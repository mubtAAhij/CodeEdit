    ...
    var body: some View {
        EmptyView()
            ...
            .alert(isPresented: $sourceControlManager.discardAllAlertIsPresented) {
                Alert(
                    title: Text(String(localized: "workspace.source-control.discard-all.title", defaultValue: "Do you want to discard all uncommitted, local changes?", comment: "Alert title for discarding all changes")),
                    message: Text(String(localized: "workspace.source-control.discard-all.message", defaultValue: "This action cannot be undone.", comment: "Alert message warning that discard cannot be undone")),
                    primaryButton: .destructive(Text(String(localized: "workspace.source-control.discard-all.button", defaultValue: "Discard", comment: "Destructive button to discard changes"))) {
            ...
            .alert(String(localized: "workspace.source-control.no-changes-to-stage.title", defaultValue: "Cannot Stage Changes", comment: "Alert title when there are no changes to stage"), isPresented: $sourceControlManager.noChangesToStageAlertIsPresented) {
                Button(String(localized: "workspace.source-control.alert.ok", defaultValue: "OK", comment: "OK button for source control alerts"), role: .cancel) {}
            } message: {
                Text(String(localized: "workspace.source-control.no-changes.message", defaultValue: "There are no uncommitted changes in the local repository for this project.", comment: "Alert message when there are no uncommitted changes"))
            }
