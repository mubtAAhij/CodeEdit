...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    private func terminateLanguageServers() {
        Task { @MainActor in
            let task = TaskNotificationModel(
                ...
                title: String(localized: "app.shutdown.lsp.title", defaultValue: "Stopping Language Servers", comment: "Title for language server shutdown notification"),
                message: String(localized: "app.shutdown.lsp.message", defaultValue: "Stopping running language server processes...", comment: "Message for language server shutdown notification"),
                isLoading: true
            )
    ...
    private func terminateTasks() {
        let task = TaskNotificationModel(
            ...
            title: String(localized: "app.shutdown.tasks.title", defaultValue: "Terminating Tasks", comment: "Title for task termination notification"),
            message: String(localized: "app.shutdown.tasks.message", defaultValue: "Interrupting all running tasks before quitting...", comment: "Message for task termination notification"),
            isLoading: true
        )
