...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    private func terminateLanguageServers() {
        Task { @MainActor in
            let task = TaskNotificationModel(
                ...
                title: String(localized: "app.quit.stopping-lsp.title", defaultValue: "Stopping Language Servers", comment: "Title for notification when stopping language servers during quit"),
                message: String(localized: "app.quit.stopping-lsp.message", defaultValue: "Stopping running language server processes...", comment: "Message for notification when stopping language servers during quit"),
                isLoading: true
            )
    ...
    private func terminateTasks() {
        let task = TaskNotificationModel(
            ...
            title: String(localized: "app.quit.terminating-tasks.title", defaultValue: "Terminating Tasks", comment: "Title for notification when terminating tasks during quit"),
            message: String(localized: "app.quit.terminating-tasks.message", defaultValue: "Interrupting all running tasks before quitting...", comment: "Message for notification when terminating tasks during quit"),
            isLoading: true
        )
