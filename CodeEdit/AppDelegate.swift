...
final class AppDelegate: NSObject, NSApplicationDelegate, ObservableObject {
    ...
    private func terminateLanguageServers() {
        Task { @MainActor in
            let task = TaskNotificationModel(
                ...
                title: String(localized: "app.stopping-language-servers", defaultValue: "Stopping Language Servers", comment: "Notification title when stopping language servers"),
                message: String(localized: "app.stopping-language-servers-message", defaultValue: "Stopping running language server processes...", comment: "Notification message when stopping language servers"),
                isLoading: true
            )
    ...
    private func terminateTasks() {
        let task = TaskNotificationModel(
            ...
            title: String(localized: "app.terminating-tasks", defaultValue: "Terminating Tasks", comment: "Notification title when terminating tasks"),
            message: String(localized: "app.terminating-tasks-message", defaultValue: "Interrupting all running tasks before quitting...", comment: "Notification message when terminating tasks"),
            isLoading: true
        )
