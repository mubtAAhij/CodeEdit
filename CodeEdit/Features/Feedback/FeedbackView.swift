        ...
        VStack {
            ...
            FeedbackToolbar {
                ...
                if feedbackModel.isSubmitted {
                    Text("feedback.submitted", comment: "Feedback submission success")
                } else if feedbackModel.failedToSubmit {
                    Text("feedback.failed_to_submit", comment: "Feedback submission failure")
                }
                Button {
                    ...
                    if isSubmitButtonPressed && feedbackModel.feedbackTitle.isEmpty {
                        HStack {
                            Image(systemName: "arrow.right.circle.fill")
                                .foregroundColor(.red)
                            Text("feedback.provide_title", comment: "Title prompt")
                        }.padding(.leading, -23)
                    } else {
                        Text("feedback.provide_title", comment: "Title prompt")
                    }
                ...
                HStack {
                    if isSubmitButtonPressed && feedbackModel.issueAreaListSelection == "none" {
                        HStack {
                            Image(systemName: "arrow.right.circle.fill")
                                .foregroundColor(.red)
                            Text("feedback.which_area", comment: "Area selection prompt")
                        }.padding(.leading, -23)
                    } else {
                        Text("feedback.which_area", comment: "Area selection prompt")
                    }
            ...
            VStack(alignment: .leading) {
                if isSubmitButtonPressed && feedbackModel.feedbackTypeListSelection == "none" {
                    HStack {
                        Image(systemName: "arrow.right.circle.fill")
                            .foregroundColor(.red)
                        Text("feedback.what_type", comment: "Feedback type prompt")
                    }.padding(.leading, -23)
                } else {
                    Text("feedback.what_type", comment: "Feedback type prompt")
                }
        ...
        VStack(alignment: .leading) {
            ...
            VStack(alignment: .leading) {
                HStack {
                    if isSubmitButtonPressed && feedbackModel.issueDescription.isEmpty {
                        HStack {
                            Image(systemName: "arrow.right.circle.fill")
                                .foregroundColor(.red)
                            Text("feedback.describe_issue", comment: "Description prompt")
                        }.padding(.leading, -23)
                    } else {
                        Text("feedback.describe_issue", comment: "Description prompt")
                    }
                ...
                TextEditor(text: $feedbackModel.issueDescription)
                           ...
                           .border(Color(NSColor.separatorColor))
                Text("feedback.issue_example", comment: "Example text")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
            .padding(.top, -5)
            ...
            VStack(alignment: .leading) {
                Text("feedback.steps_to_reproduce", comment: "Steps prompt")
                TextEditor(text: $feedbackModel.stepsReproduceDescription)
                           .frame(minHeight: 60, alignment: .leading)
                           .border(Color(NSColor.separatorColor))
                Text("feedback.steps_example", comment: "Example text")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
                Text("feedback.step1_example", comment: "Example step 1")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
                Text("feedback.step2_example", comment: "Example step 2")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
            .padding(.top)
            ...
            VStack(alignment: .leading) {
                Text("feedback.expected_behavior", comment: "Expected behavior prompt")
                TextEditor(text: $feedbackModel.expectationDescription)
                           .frame(minHeight: 60, alignment: .leading)
                           .border(Color(NSColor.separatorColor))
                Text("feedback.expected_example", comment: "Expected behavior example")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
            .padding(.top)
            ...
            VStack(alignment: .leading) {
                Text("feedback.actual_behavior", comment: "Actual behavior prompt")
                TextEditor(text: $feedbackModel.whatHappenedDescription)
                           .frame(minHeight: 60, alignment: .leading)
                           .border(Color(NSColor.separatorColor))
                // swiftlint:disable:next line_length
                Text("feedback.actual_example", comment: "Actual behavior example")
                    .font(.system(size: 10))
                    .foregroundColor(.secondary)
            }
            .padding(.top)
