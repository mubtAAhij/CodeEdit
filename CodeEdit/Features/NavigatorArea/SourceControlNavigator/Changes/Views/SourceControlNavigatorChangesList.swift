                ...
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        ...
                        Button(String(localized: "source-control.reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Context menu item to reveal file in project navigator")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "source-control.open-in-new-tab", defaultValue: "Open in New Tab", comment: "Context menu item to open file in new tab")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "source-control.open-in-new-window", defaultValue: "Open in New Window", comment: "Context menu item to open file in new window")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(format: String(localized: "source-control.discard-changes-format", defaultValue: "Discard Changes in %@...", comment: "Context menu item to discard changes, %@ is the filename"), file.fileURL.lastPathComponent)) {
