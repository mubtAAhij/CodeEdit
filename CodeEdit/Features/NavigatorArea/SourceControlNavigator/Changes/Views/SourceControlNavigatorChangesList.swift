            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "file.view-in-finder", defaultValue: "View in Finder", comment: "View in Finder menu item")) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(localized: "file.reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Reveal in Project Navigator menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "file.open-in-new-tab", defaultValue: "Open in New Tab", comment: "Open in new tab menu item")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "file.open-in-new-window", defaultValue: "Open in New Window", comment: "Open in new window menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(format: String(localized: "source-control.discard-changes", defaultValue: "Discard Changes in %@...", comment: "Discard changes menu item"), file.fileURL.lastPathComponent)) {
