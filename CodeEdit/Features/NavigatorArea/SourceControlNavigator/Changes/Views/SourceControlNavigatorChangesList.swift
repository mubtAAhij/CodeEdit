                ...
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        ...
                        Button(String(localized: "source-control.reveal-in-navigator", defaultValue: "Reveal in Project Navigator", comment: "Reveal in project navigator menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "source-control.open-new-tab", defaultValue: "Open in New Tab", comment: "Open in new tab menu item")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "source-control.open-new-window", defaultValue: "Open in New Window", comment: "Open in new window menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(format: String(localized: "source-control.discard-file-changes", defaultValue: "Discard Changes in %@...", comment: "Discard changes in file menu item"), file.fileURL.lastPathComponent)) {
