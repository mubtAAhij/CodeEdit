            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(
                            localized: "source-control.changes.view-in-finder",
                            defaultValue: "View in Finder",
                            comment: "Context menu item to view file in Finder"
                        )) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(
                            localized: "source-control.changes.reveal-in-navigator",
                            defaultValue: "Reveal in Project Navigator",
                            comment: "Context menu item to reveal file in project navigator"
                        )) {}
                    ...
                    Group {
                        Button(String(
                            localized: "source-control.changes.open-in-new-tab",
                            defaultValue: "Open in New Tab",
                            comment: "Context menu item to open file in new tab"
                        )) {
                            openGitFile(file)
                        }
                        Button(String(
                            localized: "source-control.changes.open-in-new-window",
                            defaultValue: "Open in New Window",
                            comment: "Context menu item to open file in new window"
                        )) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            ...
                            Button(String(
                                localized: "source-control.changes.discard-changes",
                                defaultValue: "Discard Changes in \(fileName)...",
                                comment: "Context menu item to discard changes in file"
                            )) {
