            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(
                            localized: "navigator.source-control.changes.view-in-finder",
                            defaultValue: "View in Finder",
                            comment: "Context menu action to reveal changed file in Finder"
                        )) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(
                            localized: "navigator.source-control.changes.reveal-in-project-navigator",
                            defaultValue: "Reveal in Project Navigator",
                            comment: "Context menu action to reveal changed file in project navigator"
                        )) {}
                    ...
                    Group {
                        Button(String(
                            localized: "navigator.source-control.changes.open-in-new-tab",
                            defaultValue: "Open in New Tab",
                            comment: "Context menu action to open changed file in a new tab"
                        )) {
                            openGitFile(file)
                        }
                        Button(String(
                            localized: "navigator.source-control.changes.open-in-new-window",
                            defaultValue: "Open in New Window",
                            comment: "Context menu action to open changed file in a new window"
                        )) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            ...
                            Button(String(format: String(
                                localized: "navigator.source-control.changes.discard-changes-in-file",
                                defaultValue: "Discard Changes in %@...",
                                comment: "Context menu action to discard changes in a specific file"
