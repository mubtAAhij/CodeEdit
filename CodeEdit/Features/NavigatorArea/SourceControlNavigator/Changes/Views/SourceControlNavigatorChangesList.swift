            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "source-control.view-in-finder", defaultValue: "View in Finder", comment: "Context menu option to show file in Finder")) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(localized: "source-control.reveal-in-navigator", defaultValue: "Reveal in Project Navigator", comment: "Context menu option to reveal file in project navigator")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "source-control.open-new-tab", defaultValue: "Open in New Tab", comment: "Context menu option to open file in new tab")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "source-control.open-new-window", defaultValue: "Open in New Window", comment: "Context menu option to open file in new window")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(format: String(localized: "source-control.discard-changes", defaultValue: "Discard Changes in %@...", comment: "Context menu option to discard changes with filename"), file.fileURL.lastPathComponent)) {
