            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "source-control.changes-list.view-in-finder", defaultValue: "View in Finder", comment: "Context menu action to reveal changed file in Finder.")) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(localized: "source-control.changes-list.reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Context menu action to reveal changed file in project navigator.")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "source-control.changes-list.open-in-new-tab", defaultValue: "Open in New Tab", comment: "Context menu action to open changed file in a new editor tab.")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "source-control.changes-list.open-in-new-window", defaultValue: "Open in New Window", comment: "Context menu action to open changed file in a new window.")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
