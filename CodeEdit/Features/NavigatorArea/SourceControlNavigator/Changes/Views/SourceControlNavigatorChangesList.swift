            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "view_in_finder", comment: "Context menu item to view file in Finder")) {
                    ...
                    Group {
                        Button(String(localized: "open_in_new_tab", comment: "Context menu item to open file in new tab")) {
                            openGitFile(file)
                        }
                    ...
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(localized: "discard_changes_in_file", arguments: [file.fileURL.lastPathComponent], comment: "Context menu item to discard changes in specific file")) {
