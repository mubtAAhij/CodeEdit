            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "context_menu.view_in_finder", comment: "Context menu item to view file in Finder")) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(localized: "context_menu.reveal_in_project_navigator", comment: "Context menu item to reveal file in project navigator")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "context_menu.open_in_new_tab", comment: "Context menu item to open file in new tab")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "context_menu.open_in_new_window", comment: "Context menu item to open file in new window")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(localized: "source_control.discard_changes_in_file", arguments: [file.fileURL.lastPathComponent], comment: "Context menu item to discard changes in specific file")) {
