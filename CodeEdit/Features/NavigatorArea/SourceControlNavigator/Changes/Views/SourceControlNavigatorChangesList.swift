            ...
            menu: { selectedFiles in
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        Button(String(localized: "view-in-finder", defaultValue: "View in Finder", comment: "View in Finder menu item")) {
                            NSWorkspace.shared.activateFileViewerSelecting([file.fileURL.absoluteURL])
                        }
                        Button(String(localized: "reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Reveal in project navigator menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "open-in-new-tab", defaultValue: "Open in New Tab", comment: "Open in new tab menu item")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "open-in-new-window", defaultValue: "Open in New Window", comment: "Open in new window menu item")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
