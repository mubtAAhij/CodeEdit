                ...
                if selectedFiles.count == 1,
                   let file = selectedFiles.first {
                    Group {
                        ...
                        Button(String(localized: "source-control.reveal-in-project-navigator", defaultValue: "Reveal in Project Navigator", comment: "Menu item to reveal in project navigator")) {}
                            .disabled(true) // TODO: Implementation Needed
                        Divider()
                    }
                    Group {
                        Button(String(localized: "source-control.open-in-new-tab", defaultValue: "Open in New Tab", comment: "Menu item to open in new tab")) {
                            openGitFile(file)
                        }
                        Button(String(localized: "source-control.open-in-new-window", defaultValue: "Open in New Window", comment: "Menu item to open in new window")) {}
                            .disabled(true) // TODO: Implementation Needed
                    }
                    if file.anyStatus() != .none {
                        Group {
                            Divider()
                            Button(String(format: String(localized: "source-control.discard-changes-in-file", defaultValue: "Discard Changes in %@...", comment: "Menu item to discard changes in specific file"), file.fileURL.lastPathComponent)) {
