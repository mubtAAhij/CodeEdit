...
struct SourceControlNavigatorSyncView: View {
        ...
        if let currentBranch = sourceControlManager.currentBranch {
            HStack {
                if currentBranch.upstream == nil {
                    Label(title: {
                        Text(String(localized: "source-control.no-tracked-branch", defaultValue: "No tracked branch for '\(sourceControlManager.currentBranch?.name ?? "")'", comment: "Message when branch has no upstream tracking"))
                    }, icon: {
                        Image(symbol: "branch")
    ...
    func formatUnsyncedlabel(ahead: Int?, behind: Int?) -> String {
        ...
        if let ahead = ahead, ahead > 0 {
            parts.append(String(localized: "source-control.commits-ahead", defaultValue: "\(ahead) ahead", comment: "Number of commits ahead of remote"))
        }

        if let behind = behind, behind > 0 {
            parts.append(String(localized: "source-control.commits-behind", defaultValue: "\(behind) behind", comment: "Number of commits behind remote"))
        }
