...
struct SourceControlNavigatorSyncView: View {
    ...
    func pull() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.pull.failed", defaultValue: "Failed to pull", comment: "Failed to pull error"), error: error)
            }
    ...
    func formatUnsyncedlabel(ahead: Int?, behind: Int?) -> String {
        ...
        if let ahead = ahead, ahead > 0 {
            parts.append(String(format: String(localized: "source-control.commits-ahead", defaultValue: "%d ahead", comment: "Commits ahead status"), ahead))
        }

        if let behind = behind, behind > 0 {
            parts.append(String(format: String(localized: "source-control.commits-behind", defaultValue: "%d behind", comment: "Commits behind status"), behind))
        }
