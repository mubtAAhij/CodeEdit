...
struct SourceControlNavigatorSyncView: View {
        ...
        if let currentBranch = sourceControlManager.currentBranch {
            HStack {
                if currentBranch.upstream == nil {
                    Label(title: {
                        Text(String(format: String(localized: "source-control.no-tracked-branch", defaultValue: "No tracked branch for '%@'", comment: "No tracked branch message"), sourceControlManager.currentBranch?.name ?? ""))
                    }, icon: {
                        Image(symbol: "branch")
    ...
    func pull() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.failed-to-pull", defaultValue: "Failed to pull", comment: "Failed to pull error"), error: error)
            }
    ...
    func formatUnsyncedlabel(ahead: Int?, behind: Int?) -> String {
        ...
        if let ahead = ahead, ahead > 0 {
            parts.append(String(format: String(localized: "source-control.commits-ahead", defaultValue: "%d ahead", comment: "Commits ahead"), ahead))
        }

        if let behind = behind, behind > 0 {
            parts.append(String(format: String(localized: "source-control.commits-behind", defaultValue: "%d behind", comment: "Commits behind"), behind))
        }
