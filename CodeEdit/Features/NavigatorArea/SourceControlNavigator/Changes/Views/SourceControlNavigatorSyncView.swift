            ...
            HStack {
                if currentBranch.upstream == nil {
                    Label(title: {
                        Text(String(format: String(
                            localized: "source-control-navigator.sync.no-tracked-branch",
                ...
                if sourceControlManager.numberOfUnsyncedCommits.behind > 0 {
                    Button {
                        ...
                        Text(String(
                            localized: "source-control-navigator.sync.pull",
                            defaultValue: "Pull...",
                            comment: "Action title to pull remote changes from sync view"
                        ))
                    }
                    .disabled(isLoading)
                    ...
                    Button {
                        ...
                        Text(String(
                            localized: "source-control-navigator.sync.push",
                            defaultValue: "Push...",
                            comment: "Action title to push local changes from sync view"
                        ))
                    }
                    .disabled(isLoading)
                }
        ...
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(
                    localized: "source-control-navigator.sync.failed-to-pull",
                    defaultValue: "Failed to pull",
                    comment: "Error title shown when pull operation fails in sync view"
                ), error: error)
            }
    ...
    func push() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(
                    localized: "source-control-navigator.sync.failed-to-push",
                    defaultValue: "Failed to push",
                    comment: "Error title shown when push operation fails in sync view"
                ), error: error)
            }
            self.isLoading = false
        }
    ...
    func formatUnsyncedlabel(ahead: Int?, behind: Int?) -> String {
        ...
        if let ahead = ahead, ahead > 0 {
            parts.append(String(format: String(
                localized: "source-control-navigator.sync.ahead-count",
                defaultValue: "%d ahead",
                comment: "Status text showing number of commits ahead of tracked branch"
            ), ahead))
        }

        if let behind = behind, behind > 0 {
            parts.append(String(format: String(
                localized: "source-control-navigator.sync.behind-count",
                defaultValue: "%d behind",
                comment: "Status text showing number of commits behind tracked branch"
            ), behind))
        }
