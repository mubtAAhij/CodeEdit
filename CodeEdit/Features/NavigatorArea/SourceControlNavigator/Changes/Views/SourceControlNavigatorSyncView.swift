...
struct SourceControlNavigatorSyncView: View {
        ...
        if let currentBranch = sourceControlManager.currentBranch {
            HStack {
                if currentBranch.upstream == nil {
                    Label(title: {
                        Text("No tracked branch for '\(sourceControlManager.currentBranch?.name ?? "")'", comment: "Message shown when current branch has no tracked upstream branch")
                    }, icon: {
                        Image(symbol: "branch")
                ...
                if sourceControlManager.numberOfUnsyncedCommits.behind > 0 {
                    Button {
                        ...
                        Text("Pull...", comment: "Button to pull changes from remote repository")
                    }
                    .disabled(isLoading)
                    ...
                    Button {
                        ...
                        Text("Push...", comment: "Button to push changes to remote repository")
                    }
                    .disabled(isLoading)
                }
            }
    ...
    func pull() {
        Task(priority: .background) {
            ...
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "Failed to pull"), error: error)
            }
    ...
    func formatUnsyncedlabel(ahead: Int?, behind: Int?) -> String {
        ...
        if let ahead = ahead, ahead > 0 {
            parts.append(String(localized: "\(ahead) ahead"))
        }

        if let behind = behind, behind > 0 {
            parts.append(String(localized: "\(behind) behind"))
        }
