...
struct SourceControlNavigatorRepositoryView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            : "String(localized: "repository_has_uncommitted_changes", comment: "Alert message indicating the repository has uncommitted changes")",
            isPresented: $applyStashedChangesIsPresented
        ) {
            if sourceControlManager.changedFiles.isEmpty {
                Button("String(localized: "apply", comment: "Button label to apply stashed changes")") {
                ...
                Button("String(localized: "apply_and_delete", comment: "Button label to apply stashed changes and delete the stash")") {
                    if let stashEntry = stashEntryToApply {
                        Task {
                            try await sourceControlManager.applyStashEntry(stashEntry: stashEntry)
                ...
                Button("String(localized: "cancel", comment: "Cancel button in dialog")", role: .cancel) {}
            } else {
                Button("String(localized: "okay", comment: "Okay button in dialog")", role: .cancel) {}
            }
        } message: {
            sourceControlManager.changedFiles.isEmpty
            ? Text("String(localized: "applying_stashed_changes_message", comment: "Message explaining what happens when applying stashed changes")")
            : Text("String(localized: "try_committing_discarding_changes", comment: "Suggestion to commit or discard changes")")
        }
        .confirmationDialog(
            "Do you want to delete the branch “\(branchToDelete?.name ?? "")”?",
            isPresented: $isPresentingConfirmDeleteBranch
        ) {
            Button("String(localized: "delete", comment: "Delete button")") {
