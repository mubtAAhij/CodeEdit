...
struct SourceControlNavigatorRepositoryView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            ? String(localized: "git.apply-stashed-changes-title", defaultValue: "Do you want to apply stashed changes?", comment: "Apply stashed changes alert title")
            : String(localized: "git.repository-has-uncommitted-changes", defaultValue: "The local repository has uncommitted changes.", comment: "Repository has uncommitted changes alert title"),
            isPresented: $applyStashedChangesIsPresented
        ) {
            if sourceControlManager.changedFiles.isEmpty {
                Button(String(localized: "git.apply", defaultValue: "Apply", comment: "Apply button")) {
                ...
                Button(String(localized: "git.apply-and-delete", defaultValue: "Apply and Delete", comment: "Apply and Delete button")) {
                    if let stashEntry = stashEntryToApply {
                        Task {
                            try await sourceControlManager.applyStashEntry(stashEntry: stashEntry)
            ...
            sourceControlManager.changedFiles.isEmpty
            ? Text(String(localized: "git.apply-stashed-changes-message", defaultValue: "Applying the stashed changes will restore modifications to files in your local repository.", comment: "Apply stashed changes message"))
            : Text(String(localized: "git.try-committing-or-discarding", defaultValue: "Try committing or discarding the changes.", comment: "Try committing or discarding message"))
        }
        .confirmationDialog(
            "Do you want to delete the branch \"\(branchToDelete?.name ?? \"\")\"?",
            isPresented: $isPresentingConfirmDeleteBranch
        ) {
            Button(String(localized: "git.delete", defaultValue: "Delete", comment: "Delete button")) {
                    ...
                    Task {
                        do {
                            ...
                            await sourceControlManager.showAlertForError(
                                title: String(localized: "git.failed-to-delete", defaultValue: "Failed to delete", comment: "Failed to delete error title"),
                                error: error
                            )
                        }
                        branchToDelete = nil
            ...
            Text(String(localized: "git.delete-branch-message", defaultValue: "The branch will be removed from the repository. You can't undo this action.", comment: "Delete branch confirmation message"))
        }
        .confirmationDialog(
            ...
            Text(String(localized: "git.delete-stash-message", defaultValue: "The stash will be removed from the repository. You can't undo this action.", comment: "Delete stash confirmation message"))
        }
        .confirmationDialog(
            ...
            Text(String(localized: "git.delete-remote-message", defaultValue: "The remote will be removed from the repository. You can't undo this action.", comment: "Delete remote confirmation message"))
        }
        .task {
