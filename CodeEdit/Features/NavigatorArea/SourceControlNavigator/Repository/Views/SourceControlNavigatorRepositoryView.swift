...
struct SourceControlNavigatorRepositoryView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            ? String(localized: "source_control.alert.apply_stashed_changes_title", comment: "Alert title asking if user wants to apply stashed changes")
            : String(localized: "source_control.alert.uncommitted_changes_title", comment: "Alert title indicating repository has uncommitted changes"),
            isPresented: $applyStashedChangesIsPresented
        ) {
            if sourceControlManager.changedFiles.isEmpty {
                Button(String(localized: "source_control.alert.apply", comment: "Button to apply stashed changes")) {
                ...
                Button(String(localized: "source_control.alert.apply_and_delete", comment: "Button to apply and delete stashed changes")) {
                    if let stashEntry = stashEntryToApply {
                        Task {
                            try await sourceControlManager.applyStashEntry(stashEntry: stashEntry)
                ...
                Button(String(localized: "source_control.alert.cancel", comment: "Button to cancel an action"), role: .cancel) {}
            } else {
                Button(String(localized: "source_control.alert.okay", comment: "Button to acknowledge an alert"), role: .cancel) {}
            }
        } message: {
            sourceControlManager.changedFiles.isEmpty
            ? Text(String(localized: "source_control.stash.apply_warning", comment: "Warning message about applying stashed changes"))
            : Text(String(localized: "source_control.stash.apply_suggestion", comment: "Suggestion for handling uncommitted changes"))
        }
        .confirmationDialog(
            "Do you want to delete the branch “\(branchToDelete?.name ?? "")”?",
            isPresented: $isPresentingConfirmDeleteBranch
        ) {
            Button(String(localized: "general.delete", comment: "Delete button label")) {
                    ...
                    Task {
                        do {
                            ...
                            await sourceControlManager.showAlertForError(
                                title: String(localized: "general.delete_failed", comment: "Error message when deletion fails"),
                                error: error
                            )
                        }
                        branchToDelete = nil
