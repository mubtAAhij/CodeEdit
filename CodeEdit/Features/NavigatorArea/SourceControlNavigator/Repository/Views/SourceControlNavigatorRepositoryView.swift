    ...
    var body: some View {
        ...
        .alert(
            ...
            if sourceControlManager.changedFiles.isEmpty {
                ...
                Button(String(localized: "common.cancel", defaultValue: "Cancel", comment: "Button to cancel action"), role: .cancel) {}
            } else {
                Button(String(localized: "common.okay", defaultValue: "Okay", comment: "Button to acknowledge alert"), role: .cancel) {}
            }
        } message: {
            sourceControlManager.changedFiles.isEmpty
            ? Text(String(localized: "source-control.stash.apply-message", defaultValue: "Applying the stashed changes will restore modifications to files in your local repository.", comment: "Message explaining what happens when applying stashed changes"))
            : Text(String(localized: "source-control.stash.uncommitted-message", defaultValue: "Try committing or discarding the changes.", comment: "Message suggesting to commit or discard changes"))
        }
        .confirmationDialog(
            String(format: String(localized: "source-control.branch.delete-prompt", defaultValue: "Do you want to delete the branch \"%@\"?", comment: "Confirmation prompt to delete a branch"), branchToDelete?.name ?? ""),
            isPresented: $isPresentingConfirmDeleteBranch
        ) {
            Button(String(localized: "common.delete", defaultValue: "Delete", comment: "Button to delete item")) {
        ...
        .confirmationDialog(
            String(format: String(localized: "source-control.stash.delete-prompt", defaultValue: "Do you want to delete the stash \"%@\"?", comment: "Confirmation prompt to delete a stash"), stashEntryToDelete?.message ?? ""),
        ...
        .confirmationDialog(
            String(format: String(localized: "source-control.remote.delete-prompt", defaultValue: "Do you want to delete the remote \"%@\"?", comment: "Confirmation prompt to delete a remote"), remoteToDelete?.name ?? ""),
