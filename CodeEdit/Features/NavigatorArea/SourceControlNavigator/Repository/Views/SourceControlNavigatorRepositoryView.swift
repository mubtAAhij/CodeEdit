...
struct SourceControlNavigatorRepositoryView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            ? String(localized: "source_control.apply_stash.title", defaultValue: "Do you want to apply stashed changes?", comment: "Alert title asking to apply stashed changes")
            : String(localized: "source_control.apply_stash.uncommitted_changes", defaultValue: "The local repository has uncommitted changes.", comment: "Alert title when repository has uncommitted changes"),
            ...
            if sourceControlManager.changedFiles.isEmpty {
                ...
                Button(String(localized: "source_control.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) {}
            } else {
                Button(String(localized: "source_control.okay", defaultValue: "Okay", comment: "Okay button"), role: .cancel) {}
            }
        } message: {
            sourceControlManager.changedFiles.isEmpty
            ? Text(String(localized: "source_control.apply_stash.message", defaultValue: "Applying the stashed changes will restore modifications to files in your local repository.", comment: "Message explaining what applying stashed changes does"))
            : Text(String(localized: "source_control.apply_stash.try_committing", defaultValue: "Try committing or discarding the changes.", comment: "Message suggesting to commit or discard changes"))
        }
        .confirmationDialog(
            "Do you want to delete the branch “\(branchToDelete?.name ?? "")”?",
            isPresented: $isPresentingConfirmDeleteBranch
        ) {
            Button(String(localized: "source_control.delete", defaultValue: "Delete", comment: "Delete button")) {
                    ...
                    Task {
                        do {
                            ...
                            await sourceControlManager.showAlertForError(
                                title: String(localized: "source_control.failed_to_delete", defaultValue: "Failed to delete", comment: "Error title when deletion fails"),
                                error: error
                            )
                        }
                        branchToDelete = nil
            ...
            Text(String(localized: "source_control.delete_branch.message", defaultValue: "The branch will be removed from the repository. You can't undo this action.", comment: "Warning message about deleting branch"))
        }
        .confirmationDialog(
            ...
            Text(String(localized: "source_control.delete_stash.message", defaultValue: "The stash will be removed from the repository. You can't undo this action.", comment: "Warning message about deleting stash"))
        }
        .confirmationDialog(
            ...
            Text(String(localized: "source_control.delete_remote.message", defaultValue: "The remote will be removed from the repository. You can't undo this action.", comment: "Warning message about deleting remote"))
        }
        .task {
