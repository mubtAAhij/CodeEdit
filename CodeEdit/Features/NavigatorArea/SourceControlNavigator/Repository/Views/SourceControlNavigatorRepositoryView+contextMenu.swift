...
extension SourceControlNavigatorRepositoryView {
    ...
    func contextMenu(for item: RepoOutlineGroupItem, branch: GitBranch) -> some View {
        Button("source_control.switch", comment: "Menu item") {
        ...
        Button(
            ...
            ? String(localized: "source_control.new_branch", comment: "Menu item")
            : String(localized: "source_control.new_branch_from \(branch.name)", comment: "Menu item")
        ) {
            showNewBranch = true
        ...
        Button(
            item.branch == nil
            ? String(localized: "source_control.rename_branch", comment: "Menu item")
            : String(localized: "source_control.rename_branch_named \(branch.name)", comment: "Menu item")
        ) {
        ...
        Divider()
        Button("source_control.add_existing_remote", comment: "Menu item") {
            sourceControlManager.addExistingRemoteSheetIsPresented = true
        }
        .disabled(item.id != "RemotesGroup")
        Divider()
        Button("source_control.apply_stashed_changes", comment: "Menu item") {
            applyStashedChangesIsPresented = true
            stashEntryToApply = item.stashEntry
        }
        .disabled(item.stashEntry == nil)
        Divider()
        Button("source_control.delete", comment: "Menu item") {
            handleDelete(item)
        }
        .disabled(
            (item.branch == nil
