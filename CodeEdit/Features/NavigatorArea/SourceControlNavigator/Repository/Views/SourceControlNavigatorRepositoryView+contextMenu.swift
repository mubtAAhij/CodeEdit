...
extension SourceControlNavigatorRepositoryView {
    ...
    func contextMenu(for item: RepoOutlineGroupItem, branch: GitBranch) -> some View {
        Button(String(localized: "source-control.switch", defaultValue: "Switch...", comment: "Switch branch menu item")) {
        ...
        Button(
            ...
            ? String(localized: "source-control.new-branch", defaultValue: "New Branch...", comment: "New branch menu item")
        ...
        Button(
            item.branch == nil
            ? String(localized: "source-control.rename-branch", defaultValue: "Rename Branch...", comment: "Rename branch menu item")
            : String(format: String(localized: "source-control.rename-branch-named", defaultValue: "Rename \"%@\"...", comment: "Rename branch named menu item"), branch.name)
        ) {
        ...
        Divider()
        Button(String(localized: "source-control.add-remote", defaultValue: "Add Existing Remote...", comment: "Add existing remote menu item")) {
            sourceControlManager.addExistingRemoteSheetIsPresented = true
        }
        .disabled(item.id != "RemotesGroup")
        Divider()
        Button(String(localized: "source-control.apply-stash", defaultValue: "Apply Stashed Changes...", comment: "Apply stashed changes menu item")) {
            applyStashedChangesIsPresented = true
            stashEntryToApply = item.stashEntry
        }
        .disabled(item.stashEntry == nil)
        Divider()
        Button(String(localized: "source-control.delete", defaultValue: "Delete...", comment: "Delete menu item")) {
            handleDelete(item)
        }
        .disabled(
            (item.branch == nil
