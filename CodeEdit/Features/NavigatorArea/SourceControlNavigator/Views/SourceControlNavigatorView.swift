...
struct SourceControlNavigatorTabs: View {
    ...
    var body: some View {
        if sourceControlManager.isGitRepository {
            SegmentedControl(
                $selectedSection,
                options: [String(localized: "source_control.changes", comment: "Changes tab label"), String(localized: "source_control.history", comment: "History tab label"), String(localized: "source_control.navigator.repository_tab", comment: "Repository tab title in source control navigator")],
                prominent: true
            )
            ...
            CEContentUnavailableView(
                String(localized: "source_control.navigator.no_repository_title", comment: "Title when no git repository is found"),
                 description: String(localized: "source_control.navigator.no_repository_description", comment: "Description when project is not a git repository"),
                 systemImage: "externaldrive.fill",
                 actions: {
                    Button(String(localized: "source_control.navigator.initialize_button", comment: "Button to initialize git repository")) {
                        Task {
