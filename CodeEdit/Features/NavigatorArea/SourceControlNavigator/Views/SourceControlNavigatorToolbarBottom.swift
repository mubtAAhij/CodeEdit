...
struct SourceControlNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            sourceControlMenu
            PaneTextField(
                String(localized: "filter", defaultValue: "Filter", comment: "Filter text field placeholder"),
                text: $text,
                leadingAccessories: {
                    Image(
                    ...
                    .padding(.leading, 4)
                    .help(String(localized: "source-control.filter-changes", defaultValue: "Filter Changes Navigator", comment: "Accessibility label for filter changes navigator"))
                },
                clearable: true
            )
        }
