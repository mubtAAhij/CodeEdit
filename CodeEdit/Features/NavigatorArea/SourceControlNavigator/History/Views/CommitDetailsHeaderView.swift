...
struct CommitDetailsHeaderView: View {
    ...
    private func coAuthDetail() -> String {
        if commit.committerEmail == "noreply@github.com" {
            ...
            return String(format: String(localized: "navigator.source-control.history.co-authored-by", defaultValue: "Co-authored by: %@\n<%@>", comment: "Commit details line showing co-author name and email on separate lines"), "\(commit.committer)", "\(commit.committerEmail)")
        }
        return ""
    }
