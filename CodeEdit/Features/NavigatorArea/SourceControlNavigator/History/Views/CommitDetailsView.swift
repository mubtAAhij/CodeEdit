    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack {
                ...
                Text(String(localized: "source-control.navigator.commit-details.title", defaultValue: "Commit Details", comment: "Title for commit details view"))
            ...
            if let commit = commit {
                ...
                if !commitChanges.isEmpty {
                    ...
                    CEContentUnavailableView(String(localized: "source-control.navigator.commit-details.no-changes", defaultValue: "No Changes", comment: "Message shown when selected commit has no file changes"))
                }
            } else {
                Spacer()
            }
