        ...
        Group {
            ...
            case .loading:
                VStack {
                    Spacer()
                    ProgressView {
                        Text(String(localized: "source-control.history.loading", defaultValue: "Loading History", comment: "Loading state title in source control history navigator."))
                    }
                    Spacer()
                }
            case .ready:
                if commitHistory.isEmpty {
                    CEContentUnavailableView(String(localized: "source-control.history.empty", defaultValue: "No History", comment: "Empty state title when no source control history is available."))
                } else {
            ...
            case .error(let error):
                VStack {
                    ...
                    CEContentUnavailableView(
                        String(localized: "source-control.history.error-loading", defaultValue: "Error Loading History", comment: "Error state title when source control history fails to load."),
                        ...
                        Button {
                            ...
                            Text(String(localized: "source-control.history.retry", defaultValue: "Retry", comment: "Button title to retry loading source control history."))
                        }
                    }
                    Spacer()
                }
