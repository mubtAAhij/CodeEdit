        ...
        Group {
            ...
            case .loading:
                VStack {
                    Spacer()
                    ProgressView {
                        Text(String(localized: "source-control.history.loading", defaultValue: "Loading History", comment: "Progress message while loading commit history"))
                    }
                    Spacer()
                }
            case .ready:
                if commitHistory.isEmpty {
                    CEContentUnavailableView(String(localized: "source-control.history.empty", defaultValue: "No History", comment: "Message when commit history is empty"))
                } else {
            ...
            case .error(let error):
                VStack {
                    ...
                    CEContentUnavailableView(
                        String(localized: "source-control.history.error", defaultValue: "Error Loading History", comment: "Error message when commit history fails to load"),
