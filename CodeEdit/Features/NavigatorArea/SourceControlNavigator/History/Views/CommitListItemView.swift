    ...
    var body: some View {
        ...
        .contextMenu {
            Group {
                ...
                Button(String(localized: "copy_identifier", comment: "Context menu item to copy commit identifier")) {
                    let pasteboard = NSPasteboard.general
                    pasteboard.clearContents()
                    pasteboard.setString(commit.commitHash, forType: .string)
                }
                Button(String(localized: "email_author", arguments: [commit.author], comment: "Context menu item to email commit author")) {
            ...
            Group {
                Button(String(localized: "tag_commit", arguments: [commit.hash], comment: "Context menu item to tag a commit")) {}
                    .disabled(true) // TODO: Implementation Needed
                Button(String(localized: "new_branch_from_commit", arguments: [commit.hash], comment: "Context menu item to create new branch from commit")) {}
                    .disabled(true) // TODO: Implementation Needed
                Button(String(localized: "cherry_pick_commit", arguments: [commit.hash], comment: "Context menu item to cherry-pick a commit")) {}
                    .disabled(true) // TODO: Implementation Needed
            }
            Group {
                Divider()
                if let commitRemoteURL = commit.commitBaseURL?.absoluteString {
                    Button(String(localized: "view_on_remote", arguments: [commit.remoteString], comment: "Context menu item to view commit on remote service")) {
                        let commitURL = "\(commitRemoteURL)/\(commit.commitHash)"
                        openCommit(URL(string: commitURL)!)
                    }
                    Divider()
                }
                Button(String(localized: "checkout_commit", arguments: [commit.hash], comment: "Context menu item to checkout a commit")) {}
                    .disabled(true) // TODO: Implementation Needed
                Divider()
