            ...
            if shouldShow {
                HStack(alignment: .center) {
                    ProgressView(value: progress, total: 1.0) {
                        ...
                        HStack {
                            Text(String(format: NSLocalizedString("navigator.find.indexing_progress", comment: ""), Int(progress * 100)))
                                .font(.system(size: 10))
                                .animation(.none)
                        }
                    }
                    // swiftlint:disable:next line_length
                    .help(NSLocalizedString("navigator.find.indexing_workspace", comment: ""))
                }
