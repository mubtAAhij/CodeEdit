        ...
        VStack {
            ...
            case .searching:
                VStack {
                    ...
                    Text(String(
                        localized: "find-navigator.state.searching.title",
                        defaultValue: "Searching",
                        comment: "Status title shown while find operation is in progress"
                    ))
            ...
            case .replacing:
                VStack {
                    ...
                    Text(String(
                        localized: "find-navigator.state.replacing.title",
                        defaultValue: "Replacing",
                        comment: "Status title shown while replace operation is in progress"
                    ))
                        .foregroundStyle(.tertiary)
                        .font(.title3)
                }
            ...
            case .found:
                if self.searchResultCount == 0 {
                    CEContentUnavailableView(
                        String(
                            localized: "find-navigator.state.no-results.title",
                            defaultValue: "No Results",
                            comment: "Title shown when no search results are found"
                        ),
                        description: String(format: String(
                            localized: "find-navigator.state.no-results.message",
                            defaultValue: "No Results for \"%@\" in Project",
                            comment: "Message shown when no results are found for the current query"
                        ), "\(state.searchQuery)"),
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    String(
                        localized: "find-navigator.state.replaced.title",
                        defaultValue: "Replaced",
                        comment: "Title shown after replace operation completes"
                    ),
                    description: String(format: String(
                        localized: "find-navigator.state.replaced.message",
                        defaultValue: "Successfully replaced terms across %d files",
                        comment: "Message shown after replacing terms across files"
                    ), updatedFiles),
            ...
            case .failed(let errorMessage):
                CEContentUnavailableView(
                    String(
                        localized: "find-navigator.state.error.title",
                        defaultValue: "An Error Occurred",
                        comment: "Title shown when find navigator encounters an error"
                    ),
                    description: String(format: String(
                        localized: "find-navigator.state.error.message",
                        defaultValue: "%@",
                        comment: "Error message content shown in find navigator"
                    ), "\(errorMessage)"),
