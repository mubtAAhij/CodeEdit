        ...
        VStack {
            ...
            if findNavigatorStatus == .found {
                HStack(alignment: .center) {
                    Text(String(format: String(localized: "find.results-in-files", defaultValue: "%d results in %d files", comment: "Search results count"), self.searchResultCount, self.foundFilesCount))
                        .font(.system(size: 10))
                }
            ...
            case .found:
                if self.searchResultCount == 0 {
                    CEContentUnavailableView(
                        ...
                        description: String(format: String(localized: "find.no-results-for-query", defaultValue: "No Results for \"%@\" in Project", comment: "No results for search query"), state.searchQuery),
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    ...
                    description: String(format: String(localized: "find.successfully-replaced", defaultValue: "Successfully replaced terms across %d files", comment: "Successfully replaced message"), updatedFiles),
