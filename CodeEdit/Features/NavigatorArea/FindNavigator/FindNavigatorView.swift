        ...
        VStack {
            ...
            if findNavigatorStatus == .found {
                HStack(alignment: .center) {
                    Text(String(format: String(localized: "find.results-summary", defaultValue: "%d results in %d files", comment: "Find results summary"), self.searchResultCount, self.foundFilesCount))
                        .font(.system(size: 10))
                }
            ...
            case .found:
                if self.searchResultCount == 0 {
                    CEContentUnavailableView(
                        ...
                        description: String(format: String(localized: "find.no-results-for", defaultValue: "No Results for \"%@\" in Project", comment: "No results description"), state.searchQuery),
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    ...
                    description: String(format: String(localized: "find.replaced-success", defaultValue: "Successfully replaced terms across %d files", comment: "Successfully replaced description"), updatedFiles),
