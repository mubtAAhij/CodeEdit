        ...
        VStack {
            ...
            if findNavigatorStatus == .found {
                HStack(alignment: .center) {
                    Text(String(format: String(localized: "find.results-summary", defaultValue: "%d results in %d files", comment: "Summary of search results"), self.searchResultCount, self.foundFilesCount))
                        .font(.system(size: 10))
                }
            ...
            case .searching:
                VStack {
                    ...
                    Text(String(localized: "find.searching", defaultValue: "Searching", comment: "Message shown while searching"))
            ...
            case .replacing:
                VStack {
                    ...
                    Text(String(localized: "find.replacing", defaultValue: "Replacing", comment: "Message shown while replacing"))
                        .foregroundStyle(.tertiary)
                        .font(.title3)
                }
                .frame(maxHeight: .infinity)
            case .found:
                if self.searchResultCount == 0 {
                    CEContentUnavailableView(
                        ...
                        description: String(format: String(localized: "find.no-results-description", defaultValue: "No Results for \"%@\" in Project", comment: "Description when no results found for search query"), state.searchQuery),
            ...
            case .replaced(let updatedFiles):
                CEContentUnavailableView(
                    ...
                    description: String(format: String(localized: "find.replaced-description", defaultValue: "Successfully replaced terms across %d files", comment: "Description of successful replacement"), updatedFiles),
                    systemImage: "checkmark.circle.fill"
                )
            case .failed(let errorMessage):
                CEContentUnavailableView(
                    String(localized: "find.error-occurred", defaultValue: "An Error Occurred", comment: "Title when an error occurs"),
