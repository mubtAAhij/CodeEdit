...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Placeholder for project navigator filter"),
                ...
                leadingAccessories: {
                    ...
                    .help(String(localized: "project-navigator.filter-help", defaultValue: "Show files with matching name", comment: "Help text for filter"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.recent-files", defaultValue: "Show only recent files", comment: "Help text for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.source-control-filter", defaultValue: "Show only files with source-control status", comment: "Help text for source control filter"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Menu item to add file")) {
            ...
            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Menu item to add folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
