...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Filter text field placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort folders on top")), (false, String(localized: "project-navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.filter.help", defaultValue: "Show files with matching name", comment: "Filter help text"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.filter.recent", defaultValue: "Show only recent files", comment: "Show recent files filter help"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.filter.source-control", defaultValue: "Show only files with source-control status", comment: "Show source control files filter help"))
                    }
        ...
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Add file button")) {
                ...
                do {
                    ...
                    alert.addButton(withTitle: String(localized: "project-navigator.error.dismiss", defaultValue: "Dismiss", comment: "Dismiss error alert button"))
                    alert.runModal()
                }
            }
            ...
            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Add folder button")) {
                ...
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project-navigator.new-folder.default-name", defaultValue: "untitled", comment: "Default name for new folder"),
                        toFile: rootFile
                    ) {
