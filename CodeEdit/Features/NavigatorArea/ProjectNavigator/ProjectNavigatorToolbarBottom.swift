...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project.navigator.filter.placeholder", defaultValue: "Filter", comment: "Placeholder text for project navigator filter field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project.navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option to show folders first")), (false, String(localized: "project.navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option for alphabetical ordering"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project.navigator.filter.matching-name.help", defaultValue: "Show files with matching name", comment: "Help text for name filter toggle"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project.navigator.filter.recent-files.help", defaultValue: "Show only recent files", comment: "Help text for recent files filter toggle"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project.navigator.filter.source-control.help", defaultValue: "Show only files with source-control status", comment: "Help text for source control status filter toggle"))
                    }
    ...
    private var addNewFileButton: some View {
            ...
            Button(String(localized: "project.navigator.add-file", defaultValue: "Add File", comment: "Menu item to add a new file")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project.navigator.new-file.default-name", defaultValue: "untitled", comment: "Default name for new files"),
                        toFile: rootFile
                    ) {
            ...
            Button(String(localized: "project.navigator.add-folder", defaultValue: "Add Folder", comment: "Menu item to add a new folder")) {
                ...
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project.navigator.new-folder.default-name", defaultValue: "untitled", comment: "Default name for new folders"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "project.navigator.add-button.accessibility-label", defaultValue: "Add Folder or File", comment: "Accessibility label for add button"))
