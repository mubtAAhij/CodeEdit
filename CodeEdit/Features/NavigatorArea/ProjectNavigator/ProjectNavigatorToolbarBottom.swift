...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Placeholder text for project navigator filter field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option to show folders before files")), (false, String(localized: "project-navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option to sort items alphabetically"))], id: \.0) { value, title in
                ...
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.filter.source-control.help", defaultValue: "Show only files with source-control status", comment: "Tooltip for source control status filter toggle"))
                    }
            ...
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Menu item to add a new file")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Default name for new files and folders"),
                        toFile: rootFile
                    ) {
            ...
            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Menu item to add a new folder")) {
                ...
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Default name for new files and folders"),
                        toFile: rootFile
                    ) {
