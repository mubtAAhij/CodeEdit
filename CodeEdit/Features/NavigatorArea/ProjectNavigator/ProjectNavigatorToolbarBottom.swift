...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Filter text field placeholder in project navigator"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([
                            (true, String(localized: "project-navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option for folders on top")),
                            (false, String(localized: "project-navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option for alphabetical order"))
                        ], id: \.0) { value, title in
                ...
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.filter.recent-files", defaultValue: "Show only recent files", comment: "Tooltip for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.filter.source-control", defaultValue: "Show only files with source-control status", comment: "Tooltip for source control filter"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Button to add a new file")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project-navigator.untitled-file", defaultValue: "untitled", comment: "Default name for new file"),
                        toFile: rootFile
                    ) {
                    ...
                    alert.addButton(withTitle: String(localized: "project-navigator.alert.dismiss", defaultValue: "Dismiss", comment: "Dismiss button for alert"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Button to add a new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project-navigator.untitled-folder", defaultValue: "untitled", comment: "Default name for new folder"),
                        toFile: rootFile
                    ) {
