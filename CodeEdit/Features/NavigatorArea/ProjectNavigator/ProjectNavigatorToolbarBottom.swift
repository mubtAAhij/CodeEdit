...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Placeholder for project navigator filter text field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.folders-on-top", defaultValue: "Folders on top", comment: "Sort option to show folders on top")), (false, String(localized: "project-navigator.alphabetically", defaultValue: "Alphabetically", comment: "Sort option to sort alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.show-files-matching", defaultValue: "Show files with matching name", comment: "Tooltip for filter button"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.show-recent-files", defaultValue: "Show only recent files", comment: "Tooltip for recent files filter toggle"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.show-source-control-files", defaultValue: "Show only files with source-control status", comment: "Tooltip for source control filter toggle"))
                    }
            ...
            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Menu item to add a new folder")) {
                ...
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Default name for new file"),
                        toFile: rootFile
                    ) {
