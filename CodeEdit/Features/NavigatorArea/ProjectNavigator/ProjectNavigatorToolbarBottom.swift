...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "navigator.filter", defaultValue: "Filter", comment: "Filter text field placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "navigator.folders-on-top", defaultValue: "Folders on top", comment: "Folders on top sort option")), (false, String(localized: "navigator.alphabetically", defaultValue: "Alphabetically", comment: "Alphabetically sort option"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "navigator.show-files-matching-name", defaultValue: "Show files with matching name", comment: "Filter help text"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "navigator.show-only-recent-files", defaultValue: "Show only recent files", comment: "Recent files filter help text"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "navigator.show-only-source-control-files", defaultValue: "Show only files with source-control status", comment: "Source control filter help text"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "navigator.add-file", defaultValue: "Add File", comment: "Add file menu item")) {
            ...
            Button(String(localized: "navigator.add-folder", defaultValue: "Add Folder", comment: "Add folder menu item")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Default filename for new files"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "navigator.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Add folder or file accessibility label"))
