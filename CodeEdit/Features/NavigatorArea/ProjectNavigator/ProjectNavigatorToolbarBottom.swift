...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.toolbar-bottom.filter", defaultValue: "Filter", comment: "Label for project navigator filter menu"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.toolbar-bottom.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option to keep folders listed before files")), (false, String(localized: "project-navigator.toolbar-bottom.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option to order items alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.toolbar-bottom.filter.matching-name", defaultValue: "Show files with matching name", comment: "Filter option to display only files matching the search name"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.toolbar-bottom.filter.recent-files", defaultValue: "Show only recent files", comment: "Filter option to display only recently opened files"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.toolbar-bottom.filter.source-control-status", defaultValue: "Show only files with source-control status", comment: "Filter option to display only files that have source control status"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.toolbar-bottom.add-file", defaultValue: "Add File", comment: "Menu action to add a new file from project navigator toolbar")) {
                ...
                do {
                    ...
                    alert.addButton(withTitle: String(localized: "project-navigator.toolbar-bottom.dismiss", defaultValue: "Dismiss", comment: "Button title to dismiss add file sheet"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project-navigator.toolbar-bottom.add-folder", defaultValue: "Add Folder", comment: "Menu action to add a new folder from project navigator toolbar")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    ...
                    let alert = NSAlert(error: error)
                    alert.addButton(withTitle: String(localized: "project-navigator.toolbar-bottom.dismiss", defaultValue: "Dismiss", comment: "Button title to dismiss add folder sheet"))
                    alert.runModal()
                }
            }
        } label: {}
        ...
        .accessibilityLabel(String(localized: "project-navigator.toolbar-bottom.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Toolbar menu action to add either a folder or a file"))
