...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator-toolbar.filter", defaultValue: "Filter", comment: "Filter placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator-toolbar.folders-on-top", defaultValue: "Folders on top", comment: "Folders on top sort option")), (false, String(localized: "project-navigator-toolbar.alphabetically", defaultValue: "Alphabetically", comment: "Alphabetically sort option"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator-toolbar.filter-help", defaultValue: "Show files with matching name", comment: "Filter help text"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator-toolbar.recent-files-help", defaultValue: "Show only recent files", comment: "Recent files filter help text"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator-toolbar.source-control-help", defaultValue: "Show only files with source-control status", comment: "Source control filter help text"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator-toolbar.add-file", defaultValue: "Add File", comment: "Add file button")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project-navigator-toolbar.untitled-file", defaultValue: "untitled", comment: "Untitled file name"),
                        toFile: rootFile
                    ) {
                    ...
                    alert.addButton(withTitle: String(localized: "project-navigator-toolbar.dismiss", defaultValue: "Dismiss", comment: "Dismiss button"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project-navigator-toolbar.add-folder", defaultValue: "Add Folder", comment: "Add folder button")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project-navigator-toolbar.untitled-folder", defaultValue: "untitled", comment: "Untitled folder name"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "project-navigator-toolbar.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Add folder or file accessibility label"))
