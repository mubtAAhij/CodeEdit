...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Placeholder for filter text field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option to show folders before files")), (false, String(localized: "project-navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option to sort alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.filter.help", defaultValue: "Show files with matching name", comment: "Help text for filter button"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.filter.recents", defaultValue: "Show only recent files", comment: "Help text for recent files filter toggle"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.filter.source-control", defaultValue: "Show only files with source-control status", comment: "Help text for source control status filter toggle"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Button to add new file")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Default name for new files"),
                        toFile: rootFile
                    ) {
            ...
            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Button to add new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Default name for new files"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "project-navigator.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Accessibility label for add button"))
