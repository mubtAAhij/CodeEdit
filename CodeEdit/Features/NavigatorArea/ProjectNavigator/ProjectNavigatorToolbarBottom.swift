...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.toolbar-bottom.filter", defaultValue: "Filter", comment: "Menu title for project navigator filter options"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.toolbar-bottom.sort.folders-on-top", defaultValue: "Folders on top", comment: "Option to keep folders listed before files")), (false, String(localized: "project-navigator.toolbar-bottom.sort.alphabetically", defaultValue: "Alphabetically", comment: "Option to sort project navigator entries alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.toolbar-bottom.filter.matching-name", defaultValue: "Show files with matching name", comment: "Filter option to show files with names matching query"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.toolbar-bottom.filter.recent-files", defaultValue: "Show only recent files", comment: "Filter option to show only recently opened files"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.toolbar-bottom.filter.source-control-status", defaultValue: "Show only files with source-control status", comment: "Filter option to show only files with source control changes"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.toolbar-bottom.add-file", defaultValue: "Add File", comment: "Action title for adding a new file")) {
                ...
                do {
                    ...
                    alert.addButton(withTitle: String(localized: "project-navigator.toolbar-bottom.add-file.dismiss", defaultValue: "Dismiss", comment: "Dismiss action for add file dialog"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project-navigator.toolbar-bottom.add-folder", defaultValue: "Add Folder", comment: "Action title for adding a new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    ...
                    let alert = NSAlert(error: error)
                    alert.addButton(withTitle: String(localized: "project-navigator.toolbar-bottom.add-folder.dismiss", defaultValue: "Dismiss", comment: "Dismiss action for add folder dialog"))
                    alert.runModal()
                }
            }
        } label: {}
        ...
        .accessibilityLabel(String(localized: "project-navigator.toolbar-bottom.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Action title for adding either a folder or a file"))
