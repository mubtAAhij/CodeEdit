...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "filter", comment: "Filter text field placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "folders_on_top", comment: "Sort option to show folders first")), (false, String(localized: "alphabetically", comment: "Sort option for alphabetical order"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "show_files_with_matching_name", comment: "Help text for filter button"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "show_only_recent_files", comment: "Help text for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "show_only_files_with_source_control_status", comment: "Help text for source control filter"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "add_file", comment: "Menu item to add new file")) {
            ...
            Button(String(localized: "add_folder", comment: "Menu item to add new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
