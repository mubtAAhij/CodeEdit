...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project_navigator.filter", defaultValue: "Filter", comment: "Placeholder text for project navigator filter field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project_navigator.sort.folders_on_top", defaultValue: "Folders on top", comment: "Sort option to show folders before files")), (false, String(localized: "project_navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option to sort items alphabetically"))], id: \.0) { value, title in
                ...
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project_navigator.filter.recents", defaultValue: "Show only recent files", comment: "Help text for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project_navigator.filter.source_control", defaultValue: "Show only files with source-control status", comment: "Help text for source control status filter"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project_navigator.add_file", defaultValue: "Add File", comment: "Menu option to add a new file")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "project_navigator.untitled", defaultValue: "untitled", comment: "Default name for new files"),
                        toFile: rootFile
                    ) {
                    ...
                    alert.addButton(withTitle: String(localized: "project_navigator.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss alert"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project_navigator.add_folder", defaultValue: "Add Folder", comment: "Menu option to add a new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "project_navigator.untitled", defaultValue: "untitled", comment: "Default name for new files"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "project_navigator.add_button_label", defaultValue: "Add Folder or File", comment: "Accessibility label for add button"))
