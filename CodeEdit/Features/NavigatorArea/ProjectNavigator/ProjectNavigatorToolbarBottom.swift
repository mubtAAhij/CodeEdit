...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Project navigator filter field placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.sort.folders-on-top", defaultValue: "Folders on top", comment: "Sort option: folders on top")), (false, String(localized: "project-navigator.sort.alphabetically", defaultValue: "Alphabetically", comment: "Sort option: alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.filter.help", defaultValue: "Show files with matching name", comment: "Help text for filter field"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.filter.recent-files", defaultValue: "Show only recent files", comment: "Help text for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.filter.source-control", defaultValue: "Show only files with source-control status", comment: "Help text for source control filter"))
                    }
        ...
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Menu item to add a new file")) {
                ...
                do {
                    ...
                    alert.addButton(withTitle: String(localized: "general.dismiss", defaultValue: "Dismiss", comment: "Dismiss button"))
                    alert.runModal()
                }
            }
