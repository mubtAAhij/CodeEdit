...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "navigator.project.filter", comment: "Placeholder text for project navigator filter field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "navigator.project.sort.folders_on_top", comment: "Menu option to sort folders on top")), (false, String(localized: "navigator.project.sort.alphabetically", comment: "Menu option to sort alphabetically"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "navigator.project.filter.help", comment: "Help text for filter functionality"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "navigator.project.filter.recent_files", comment: "Help text for recent files filter"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "navigator.project.filter.source_control", comment: "Help text for source control filter"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "navigator.project.add_file", comment: "Menu option to add a new file")) {
            ...
            Button(String(localized: "navigator.project.add_folder", comment: "Menu option to add a new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
        ...
        .accessibilityLabel(String(localized: "navigator.project.add_folder_or_file", comment: "Accessibility label for add button"))
