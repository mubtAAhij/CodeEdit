...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "Filter", comment: "Placeholder for filter text field"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([
                            (true, String(localized: "Folders on top", comment: "Sort option to display folders first")),
                            (false, String(localized: "Alphabetically", comment: "Sort option to display items alphabetically"))
                        ], id: \.0) { value, title in
                    ...
                    .help(String(localized: "Show files with matching name", comment: "Tooltip for filter icon button"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "Show only recent files", comment: "Tooltip for recent files filter toggle"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "Show only files with source-control status", comment: "Tooltip for source control filter toggle"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "Add File", comment: "Menu option to add a new file")) {
            ...
            Button(String(localized: "Add Folder", comment: "Menu option to add a new folder")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
        ...
        .accessibilityLabel(String(localized: "Add Folder or File", comment: "Accessibility label for add button"))
