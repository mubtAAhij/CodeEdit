...
struct ProjectNavigatorToolbarBottom: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            addNewFileButton
            PaneTextField(
                String(localized: "project-navigator.filter", defaultValue: "Filter", comment: "Filter field placeholder"),
                text: $workspace.navigatorFilter,
                leadingAccessories: {
                    FilterDropDownIconButton(menu: {
                        ForEach([(true, String(localized: "project-navigator.folders-on-top", defaultValue: "Folders on top", comment: "Folders on top sort option")), (false, String(localized: "project-navigator.alphabetically", defaultValue: "Alphabetically", comment: "Alphabetically sort option"))], id: \.0) { value, title in
                    ...
                    .help(String(localized: "project-navigator.show-matching-files", defaultValue: "Show files with matching name", comment: "Show files with matching name tooltip"))
                },
                trailingAccessories: {
                    HStack(spacing: 0) {
                        ...
                        .help(String(localized: "project-navigator.show-recent-files", defaultValue: "Show only recent files", comment: "Show only recent files tooltip"))
                        Toggle(isOn: $workspace.sourceControlFilter) {
                            Image(systemName: "plusminus.circle")
                        }
                        .help(String(localized: "project-navigator.show-source-control-files", defaultValue: "Show only files with source-control status", comment: "Show only files with source-control status tooltip"))
                    }
    ...
    private var addNewFileButton: some View {
        Menu {
            Button(String(localized: "project-navigator.add-file", defaultValue: "Add File", comment: "Add File button")) {
                ...
                do {
                    if let newFile = try workspace.workspaceFileManager?.addFile(
                        fileName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Untitled file name"),
                        toFile: rootFile
                    ) {
                    ...
                    alert.addButton(withTitle: String(localized: "alert.dismiss", defaultValue: "Dismiss", comment: "Dismiss button"))
                    alert.runModal()
                }
            }

            Button(String(localized: "project-navigator.add-folder", defaultValue: "Add Folder", comment: "Add Folder button")) {
                let filePathURL = activeTabURL()
                guard let rootFile = workspace.workspaceFileManager?.getFile(filePathURL.path) else { return }
                do {
                    if let newFolder = try workspace.workspaceFileManager?.addFolder(
                        folderName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Untitled file name"),
                        toFile: rootFile
                    ) {
        ...
        .accessibilityLabel(String(localized: "project-navigator.add-folder-or-file", defaultValue: "Add Folder or File", comment: "Add Folder or File accessibility label"))
