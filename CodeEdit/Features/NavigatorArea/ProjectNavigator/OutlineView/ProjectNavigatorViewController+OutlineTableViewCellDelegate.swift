...
extension ProjectNavigatorViewController: OutlineTableViewCellDelegate {
    func moveFile(file: CEWorkspaceFile, to destination: URL) {
        do {
            ...
            alert.addButton(withTitle: String(localized: "project-navigator.outline-table.dismiss-button-title", defaultValue: "Dismiss", comment: "Button title to dismiss the project navigator tooltip or popover"))
            alert.runModal()
        }
    }
