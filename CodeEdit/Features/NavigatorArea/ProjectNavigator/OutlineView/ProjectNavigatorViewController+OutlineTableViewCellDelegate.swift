...
extension ProjectNavigatorViewController: OutlineTableViewCellDelegate {
    func moveFile(file: CEWorkspaceFile, to destination: URL) {
        do {
            ...
            alert.addButton(withTitle: String(localized: "project-navigator.move-error.dismiss", defaultValue: "Dismiss", comment: "Dismiss button in move file error alert"))
            alert.runModal()
        }
    }
    ...
    func copyFile(file: CEWorkspaceFile, to destination: URL) {
        do {
            ...
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project-navigator.copy-error.dismiss", defaultValue: "Dismiss", comment: "Dismiss button in copy file error alert"))
            alert.runModal()
        }
    }
