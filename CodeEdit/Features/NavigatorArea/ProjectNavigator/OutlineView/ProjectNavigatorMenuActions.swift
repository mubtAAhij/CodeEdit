...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Untitled file name"), toFile: item) {
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "file.untitled", defaultValue: "untitled", comment: "Untitled file name"), toFile: item) {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "file.new-folder-with-items", defaultValue: "New Folder With Items", comment: "New folder with items name"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "file.new-folder-with-items-numbered", defaultValue: "New Folder With Items %d", comment: "New folder with items numbered name"), folderNumber))
        }
