...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "project_navigator.new_file.default_name", defaultValue: "untitled", comment: "Default name for new file"), toFile: item) {
                workspace?.listenerModel.highlightedFileItem = newFile
                workspace?.editorManager?.openTab(item: newFile)
            }
        } catch {
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project_navigator.alert.dismiss", defaultValue: "Dismiss", comment: "Dismiss alert button"))
            alert.runModal()
        }
    }
    ...
    func newFileFromClipboard() {
        ...
        do {
            ...
            if let clipBoardContent, !clipBoardContent.isEmpty, let newFile = try workspace?
                ...
                .addFile(
                    fileName: String(localized: "project_navigator.new_file.default_name", defaultValue: "untitled", comment: "Default name for new file"),
                    toFile: item,
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "project_navigator.new_folder.default_name", defaultValue: "untitled", comment: "Default name for new folder"), toFile: item) {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "project_navigator.new_folder_with_items.default_name", defaultValue: "New Folder With Items", comment: "Default name for new folder with items"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project_navigator.new_folder_with_items.numbered_name", defaultValue: "New Folder With Items %d", comment: "Numbered name for new folder with items"), folderNumber))
        }
