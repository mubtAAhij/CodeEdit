...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "file.default.untitled.name", defaultValue: "untitled", comment: "Default name for new untitled file"), toFile: item) {
                workspace?.listenerModel.highlightedFileItem = newFile
                workspace?.editorManager?.openTab(item: newFile)
            }
        } catch {
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project-navigator.alert.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss alert"))
            alert.runModal()
        }
    }
    ...
    func newFileFromClipboard() {
        ...
        do {
            ...
            if let clipBoardContent, !clipBoardContent.isEmpty, let newFile = try workspace?
                ...
                .addFile(
                    fileName: String(localized: "file.default.untitled.name", defaultValue: "untitled", comment: "Default name for new untitled file"),
                    toFile: item,
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "folder.default.untitled.name", defaultValue: "untitled", comment: "Default name for new untitled folder"), toFile: item) {
