...
extension ProjectNavigatorMenu {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "project-navigator.new-folder-with-items.name", defaultValue: "New Folder With Items", comment: "New folder with items name"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project-navigator.new-folder-with-items.numbered-name", defaultValue: "New Folder With Items %d", comment: "Numbered new folder with items name"), folderNumber))
        }
