...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            ...
            alert.addButton(withTitle: String(localized: "project-navigator.alert.dismiss", defaultValue: "Dismiss", comment: "Dismiss button in error alert"))
            alert.runModal()
        }
    }
    ...
    func newFolderFromSelection() {
        ...
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project-navigator.new-folder-with-items-numbered", defaultValue: "New Folder With Items %d", comment: "Default name for numbered folder created from selection"), folderNumber))
        }
