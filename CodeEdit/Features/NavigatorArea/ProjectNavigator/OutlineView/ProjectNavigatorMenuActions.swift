...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "project-navigator.new-file-name", defaultValue: "untitled", comment: "Default name for new file"), toFile: item) {
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "project-navigator.new-folder-name", defaultValue: "untitled", comment: "Default name for new folder"), toFile: item) {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "project-navigator.new-folder-with-items", defaultValue: "New Folder With Items", comment: "Default name for new folder created with items"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project-navigator.new-folder-with-items-numbered", defaultValue: "New Folder With Items %d", comment: "Numbered name for new folder with items"), folderNumber))
        }
