...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Untitled file name"), toFile: item) {
                workspace?.listenerModel.highlightedFileItem = newFile
                workspace?.editorManager?.openTab(item: newFile)
            }
        } catch {
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project-navigator.dismiss", defaultValue: "Dismiss", comment: "Dismiss button"))
            alert.runModal()
        }
    }
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "project-navigator.untitled", defaultValue: "untitled", comment: "Untitled file name"), toFile: item) {
    ...
    func newFolderFromSelection() {
        ...
        /// Get 'New Folder' name.
        var newFolderURL = parent.url.appendingPathComponent(String(localized: "project-navigator.new-folder-with-items", defaultValue: "New Folder With Items", comment: "New folder with items name"), conformingTo: .folder)
        var folderNumber = 0
        while workspaceFileManager.fileManager.fileExists(atPath: newFolderURL.path) {
            folderNumber += 1
            newFolderURL = parent.url.appending(path: String(format: String(localized: "project-navigator.new-folder-with-items-numbered", defaultValue: "New Folder With Items %d", comment: "New folder with items numbered name"), folderNumber))
        }
