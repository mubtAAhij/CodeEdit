...
extension ProjectNavigatorMenu {
    ...
    func newFile() {
        ...
        do {
            if let newFile = try workspace?.workspaceFileManager?.addFile(fileName: String(localized: "project-navigator.menu-actions.default-name.untitled", defaultValue: "untitled", comment: "Default placeholder name when creating a new untitled item."), toFile: item) {
                workspace?.listenerModel.highlightedFileItem = newFile
                workspace?.editorManager?.openTab(item: newFile)
            }
        } catch {
            let alert = NSAlert(error: error)
            alert.addButton(withTitle: String(localized: "project-navigator.menu-actions.dismiss.button", defaultValue: "Dismiss", comment: "Button title to dismiss a project navigator prompt or alert."))
            alert.runModal()
        }
    }
    ...
    func newFileFromClipboard() {
        ...
        do {
            ...
            if let clipBoardContent, !clipBoardContent.isEmpty, let newFile = try workspace?
                ...
                .addFile(
                    fileName: String(localized: "project-navigator.menu-actions.default-name.untitled", defaultValue: "untitled", comment: "Default placeholder name when creating a new untitled item."),
                    toFile: item,
    ...
    func newFolder() {
        guard let item else { return }
        do {
            if let newFolder = try workspace?.workspaceFileManager?.addFolder(folderName: String(localized: "project-navigator.menu-actions.default-name.untitled", defaultValue: "untitled", comment: "Default placeholder name when creating a new untitled item."), toFile: item) {
