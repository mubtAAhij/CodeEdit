...
final class ProjectNavigatorMenu: NSMenu {
    ...
    required init(coder _: NSCoder) {
        fatalError(String(localized: "project-navigator.menu.init-coder-not-implemented", defaultValue: "init(coder:) has not been implemented", comment: "Fatal error message for unavailable initializer"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let duplicate = menuItem(String(format: String(localized: "project-navigator.menu.duplicate-item", defaultValue: "Duplicate %@", comment: "Menu item to duplicate folder or file"), item.isFolder ? String(localized: "project-navigator.menu.folder", defaultValue: "Folder", comment: "Folder label") : String(localized: "project-navigator.menu.file", defaultValue: "File", comment: "File label")), action: #selector(duplicate))
        ...
        let sourceControl = menuItem(String(localized: "project-navigator.menu.source-control", defaultValue: "Source Control", comment: "Source control submenu"), action: nil)
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(localized: "project-navigator.menu.open-as", defaultValue: "Open As", comment: "Open As submenu title"))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.interface-builder-xib", defaultValue: "Interface Builder XIB Document", comment: "Menu item for XIB document type"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.xcode-project", defaultValue: "Xcode Project", comment: "Menu item for Xcode project type"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        let sourceControlMenu = NSMenu(title: String(localized: "project-navigator.menu.source-control", defaultValue: "Source Control", comment: "Source control submenu title"))
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project-navigator.menu.commit-file", defaultValue: "Commit \"%@\"...", comment: "Menu item to commit file"), String(describing: item.fileName())),
            action: nil,
            keyEquivalent: ""
        )
        ...
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.menu.mark-files-resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Menu item to mark selected files as resolved"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.menu.none", defaultValue: "<None>", comment: "Menu item for no selection"), action: nil, keyEquivalent: "")
