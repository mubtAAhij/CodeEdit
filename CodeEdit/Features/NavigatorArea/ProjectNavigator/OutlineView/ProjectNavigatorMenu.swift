...
final class ProjectNavigatorMenu: NSMenu {
    ...
    init(_ sender: ProjectNavigatorViewController) {
        self.sender = sender
        super.init(title: String(localized: "project-navigator.menu.options", defaultValue: "Options", comment: "Title of the project navigator context menu"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let newFileFromClipboard = menuItem(
            String(localized: "project-navigator.menu.new-file-from-clipboard", defaultValue: "New File from Clipboard", comment: "Menu item to create a new file from clipboard"),
        ...
        let itemType = item.isFolder ? String(localized: "project-navigator.menu.item-type.folder", defaultValue: "Folder", comment: "Item type for folder") : String(localized: "project-navigator.menu.item-type.file", defaultValue: "File", comment: "Item type for file")
        let duplicate = menuItem(String(format: String(localized: "project-navigator.menu.duplicate", defaultValue: "Duplicate %@", comment: "Menu item to duplicate file or folder"), itemType), action: #selector(duplicate))
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.open-as.asset-catalog", defaultValue: "Asset Catalog Document", comment: "Menu item to open file as asset catalog document"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.open-as.interface-builder-xib", defaultValue: "Interface Builder XIB Document", comment: "Menu item to open file as Interface Builder XIB document"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project-navigator.menu.source-control.commit", defaultValue: "Commit \"%@\"...", comment: "Menu item to commit file"), String(describing: item.fileName())),
            action: nil,
            keyEquivalent: ""
        )
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.menu.open-as.none", defaultValue: "<None>", comment: "Menu item for none option in open as menu"), action: nil, keyEquivalent: "")
    ...
    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.hex", defaultValue: "Hex", comment: "Menu item to open file as hex"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.quick-look", defaultValue: "Quick Look", comment: "Menu item to quick look file"), action: nil, keyEquivalent: "")
    }
}
