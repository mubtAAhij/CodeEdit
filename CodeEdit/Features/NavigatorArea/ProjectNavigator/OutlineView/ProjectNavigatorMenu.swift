...
final class ProjectNavigatorMenu: NSMenu {
    ...
    init(_ sender: ProjectNavigatorViewController) {
        self.sender = sender
        super.init(title: String(localized: "project_navigator.menu.title", defaultValue: "Options", comment: "Title for project navigator context menu"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let rename = menuItem(String(localized: "project_navigator.menu.rename", defaultValue: "Rename", comment: "Menu item to rename file or folder"), action: #selector(renameFile))
        ...
        let duplicate = menuItem("Duplicate \(item.isFolder ? String(localized: "project_navigator.menu.folder", defaultValue: "Folder", comment: "Label for folder in duplicate menu item") : String(localized: "project_navigator.menu.file", defaultValue: "File", comment: "Label for file in duplicate menu item"))", action: #selector(duplicate))
        ...
        let sourceControl = menuItem(String(localized: "project_navigator.menu.source_control", defaultValue: "Source Control", comment: "Menu item for source control operations"), action: nil)
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(localized: "project_navigator.menu.open_as", defaultValue: "Open As", comment: "Menu item to open file as specific type"))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project_navigator.open_as.asset_catalog", defaultValue: "Asset Catalog Document", comment: "Menu item to open as asset catalog document"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project_navigator.open_as.xib_document", defaultValue: "Interface Builder XIB Document", comment: "Menu item to open as Interface Builder XIB document"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project_navigator.open_as.xcode_project", defaultValue: "Xcode Project", comment: "Menu item to open as Xcode project"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        let sourceControlMenu = NSMenu(title: String(localized: "project_navigator.menu.source_control", defaultValue: "Source Control", comment: "Menu item for source control operations"))
        let fileName = String(describing: item.fileName())
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project_navigator.source_control.commit_file", defaultValue: "Commit \"%@\"...", comment: "Menu item to commit specific file"), fileName),
            action: nil,
            keyEquivalent: ""
        )
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project_navigator.source_control.discard_changes", defaultValue: "Discard Changes...", comment: "Menu item to discard changes"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project_navigator.source_control.add_selected_files", defaultValue: "Add Selected Files", comment: "Menu item to add selected files to source control"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(withTitle: String(localized: "project_navigator.source_control.mark_resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Menu item to mark selected files as resolved"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project_navigator.open_as.none", defaultValue: "<None>", comment: "Menu item for no selection"), action: nil, keyEquivalent: "")
        item.isEnabled = false
        return item
    }

    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project_navigator.open_as.source_code", defaultValue: "Source Code", comment: "Menu item to open as source code"), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project_navigator.open_as.property_list", defaultValue: "Property List", comment: "Menu item to open as property list"), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project_navigator.open_as.ascii_property_list", defaultValue: "ASCII Property List", comment: "Menu item to open as ASCII property list"), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project_navigator.open_as.hex", defaultValue: "Hex", comment: "Menu item to open as hex"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project_navigator.quick_look", defaultValue: "Quick Look", comment: "Menu item for Quick Look preview"), action: nil, keyEquivalent: "")
    }
}
