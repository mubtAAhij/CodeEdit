...
final class ProjectNavigatorMenu: NSMenu {
    ...
    init(_ sender: ProjectNavigatorViewController) {
        ...
        super.init(title: String(
            localized: "navigator.project.context-menu.options.title",
            defaultValue: "Options",
            comment: "Title for project navigator context menu options submenu"
        ))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let showInFinder = menuItem(String(
            localized: "navigator.project.context-menu.show-in-finder.action",
            defaultValue: "Show in Finder",
            comment: "Context menu action to reveal selected item in Finder"
        ), action: #selector(showInFinder))

        let openInTab = menuItem(String(
            localized: "navigator.project.context-menu.open-in-tab.action",
            defaultValue: "Open in Tab",
            comment: "Context menu action to open selected item in a tab"
        ), action: #selector(openInTab))
        let openInNewWindow = menuItem(String(
            localized: "navigator.project.context-menu.open-in-new-window.action",
        ...
        let openExternalEditor = menuItem(String(
            localized: "navigator.project.context-menu.open-with-external-editor.action",
        ...
        let openAs = menuItem(String(
            localized: "navigator.project.context-menu.open-as.submenu",
            defaultValue: "Open As",
            comment: "Context menu submenu title for open-as actions"
        ), action: nil)

        let copyPath = menuItem(String(
            localized: "navigator.project.context-menu.copy-path.action",
            defaultValue: "Copy Path",
            comment: "Context menu action to copy full file path"
        ), action: #selector(copyPath))
        let copyRelativePath = menuItem(String(
            localized: "navigator.project.context-menu.copy-relative-path.action",
        ...
        let showFileInspector = menuItem(String(
            localized: "navigator.project.context-menu.show-file-inspector.action",
            defaultValue: "Show File Inspector",
            comment: "Context menu action to show file inspector"
        ), action: nil)

        let newFile = menuItem(String(
            localized: "navigator.project.context-menu.new-file.action",
            defaultValue: "New File...",
            comment: "Context menu action to create a new file"
        ), action: #selector(newFile))
        let newFileFromClipboard = menuItem(
            String(
                localized: "navigator.project.context-menu.new-file-from-clipboard.action",
                defaultValue: "New File from Clipboard",
                comment: "Context menu action to create new file from clipboard contents"
            ),
        ...
        let delete = menuItem(String(
            localized: "navigator.project.context-menu.delete-immediately.action",
        ...
        let duplicate = menuItem(String(format: String(
            localized: "navigator.project.context-menu.duplicate-item.action",
            defaultValue: "Duplicate %@",
            comment: "Context menu action to duplicate selected folder or file"
        ), (item.isFolder
            ? String(
                localized: "navigator.project.context-menu.duplicate-item.file-type.folder",
                defaultValue: String(
                    localized: "navigator.project.context-menu.duplicate-folder.noun",
                    defaultValue: "Folder",
                    comment: "Noun used when duplicating a folder"
                ),
                comment: "File type label for folder in duplicate-item action"
            )
            : String(
                localized: "navigator.project.context-menu.duplicate-item.file-type.file",
                defaultValue: String(
                    localized: "navigator.project.context-menu.duplicate-file.noun",
                    defaultValue: "File",
                    comment: "Noun used when duplicating a file"
                ),
                comment: "File type label for file in duplicate-item action"
            ))), action: #selector(duplicate))

        let sortByName = menuItem(String(
            localized: "navigator.project.context-menu.sort-by-name.action",
        ...
        let sortByType = menuItem(String(
            localized: "navigator.project.context-menu.sort-by-type.action",
        ...
        let sourceControl = menuItem(String(
            localized: "navigator.project.context-menu.source-control.submenu",
            defaultValue: "Source Control",
            comment: "Context menu submenu title for source control actions"
        ), action: nil)
        ...
        if canCreateFolderFromSelection() {
            items.append(menuItem(String(
                localized: "navigator.project.context-menu.new-folder-from-selection.action",
                defaultValue: "New Folder from Selection",
                comment: "Context menu action to create a new folder from selected items"
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(
            localized: "navigator.project.context-menu.open-as.editor.submenu",
            defaultValue: "Open As",
            comment: "Context menu submenu title for opening file as a specific type"
        ))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(
                    localized: "navigator.project.context-menu.open-as.asset-catalog-document.option",
                    defaultValue: "Asset Catalog Document",
                    comment: "Open As option for asset catalog document type"
                ), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(
                    localized: "navigator.project.context-menu.open-as.interface-builder-xib-document.option",
                    defaultValue: "Interface Builder XIB Document",
                    comment: "Open As option for Interface Builder XIB document type"
                ), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(
                    localized: "navigator.project.context-menu.open-as.xcode-project.option",
                    defaultValue: "Xcode Project",
                    comment: "Open As option for Xcode project document type"
                ), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        let sourceControlMenu = NSMenu(title: String(
            localized: "navigator.project.context-menu.file.source-control.submenu",
            defaultValue: "Source Control",
            comment: "File context menu submenu title for source control actions"
        ))
        sourceControlMenu.addItem(
            withTitle: String(format: String(
                localized: "navigator.project.context-menu.file.source-control.commit-file.action",
                defaultValue: "Commit \"%@\"...",
                comment: "Source control action to commit the selected file"
        ...
        sourceControlMenu.addItem(withTitle: String(
            localized: "navigator.project.context-menu.file.source-control.discard-changes.action",
            defaultValue: "Discard Changes...",
            comment: "Source control action to discard changes in selected file"
        ), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(
            localized: "navigator.project.context-menu.file.source-control.add-selected-files.action",
            defaultValue: "Add Selected Files",
            comment: "Source control action to add selected files"
        ), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(withTitle: String(
            localized: "navigator.project.context-menu.file.source-control.mark-selected-files-resolved.action",
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.none.option",
            defaultValue: "<None>",
            comment: "Open As option representing no explicit file type"
        ), action: nil, keyEquivalent: "")
    ...
    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.source-code.option",
            defaultValue: "Source Code",
            comment: "Open As option for source code file type"
        ), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.property-list.option",
            defaultValue: "Property List",
            comment: "Open As option for property list file type"
        ), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.ascii-property-list.option",
            defaultValue: "ASCII Property List",
            comment: "Open As option for ASCII property list file type"
        ), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.hex.option",
            defaultValue: "Hex",
            comment: "Open As option for hex view"
        ), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(
            localized: "navigator.project.context-menu.open-as.quick-look.option",
            defaultValue: "Quick Look",
            comment: "Open As option for quick look preview"
        ), action: nil, keyEquivalent: "")
    }
}
