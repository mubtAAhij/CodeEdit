...
final class ProjectNavigatorMenu: NSMenu {
    ...
    required init(coder _: NSCoder) {
        fatalError(String(localized: "project.navigator.menu.init-coder-error", defaultValue: "init(coder:) has not been implemented", comment: "Fatal error message for init(coder:)"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let newFileFromClipboard = menuItem(
            String(localized: "project.navigator.menu.new-file-from-clipboard", defaultValue: "New File from Clipboard", comment: "Menu item to create file from clipboard"),
        ...
        let duplicate = menuItem(String(format: String(localized: "project.navigator.menu.duplicate", defaultValue: "Duplicate %@", comment: "Menu item to duplicate file or folder"), item.isFolder ? String(localized: "project.navigator.menu.folder", defaultValue: "Folder", comment: "Folder type label") : String(localized: "project.navigator.menu.file", defaultValue: "File", comment: "File type label")), action: #selector(duplicate))
        ...
        if canCreateFolderFromSelection() {
            items.append(menuItem(String(localized: "project.navigator.menu.new-folder-from-selection", defaultValue: "New Folder from Selection", comment: "Menu item to create folder from selected files"), action: #selector(newFolderFromSelection)))
        }
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project.navigator.menu.open-as.asset-catalog", defaultValue: "Asset Catalog Document", comment: "Menu item to open as asset catalog"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project.navigator.menu.open-as.xib", defaultValue: "Interface Builder XIB Document", comment: "Menu item to open as XIB document"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        sourceControlMenu.addItem(withTitle: String(localized: "project.navigator.menu.source-control.discard", defaultValue: "Discard Changes...", comment: "Menu item to discard changes"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project.navigator.menu.source-control.add", defaultValue: "Add Selected Files", comment: "Menu item to add selected files"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project.navigator.menu.open-as.none", defaultValue: "<None>", comment: "Menu item indicating no option available"), action: nil, keyEquivalent: "")
    ...
    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project.navigator.menu.open-as.hex", defaultValue: "Hex", comment: "Menu item to open as hex"), action: nil, keyEquivalent: "")
    }
