...
final class ProjectNavigatorMenu: NSMenu {
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let duplicateType = item.isFolder ? String(localized: "project-navigator.folder", defaultValue: "Folder", comment: "Folder type") : String(localized: "project-navigator.file", defaultValue: "File", comment: "File type")
        let duplicate = menuItem(String(format: String(localized: "project-navigator.duplicate", defaultValue: "Duplicate %@", comment: "Duplicate menu item"), duplicateType), action: #selector(duplicate))
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.asset-catalog-document", defaultValue: "Asset Catalog Document", comment: "Asset Catalog Document menu item"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.interface-builder-xib-document", defaultValue: "Interface Builder XIB Document", comment: "Interface Builder XIB Document menu item"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.xcode-project", defaultValue: "Xcode Project", comment: "Xcode Project menu item"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project-navigator.commit-file", defaultValue: "Commit \"%@\"...", comment: "Commit file menu item"), String(describing: item.fileName())),
            action: nil,
            keyEquivalent: ""
        )
        ...
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.mark-selected-files-resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Mark Selected Files as Resolved menu item"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.none", defaultValue: "<None>", comment: "None menu item"), action: nil, keyEquivalent: "")
        item.isEnabled = false
        return item
    }

    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.source-code", defaultValue: "Source Code", comment: "Source Code menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.property-list", defaultValue: "Property List", comment: "Property List menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.ascii-property-list", defaultValue: "ASCII Property List", comment: "ASCII Property List menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.hex", defaultValue: "Hex", comment: "Hex menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.quick-look", defaultValue: "Quick Look", comment: "Quick Look menu item"), action: nil, keyEquivalent: "")
    }
}
