...
final class ProjectNavigatorMenu: NSMenu {
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let itemType = item.isFolder ? String(localized: "project-navigator.menu.item-type.folder", defaultValue: "Folder", comment: "Item type: Folder") : String(localized: "project-navigator.menu.item-type.file", defaultValue: "File", comment: "Item type: File")
        let duplicate = menuItem(String(format: String(localized: "project-navigator.menu.duplicate-item", defaultValue: "Duplicate %@", comment: "Menu item to duplicate file or folder"), itemType), action: #selector(duplicate))
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.open-as.asset-catalog", defaultValue: "Asset Catalog Document", comment: "Open As option: Asset Catalog Document"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.open-as.xib", defaultValue: "Interface Builder XIB Document", comment: "Open As option: Interface Builder XIB Document"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.open-as.xcode-project", defaultValue: "Xcode Project", comment: "Open As option: Xcode Project"), action: nil, keyEquivalent: ""))
            }
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.menu.open-as.none", defaultValue: "<None>", comment: "Open As option: None available"), action: nil, keyEquivalent: "")
        item.isEnabled = false
        return item
    }

    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.source-code", defaultValue: "Source Code", comment: "Open As option: Source Code"), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.property-list", defaultValue: "Property List", comment: "Open As option: Property List"), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.ascii-property-list", defaultValue: "ASCII Property List", comment: "Open As option: ASCII Property List"), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.hex", defaultValue: "Hex", comment: "Open As option: Hex"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.open-as.quick-look", defaultValue: "Quick Look", comment: "Open As option: Quick Look"), action: nil, keyEquivalent: "")
    }
}
