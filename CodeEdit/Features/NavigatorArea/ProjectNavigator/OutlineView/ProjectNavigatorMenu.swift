...
final class ProjectNavigatorMenu: NSMenu {
    ...
    init(_ sender: ProjectNavigatorViewController) {
        self.sender = sender
        super.init(title: String(localized: "project-navigator-menu.options", defaultValue: "Options", comment: "Options menu title"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let duplicate = menuItem(String(format: String(localized: "project-navigator-menu.duplicate-item", defaultValue: "Duplicate %@", comment: "Duplicate item menu item"), item.isFolder ? String(localized: "project-navigator-menu.folder", defaultValue: "Folder", comment: "Folder label") : String(localized: "project-navigator-menu.file", defaultValue: "File", comment: "File label")), action: #selector(duplicate))
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(localized: "project-navigator-menu.open-as-submenu", defaultValue: "Open As", comment: "Open as submenu title"))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator-menu.asset-catalog-document", defaultValue: "Asset Catalog Document", comment: "Asset catalog document menu item"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator-menu.interface-builder-xib", defaultValue: "Interface Builder XIB Document", comment: "Interface builder XIB document menu item"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xcodeproj")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator-menu.xcode-project", defaultValue: "Xcode Project", comment: "Xcode project menu item"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        let sourceControlMenu = NSMenu(title: String(localized: "project-navigator-menu.source-control-submenu", defaultValue: "Source Control", comment: "Source control submenu title"))
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project-navigator-menu.commit-file", defaultValue: "Commit \"%@\"...", comment: "Commit file menu item"), String(describing: item.fileName())),
            action: nil,
            keyEquivalent: ""
        )
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator-menu.discard-changes", defaultValue: "Discard Changes...", comment: "Discard changes menu item"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator-menu.add-selected-files", defaultValue: "Add Selected Files", comment: "Add selected files menu item"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator-menu.mark-resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Mark selected files as resolved menu item"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator-menu.none", defaultValue: "<None>", comment: "None menu item"), action: nil, keyEquivalent: "")
        item.isEnabled = false
        return item
    }

    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator-menu.source-code", defaultValue: "Source Code", comment: "Source code menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator-menu.property-list", defaultValue: "Property List", comment: "Property list menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator-menu.ascii-property-list", defaultValue: "ASCII Property List", comment: "ASCII property list menu item"), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator-menu.hex", defaultValue: "Hex", comment: "Hex menu item"), action: nil, keyEquivalent: "")
    }
