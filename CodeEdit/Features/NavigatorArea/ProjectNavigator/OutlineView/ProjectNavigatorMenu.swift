...
final class ProjectNavigatorMenu: NSMenu {
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let openAs = menuItem(String(localized: "project-navigator.menu.open-as", defaultValue: "Open As", comment: "Menu item for Open As submenu"), action: nil)
        ...
        let copyRelativePath = menuItem(String(localized: "project-navigator.menu.copy-relative-path", defaultValue: "Copy Relative Path", comment: "Menu item to copy relative path"), action: #selector(copyRelativePath))
        ...
        let duplicate = menuItem(String(format: String(localized: "project-navigator.menu.duplicate", defaultValue: "Duplicate %@", comment: "Menu item to duplicate file or folder"), item.isFolder ? String(localized: "project-navigator.folder", defaultValue: "Folder", comment: "Folder type label") : String(localized: "project-navigator.file", defaultValue: "File", comment: "File type label")), action: #selector(duplicate))
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(localized: "project-navigator.menu.open-as", defaultValue: "Open As", comment: "Menu item for Open As submenu"))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: "xcassets")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.asset-catalog", defaultValue: "Asset Catalog Document", comment: "Menu item for asset catalog document type"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: "xib")!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.interface-builder-xib", defaultValue: "Interface Builder XIB Document", comment: "Menu item for Interface Builder XIB document type"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        ...
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.menu.mark-resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Menu item to mark files as resolved"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.menu.none", defaultValue: "<None>", comment: "Menu item for no file type"), action: nil, keyEquivalent: "")
    ...
    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.hex", defaultValue: "Hex", comment: "Menu item for hex file view"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.quick-look", defaultValue: "Quick Look", comment: "Menu item for Quick Look view"), action: nil, keyEquivalent: "")
    }
}
