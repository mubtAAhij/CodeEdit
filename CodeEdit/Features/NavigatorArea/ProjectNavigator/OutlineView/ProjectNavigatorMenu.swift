...
final class ProjectNavigatorMenu: NSMenu {
    ...
    required init(coder _: NSCoder) {
        fatalError(String(localized: "project-navigator.menu.init-coder-not-implemented", defaultValue: "init(coder:) has not been implemented", comment: "Fatal error message for unimplemented init(coder:)"))
    }
    ...
    private func setupMenu() { // swiftlint:disable:this function_body_length
        ...
        let openExternalEditor = menuItem(String(localized: "project-navigator.menu.open-with-external-editor", defaultValue: "Open with External Editor", comment: "Menu item to open file with external editor"), action: #selector(openWithExternalEditor))
        ...
        let newFileFromClipboard = menuItem(
            ...
            key: String(localized: "project-navigator.menu.new-file-from-clipboard-key", defaultValue: "v", comment: "Keyboard shortcut key for new file from clipboard")
        )
        ...
        let duplicate = menuItem(String(format: String(localized: "project-navigator.menu.duplicate", defaultValue: "Duplicate %@", comment: "Menu item to duplicate file or folder"), item.isFolder ? String(localized: "project-navigator.menu.folder", defaultValue: "Folder", comment: "Label for folder") : String(localized: "project-navigator.menu.file", defaultValue: "File", comment: "Label for file")), action: #selector(duplicate))
        ...
        if canCreateFolderFromSelection() {
            items.append(menuItem(String(localized: "project-navigator.menu.new-folder-from-selection", defaultValue: "New Folder from Selection", comment: "Menu item to create new folder from selection"), action: #selector(newFolderFromSelection)))
        }
    ...
    private func openAsMenu(item: CEWorkspaceFile) -> NSMenu {
        let openAsMenu = NSMenu(title: String(localized: "project-navigator.menu.open-as", defaultValue: "Open As", comment: "Title for Open As submenu"))
        func getMenusItems() -> ([NSMenuItem], [NSMenuItem]) {
            ...
            if type.conforms(to: UTType(filenameExtension: String(localized: "project-navigator.menu.file-extension.xcassets", defaultValue: "xcassets", comment: "File extension for asset catalogs"))!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.asset-catalog-document", defaultValue: "Asset Catalog Document", comment: "Menu item for asset catalog document type"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: String(localized: "project-navigator.menu.file-extension.xib", defaultValue: "xib", comment: "File extension for XIB files"))!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.interface-builder-xib-document", defaultValue: "Interface Builder XIB Document", comment: "Menu item for Interface Builder XIB document type"), action: nil, keyEquivalent: ""))
            }
            if type.conforms(to: UTType(filenameExtension: String(localized: "project-navigator.menu.file-extension.xcodeproj", defaultValue: "xcodeproj", comment: "File extension for Xcode projects"))!) {
                primaryItems.append(NSMenuItem(title: String(localized: "project-navigator.menu.xcode-project", defaultValue: "Xcode Project", comment: "Menu item for Xcode project document type"), action: nil, keyEquivalent: ""))
            }
    ...
    private func sourceControlMenu(item: CEWorkspaceFile) -> NSMenu {
        let sourceControlMenu = NSMenu(title: String(localized: "project-navigator.menu.source-control", defaultValue: "Source Control", comment: "Title for source control submenu"))
        sourceControlMenu.addItem(
            withTitle: String(format: String(localized: "project-navigator.menu.commit-file", defaultValue: "Commit \"%@\"...", comment: "Menu item to commit a file"), String(describing: item.fileName())),
            action: nil,
            keyEquivalent: ""
        )
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.menu.discard-changes", defaultValue: "Discard Changes...", comment: "Menu item to discard changes"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(.separator())
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.menu.add-selected-files", defaultValue: "Add Selected Files", comment: "Menu item to add selected files to source control"), action: nil, keyEquivalent: "")
        sourceControlMenu.addItem(withTitle: String(localized: "project-navigator.menu.mark-selected-files-resolved", defaultValue: "Mark Selected Files as Resolved", comment: "Menu item to mark selected files as resolved"), action: nil, keyEquivalent: "")
...
extension NSMenuItem {
    fileprivate static func none() -> NSMenuItem {
        let item = NSMenuItem(title: String(localized: "project-navigator.menu.none", defaultValue: "<None>", comment: "Placeholder for no available option"), action: nil, keyEquivalent: "")
        item.isEnabled = false
        return item
    }

    fileprivate static func sourceCode() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.source-code", defaultValue: "Source Code", comment: "Menu item for source code file type"), action: nil, keyEquivalent: "")
    }

    fileprivate static func propertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.property-list", defaultValue: "Property List", comment: "Menu item for property list file type"), action: nil, keyEquivalent: "")
    }

    fileprivate static func asciiPropertyList() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.ascii-property-list", defaultValue: "ASCII Property List", comment: "Menu item for ASCII property list file type"), action: nil, keyEquivalent: "")
    }

    fileprivate static func hex() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.hex", defaultValue: "Hex", comment: "Menu item for hex file type"), action: nil, keyEquivalent: "")
    }

    fileprivate static func quickLook() -> NSMenuItem {
        NSMenuItem(title: String(localized: "project-navigator.menu.quick-look", defaultValue: "Quick Look", comment: "Menu item for Quick Look"), action: nil, keyEquivalent: "")
    }
}
