...
extension ProjectNavigatorViewController: NSOutlineViewDelegate {
    ...
    public func reveal(_ fileItem: CEWorkspaceFile) {
        ...
        if row < 0 {
            let alert = NSAlert()
            alert.messageText = String(localized: "navigator.project.file-not-found", defaultValue: "Could not find file", comment: "Alert message when selected file cannot be found")
            alert.runModal()
            return
        } else {
