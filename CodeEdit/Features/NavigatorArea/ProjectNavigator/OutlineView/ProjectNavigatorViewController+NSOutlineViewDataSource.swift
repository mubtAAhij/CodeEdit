...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "navigator.project.replace-file.message", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Replace file confirmation message"), fileName)
        alert.informativeText = String(localized: "navigator.project.replace-file.warning", defaultValue: "This action is irreversible!", comment: "Replace file irreversible warning")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "navigator.project.replace-file.replace", defaultValue: "Replace", comment: "Replace button"))
        alert.addButton(withTitle: String(localized: "navigator.project.replace-file.cancel", defaultValue: "Cancel", comment: "Cancel button"))
