...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "project-navigator.replace.message", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Alert message when file already exists"), fileName)
        alert.informativeText = String(localized: "project-navigator.replace.warning", defaultValue: "This action is irreversible!", comment: "Warning that replace action cannot be undone")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "project-navigator.replace.button", defaultValue: "Replace", comment: "Replace button title"))
        alert.addButton(withTitle: String(localized: "project-navigator.replace.cancel", defaultValue: "Cancel", comment: "Cancel button title"))
