...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        ...
        alert.messageText = String(
            format: String(
                localized: "file.replace.message",
                defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?",
                comment: "Alert message asking to replace existing file or folder"
            ),
            fileName
        )
        alert.informativeText = String(localized: "file.replace.warning", defaultValue: "This action is irreversible!", comment: "Warning that file replacement cannot be undone")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "file.replace.button", defaultValue: "Replace", comment: "Button to replace existing file"))
        alert.addButton(withTitle: String(localized: "alert.cancel", defaultValue: "Cancel", comment: "Button to cancel the operation"))
