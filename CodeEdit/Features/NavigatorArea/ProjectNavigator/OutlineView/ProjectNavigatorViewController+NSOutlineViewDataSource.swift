...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "project-navigator.replace-file.message", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Alert message when file already exists, %@ is the file name"), fileName)
        alert.informativeText = String(localized: "project-navigator.replace-file.warning", defaultValue: "This action is irreversible!", comment: "Warning that file replacement cannot be undone")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "project-navigator.replace-file.replace", defaultValue: "Replace", comment: "Button to replace existing file"))
        alert.addButton(withTitle: String(localized: "project-navigator.replace-file.cancel", defaultValue: "Cancel", comment: "Button to cancel file replacement"))
