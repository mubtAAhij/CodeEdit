...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: NSLocalizedString("navigator.replace-file-message", comment: "Replace file message"), fileName)
        alert.informativeText = NSLocalizedString("navigator.irreversible-action", comment: "This action is irreversible!")
        alert.alertStyle = .warning
        alert.addButton(withTitle: NSLocalizedString("common.replace", comment: "Replace"))
