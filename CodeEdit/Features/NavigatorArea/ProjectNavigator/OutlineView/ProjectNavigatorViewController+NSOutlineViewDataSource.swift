...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "project-navigator.file-exists", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Alert message when a file or folder with the same name already exists"), fileName)
        alert.informativeText = String(localized: "project-navigator.action-irreversible", defaultValue: "This action is irreversible!", comment: "Warning that the replace action cannot be undone")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "replace", defaultValue: "Replace", comment: "Replace button"))
        alert.addButton(withTitle: String(localized: "cancel", defaultValue: "Cancel", comment: "Cancel button"))
