...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(localized: "project-navigator.replace-file-message", defaultValue: "A file or folder with the name \(fileName) already exists in the destination folder. Do you want to replace it?", comment: "Message asking to replace existing file")
        alert.informativeText = String(localized: "project-navigator.irreversible-action", defaultValue: "This action is irreversible!", comment: "Warning that file replacement cannot be undone")
        alert.alertStyle = .warning
        alert.addButton(withTitle: String(localized: "button.replace", defaultValue: "Replace", comment: "Replace button in file conflict dialog"))
        alert.addButton(withTitle: String(localized: "button.cancel", defaultValue: "Cancel", comment: "Cancel button in file conflict dialog"))
