...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "project-navigator.replace-file-prompt", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Replace file prompt"), fileName)
        alert.informativeText = String(localized: "project-navigator.action-irreversible", defaultValue: "This action is irreversible!", comment: "Action is irreversible warning")
