...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        ...
        alert.messageText = String(
            format: String(localized: "project.navigator.replace.file.message", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Message asking if user wants to replace existing file"),
            fileName
        )
        ...
        alert.addButton(withTitle: String(localized: "project.navigator.replace.file.button", defaultValue: "Replace", comment: "Replace file button"))
        alert.addButton(withTitle: String(localized: "project.navigator.replace.file.cancel", defaultValue: "Cancel", comment: "Cancel button"))
