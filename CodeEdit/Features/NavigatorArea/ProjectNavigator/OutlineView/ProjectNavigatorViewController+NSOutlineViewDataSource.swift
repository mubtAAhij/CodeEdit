...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(format: String(localized: "project.navigator.replace.file.message", defaultValue: "A file or folder with the name %@ already exists in the destination folder. Do you want to replace it?", comment: "Alert message for file replacement"), fileName)
        ...
        alert.addButton(withTitle: String(localized: "alert.cancel", defaultValue: "Cancel", comment: "Cancel button"))
