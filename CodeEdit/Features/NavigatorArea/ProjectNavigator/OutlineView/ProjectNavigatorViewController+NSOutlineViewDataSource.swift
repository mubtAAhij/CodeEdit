...
extension ProjectNavigatorViewController: NSOutlineViewDataSource {
    ...
    func replaceFileDialog(fileName: String) -> Bool {
        let alert = NSAlert()
        alert.messageText = String(localized: "navigator.project.replace-file-message", defaultValue: "A file or folder with the name \(fileName) already exists in the destination folder. Do you want to replace it?", comment: "Alert message asking to replace existing file or folder")
        ...
        alert.addButton(withTitle: String(localized: "navigator.project.replace-button", defaultValue: "Replace", comment: "Button to replace the file"))
        alert.addButton(withTitle: String(localized: "navigator.project.replace-cancel", defaultValue: "Cancel", comment: "Button to cancel file replacement"))
