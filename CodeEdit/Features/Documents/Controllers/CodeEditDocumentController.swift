...
extension NSDocumentController {
    final func openDocument(onCompletion: @escaping (NSDocument?, Bool) -> Void, onCancel: @escaping () -> Void) {
        ...
        dialog.title = String(localized: "document.open-workspace-or-file", defaultValue: "Open Workspace or File", comment: "Open panel title for opening workspace or file")
        ...
        dialog.begin { result in
            if result ==  NSApplication.ModalResponse.OK, let url = dialog.url {
                self.openDocument(withContentsOf: url, display: true) { document, documentWasAlreadyOpen, error in
                    ...
                    guard let document else {
                        let alert = NSAlert()
                        alert.messageText = String(localized: "document.error.failed-to-get", defaultValue: "Failed to get document", comment: "Error message when document cannot be retrieved")
                        alert.runModal()
                        return
                    }
