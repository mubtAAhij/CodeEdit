...
extension NSDocumentController {
    final func openDocument(onCompletion: @escaping (NSDocument?, Bool) -> Void, onCancel: @escaping () -> Void) {
        ...
        dialog.title = String(localized: "dialog.open-workspace-or-file", defaultValue: "Open Workspace or File", comment: "Title for open file dialog")
        ...
        dialog.begin { result in
            if result ==  NSApplication.ModalResponse.OK, let url = dialog.url {
                self.openDocument(withContentsOf: url, display: true) { document, documentWasAlreadyOpen, error in
                    ...
                    guard let document else {
                        let alert = NSAlert()
                        alert.messageText = String(localized: "error.document.failed-to-get", defaultValue: "Failed to get document", comment: "Error message when document cannot be retrieved")
                        alert.runModal()
                        return
                    }
