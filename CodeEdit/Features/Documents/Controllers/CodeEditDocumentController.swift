        ...
        super.openDocument(withContentsOf: url, display: displayDocument) { document, documentWasAlreadyOpen, error in
            MainActor.assumeIsolated {
                if let document {
                    ...
                    let errorMessage = String(format: "%@", error?.localizedDescription ?? String(localized: "error.unknown", defaultValue: "unknown error", comment: "Fallback error message"))
...
extension NSDocumentController {
    final func openDocument(onCompletion: @escaping (NSDocument?, Bool) -> Void, onCancel: @escaping () -> Void) {
        ...
        dialog.title = String(localized: "document.open-workspace-or-file", defaultValue: "Open Workspace or File", comment: "Open panel message")
