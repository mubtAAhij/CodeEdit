...
extension NSDocumentController {
    final func openDocument(onCompletion: @escaping (NSDocument?, Bool) -> Void, onCancel: @escaping () -> Void) {
        ...
        dialog.title = String(localized: "open_workspace_or_file", comment: "Dialog title for opening workspace or file")
                ...
                self.openDocument(withContentsOf: url, display: true) { document, documentWasAlreadyOpen, error in
                    ...
                    guard let document else {
                        ...
                        alert.messageText = NSLocalizedString(
                            String(localized: "failed_to_get_document", comment: "Error message when document cannot be retrieved"),
                            comment: String(localized: "failed_to_get_document", comment: "Error message when document cannot be retrieved")
                        )
                        alert.runModal()
                        return
                    }
