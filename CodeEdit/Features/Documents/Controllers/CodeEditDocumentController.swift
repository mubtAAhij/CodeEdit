        ...
        super.openDocument(withContentsOf: url, display: displayDocument) { document, documentWasAlreadyOpen, error in
            MainActor.assumeIsolated {
                if let document {
                    ...
                    let errorMessage = error?.localizedDescription ?? String(localized: "document.unknown-error", defaultValue: "unknown error", comment: "Unknown error message when opening document fails")
...
extension NSDocumentController {
    final func openDocument(onCompletion: @escaping (NSDocument?, Bool) -> Void, onCancel: @escaping () -> Void) {
        ...
        dialog.title = String(localized: "document.open-workspace-or-file", defaultValue: "Open Workspace or File", comment: "Open workspace or file dialog title")
