...
extension CodeEditWindowController {
    ...
    func registerCommands() {
        CommandManager.shared.addCommand(
            name: String(localized: "command.quick-open.name", defaultValue: "Quick Open", comment: "Quick Open command name"),
            title: String(localized: "command.quick-open.title", defaultValue: "Quick Open", comment: "Quick Open command title"),
            id: "quick_open",
            command: { [weak self] in self?.openQuickly(nil) }
        )

        CommandManager.shared.addCommand(
            name: String(localized: "command.toggle-navigator.name", defaultValue: "Toggle Navigator", comment: "Toggle Navigator command name"),
            title: String(localized: "command.toggle-navigator.title", defaultValue: "Toggle Navigator", comment: "Toggle Navigator command title"),
            id: "toggle_left_sidebar",
            command: { [weak self] in self?.toggleFirstPanel() }
        )

        CommandManager.shared.addCommand(
            name: String(localized: "command.toggle-inspector.name", defaultValue: "Toggle Inspector", comment: "Toggle Inspector command name"),
            title: String(localized: "command.toggle-inspector.title", defaultValue: "Toggle Inspector", comment: "Toggle Inspector command title"),
    ...
    @IBAction func openWorkspaceSettings(_ sender: Any) {
        ...
        if let workspaceSettingsWindow, workspaceSettingsWindow.isVisible {
            ...
            settingsWindow.setAccessibilityTitle(String(localized: "workspace-settings.title", defaultValue: "Workspace Settings", comment: "Workspace Settings window title"))
