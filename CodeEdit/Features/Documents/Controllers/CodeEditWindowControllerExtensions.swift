...
extension CodeEditWindowController {
    ...
    func registerCommands() {
        CommandManager.shared.addCommand(
            name: String(localized: "window.command.quick-open.name", defaultValue: "Quick Open", comment: "Command name for quick open"),
            title: String(localized: "window.command.quick-open.title", defaultValue: "Quick Open", comment: "Command title for quick open"),
            id: "quick_open",
            command: { [weak self] in self?.openQuickly(nil) }
        )

        CommandManager.shared.addCommand(
            name: String(localized: "window.command.toggle-navigator.name", defaultValue: "Toggle Navigator", comment: "Command name for toggle navigator"),
            title: String(localized: "window.command.toggle-navigator.title", defaultValue: "Toggle Navigator", comment: "Command title for toggle navigator"),
            id: "toggle_left_sidebar",
            command: { [weak self] in self?.toggleFirstPanel() }
        )

        CommandManager.shared.addCommand(
            name: String(localized: "window.command.toggle-inspector.name", defaultValue: "Toggle Inspector", comment: "Command name for toggle inspector"),
            title: String(localized: "window.command.toggle-inspector.title", defaultValue: "Toggle Inspector", comment: "Command title for toggle inspector"),
    ...
    @IBAction func openWorkspaceSettings(_ sender: Any) {
        ...
        if let workspaceSettingsWindow, workspaceSettingsWindow.isVisible {
            ...
            settingsWindow.setAccessibilityTitle(String(localized: "window.workspace-settings.accessibility-title", defaultValue: "Workspace Settings", comment: "Accessibility title for workspace settings window"))
