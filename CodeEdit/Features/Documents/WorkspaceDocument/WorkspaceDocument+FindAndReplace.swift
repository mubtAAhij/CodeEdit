...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "workspace.find-replace.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "Error message when no files match the search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "workspace.find-replace.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "Error message when all files fail to update during replacement"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "workspace.find-replace.partial-success", defaultValue: "%#@updated@ successfully updated, %#@errors@ occurred. Please check logs for more information.", comment: "Error message when some files update successfully but some fail"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "workspace.find-replace.error-title", defaultValue: "Error", comment: "Error alert title when reading file fails")
            alert.informativeText = String(format: String(localized: "workspace.find-replace.read-error", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error message when reading file contents fails"), file.path)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "workspace.find-replace.ok-button", defaultValue: "OK", comment: "OK button title in error alert"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "workspace.find-replace.error-title", defaultValue: "Error", comment: "Error alert title when reading file fails")
            alert.informativeText = String(format: String(localized: "workspace.find-replace.write-error", defaultValue: "An error occurred while writing to: %@", comment: "Error message when writing to file fails"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "workspace.find-replace.ok-button", defaultValue: "OK", comment: "OK button title in error alert"))
            alert.runModal()
        }
