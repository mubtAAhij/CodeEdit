...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "workspace.replace.no-matches", defaultValue: "No files in the workspace matched: \(query)", comment: "Error message when no files match for replacement")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "workspace.replace.all-failed", defaultValue: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information", comment: "Error message when all file replacements fail")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "workspace.replace.partial-success", defaultValue: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.", comment: "Error message when some file replacements fail")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "workspace.replace.error.read-title", defaultValue: "Error", comment: "Error alert title for reading file")
            alert.informativeText = String(localized: "workspace.replace.error.read-message", defaultValue: "An error occurred while reading file contents of: \(file)", comment: "Error message when reading file contents fails")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "workspace.replace.error.ok", defaultValue: "OK", comment: "OK button in error alert"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "workspace.replace.error.write-title", defaultValue: "Error", comment: "Error alert title for writing file")
            alert.informativeText = String(localized: "workspace.replace.error.write-message", defaultValue: "An error occurred while writing to: \(error.localizedDescription)", comment: "Error message when writing file fails")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "workspace.replace.error.ok", defaultValue: "OK", comment: "OK button in error alert"))
            alert.runModal()
        }
