...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "findAndReplace.noFilesMatched", comment: "Error message", defaultValue: "No files in the workspace matched: \(query)")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "findAndReplace.allFilesFailed", comment: "Error message", defaultValue: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "findAndReplace.partialSuccess", comment: "Error message", defaultValue: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "alert.error", comment: "Error message")
            alert.informativeText = String(localized: "findAndReplace.readError", comment: "Error message", defaultValue: "An error occurred while reading file contents of: \(file)")
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "alert.error", comment: "Error message")
            alert.informativeText = String(localized: "findAndReplace.writeError", comment: "Error message", defaultValue: "An error occurred while writing to: \(error.localizedDescription)")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "alert.ok", comment: "Button text"))
            alert.runModal()
        }
