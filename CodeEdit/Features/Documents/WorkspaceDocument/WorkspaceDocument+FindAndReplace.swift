...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "No files in the workspace matched: \(query)", comment: "Error message when no files match the search query during find and replace")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information", comment: "Error message when all files fail to update during find and replace")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.", comment: "Error message when some files fail to update during find and replace")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "Error", comment: "Error alert title")
            alert.informativeText = String(localized: "An error occurred while reading file contents of: \(file)", comment: "Error message when file contents cannot be read")
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "Error", comment: "Error alert title")
            alert.informativeText = String(localized: "An error occurred while writing to: \(error.localizedDescription)", comment: "Error message when file write fails")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "OK", comment: "OK button title"))
            alert.runModal()
        }
