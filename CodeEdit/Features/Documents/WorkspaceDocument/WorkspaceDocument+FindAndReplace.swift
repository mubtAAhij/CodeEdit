...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find.no-files-matched", defaultValue: "No files in the workspace matched: %@", comment: "No files matched search query error"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find.all-files-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "All files failed to update error"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find.partial-success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Partial success with errors message"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            ...
            alert.informativeText = String(format: String(localized: "find.error-reading-file", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error reading file contents"), file.path)
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "common.error", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(format: String(localized: "find.error-writing-file", defaultValue: "An error occurred while writing to: %@", comment: "Error writing to file"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        }
