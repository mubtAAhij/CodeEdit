...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "No files in the workspace matched: \(query)")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            ...
            alert.informativeText = String(localized: "An error occurred while reading file contents of: \(file)")
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "Error")
            alert.informativeText = String(localized: "An error occurred while writing to: \(error.localizedDescription)")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "OK"))
            alert.runModal()
        }
