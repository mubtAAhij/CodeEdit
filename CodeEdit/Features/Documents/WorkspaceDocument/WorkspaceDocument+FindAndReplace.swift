...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find_replace.no_matches", defaultValue: "No files in the workspace matched: %@", comment: "No files matched search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find_replace.all_failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "All files failed to update with error count"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find_replace.partial_success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Partial success message with updated and error counts"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "find_replace.error", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(format: String(localized: "find_replace.read_error", defaultValue: "An error occurred while reading file contents of: %@", comment: "File read error message"), file.path)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find_replace.ok", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find_replace.error", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(format: String(localized: "find_replace.write_error", defaultValue: "An error occurred while writing to: %@", comment: "File write error message"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "find_replace.ok", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        }
