...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(format: String(localized: "find-replace.no-matches", defaultValue: "No files in the workspace matched: %@", comment: "No files matched search query"), query)))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.all-failed", defaultValue: "All files failed to update. (%d) errors occurred. Check logs for more information", comment: "All files failed to update"), errorCount)
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(format: String(localized: "find-replace.partial-success", defaultValue: "%d successfully updated, %d errors occurred. Please check logs for more information.", comment: "Partial success with errors"), updatedFilesCount, errorCount)
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            ...
            alert.informativeText = String(format: String(localized: "find-replace.error.read-file", defaultValue: "An error occurred while reading file contents of: %@", comment: "Error reading file contents"), file.absoluteString)
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "find-replace.error.title", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(format: String(localized: "find-replace.error.write-file", defaultValue: "An error occurred while writing to: %@", comment: "Error writing file"), error.localizedDescription)
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "common.ok", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        }
