...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "find_replace_no_matches", arguments: [query], comment: "Message when no files match the search query")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find_replace_all_failed", arguments: [errorCount], comment: "Message when all files fail to update during find and replace")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find_replace_partial_success", arguments: [updatedFilesCount, errorCount], comment: "Message when some files succeed and some fail during find and replace")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "error_title", comment: "Generic error dialog title")
            alert.informativeText = String(localized: "file_read_error_message", arguments: [file.absoluteString], comment: "Error message when file reading fails")
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "error_title", comment: "Generic error dialog title")
            alert.informativeText = String(localized: "error_writing_to_file", comment: "Error message when writing to file fails")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "ok", comment: "OK button"))
            alert.runModal()
        }
