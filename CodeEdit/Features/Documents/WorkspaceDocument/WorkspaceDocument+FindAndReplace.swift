...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "find-replace.no-files-matched", defaultValue: "No files in the workspace matched: \(query)", comment: "Error message when no files matched search query")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find-replace.all-failed", defaultValue: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information", comment: "Error message when all files failed to update")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find-replace.partial-success", defaultValue: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.", comment: "Error message when some files updated successfully but some failed")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "error.title", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(localized: "find-replace.read-error", defaultValue: "An error occurred while reading file contents of: \(file)", comment: "Error message when reading file fails")
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "error.title", defaultValue: "Error", comment: "Error alert title")
            alert.informativeText = String(localized: "find-replace.write-error", defaultValue: "An error occurred while writing to: \(error.localizedDescription)", comment: "Error message when writing file fails")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "button.ok", defaultValue: "OK", comment: "OK button"))
            alert.runModal()
        }
