...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "find.no_files_matched \(query)", comment: "Error when no files matched search")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find.all_files_failed \(errorCount)", comment: "Error when all files failed to update")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find.partial_update \(updatedFilesCount) \(errorCount)", comment: "Error when some files updated with errors")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "errors.error", comment: "Error title")
            alert.informativeText = String(localized: "errors.reading_file \(file)", comment: "Error reading file contents")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "actions.ok", comment: "OK button"))
            alert.runModal()
        ...
        do {
            ...
            let alert = NSAlert()
            alert.messageText = String(localized: "errors.error", comment: "Error title")
            alert.informativeText = String(localized: "errors.writing_file \(error.localizedDescription)", comment: "Error writing to file")
            alert.alertStyle = .critical
            alert.addButton(withTitle: String(localized: "actions.ok", comment: "OK button"))
            alert.runModal()
        }
