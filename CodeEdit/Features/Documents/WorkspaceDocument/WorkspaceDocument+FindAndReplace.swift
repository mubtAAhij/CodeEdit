...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(.failed(errorMessage: String(localized: "find.replace.no-matches", defaultValue: "No files in the workspace matched: \(query)", comment: "Error when no files match search query")))
        } else if updatedFilesCount == 0 && errorCount > 0 {
            // All files failed to updated
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find.replace.all-failed", defaultValue: "All files failed to update. (\(errorCount)) errors occurred. Check logs for more information", comment: "Error when all file updates fail")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: String(localized: "find.replace.partial-success", defaultValue: "\(updatedFilesCount) successfully updated, \(errorCount) errors occurred. Please check logs for more information.", comment: "Message when some files update successfully and some fail")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            ...
            alert.addButton(withTitle: String(localized: "button.ok", defaultValue: "OK", comment: "OK button for error alert"))
            alert.runModal()
