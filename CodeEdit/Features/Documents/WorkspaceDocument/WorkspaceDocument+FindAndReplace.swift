...
extension WorkspaceDocument.SearchState {
    ...
    func findAndReplace(query: String, replacingTerm: String) async throws {
        ...
        if updatedFilesCount == 0 && errorCount == 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: "String(localized: "all_files_failed_update_count", comment: "Error message when all files fail to update with error count") " +
                    String(localized: "errors_occurred_check_logs", comment: "Error message instructing user to check logs")
                )
            )
        } else if updatedFilesCount > 0 && errorCount > 0 {
            ...
            await setStatus(
                .failed(
                    errorMessage: "String(localized: "files_successfully_updated_count", comment: "Message showing count of successfully updated files") " +
                    String(localized: "error_count_occurred_check_logs", comment: "Error message with count and instruction to check logs")
                )
            )
        } else {
    ...
    func replaceRange(
        ...
        guard let fileContent = try? String(contentsOf: file, encoding: .utf8) else {
            let alert = NSAlert()
            alert.messageText = String(localized: "error", comment: "Error dialog title")
