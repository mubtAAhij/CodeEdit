...
extension WorkspaceDocument.SearchState {
    ...
    func search(_ query: String) async {
        ...
        guard let indexer = indexer else {
            await setStatus(.failed(errorMessage: "String(localized: "no_index_rebuild", comment: "Error message when search index is not available")"))
            return
        }
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let fileContent = String(bytes: data, encoding: .utf8) else {
            await setStatus(.failed(errorMessage: "String(localized: "failed_decode_content", comment: "Error message when file content cannot be decoded")"))
            return
        }
