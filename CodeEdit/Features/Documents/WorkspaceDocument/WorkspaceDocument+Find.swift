...
extension WorkspaceDocument.SearchState {
    ...
    func search(_ query: String) async {
        ...
        guard let indexer = indexer else {
            await setStatus(.failed(errorMessage: String(localized: "search.no_index_found", defaultValue: "No index found. Try rebuilding the index.", comment: "Error message")))
            return
        }
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let fileContent = String(bytes: data, encoding: .utf8) else {
            await setStatus(.failed(errorMessage: String(localized: "search.failed_decode_file", defaultValue: "Failed to decode file content.", comment: "Error message")))
            return
        }
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "search.invalid_regex", defaultValue: "Invalid regular expression.", comment: "Error message")))
            return
        }
