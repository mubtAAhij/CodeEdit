...
extension WorkspaceDocument.SearchState {
    ...
    func search(_ query: String) async {
        ...
        guard let indexer = indexer else {
            await setStatus(.failed(errorMessage: String(localized: "search.error.no-index", defaultValue: "No index found. Try rebuilding the index.", comment: "Error message when search index is not found")))
            return
        }
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let fileContent = String(bytes: data, encoding: .utf8) else {
            await setStatus(.failed(errorMessage: String(localized: "search.error.decode-failed", defaultValue: "Failed to decode file content.", comment: "Error message when file content cannot be decoded")))
            return
        }
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "search.error.invalid-regex", defaultValue: "Invalid regular expression.", comment: "Error message when regular expression is invalid")))
            return
        }
