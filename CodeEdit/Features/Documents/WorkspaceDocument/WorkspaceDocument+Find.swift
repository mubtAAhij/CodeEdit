...
extension WorkspaceDocument.SearchState {
    ...
    func search(_ query: String) async {
        ...
        guard let indexer = indexer else {
            await setStatus(.failed(errorMessage: String(localized: "no_index_found_rebuild", comment: "Error message when search index is not found")))
            return
        }
    ...
    private func evaluateFile(query: String, searchResult: inout SearchResultModel) async {
        ...
        guard let fileContent = String(bytes: data, encoding: .utf8) else {
            await setStatus(.failed(errorMessage: String(localized: "failed_decode_file_content", comment: "Error message when file content cannot be decoded")))
            return
        }
        ...
        guard let regex = try? NSRegularExpression(
            ...
            await setStatus(.failed(errorMessage: String(localized: "invalid_regular_expression", comment: "Error message when regular expression is invalid")))
            return
        }
