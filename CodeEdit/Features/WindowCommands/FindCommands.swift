...
struct FindCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "find.menu", defaultValue: "Find", comment: "Find menu title")) {
            Group {
                Button(String(localized: "find.show-panel", defaultValue: "Find...", comment: "Find panel menu item")) {
                    send(.showFindPanel)
                }
                .keyboardShortcut("f")

                Button(String(localized: "find.show-replace-panel", defaultValue: "Find and Replace...", comment: "Find and replace panel menu item")) {
                    send(.init(rawValue: 12)!)
                }
                .keyboardShortcut("f", modifiers: [.option, .command])

                Button(String(localized: "find.next", defaultValue: "Find Next", comment: "Find next occurrence menu item")) {
                    send(.next)
                }
                .keyboardShortcut("g")

                Button(String(localized: "find.previous", defaultValue: "Find Previous", comment: "Find previous occurrence menu item")) {
                    send(.previous)
                }
