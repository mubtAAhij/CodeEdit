    ...
    var body: some Commands {
        CommandMenu(String(localized: "find-menu.title", defaultValue: "Find", comment: "Find menu title")) {
            Group {
                ...
                Button(String(localized: "find-menu.find-replace", defaultValue: "Find and Replace...", comment: "Find and replace menu item")) {
                    send(.init(rawValue: 12)!)
                }
                .keyboardShortcut("f", modifiers: [.option, .command])

                Button(String(localized: "find-menu.find-next", defaultValue: "Find Next", comment: "Find next menu item")) {
                    send(.next)
                }
                .keyboardShortcut("g")

                Button(String(localized: "find-menu.find-previous", defaultValue: "Find Previous", comment: "Find previous menu item")) {
                    send(.previous)
                }
