    ...
    var body: some Commands {
        CommandMenu(String(localized: "menu.tasks", defaultValue: "Tasks", comment: "Tasks menu title")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                ...
                String(localized: "task.no-selected-task", defaultValue: "(No Selected Task)", comment: "Placeholder when no task is selected")
            }

            Button(String(localized: "menu.tasks.run-task", defaultValue: "Run \(selectedTaskName)", comment: "Menu item to run selected task"), systemImage: "play.fill") {
            ...
            Button(String(localized: "menu.tasks.stop-task", defaultValue: "Stop \(selectedTaskName)", comment: "Menu item to stop selected task"), systemImage: "stop.fill") {
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Menu item to create tasks")) {
                        openSettings()
                    }
                }
            } label: {
            ...
            Button(String(localized: "task.manage-tasks", defaultValue: "Manage Tasks...", comment: "Menu item to manage tasks")) {
                openSettings()
            }
