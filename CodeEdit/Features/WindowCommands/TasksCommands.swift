...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "tasks.menu", defaultValue: "Tasks", comment: "Menu title")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                "\"" + selectedTask.name + "\""
            } else {
                String(localized: "tasks.no_selected", defaultValue: "(No Selected Task)", comment: "No selected task label")
            }

            Button(String(localized: "tasks.run \(selectedTaskName)", defaultValue: "Run \(selectedTaskName)", comment: "Button text"), systemImage: "play.fill") {
            ...
            Button(String(localized: "tasks.stop \(selectedTaskName)", defaultValue: "Stop \(selectedTaskName)", comment: "Button text"), systemImage: "stop.fill") {
            ...
            Button(String(localized: "tasks.show_output \(selectedTaskName)", defaultValue: "Show \(selectedTaskName) Output", comment: "Button text")) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "tasks.create", defaultValue: "Create Tasks", comment: "Button text")) {
                        openSettings()
                    }
                }
            } label: {
                Text("tasks.choose", comment: "Menu label")
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "tasks.manage", defaultValue: "Manage Tasks...", comment: "Button text")) {
                openSettings()
            }
