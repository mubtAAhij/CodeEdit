    ...
    var body: some Commands {
        CommandMenu(String(localized: "menu.tasks", defaultValue: "Tasks", comment: "Tasks menu")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                ...
                String(localized: "menu.tasks.no-selected-task", defaultValue: "(No Selected Task)", comment: "No selected task label")
            }

            Button(String(format: String(localized: "menu.tasks.run", defaultValue: "Run %@", comment: "Run task menu item"), selectedTaskName), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "menu.tasks.stop", defaultValue: "Stop %@", comment: "Stop task menu item"), selectedTaskName), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "menu.tasks.show-output", defaultValue: "Show %@ Output", comment: "Show task output menu item"), selectedTaskName)) {
                showOutput()
            }
            ...
            Menu {
                ...
                Text(String(localized: "menu.tasks.choose-task", defaultValue: "Choose Task...", comment: "Choose task menu label"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "task.manage-tasks", defaultValue: "Manage Tasks...", comment: "Manage tasks menu item")) {
                openSettings()
            }
