...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "window-commands.tasks.menu-title", defaultValue: "Tasks", comment: "Top-level tasks command menu title")) {
            ...
            Button(String(format: String(localized: "window-commands.tasks.run-selected-task", defaultValue: "Run %@", comment: "Menu item to run the selected task"), "\(selectedTaskName)"), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.stop-selected-task", defaultValue: "Stop %@", comment: "Menu item to stop the selected task"), "\(selectedTaskName)"), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.show-selected-task-output", defaultValue: "Show %@ Output", comment: "Menu item to show output for the selected task"), "\(selectedTaskName)")) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "window-commands.tasks.create-tasks", defaultValue: "Create Tasks", comment: "Menu item to create tasks configuration")) {
                        openSettings()
                    }
                }
            } label: {
