...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "window-commands.tasks.menu-title", defaultValue: "Tasks", comment: "Tasks command menu title")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                "\"" + selectedTask.name + "\""
            } else {
                String(localized: "window-commands.tasks.no-selected-task", defaultValue: "(No Selected Task)", comment: "Placeholder title shown when no task is selected")
            }

            Button(String(format: String(localized: "window-commands.tasks.run-selected", defaultValue: "Run %@", comment: "Menu item title to run the selected task"), "\(selectedTaskName)"), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.stop-selected", defaultValue: "Stop %@", comment: "Menu item title to stop the selected task"), "\(selectedTaskName)"), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.show-selected-output", defaultValue: "Show %@ Output", comment: "Menu item title to show output for the selected task"), "\(selectedTaskName)")) {
                showOutput()
            }
