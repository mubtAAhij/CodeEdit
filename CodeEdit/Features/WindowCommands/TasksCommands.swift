    ...
    var body: some Commands {
        CommandMenu(String(localized: "window-commands.tasks.menu-title", defaultValue: "Tasks", comment: "Title of the Tasks command menu")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                ...
                String(localized: "window-commands.tasks.no-selected-task", defaultValue: "(No Selected Task)", comment: "Placeholder task name when no task is selected")
            }

            Button(
                String(
                    format: String(
                        localized: "window-commands.tasks.run",
                        defaultValue: "Run %@",
                        comment: "Command title to run the selected task"
                    ),
                    selectedTaskName
                ),
            ...
            Button(
                String(
                    format: String(
                        localized: "window-commands.tasks.stop",
                        defaultValue: "Stop %@",
                        comment: "Command title to stop the selected task"
                    ),
                    selectedTaskName
                ),
            ...
            Button(
                String(
                    format: String(
                        localized: "window-commands.tasks.show-output",
                        defaultValue: "Show %@ Output",
                        comment: "Command title to show output for the selected task"
                    ),
                    selectedTaskName
                )
            ) {
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "window-commands.tasks.create-tasks", defaultValue: "Create Tasks", comment: "Command title to create tasks when none are available")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "window-commands.tasks.choose-task", defaultValue: "Choose Task...", comment: "Menu title to choose a task"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "window-commands.tasks.manage-tasks", defaultValue: "Manage Tasks...", comment: "Command title to open task management settings")) {
                openSettings()
            }
