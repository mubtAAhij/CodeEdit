...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "tasks-commands.menu", defaultValue: "Tasks", comment: "Tasks menu title")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                "\"" + selectedTask.name + "\""
            } else {
                String(localized: "tasks-commands.no-selected-task", defaultValue: "(No Selected Task)", comment: "No selected task placeholder")
            }

            Button(String(format: String(localized: "tasks-commands.run", defaultValue: "Run %@", comment: "Run task menu item"), selectedTaskName), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "tasks-commands.stop", defaultValue: "Stop %@", comment: "Stop task menu item"), selectedTaskName), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "tasks-commands.show-output", defaultValue: "Show %@ Output", comment: "Show task output menu item"), selectedTaskName)) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "tasks-commands.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks menu item")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "tasks-commands.choose-task", defaultValue: "Choose Task...", comment: "Choose task menu label"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "tasks-commands.manage-tasks", defaultValue: "Manage Tasks...", comment: "Manage tasks menu item")) {
                openSettings()
            }
