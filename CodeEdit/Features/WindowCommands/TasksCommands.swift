...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "window.tasks.title", defaultValue: "Tasks", comment: "Top-level tasks command menu title")) {
            let selectedTaskName = if let selectedTask = taskManager?.selectedTask {
                "\"" + selectedTask.name + "\""
            } else {
                String(localized: "window.tasks.no-selected-task", defaultValue: "(No Selected Task)", comment: "Placeholder when no task is selected")
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "window.tasks.create-tasks", defaultValue: "Create Tasks", comment: "Command title to create tasks configuration")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "window.tasks.choose-task", defaultValue: "Choose Task...", comment: "Command title to choose a task"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "window.tasks.manage-tasks", defaultValue: "Manage Tasks...", comment: "Command title to manage tasks")) {
                openSettings()
            }
