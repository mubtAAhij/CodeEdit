    ...
    var body: some Commands {
        CommandMenu(String(localized: "menu.tasks", defaultValue: "Tasks", comment: "Tasks menu title")) {
            let selectedTaskName: String = if let selectedTask = taskManager?.selectedTask {
                ...
                String(localized: "menu.tasks.no-selection", defaultValue: "(No Selected Task)", comment: "Placeholder when no task is selected")
            }

            Button(String(format: String(localized: "menu.tasks.run", defaultValue: "Run %@", comment: "Run task command"), selectedTaskName), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "menu.tasks.stop", defaultValue: "Stop %@", comment: "Stop task command"), selectedTaskName), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "menu.tasks.show-output", defaultValue: "Show %@ Output", comment: "Show task output command"), selectedTaskName)) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "menu.tasks.create", defaultValue: "Create Tasks", comment: "Create tasks button")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "menu.tasks.choose", defaultValue: "Choose Task...", comment: "Choose task menu label"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "menu.tasks.manage", defaultValue: "Manage Tasks...", comment: "Manage tasks button")) {
                openSettings()
            }
