...
struct TasksCommands: Commands {
    ...
    var body: some Commands {
        CommandMenu(String(localized: "window-commands.tasks.title", defaultValue: "Tasks", comment: "Top-level Tasks menu title in window commands")) {
            ...
            Button(String(format: String(localized: "window-commands.tasks.run-selected-task", defaultValue: "Run %@", comment: "Menu command to run the selected task"), "\(selectedTaskName)"), systemImage: "play.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.stop-selected-task", defaultValue: "Stop %@", comment: "Menu command to stop the selected task"), "\(selectedTaskName)"), systemImage: "stop.fill") {
            ...
            Button(String(format: String(localized: "window-commands.tasks.show-selected-task-output", defaultValue: "Show %@ Output", comment: "Menu command to show output for the selected task"), "\(selectedTaskName)")) {
                showOutput()
            }
            ...
            Menu {
                ...
                if taskManager?.availableTasks.isEmpty ?? true {
                    Button(String(localized: "window-commands.tasks.create-tasks", defaultValue: "Create Tasks", comment: "Menu command to create tasks configuration")) {
                        openSettings()
                    }
                }
            } label: {
                Text(String(localized: "window-commands.tasks.choose-task", defaultValue: "Choose Task...", comment: "Menu command to choose a task"))
            }
            .disabled(taskManager?.availableTasks.isEmpty == true)

            Button(String(localized: "window-commands.tasks.manage-tasks", defaultValue: "Manage Tasks...", comment: "Menu command to manage tasks")) {
                openSettings()
            }
