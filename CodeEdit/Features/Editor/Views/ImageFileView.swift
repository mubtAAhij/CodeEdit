    ...
    var body: some View {
        if let nsImage = NSImage(contentsOf: imageURL),
           let imageReps = nsImage.representations.first {
            ...
            Text(String(localized: "editor.image-file.cannot-preview-image", defaultValue: "Cannot preview image", comment: "Message shown when an image cannot be previewed"))
        }
    }
