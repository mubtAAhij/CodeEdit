    ...
    var body: some View {
        Group {
            if let fileURL = fileDocument.fileURL {
                ...
                ZStack {
                    Text(String(localized: "editor.error.no.url", defaultValue: "Cannot retrieve URL to the file you opened.", comment: "Error message when file URL cannot be retrieved"))
                }
            }
        }
    }
