...
struct LoadingFileView: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            Spacer()
            ProgressView()
            Text(String(format: String(localized: "editor.loading.opening-file", defaultValue: "Opening %@...", comment: "Loading message shown when opening a file, %@ is the filename"), filename))
            Spacer()
        }
    }
}
