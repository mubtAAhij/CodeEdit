...
struct LoadingFileView: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            Spacer()
            ProgressView()
            Text(String(format: String(localized: "editor.loading-file.opening-filename", defaultValue: "Opening %@...", comment: "Status message while opening a file by name"), "\(filename)"))
            Spacer()
        }
    }
}
