...
struct EditorTabBarTrailingAccessories: View {
    ...
    func editorOptionsMenu(codeFile: CodeFileDocument) -> some View {
        ...
        Button(action: {}, label: { Image(systemName: "slider.horizontal.3") })
            .overlay {
                Menu {
                    Toggle(String(localized: "editor.tab-bar.trailing-accessories.show-minimap", defaultValue: "Show Minimap", comment: "Menu item title to toggle minimap visibility in editor tab bar accessories"), isOn: $showMinimap)
                        .keyboardShortcut("M", modifiers: [.command, .shift, .control])
                    Divider()
                    Toggle(
                        String(localized: "editor.tab-bar.trailing-accessories.wrap-lines", defaultValue: "Wrap Lines", comment: "Menu item title to toggle line wrapping in editor tab bar accessories"),
                        isOn: Binding(
