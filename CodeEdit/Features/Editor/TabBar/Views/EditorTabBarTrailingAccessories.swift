...
struct EditorTabBarTrailingAccessories: View {
    ...
    func editorOptionsMenu(codeFile: CodeFileDocument) -> some View {
        ...
        Button(action: {}, label: { Image(systemName: "slider.horizontal.3") })
            .overlay {
                Menu {
                    Toggle(String(localized: "editor.tab-bar.trailing.show-minimap", defaultValue: "Show Minimap", comment: "Menu option to show the minimap in the editor"), isOn: $showMinimap)
                        .keyboardShortcut("M", modifiers: [.command, .shift, .control])
                    Divider()
                    Toggle(
                        String(localized: "editor.tab-bar.trailing.wrap-lines", defaultValue: "Wrap Lines", comment: "Menu option to toggle line wrapping in the editor"),
                        isOn: Binding(
    ...
    var splitviewButton: some View {
        Group {
            ...
            case (.horizontal, true), (.vertical, false):
                ...
                .help(String(localized: "editor.tab-bar.trailing.split-vertically", defaultValue: "Split Vertically", comment: "Menu option to split the editor vertically"))
            ...
            case (.vertical, true), (.horizontal, false):
                ...
                .help(String(localized: "editor.tab-bar.trailing.split-horizontally", defaultValue: "Split Horizontally", comment: "Menu option to split the editor horizontally"))
