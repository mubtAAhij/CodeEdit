...
struct EditorTabBarTrailingAccessories: View {
    ...
    func editorOptionsMenu(codeFile: CodeFileDocument) -> some View {
        ...
        Button(action: {}, label: { Image(systemName: "slider.horizontal.3") })
            .overlay {
                Menu {
                    Toggle(String(localized: "editor.show-minimap", defaultValue: "Show Minimap", comment: "Toggle to show/hide minimap in editor"), isOn: $showMinimap)
                        .keyboardShortcut("M", modifiers: [.command, .shift, .control])
                    Divider()
                    Toggle(
                        String(localized: "editor.wrap-lines", defaultValue: "Wrap Lines", comment: "Toggle to wrap lines in editor"),
                        isOn: Binding(
