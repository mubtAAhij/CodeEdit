...
struct EditorTabBarTrailingAccessories: View {
    ...
    func editorOptionsMenu(codeFile: CodeFileDocument) -> some View {
        ...
        Button(action: {}, label: { Image(systemName: "slider.horizontal.3") })
            .overlay {
                Menu {
                    Toggle("String(localized: "show_minimap", comment: "Toggle option to show/hide minimap in editor")", isOn: $showMinimap)
                        .keyboardShortcut("M", modifiers: [.command, .shift, .control])
                    Divider()
                    Toggle(
                        "String(localized: "wrap_lines", comment: "Toggle option to wrap long lines in editor")",
                        isOn: Binding(
