    ...
    func editorOptionsMenu(codeFile: CodeFileDocument) -> some View {
        ...
        Button(action: {}, label: { Image(systemName: "slider.horizontal.3") })
            .overlay {
                Menu {
                    Toggle(String(
                        localized: "editor.tab-bar.trailing-accessories.show-minimap",
                        defaultValue: "Show Minimap",
                        comment: "Menu item title to show the minimap in editor tab bar trailing accessories"
                    ), isOn: $showMinimap)
                        .keyboardShortcut("M", modifiers: [.command, .shift, .control])
                    Divider()
                    Toggle(
                        String(
                            localized: "editor.tab-bar.trailing-accessories.wrap-lines",
                            defaultValue: "Wrap Lines",
                            comment: "Menu item title to toggle line wrapping in editor tab bar trailing accessories"
                        ),
                        isOn: Binding(
    ...
    var splitviewButton: some View {
        Group {
            ...
            case (.horizontal, true), (.vertical, false):
                ...
                .help(String(
                    localized: "editor.tab-bar.trailing-accessories.split-vertically",
                    defaultValue: "Split Vertically",
                    comment: "Menu item title to split the editor vertically"
                ))
            ...
            case (.vertical, true), (.horizontal, false):
                ...
                .help(String(
                    localized: "editor.tab-bar.trailing-accessories.split-horizontally",
                    defaultValue: "Split Horizontally",
                    comment: "Menu item title to split the editor horizontally"
                ))
