            ...
            let (channel, process) = try DataChannel.localProcessChannel(
                ...
                terminationHandler: { [weak logContainer] in
                    ...
                    logContainer?.appendLog(
                        LogMessageParams(type: .error, message: String(localized: "lsp.language-server.data-channel-unexpected-termination", defaultValue: "Data Channel Terminated Unexpectedly", comment: "Error title when LSP data channel terminates unexpectedly"))
                    )
                }
            )
