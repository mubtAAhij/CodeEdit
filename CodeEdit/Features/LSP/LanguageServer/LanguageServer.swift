            ...
            let (channel, process) = try DataChannel.localProcessChannel(
                ...
                terminationHandler: { [weak logContainer] in
                    ...
                    logContainer?.appendLog(
                        LogMessageParams(type: .error, message: String(localized: "language-server.channel-terminated", defaultValue: "Data Channel Terminated Unexpectedly", comment: "Error message when data channel terminates unexpectedly"))
                    )
                }
            )
