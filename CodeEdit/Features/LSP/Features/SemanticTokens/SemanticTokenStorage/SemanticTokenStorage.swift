...
final class SemanticTokenStorage: GenericSemanticTokenStorage {
    ...
    func applyDelta(_ deltas: SemanticTokensDelta) -> [SemanticTokenRange] {
        assert(state != nil, String(localized: "lsp.error.state-not-set", defaultValue: "State should be set before applying any deltas.", comment: "Developer fatalError message - should not be localized"))
