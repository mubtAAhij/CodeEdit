        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                ...
                "title": String(format: String(
                    localized: "lsp.registry.registry-manager.removing-package",
                    defaultValue: "Removing %@",
                    comment: "Activity title while removing an installed package"
                ), "\(packageName)")
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(
                    localized: "lsp.registry.registry-manager.installing-package",
                    defaultValue: "Installing %@",
                    comment: "Activity title while installing a package"
                ), "\(activityTitle)"))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(
                    localized: "lsp.registry.registry-manager.could-not-install",
                    defaultValue: "Could not install %@",
                    comment: "Alert title shown when package installation fails"
                ), "\(activityName)"),
                description: String(
                    localized: "lsp.registry.registry-manager.installation-problem",
                    defaultValue: "There was a problem during installation.",
                    comment: "Alert message shown when package installation encounters an error"
                ),
                actionButtonTitle: String(
                    localized: "lsp.registry.registry-manager.done",
                    defaultValue: "Done",
                    comment: "Button title to dismiss installation failure alert"
                ),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(
                    localized: "lsp.registry.registry-manager.successfully-installed",
                    defaultValue: "Successfully installed %@",
                    comment: "Success message shown when package installation completes"
                ), "\(activityName)"), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
