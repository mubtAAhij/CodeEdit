    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "registry.removing-package", defaultValue: "Removing %@", comment: "Title for removing package task"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "registry.installing-package", defaultValue: "Installing %@", comment: "Title for installing package task"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "registry.install-failed", defaultValue: "Could not install %@", comment: "Title when package installation fails"), activityName),
                description: String(localized: "registry.install-problem", defaultValue: "There was a problem during installation.", comment: "Description when package installation fails"),
                actionButtonTitle: String(localized: "done", defaultValue: "Done", comment: "Done button"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "registry.install-success", defaultValue: "Successfully installed %@", comment: "Title when package installation succeeds"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
