    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "lsp.registry.removing-package", defaultValue: "Removing %@", comment: "Status message when removing language server package"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "lsp.registry.installing-package", defaultValue: "Installing %@", comment: "Status message when installing language server package"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "lsp.registry.install-failed", defaultValue: "Could not install %@", comment: "Error message when installation fails"), activityName),
                description: String(localized: "lsp.registry.install-problem", defaultValue: "There was a problem during installation.", comment: "Description of installation problem"),
                actionButtonTitle: String(localized: "lsp.registry.done", defaultValue: "Done", comment: "Done button title"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "lsp.registry.install-success", defaultValue: "Successfully installed %@", comment: "Success message when installation completes"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
