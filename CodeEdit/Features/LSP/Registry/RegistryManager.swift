    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "lsp.registry.removing", defaultValue: "Removing %@", comment: "Activity title when removing a language server package"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "lsp.registry.installing", defaultValue: "Installing %@", comment: "Activity title when installing a language server package"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "lsp.registry.install_failed", defaultValue: "Could not install %@", comment: "Error notification title when language server installation fails"), activityName),
                description: String(localized: "lsp.registry.install_failed.description", defaultValue: "There was a problem during installation.", comment: "Error notification description when installation fails"),
                actionButtonTitle: String(localized: "done", defaultValue: "Done", comment: "Button title to dismiss notification"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "lsp.registry.install_success", defaultValue: "Successfully installed %@", comment: "Success notification title when language server installation succeeds"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
