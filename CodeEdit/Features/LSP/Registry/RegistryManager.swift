...
final class RegistryManager: ObservableObject {
    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "registry.removing-package", defaultValue: "Removing %@", comment: "Activity title for removing package"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            // Add to activity viewer
            let versionSuffix = "@" + (method.version ?? "latest")
            let activityTitle = operation.package.name + versionSuffix
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "registry.installing-package", defaultValue: "Installing %@", comment: "Activity title for installing package"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "registry.install-failed", defaultValue: "Could not install %@", comment: "Error title for failed installation"), activityName),
                description: String(localized: "registry.install-problem", defaultValue: "There was a problem during installation.", comment: "Error description for installation problem"),
                actionButtonTitle: String(localized: "registry.done-button", defaultValue: "Done", comment: "Done button title"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "registry.install-success", defaultValue: "Successfully installed %@", comment: "Success title for completed installation"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
