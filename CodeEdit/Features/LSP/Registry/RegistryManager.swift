    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "lsp.install-failed", defaultValue: "Could not install %@", comment: "Installation failed title"), activityName),
                description: String(localized: "lsp.install-problem", defaultValue: "There was a problem during installation.", comment: "Installation problem description"),
                actionButtonTitle: String(localized: "lsp.done", defaultValue: "Done", comment: "Done button"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "lsp.install-success", defaultValue: "Successfully installed %@", comment: "Installation success message"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
