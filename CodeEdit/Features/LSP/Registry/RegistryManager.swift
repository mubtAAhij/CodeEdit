    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "lsp.registry.removing-title", defaultValue: "Removing %@", comment: "Notification title when removing a language server"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "lsp.registry.installing-title", defaultValue: "Installing %@", comment: "Notification title when installing a language server"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "lsp.registry.install-failed-title", defaultValue: "Could not install %@", comment: "Error title when language server installation fails"), activityName),
                description: String(localized: "lsp.registry.install-failed-message", defaultValue: "There was a problem during installation.", comment: "Error message when installation fails"),
                actionButtonTitle: String(localized: "lsp.registry.install-failed-button", defaultValue: "Done", comment: "Button to dismiss installation error"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
