    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": "String(localized: "removing_package", comment: "Status message when removing a language server package")"
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: "String(localized: "installing_package", comment: "Status message when installing a language server package")")
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: "String(localized: "could_not_install_package", comment: "Error notification title when package installation fails")",
                description: "String(localized: "installation_problem_description", comment: "Error notification description for installation failure")",
                actionButtonTitle: "String(localized: "done", comment: "Button title to dismiss notification")",
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: "String(localized: "successfully_installed_package", comment: "Success message when package installation completes")", isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
