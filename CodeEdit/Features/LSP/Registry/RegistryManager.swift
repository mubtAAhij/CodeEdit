    ...
    func removeLanguageServer(packageName: String) async throws {
        ...
        NotificationCenter.default.post(
            ...
            userInfo: [
                "id": packageName,
                "action": "create",
                "title": String(format: String(localized: "lsp.removing-package", defaultValue: "Removing %@", comment: "Removing package status"), packageName)
            ]
        )
    ...
    private func installPackage(operation: PackageManagerInstallOperation, method: InstallationMethod) {
        installTask = Task { [weak self] in
            ...
            TaskNotificationHandler.postTask(
                action: .create,
                model: TaskNotificationModel(id: operation.package.name, title: String(format: String(localized: "lsp.installing-package", defaultValue: "Installing %@", comment: "Installing package status"), activityTitle))
            )
    ...
    private func updateActivityViewer(
        ...
        if failed {
            NotificationManager.shared.post(
                ...
                iconColor: .clear,
                title: String(format: String(localized: "lsp.could-not-install", defaultValue: "Could not install %@", comment: "Could not install package error"), activityName),
                description: String(localized: "lsp.installation-problem", defaultValue: "There was a problem during installation.", comment: "Installation problem description"),
                actionButtonTitle: String(localized: "common.done", defaultValue: "Done", comment: "Done button"),
                action: {},
            )
        } else {
            TaskNotificationHandler.postTask(
                action: .update,
                model: TaskNotificationModel(id: id, title: String(format: String(localized: "lsp.successfully-installed", defaultValue: "Successfully installed %@", comment: "Successfully installed package status"), activityName), isLoading: false)
            )
            NotificationCenter.default.post(
                name: .taskNotification,
                object: nil,
