...
enum RegistryManagerError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .installationRunning:
            String(localized: "registry_manager.error.installation_running", comment: "Error when another package installation is already in progress")
        case .invalidResponse(let statusCode):
            String(localized: "registry_manager.error.invalid_response", arguments: [statusCode], comment: "Error for invalid HTTP response with status code")
        case .downloadFailed(let url, _):
            String(localized: "registry_manager.error.download_failed", arguments: [url], comment: "Error when download fails for a specific URL")
