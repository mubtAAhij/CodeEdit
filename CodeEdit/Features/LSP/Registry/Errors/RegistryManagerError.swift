...
enum RegistryManagerError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .installationRunning:
            String(localized: "lsp.registry.error.installation-running", defaultValue: "A package is already being installed.", comment: "Error message when a package installation is already in progress")
        case .invalidResponse(let statusCode):
            String(localized: "lsp.registry.error.invalid-response-code", defaultValue: "Invalid response received: \(statusCode)", comment: "Error message when an invalid HTTP response code is received")
        case .downloadFailed(let url, _):
            String(localized: "lsp.registry.error.download-failed", defaultValue: "Download for \(url) error.", comment: "Error message when download fails for a URL")
