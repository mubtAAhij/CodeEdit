...
enum RegistryItemTemplateParser {
    ...
    private enum Filter {
        ...
        static func parse(_ filterString: String) throws -> Filter {
            ...
            if components.count >= 2 && components[0] == String(localized: "swift.preflight.literal.412542", defaultValue: "strip_prefix", comment: "") {
    ...
    static func process(template: String, with context: [String: Any]) throws -> String {
        ...
        for match in matches.reversed() {
            ...
            let components = expression.components(separatedBy: String(localized: "swift.preflight.literal.412545", defaultValue: "|", comment: "")).filter { !$0.isEmpty }
    ...
    private static func getValueFromPath(_ path: String, in context: [String: Any]) throws -> String {
        let pathComponents = path.components(separatedBy: String(localized: "swift.preflight.literal.412546", defaultValue: ".", comment: ""))
        ...
        if let stringValue = currentValue as? String {
            ...
            throw TemplateError.invalidPath(String(localized: "swift.preflight.literal.412549", defaultValue: "Path resolves to a complex object, not a simple value", comment: ""))
        } else {
