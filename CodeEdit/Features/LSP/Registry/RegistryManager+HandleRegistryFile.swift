    ...
    func handleUpdateError(_ error: Error) {
        ...
        if let regError = error as? RegistryManagerError {
            ...
            case .invalidResponse(let statusCode):
                logger.error(String(localized: "invalid_response_received", comment: "Error message for invalid HTTP response").replacingOccurrences(of: "{statusCode}", with: "\(statusCode)"))
