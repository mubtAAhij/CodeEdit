    ...
    func handleUpdateError(_ error: Error) {
        ...
        if let regError = error as? RegistryManagerError {
            ...
            case .invalidResponse(let statusCode):
                let code = statusCode
                logger.error(String(localized: "lsp.registry.error.invalid-response", defaultValue: "Invalid response received: \(code)", comment: "Error message when invalid HTTP response is received"))
