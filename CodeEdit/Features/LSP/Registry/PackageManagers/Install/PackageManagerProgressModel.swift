...
final class PackageManagerProgressModel: ObservableObject {
    ...
    func createDirectoryStructure(for packagePath: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: decodedPath) {
            status(String(localized: "removing_existing_installation", comment: "Status message for removing existing installation"))
            try FileManager.default.removeItem(at: packagePath)
        }

        status(String(localized: "creating_directory_format", comment: "Status message for creating directory").replacingOccurrences(of: "{path}", with: decodedPath))
