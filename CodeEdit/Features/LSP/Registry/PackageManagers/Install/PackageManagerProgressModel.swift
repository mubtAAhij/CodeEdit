...
final class PackageManagerProgressModel: ObservableObject {
    ...
    func createDirectoryStructure(for packagePath: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: decodedPath) {
            status(String(localized: "lsp.install.removing-existing", defaultValue: "Removing existing installation.", comment: "Status message when removing existing language server installation"))
            try FileManager.default.removeItem(at: packagePath)
        }

        status(String(format: String(localized: "lsp.install.creating-directory", defaultValue: "Creating directory: %@", comment: "Status message when creating installation directory, %@ is the directory path"), decodedPath))
