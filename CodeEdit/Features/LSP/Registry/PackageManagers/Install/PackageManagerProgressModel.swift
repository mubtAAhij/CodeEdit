...
final class PackageManagerProgressModel: ObservableObject {
    ...
    func createDirectoryStructure(for packagePath: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: decodedPath) {
            status(String(localized: "lsp.removing-installation", defaultValue: "Removing existing installation.", comment: "Progress message when removing existing LSP installation"))
            try FileManager.default.removeItem(at: packagePath)
        }

        status(String(format: String(localized: "lsp.creating-directory", defaultValue: "Creating directory: %@", comment: "Progress message when creating directory"), decodedPath))
