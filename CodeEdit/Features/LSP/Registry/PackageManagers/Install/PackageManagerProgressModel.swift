...
final class PackageManagerProgressModel: ObservableObject {
    ...
    func createDirectoryStructure(for packagePath: URL) throws {
        ...
        if FileManager.default.fileExists(atPath: decodedPath) {
            status(String(localized: "lsp.removing-existing", defaultValue: "Removing existing installation.", comment: "Removing existing installation status"))
            try FileManager.default.removeItem(at: packagePath)
        }

        status(String(format: String(localized: "lsp.creating-directory", defaultValue: "Creating directory: %@", comment: "Creating directory status"), decodedPath))
