...
final class PackageManagerInstallOperation: ObservableObject, Identifiable {
    ...
    private func runNext() async throws {
        ...
        accumulatedOutput.append(OutputItem(isStepDivider: true, contents: String(format: String(localized: "package-manager.install.step", defaultValue: "Step %d: %@", comment: "Installation step divider showing step number and task name"), currentStepIdx + 1, task.name)))
