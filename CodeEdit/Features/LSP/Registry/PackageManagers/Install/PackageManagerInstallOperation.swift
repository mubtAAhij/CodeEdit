...
final class PackageManagerInstallOperation: ObservableObject, Identifiable {
    ...
    private func runNext() async throws {
        ...
        accumulatedOutput.append(OutputItem(isStepDivider: true, contents: String(localized: "installation_step_format", comment: "Installation step progress format").replacingOccurrences(of: "{step}", with: "\(currentStepIdx + 1)").replacingOccurrences(of: "{name}", with: task.name)))
