...
final class PackageManagerInstallOperation: ObservableObject, Identifiable {
    ...
    private func runNext() async throws {
        ...
        accumulatedOutput.append(OutputItem(isStepDivider: true, contents: String(format: String(localized: "lsp.package-install.step-progress", defaultValue: "Step %d: %@", comment: "Progress message showing current installation step and task name"), currentStepIdx + 1, "\(task.name)")))
