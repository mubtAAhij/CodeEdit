...
final class PackageManagerInstallOperation: ObservableObject, Identifiable {
    ...
    private func runNext() async throws {
        ...
        accumulatedOutput.append(OutputItem(isStepDivider: true, contents: String(format: String(localized: "package-manager.install.step-label", defaultValue: "Step %d: %@", comment: "Step label in package manager install operation"), currentStepIdx + 1, task.name)))
