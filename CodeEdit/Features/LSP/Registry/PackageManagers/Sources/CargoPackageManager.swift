...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "cargo.initialize_directory", comment: "Package installation step"),
            confirmation: .none
        ) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "cargo.install_package", comment: "Package installation step"),
            confirmation: .required(
                message: String(
                    localized: "cargo.install_confirmation \(qualifiedPackageName)",
                    comment: "Confirmation message for package installation"
                )
            )
        ) { model in
