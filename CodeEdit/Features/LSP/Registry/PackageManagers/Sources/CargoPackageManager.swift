...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.cargo.initialize_directory_structure", comment: "Cargo initialization step name"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.cargo.install_package", comment: "Cargo install step name"),
            confirmation: .required(
                message: String(localized: "lsp.cargo.install_confirmation \(qualifiedPackageName)", comment: "Cargo install confirmation message")
            )
        ) { model in
