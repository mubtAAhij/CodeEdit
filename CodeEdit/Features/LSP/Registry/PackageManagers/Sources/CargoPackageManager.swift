...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.cargo.init-directory", defaultValue: "Initialize Directory Structure", comment: "Progress message for initializing directory"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.cargo.install-title", defaultValue: "Install Package Using cargo", comment: "Alert title for cargo package installation"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.cargo.install-message", defaultValue: "This requires the cargo package %@.\nAllow CodeEdit to install this package?", comment: "Alert message asking permission to install cargo package"), qualifiedPackageName)
            )
        ) { model in
