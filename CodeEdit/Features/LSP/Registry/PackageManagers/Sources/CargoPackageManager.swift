...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.cargo.initialize-directory-structure", defaultValue: "Initialize Directory Structure", comment: "Confirmation dialog title for creating Cargo package directory structure"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.cargo.install-package-title", defaultValue: "Install Package Using cargo", comment: "Confirmation dialog title for installing package using cargo"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.cargo.install-package-confirmation", defaultValue: "This requires the cargo package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation dialog message asking user to allow installing a Cargo package"), "\(qualifiedPackageName)")
            )
        ) { model in
