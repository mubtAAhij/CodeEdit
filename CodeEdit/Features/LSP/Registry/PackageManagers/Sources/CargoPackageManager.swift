...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "initialize_directory_structure", comment: "Installation step name"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "install_package_using_cargo", comment: "Installation step name"),
            confirmation: .required(
                message: String(localized: "requires_cargo_package", comment: "Installation confirmation message")
                + String(localized: "allow_codeedit_install_package", comment: "Installation confirmation prompt")
            )
        ) { model in
