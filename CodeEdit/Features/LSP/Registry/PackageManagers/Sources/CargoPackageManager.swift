...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.cargo.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure in cargo installation"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.cargo.install-package", defaultValue: "Install Package Using cargo", comment: "Step name for installing package using cargo"),
            confirmation: .required(
                message: String(localized: "lsp.cargo.install-confirmation", defaultValue: "This requires the cargo package \(qualifiedPackageName).\nAllow CodeEdit to install this package?", comment: "Confirmation message for cargo package installation")
            )
        ) { model in
