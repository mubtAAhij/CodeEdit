...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "cargo.initialize.name", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing cargo package directory structure"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "cargo.install.name", defaultValue: "Install Package Using cargo", comment: "Installation step name for installing cargo package"),
            confirmation: .required(
                message: String(format: String(localized: "cargo.install.confirmation-message", defaultValue: "This requires the cargo package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation message for installing cargo package"), qualifiedPackageName)
            )
        ) { model in
