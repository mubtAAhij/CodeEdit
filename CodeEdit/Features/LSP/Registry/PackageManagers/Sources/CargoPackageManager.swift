...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "package.manager.cargo.initialize", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let qualifiedPackageName = String(format: String(localized: "package.manager.cargo.qualified-name", defaultValue: "%@@%@", comment: "Qualified package name with version"), source.pkgName, source.version)

        return PackageManagerInstallStep(
            name: String(localized: "package.manager.cargo.install", defaultValue: "Install Package Using cargo", comment: "Step name for installing cargo package"),
            confirmation: .required(
                message: String(format: String(localized: "package.manager.cargo.install-confirm", defaultValue: "This requires the cargo package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation message for cargo package installation"), qualifiedPackageName)
            )
        ) { model in
