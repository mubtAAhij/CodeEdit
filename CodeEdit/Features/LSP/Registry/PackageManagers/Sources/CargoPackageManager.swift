...
final class CargoPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "package-manager.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Installation step to initialize directory structure"), confirmation: .none) { model in
    ...
    func runCargoInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let qualifiedPackageName = String(format: String(localized: "package-manager.cargo.qualified-package", defaultValue: "%@@@%@", comment: "Qualified package name with version"), source.pkgName, source.version)

        return PackageManagerInstallStep(
            name: String(localized: "package-manager.cargo.install-step", defaultValue: "Install Package Using cargo", comment: "Installation step to install package using cargo"),
            confirmation: .required(
                message: String(format: String(localized: "package-manager.cargo.install-confirmation", defaultValue: "This requires the cargo package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation message to install cargo package"), qualifiedPackageName)
            )
        ) { model in
