...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "lsp.github.requires-git-permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Permission message for allowing git installation")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.github.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing directory"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github.download-binary", defaultValue: "Download Binary Executable", comment: "Installation step name for downloading binary"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "lsp.github.downloading", defaultValue: "Downloading %@", comment: "Status message while downloading package"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.github.http-error", defaultValue: "HTTP error", comment: "Error domain for HTTP errors"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.github.download-failed", defaultValue: "Could not download package", comment: "Error message when package download fails"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Installation step name for decompressing binary"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "lsp.github.decompressing", defaultValue: "Decompressing %@", comment: "Status message while decompressing file"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.github.decompressed-to", defaultValue: "Decompressed to '%@'", comment: "Status message after successful decompression"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "lsp.github.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Status message while decompressing with tar"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.github.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Status message while decompressing with gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.github.clone-with-git", defaultValue: "Clone with Git", comment: "Installation step name for git clone"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "lsp.github.clone-command", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Permission message for git clone command"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github.install-from-source", defaultValue: "Install From Source", comment: "Installation step name for installing from source"),
            confirmation: .required(message: String(format: String(localized: "lsp.github.install-command", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Permission message for install command"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
