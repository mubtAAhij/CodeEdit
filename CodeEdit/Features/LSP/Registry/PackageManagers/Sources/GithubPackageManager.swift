...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "github.git-permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Confirmation message asking permission to run git commands")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "github.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github.download-binary", defaultValue: "Download Binary Executable", comment: "Step name for downloading binary executable"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "github.downloading-url", defaultValue: "Downloading %@", comment: "Status message for downloading from URL"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "github.http-error", defaultValue: "HTTP error", comment: "Error domain for HTTP errors"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "github.download-failed", defaultValue: "Could not download package", comment: "Error message when package download fails"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Step name for decompressing binary executable"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "github.decompressing-file", defaultValue: "Decompressing %@", comment: "Status message for decompressing file"), fileName))
                ...
                await model.status(String(format: String(localized: "github.decompressed-to-path", defaultValue: "Decompressed to '%@'", comment: "Status message showing where file was decompressed"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "github.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Status message for decompressing with tar"), fileName))
                ...
                await model.status(String(format: String(localized: "github.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Status message for decompressing with gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "github.clone-with-git", defaultValue: "Clone with Git", comment: "Step name for cloning repository with git"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "github.clone-confirmation", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Confirmation message for git clone command"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github.install-from-source", defaultValue: "Install From Source", comment: "Step name for installing from source"),
            confirmation: .required(message: String(format: String(localized: "github.install-confirmation", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Confirmation message for install command"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "github.source-build-failed", defaultValue: "Source build failed.", comment: "Error message when source build fails"))
            }
        }
    }
}
