...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "lsp.git-install-confirmation", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Git install confirmation message")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.initialize-directory-structure", defaultValue: "Initialize Directory Structure", comment: "Initialize Directory Structure step name"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.download-binary-executable", defaultValue: "Download Binary Executable", comment: "Download Binary Executable step name"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "lsp.downloading-url", defaultValue: "Downloading %@", comment: "Downloading URL status"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.http-error", defaultValue: "HTTP error", comment: "HTTP error domain"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.could-not-download-package", defaultValue: "Could not download package", comment: "Could not download package error"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.decompress-binary-executable", defaultValue: "Decompress Binary Executable", comment: "Decompress Binary Executable step name"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "lsp.decompressing-file", defaultValue: "Decompressing %@", comment: "Decompressing file status"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.decompressed-to-path", defaultValue: "Decompressed to '%@'", comment: "Decompressed to path status"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "lsp.decompressing-file-using-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Decompressing file using tar status"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.decompressing-file-using-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Decompressing file using gunzip status"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.clone-with-git", defaultValue: "Clone with Git", comment: "Clone with Git step name"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "lsp.clone-git-confirmation", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Clone with Git confirmation message"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.install-from-source", defaultValue: "Install From Source", comment: "Install From Source step name"),
            confirmation: .required(message: String(format: String(localized: "lsp.install-from-source-confirmation", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Install From Source confirmation message"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "lsp.source-build-failed", defaultValue: "Source build failed.", comment: "Source build failed error"))
            }
        }
    }
}
