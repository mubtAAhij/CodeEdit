...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "package-manager.github.git-permission-message", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Permission message for running git commands")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.initialize-step", defaultValue: "Initialize Directory Structure", comment: "Installation step to initialize directory structure"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.download-step", defaultValue: "Download Binary Executable", comment: "Installation step to download binary executable"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(localized: "package-manager.github.downloading-status", defaultValue: "Downloading \(url)", comment: "Status message while downloading"))
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.decompress-step", defaultValue: "Decompress Binary Executable", comment: "Installation step to decompress binary executable"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(localized: "package-manager.github.decompressing-status", defaultValue: "Decompressing \(fileName)", comment: "Status message while decompressing"))
                ...
                await model.status(String(localized: "package-manager.github.decompressed-status", defaultValue: "Decompressed to '\(downloadPath.path(percentEncoded: false))'", comment: "Status message after decompression completes"))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(localized: "package-manager.github.decompressing-tar-status", defaultValue: "Decompressing \(fileName) using `tar`", comment: "Status message while decompressing with tar"))
                ...
                await model.status(String(localized: "package-manager.github.decompressing-gunzip-status", defaultValue: "Decompressing \(fileName) using `gunzip`", comment: "Status message while decompressing with gunzip"))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.github.clone-step", defaultValue: "Clone with Git", comment: "Installation step to clone with git"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(localized: "package-manager.github.clone-confirmation-message", defaultValue: "This step will run the following command to clone the package from source control:\n`\(command.joined(separator: " "))`", comment: "Confirmation message for git clone step"))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.install-from-source-step", defaultValue: "Install From Source", comment: "Installation step to install from source"),
            confirmation: .required(message: String(localized: "package-manager.github.install-from-source-confirmation-message", defaultValue: "This step will run the following to finish installing:\n`\(command)`", comment: "Confirmation message for install from source step"))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
