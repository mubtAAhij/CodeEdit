...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "github-package-manager.git-permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Git permission request")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "github-package-manager.initialize", defaultValue: "Initialize Directory Structure", comment: "Initialize directory structure step"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github-package-manager.download-binary", defaultValue: "Download Binary Executable", comment: "Download binary executable step"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "github-package-manager.downloading", defaultValue: "Downloading %@", comment: "Downloading status"), url.absoluteString))
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "github-package-manager.download-failed", defaultValue: "Could not download package", comment: "Download failed error"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github-package-manager.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Decompress binary executable step"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "github-package-manager.decompressing", defaultValue: "Decompressing %@", comment: "Decompressing status"), fileName))
                ...
                await model.status(String(format: String(localized: "github-package-manager.decompressed-to", defaultValue: "Decompressed to '%@'", comment: "Decompressed to path status"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "github-package-manager.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Decompressing using tar status"), fileName))
                ...
                await model.status(String(format: String(localized: "github-package-manager.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Decompressing using gunzip status"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "github-package-manager.install-from-source", defaultValue: "Install From Source", comment: "Install from source step"),
            confirmation: .required(message: String(format: String(localized: "github-package-manager.install-confirmation", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Install confirmation message"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "github-package-manager.source-build-failed", defaultValue: "Source build failed.", comment: "Source build failed error"))
            }
        }
    }
}
