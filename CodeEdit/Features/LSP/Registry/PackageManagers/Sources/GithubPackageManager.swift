...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "github_package_manager.git_required", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Confirmation message asking user permission to run git commands")
                )
            ) { model in
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            ...
            do {
                await model.status(String(format: String(localized: "github_package_manager.downloading", defaultValue: "Downloading %@", comment: "Status message while downloading from URL"), url.absoluteString))
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "github_package_manager.error.download_failed", defaultValue: "Could not download package", comment: "Error message when package download fails"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "github_package_manager.decompressing", defaultValue: "Decompressing %@", comment: "Status message while decompressing file"), fileName))
                ...
                await model.status(String(format: String(localized: "github_package_manager.decompressed_to", defaultValue: "Decompressed to '%@'", comment: "Status message after successful decompression"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "github_package_manager.decompressing_tar", defaultValue: "Decompressing %@ using `tar`", comment: "Status message while decompressing using tar"), fileName))
                ...
                await model.status(String(format: String(localized: "github_package_manager.decompressing_gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Status message while decompressing using gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "github_package_manager.clone_with_git", defaultValue: "Clone with Git", comment: "Step name for cloning repository with git"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "github_package_manager.clone_confirmation", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Confirmation message for git clone command"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            ...
            confirmation: .required(message: String(format: String(localized: "github_package_manager.install_confirmation", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Confirmation message for source installation command"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "github_package_manager.error.source_build_failed", defaultValue: "Source build failed.", comment: "Error message when source build fails"))
            }
        }
    }
}
