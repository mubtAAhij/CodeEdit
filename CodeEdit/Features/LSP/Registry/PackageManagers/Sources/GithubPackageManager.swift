...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(
                        localized: "github-package-manager.install-confirmation",
                        defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?",
                        comment: "Confirmation message asking user permission to run git commands"
                    )
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(
                localized: "github-package-manager.initialize-directory",
                defaultValue: "Initialize Directory Structure",
                comment: "Step name for initializing directory structure"
            ),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(
                localized: "github-package-manager.download-binary",
                defaultValue: "Download Binary Executable",
                comment: "Step name for downloading binary executable"
            ),
            confirmation: .none
        ) { model in
            do {
                await model.status(
                    String(
                        format: String(
                            localized: "github-package-manager.downloading",
                            defaultValue: "Downloading %@",
                            comment: "Status message while downloading from URL"
                        ),
                        url.absoluteString
                    )
                )
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        ...
                        error: NSError(
                            domain: String(
                                localized: "github-package-manager.error.http-error",
                                defaultValue: "HTTP error",
                                comment: "Error domain for HTTP errors"
                            ),
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        ...
                        error: NSError(
                            domain: String(
                                localized: "github-package-manager.error.download-failed",
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(
                localized: "github-package-manager.decompress-binary",
                defaultValue: "Decompress Binary Executable",
                comment: "Step name for decompressing binary executable"
            ),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(
                    String(
                        format: String(
                            localized: "github-package-manager.decompressing",
                            defaultValue: "Decompressing %@",
                            comment: "Status message while decompressing file"
                        ),
                        fileName
                    )
                )
                ...
                await model.status(
                    String(
                        format: String(
                            localized: "github-package-manager.decompressed-to",
                            defaultValue: "Decompressed to '%@'",
                            comment: "Status message after successful decompression"
                        ),
                ...
                await model.status(
                    String(
                        format: String(
                            localized: "github-package-manager.decompressing-tar",
                            defaultValue: "Decompressing %@ using `tar`",
                            comment: "Status message while decompressing with tar"
                        ),
                        fileName
                    )
                )
                ...
                await model.status(
                    String(
                        format: String(
                            localized: "github-package-manager.decompressing-gunzip",
                            defaultValue: "Decompressing %@ using `gunzip`",
                            comment: "Status message while decompressing with gunzip"
                        ),
                        fileName
                    )
                )
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "github-package-manager.clone-with-git",
                defaultValue: "Clone with Git",
                comment: "Step name for cloning with git"
            ),
            // swiftlint:disable:next line_length
            confirmation: .required(
                message: String(
                    format: String(
                        localized: "github-package-manager.clone-confirmation",
                        defaultValue: "This step will run the following command to clone the package from source control:\n`%@`",
                        comment: "Confirmation message for git clone command"
                    ),
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(
                localized: "github-package-manager.install-from-source",
                defaultValue: "Install From Source",
                comment: "Step name for installing from source"
            ),
            confirmation: .required(
                message: String(
                    format: String(
                        ...
                        comment: "Confirmation message for source install command"
                    ),
                    command
                )
            )
