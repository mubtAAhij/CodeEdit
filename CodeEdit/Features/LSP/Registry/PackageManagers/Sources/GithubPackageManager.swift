...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "lsp.github-package-manager.git-permission-prompt", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Prompt asking user permission to run git commands for package installation")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.github-package-manager.step.initialize-directory-structure", defaultValue: "Initialize Directory Structure", comment: "Progress step title for creating package directory structure"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github-package-manager.step.download-binary-executable", defaultValue: "Download Binary Executable", comment: "Progress step title for downloading binary executable"),
            confirmation: .none
        ) { model in
            do {
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.github-package-manager.error.http-error", defaultValue: "HTTP error", comment: "Error title for HTTP failure during package download"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.github-package-manager.error.could-not-download-package", defaultValue: "Could not download package", comment: "Error message when package download fails"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github-package-manager.step.decompress-binary-executable", defaultValue: "Decompress Binary Executable", comment: "Progress step title for decompressing downloaded binary"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.github-package-manager.step.clone-with-git", defaultValue: "Clone with Git", comment: "Progress step title for cloning package repository with git"),
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.github-package-manager.step.install-from-source", defaultValue: "Install From Source", comment: "Progress step title for installing package from source"),
            ...
            do {
                ...
                throw PackageManagerError.installationFailed(String(localized: "lsp.github-package-manager.error.source-build-failed", defaultValue: "Source build failed.", comment: "Error message when building package from source fails"))
            }
        }
    }
}
