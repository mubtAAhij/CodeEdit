...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "lsp.package-manager.git-permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Git permission confirmation message")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.initialize", defaultValue: "Initialize Directory Structure", comment: "Initialize directory structure step"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.download-binary", defaultValue: "Download Binary Executable", comment: "Download binary executable step"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "lsp.package-manager.downloading", defaultValue: "Downloading %@", comment: "Downloading status message"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.package-manager.http-error", defaultValue: "HTTP error", comment: "HTTP error domain"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "lsp.package-manager.download-failed", defaultValue: "Could not download package", comment: "Download failed error domain"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Decompress binary executable step"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "lsp.package-manager.decompressing", defaultValue: "Decompressing %@", comment: "Decompressing status message"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.package-manager.decompressed-to", defaultValue: "Decompressed to '%@'", comment: "Decompressed to path message"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "lsp.package-manager.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Decompressing with tar message"), fileName))
                ...
                await model.status(String(format: String(localized: "lsp.package-manager.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Decompressing with gunzip message"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.clone-with-git", defaultValue: "Clone with Git", comment: "Clone with git step"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "lsp.package-manager.clone-command", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Clone command confirmation message"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.install-from-source", defaultValue: "Install From Source", comment: "Install from source step"),
            confirmation: .required(message: String(format: String(localized: "lsp.package-manager.install-command", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Install command confirmation message"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "lsp.package-manager.source-build-failed", defaultValue: "Source build failed.", comment: "Source build failed error"))
            }
        }
    }
}
