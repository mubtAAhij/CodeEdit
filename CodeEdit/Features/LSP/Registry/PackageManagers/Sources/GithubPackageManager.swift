...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "package-manager.github.git-permission", defaultValue: "This package requires the use of git to install. Allow CodeEdit to run git commands?", comment: "Confirmation message requesting permission to run git commands")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.initialize", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.download-binary", defaultValue: "Download Binary Executable", comment: "Step name for downloading binary"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: NSLocalizedString("package-manager.github.downloading", comment: "Status message while downloading"), url.absoluteString))
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Step name for decompressing binary"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: NSLocalizedString("package-manager.github.decompressing", comment: "Status message while decompressing file"), fileName))
                ...
                await model.status(String(format: NSLocalizedString("package-manager.github.decompressed-to", comment: "Status message after decompression"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: NSLocalizedString("package-manager.github.decompressing-tar", comment: "Status message while decompressing with tar"), fileName))
                ...
                await model.status(String(format: NSLocalizedString("package-manager.github.decompressing-gunzip", comment: "Status message while decompressing with gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.github.git-clone", defaultValue: "Clone with Git", comment: "Step name for git clone"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: NSLocalizedString("package-manager.github.git-clone-confirmation", comment: "Confirmation message for git clone command"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.install-from-source", defaultValue: "Install From Source", comment: "Step name for installing from source"),
            confirmation: .required(message: String(format: NSLocalizedString("package-manager.github.install-confirmation", comment: "Confirmation message for install command"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "package-manager.github.source-build-failed", defaultValue: "Source build failed.", comment: "Error when source build fails"))
            }
        }
    }
}
