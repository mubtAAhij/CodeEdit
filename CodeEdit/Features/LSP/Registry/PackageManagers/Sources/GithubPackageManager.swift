...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "package-manager.github.permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Permission prompt for running git commands")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.initialize", defaultValue: "Initialize Directory Structure", comment: "GitHub package manager initialization step"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.download", defaultValue: "Download Binary Executable", comment: "GitHub package binary download step"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "package-manager.github.downloading", defaultValue: "Downloading %@", comment: "Status message while downloading package"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "package-manager.github.error.http", defaultValue: "HTTP error", comment: "HTTP error domain for download failures"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "package-manager.github.error.download-failed", defaultValue: "Could not download package", comment: "Error message when package download fails"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.decompress", defaultValue: "Decompress Binary Executable", comment: "GitHub package decompression step"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "package-manager.github.decompressing", defaultValue: "Decompressing %@", comment: "Status message while decompressing package"), fileName))
                ...
                await model.status(String(format: String(localized: "package-manager.github.decompressed-to", defaultValue: "Decompressed to '%@'", comment: "Status message after successful decompression"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "package-manager.github.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Status message while decompressing with tar"), fileName))
                ...
                await model.status(String(format: String(localized: "package-manager.github.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Status message while decompressing with gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.github.clone", defaultValue: "Clone with Git", comment: "GitHub package git clone step"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "package-manager.github.clone-confirmation", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "GitHub package git clone confirmation message"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package-manager.github.install-from-source", defaultValue: "Install From Source", comment: "GitHub package install from source step"),
            confirmation: .required(message: String(format: String(localized: "package-manager.github.install-from-source-confirmation", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "GitHub package install from source confirmation message"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
