...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "package.git.permission", defaultValue: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Permission request to run git commands for package installation")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package.initialize", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing package directory structure"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package.download-binary", defaultValue: "Download Binary Executable", comment: "Step name for downloading binary executable"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(format: String(localized: "package.downloading", defaultValue: "Downloading %@", comment: "Status message for downloading package"), url.absoluteString))
                ...
                guard let httpResponse = response as? HTTPURLResponse,
                    ...
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "package.http-error", defaultValue: "HTTP error", comment: "Error domain for HTTP errors"), code: (response as? HTTPURLResponse)?.statusCode ?? -1)
                    )
                }
                ...
                if !FileManager.default.fileExists(atPath: packagePath.path) {
                    throw RegistryManagerError.downloadFailed(
                        url: url,
                        error: NSError(domain: String(localized: "package.download-failed", defaultValue: "Could not download package", comment: "Error message for failed package download"), code: -1)
                    )
                }
            } catch {
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package.decompress-binary", defaultValue: "Decompress Binary Executable", comment: "Step name for decompressing binary executable"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(format: String(localized: "package.decompressing", defaultValue: "Decompressing %@", comment: "Status message for decompressing file"), fileName))
                ...
                await model.status(String(format: String(localized: "package.decompressed-to", defaultValue: "Decompressed to '%@'", comment: "Status message for successful decompression"), downloadPath.path(percentEncoded: false)))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(format: String(localized: "package.decompressing-tar", defaultValue: "Decompressing %@ using `tar`", comment: "Status message for decompressing with tar"), fileName))
                ...
                await model.status(String(format: String(localized: "package.decompressing-gunzip", defaultValue: "Decompressing %@ using `gunzip`", comment: "Status message for decompressing with gunzip"), fileName))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package.git-clone", defaultValue: "Clone with Git", comment: "Step name for cloning repository with git"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(format: String(localized: "package.git-clone-confirm", defaultValue: "This step will run the following command to clone the package from source control:\n`%@`", comment: "Confirmation message for git clone command"), command.joined(separator: " ")))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "package.install-from-source", defaultValue: "Install From Source", comment: "Step name for installing from source"),
            confirmation: .required(message: String(format: String(localized: "package.install-from-source-confirm", defaultValue: "This step will run the following to finish installing:\n`%@`", comment: "Confirmation message for install from source command"), command))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "package.source-build-failed", defaultValue: "Source build failed.", comment: "Error message for failed source build"))
            }
        }
    }
}
