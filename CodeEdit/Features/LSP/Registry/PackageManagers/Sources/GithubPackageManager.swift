...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "This package requires git to install. Allow CodeEdit to run git commands?", comment: "Permission prompt for git commands")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "Initialize Directory Structure", comment: "Installation step name"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func downloadBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "Download Binary Executable", comment: "Installation step name"),
            confirmation: .none
        ) { model in
            do {
                await model.status(String(localized: "Downloading \(url)", comment: "Download status message"))
    ...
    private func decompressBinary(
        ...
        PackageManagerInstallStep(
            name: String(localized: "Decompress Binary Executable", comment: "Installation step name"),
            confirmation: .none,
        ) { model in
            let fileName = url.lastPathComponent
            let downloadPath = installationDirectory.appending(path: source.entryName)
            ...
            if packagePath.pathExtension == "tar" || packagePath.pathExtension == ".zip" {
                await model.status(String(localized: "Decompressing \(fileName)", comment: "Decompression status message"))
                ...
                await model.status(String(localized: "Decompressed to '\(downloadPath.path(percentEncoded: false))'", comment: "Decompression completion message"))
            } else if packagePath.lastPathComponent.hasSuffix(".tar.gz") {
                await model.status(String(localized: "Decompressing \(fileName) using `tar`", comment: "Decompression status using tar"))
                ...
                await model.status(String(localized: "Decompressing \(fileName) using `gunzip`", comment: "Decompression status using gunzip"))
                _ = try await model.executeInDirectory(
                    in: packagePath.deletingLastPathComponent().path(percentEncoded: false),
                    [
                        "gunzip",
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "Clone with Git", comment: "Installation step name"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(localized: "This step will run the following command to clone the package from source control:\n`\(command.joined(separator: " "))`", comment: "Git clone confirmation message"))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "Install From Source", comment: "Installation step name"),
            confirmation: .required(message: String(localized: "This step will run the following to finish installing:\n`\(command)`", comment: "Source installation confirmation message"))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
                ...
                throw PackageManagerError.installationFailed(String(localized: "Source build failed.", comment: "Error message for source build failure"))
            }
        }
    }
}
