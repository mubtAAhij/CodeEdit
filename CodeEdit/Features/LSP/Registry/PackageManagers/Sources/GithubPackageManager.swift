...
final class GithubPackageManager: PackageManagerProtocol {
        ...
        case .sourceBuild:
            PackageManagerInstallStep(
                ...
                confirmation: .required(
                    message: String(localized: "package_requires_git_allow_commands", comment: "Git permission confirmation message")
                )
            ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "initialize_directory_structure", comment: "Installation step name"),
            confirmation: .none
        ) { model in
            do {
    ...
    private func gitClone(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "clone_with_git", comment: "Installation step name"),
            // swiftlint:disable:next line_length
            confirmation: .required(message: String(localized: "git_clone_command_confirmation", comment: "Git clone confirmation message"))
        ) { model in
    ...
    private func installFromSource(
        ...
        PackageManagerInstallStep(
            name: String(localized: "install_from_source", comment: "Step name for installing package from source code"),
            confirmation: .required(message: String(localized: "install_source_command_message", comment: "Message explaining the command that will be run to install from source"))
        ) { model in
            do {
                let installPath = installationDirectory.appending(path: source.entryName, directoryHint: .isDirectory)
