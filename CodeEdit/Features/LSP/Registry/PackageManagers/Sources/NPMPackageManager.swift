...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm.requires-npm", defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?", comment: "NPM installation confirmation")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm.init.title", defaultValue: "Initialize Directory Structure", comment: "Initialize directory step"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "npm.install.these-packages", defaultValue: "these packages", comment: "These packages plural") : String(localized: "npm.install.this-package", defaultValue: "this package", comment: "This package singular")

        return PackageManagerInstallStep(
            name: String(localized: "npm.install.title", defaultValue: "Install Package Using npm", comment: "Install package step"),
            confirmation: .required(
                message: String(format: String(localized: "npm.install.confirmation", defaultValue: "This requires the npm package%@ %@.\nAllow CodeEdit to install %@?", comment: "Confirmation message for npm install"), sSuffix, packagesDescription, suffix)
            )
        ) { model in
            do {
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm.verify.title", defaultValue: "Verify Installation", comment: "Verify installation step"),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(String(localized: "npm.verify.not-in-package-json", defaultValue: "Package not found in package.json", comment: "Package not found in package.json error"))
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(format: String(localized: "npm.verify.version-mismatch", defaultValue: "Version mismatch: Expected %@, but found %@", comment: "Version mismatch error"), version, installedVersion)
                )
            }
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(String(localized: "npm.verify.not-in-node-modules", defaultValue: "Package not found in node_modules", comment: "Package not found in node_modules error"))
            }
        }
    }
}
