...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm.permission_request", comment: "Permission request message")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm.initialize_directory", comment: "Initialization step name"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "npm.packages_plural", comment: "Multiple packages") : String(localized: "npm.package_singular", comment: "Single package")

        return PackageManagerInstallStep(
            name: String(localized: "npm.install_package", comment: "Installation step name"),
            confirmation: .required(
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm.verify_installation", comment: "Verification step name"),
            confirmation: .none
        ) { _ in
