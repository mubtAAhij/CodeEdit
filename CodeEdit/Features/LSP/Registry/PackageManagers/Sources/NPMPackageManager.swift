...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm.allow-npm-commands", defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?", comment: "NPM permission prompt")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Initialize directory step name"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "npm.these-packages", defaultValue: "these packages", comment: "Multiple packages reference") : String(localized: "npm.this-package", defaultValue: "this package", comment: "Single package reference")

        return PackageManagerInstallStep(
            name: String(localized: "npm.install-package", defaultValue: "Install Package Using npm", comment: "Install package step name"),
            confirmation: .required(
                message: String(format: String(localized: "npm.install-confirmation", defaultValue: "This requires the npm package%@ %@.\nAllow CodeEdit to install %@?", comment: "NPM package install confirmation"), sSuffix, packagesDescription, suffix)
            )
        ) { model in
            do {
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm.verify-installation", defaultValue: "Verify Installation", comment: "Verify installation step name"),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(String(localized: "npm.package-not-in-json", defaultValue: "Package not found in package.json", comment: "Package not found in package.json error"))
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(format: String(localized: "npm.version-mismatch", defaultValue: "Version mismatch: Expected %@, but found %@", comment: "Version mismatch error"), version, installedVersion)
                )
            }
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(String(localized: "npm.package-not-in-node-modules", defaultValue: "Package not found in node_modules", comment: "Package not found in node_modules error"))
            }
        }
    }
}
