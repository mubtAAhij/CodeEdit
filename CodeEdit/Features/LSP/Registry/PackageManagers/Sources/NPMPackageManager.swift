...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "This package requires npm to install. Allow CodeEdit to run npm commands?")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "Initialize Directory Structure"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "these packages") : String(localized: "this package")

        return PackageManagerInstallStep(
            name: String(localized: "Install Package Using npm"),
            confirmation: .required(
                message: String(localized: "This requires the npm package\(sSuffix) \(packagesDescription).\nAllow CodeEdit to install \(suffix)?")
            )
        ) { model in
            do {
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "Verify Installation"),
            confirmation: .none
        ) { _ in
