...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm_install_permission", comment: "Confirmation message for npm installation permission")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "initialize_directory_structure", comment: "Activity step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "these_packages", comment: "Plural form for package references") : String(localized: "this_package", comment: "Singular form for package references")

        return PackageManagerInstallStep(
            name: String(localized: "install_package_using_npm", comment: "Activity step name for installing npm package"),
            confirmation: .required(
                message: String(localized: "npm_package_install_confirmation", arguments: [sSuffix, packagesDescription, suffix], comment: "Confirmation message for npm package installation")
            )
        ) { model in
            do {
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "verify_installation", comment: "Activity step name for verifying installation"),
            confirmation: .none
        ) { _ in
