...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm-package-manager.allow-npm", defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?", comment: "Allow npm commands confirmation message")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm-package-manager.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Initialize directory structure step name"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "npm-package-manager.these-packages", defaultValue: "these packages", comment: "These packages (plural)") : String(localized: "npm-package-manager.this-package", defaultValue: "this package", comment: "This package (singular)")

        return PackageManagerInstallStep(
            name: String(localized: "npm-package-manager.install-package", defaultValue: "Install Package Using npm", comment: "Install package using npm step name"),
            confirmation: .required(
                message: String(format: String(localized: "npm-package-manager.install-confirmation", defaultValue: "This requires the npm package%@ %@.\nAllow CodeEdit to install %@?", comment: "Install package confirmation message"), sSuffix, packagesDescription, suffix)
            )
        ) { model in
            do {
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm-package-manager.verify-installation", defaultValue: "Verify Installation", comment: "Verify installation step name"),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(String(localized: "npm-package-manager.package-not-in-json", defaultValue: "Package not found in package.json", comment: "Package not found in package.json error"))
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(format: String(localized: "npm-package-manager.version-mismatch", defaultValue: "Version mismatch: Expected %@, but found %@", comment: "Version mismatch error"), version, installedVersion)
                )
            }
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(String(localized: "npm-package-manager.package-not-in-modules", defaultValue: "Package not found in node_modules", comment: "Package not found in node_modules error"))
            }
        }
    }
}
