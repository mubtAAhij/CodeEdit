...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm.install-permission", defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?", comment: "Confirmation message asking permission to run npm commands")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing npm directory structure"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        let suffix = plural ? String(localized: "npm.these-packages", defaultValue: "these packages", comment: "Plural reference to multiple npm packages") : String(localized: "npm.this-package", defaultValue: "this package", comment: "Singular reference to one npm package")

        return PackageManagerInstallStep(
            name: String(localized: "npm.install-package", defaultValue: "Install Package Using npm", comment: "Step name for installing npm package"),
            confirmation: .required(
                message: String(
                    format: String(
                        localized: "npm.install-confirmation",
                        defaultValue: "This requires the npm package%@ %@.\nAllow CodeEdit to install %@?",
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm.verify-installation", defaultValue: "Verify Installation", comment: "Step name for verifying npm package installation"),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(String(localized: "npm.package-not-found-json", defaultValue: "Package not found in package.json", comment: "Error message when package is not found in package.json"))
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(
                        format: String(
                            localized: "npm.version-mismatch",
                            defaultValue: "Version mismatch: Expected %@, but found %@",
                            comment: "Error message when installed npm package version doesn't match expected version"
                        ),
                        version,
                        installedVersion
                    )
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(String(localized: "npm.package-not-found-modules", defaultValue: "Package not found in node_modules", comment: "Error message when package is not found in node_modules directory"))
            }
        }
    }
}
