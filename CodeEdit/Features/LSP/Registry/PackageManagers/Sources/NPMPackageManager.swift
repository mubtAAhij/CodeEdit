...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(localized: "npm.package-manager.npm-required-permission", defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?", comment: "Permission prompt asking if CodeEdit can run npm commands")
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "npm.package-manager.step.initialize", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing npm package directory"), confirmation: .none) { model in
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        if packageList.count > 1 {
            message = String(
                format: String(localized: "npm.package-manager.install-permission.multiple", defaultValue: "This requires the npm packages %@.\nAllow CodeEdit to install these packages?", comment: "Permission prompt asking if CodeEdit can install multiple npm packages"),
                packagesDescription
            )
        } else {
            message = String(
                format: String(localized: "npm.package-manager.install-permission.single", defaultValue: "This requires the npm package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt asking if CodeEdit can install a single npm package"),
                packagesDescription
            )
        }

        return PackageManagerInstallStep(
            name: String(localized: "npm.package-manager.step.install", defaultValue: "Install Package Using npm", comment: "Installation step name for installing npm package"),
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(localized: "npm.package-manager.step.verify", defaultValue: "Verify Installation", comment: "Installation step name for verifying npm package installation"),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(String(localized: "npm.package-manager.error.package-not-found-json", defaultValue: "Package not found in package.json", comment: "Error message when npm package is not found in package.json"))
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(format: String(localized: "npm.package-manager.error.version-mismatch", defaultValue: "Version mismatch: Expected %@, but found %@", comment: "Error message when installed npm package version doesn't match expected version"), version, installedVersion)
                )
            }
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(String(localized: "npm.package-manager.error.package-not-found-modules", defaultValue: "Package not found in node_modules", comment: "Error message when npm package is not found in node_modules directory"))
            }
        }
    }
}
