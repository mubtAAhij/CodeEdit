...
final class NPMPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            ...
            confirmation: .required(
                message: String(
                    localized: "package-manager.npm.permission",
                    defaultValue: "This package requires npm to install. Allow CodeEdit to run npm commands?",
                    comment: "Permission message for running npm commands"
                )
            )
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(
                localized: "package-manager.initialize-directory",
    ...
    func runNpmInstall(_ source: PackageSource, installDir installationDirectory: URL) -> PackageManagerInstallStep {
        ...
        // Use ListFormatter for proper localization of lists
        ...
        if packageList.count > 1 {
            message = String(
                format: String(
                    localized: "package-manager.npm.install-confirmation-plural",
                    defaultValue: "This requires the npm packages %@.\nAllow CodeEdit to install these packages?",
                    comment: "Confirmation message for npm package installation (multiple packages)"
                ),
                packagesDescription
            )
        } else {
            message = String(
                format: String(
                    localized: "package-manager.npm.install-confirmation-singular",
                    defaultValue: "This requires the npm package %@.\nAllow CodeEdit to install this package?",
                    comment: "Confirmation message for npm package installation (single package)"
                ),
                packagesDescription
            )
        }
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "package-manager.npm.install-step",
    ...
    private func verifyInstallation(
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "package-manager.verify-installation",
                defaultValue: "Verify Installation",
                comment: "Step name for verifying package installation"
            ),
            confirmation: .none
        ) { _ in
            ...
            guard let packageJsonData = FileManager.default.contents(atPath: packageJsonPath),
                ...
                throw PackageManagerError.installationFailed(
                    String(
                        localized: "package-manager.npm.package-not-found-in-json",
                        defaultValue: "Package not found in package.json",
                        comment: "Error message when package is not found in package.json"
                    )
                )
            }
            ...
            if normalizedInstalledVersion != normalizedRequestedVersion &&
                ...
                throw PackageManagerError.installationFailed(
                    String(
                        format: String(
                            localized: "package-manager.npm.version-mismatch",
                            defaultValue: "Version mismatch: Expected %@, but found %@",
                            comment: "Error message when installed version doesn't match expected version"
                        ),
                        version,
                        installedVersion
                    )
            ...
            guard FileManager.default.fileExists(atPath: packageDirectory.path) else {
                throw PackageManagerError.installationFailed(
                    String(
                        localized: "package-manager.npm.package-not-found-in-modules",
