...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.pip.initialize", defaultValue: "Initialize Directory Structure", comment: "Pip installation step: initialize directory"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.pip.install", defaultValue: "Install Package Using pip", comment: "Pip installation step: install package"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.pip.install-confirmation", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation for pip package installation"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.pip.update-requirements", defaultValue: "Update requirements.txt", comment: "Pip installation step: update requirements"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "lsp.pip.writing-requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Status message when writing requirements"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.pip.verify", defaultValue: "Verify Installation", comment: "Pip installation step: verify installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "lsp.pip.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Error when package not found in pip list"), source.pkgName))
            }
        }
    }
