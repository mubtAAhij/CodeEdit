...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "pip.initialize", defaultValue: "Initialize Directory Structure", comment: "Initialize directory structure step"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "pip.install-package", defaultValue: "Install Package Using pip", comment: "Install package using pip step"),
            confirmation: .required(
                message: String(format: String(localized: "pip.install-confirmation", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Install package confirmation message"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            ...
            await model.status(String(localized: "pip.writing-requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Writing requirements status message"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "pip.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Package not found error message"), source.pkgName))
            }
        }
    }
