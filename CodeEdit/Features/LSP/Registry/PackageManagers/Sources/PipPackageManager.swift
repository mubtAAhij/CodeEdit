...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "pip.init-directory", defaultValue: "Initialize Directory Structure", comment: "Pip package manager step to initialize directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "pip.install-package", defaultValue: "Install Package Using pip", comment: "Pip package manager step to install package"),
            confirmation: .required(
                message: String(localized: "pip.install-prompt", defaultValue: "This requires the pip package \(source.pkgName).\nAllow CodeEdit to install this package?", comment: "Prompt asking user permission to install a pip package")
            )
        ) { model in
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.verify-installation", defaultValue: "Verify Installation", comment: "Pip package manager step to verify installation"),
            confirmation: .none
        ) { model in
