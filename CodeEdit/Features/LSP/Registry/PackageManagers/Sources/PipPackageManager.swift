...
final class PipPackageManager: PackageManagerProtocol {
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.update-requirements", defaultValue: "Update requirements.txt", comment: "Step name for updating requirements.txt"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "pip.writing-requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Status message while writing requirements"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.verify-installation", defaultValue: "Verify Installation", comment: "Step name for verifying installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "pip.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Error when package is not found in pip list"), source.pkgName))
            }
        }
    }
