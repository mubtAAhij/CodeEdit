...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "pip_package_manager.initialize_directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "pip_package_manager.install_package", defaultValue: "Install Package Using pip", comment: "Step name for installing pip package"),
            confirmation: .required(
                message: String(format: String(localized: "pip_package_manager.install_package_permission", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt to install pip package"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip_package_manager.update_requirements", defaultValue: "Update requirements.txt", comment: "Step name for updating requirements.txt"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "pip_package_manager.writing_requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Status message when writing requirements"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip_package_manager.verify_installation", defaultValue: "Verify Installation", comment: "Step name for verifying installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "pip_package_manager.package_not_found_in_list", defaultValue: "Package %@ not found in pip list", comment: "Error message when package is not found in pip list"), source.pkgName))
            }
        }
    }
