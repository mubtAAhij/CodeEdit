...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.pip.initialize", comment: "Initialize Directory Structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.pip.install", comment: "Install Package Using pip"),
            confirmation: .required(
                message: String(localized: "lsp.pip.install_confirm \(source.pkgName)", comment: "This requires the pip package")
            )
        ) { model in
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.pip.verify", comment: "Verify Installation"),
            confirmation: .none
        ) { model in
