...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "package-manager.pip.initialize-step", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.pip.install-step", defaultValue: "Install Package Using pip", comment: "Installation step name for installing package with pip"),
            confirmation: .required(
                message: String(format: String(localized: "package-manager.pip.install-confirmation", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation message for pip package installation"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.pip.update-requirements-step", defaultValue: "Update requirements.txt", comment: "Installation step name for updating requirements.txt"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "package-manager.pip.writing-requirements-status", defaultValue: "Writing requirements to requirements.txt", comment: "Status message for writing requirements file"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.pip.verify-step", defaultValue: "Verify Installation", comment: "Installation step name for verifying installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "package-manager.pip.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Error message when package not found in pip list"), source.pkgName))
            }
        }
    }
