...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "Initialize Directory Structure", comment: "Step name for initializing package directory"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "Install Package Using pip", comment: "Step name for installing pip package"),
            confirmation: .required(
                message: String(localized: "This requires the pip package \(source.pkgName).\nAllow CodeEdit to install this package?", comment: "Confirmation message for pip package installation")
            )
        ) { model in
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "Verify Installation", comment: "Step name for verifying package installation"),
            confirmation: .none
        ) { model in
