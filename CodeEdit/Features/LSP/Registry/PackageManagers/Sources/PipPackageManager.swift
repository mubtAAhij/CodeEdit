...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "pip.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "pip.install-package", defaultValue: "Install Package Using pip", comment: "Step name for installing package using pip"),
            confirmation: .required(
                message: String(
                    format: String(
                        localized: "pip.install-confirmation",
                        defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?",
                        comment: "Confirmation message for pip package installation"
                    ),
                    source.pkgName
                )
            )
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.update-requirements", defaultValue: "Update requirements.txt", comment: "Step name for updating requirements.txt file"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "pip.writing-requirements", defaultValue: "Writing requirements to requirements.txt", comment: "Status message when writing requirements file"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.verify-installation", defaultValue: "Verify Installation", comment: "Step name for verifying package installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(
                    String(format: String(localized: "pip.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Error when package is not found in pip list"), source.pkgName)
                )
            }
        }
    }
