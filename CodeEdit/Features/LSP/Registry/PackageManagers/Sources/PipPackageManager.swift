...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "initialize_directory_structure", comment: "Step name for initializing directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "install_package_using_pip", comment: "Step name for installing package using pip"),
            confirmation: .required(
                message: String(localized: "pip_package_requirement_message", comment: "Message explaining pip package requirement")
                + String(localized: "allow_pip_package_install", comment: "Permission request for pip package installation")
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "update_requirements_txt", comment: "Step name for updating requirements.txt file"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "writing_requirements_to_file", comment: "Status message when writing requirements to requirements.txt file"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "verify_installation", comment: "Installation step name for verifying package installation"),
            confirmation: .none
        ) { model in
