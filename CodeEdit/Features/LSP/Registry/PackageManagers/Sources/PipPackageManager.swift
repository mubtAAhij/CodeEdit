...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "pip.package-manager.step.initialize", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing pip package directory"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "pip.package-manager.step.install", defaultValue: "Install Package Using pip", comment: "Installation step name for installing pip package"),
            confirmation: .required(
                message: String(format: String(localized: "pip.package-manager.install-permission", defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt asking if CodeEdit can install a pip package"), source.pkgName)
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.package-manager.step.update-requirements", defaultValue: "Update requirements.txt", comment: "Installation step name for updating requirements.txt file"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "pip.package-manager.step.verify", defaultValue: "Verify Installation", comment: "Installation step name for verifying pip package installation"),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(localized: "pip.package-manager.error.package-not-found", defaultValue: "Package %@ not found in pip list", comment: "Error message when pip package is not found in pip list"), source.pkgName))
            }
        }
    }
