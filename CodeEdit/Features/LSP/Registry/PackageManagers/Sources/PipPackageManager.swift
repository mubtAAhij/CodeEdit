...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(localized: "lsp.package-manager.pip.initialize-directory-structure", defaultValue: "Initialize Directory Structure", comment: "Progress step title for setting up pip package directory structure"), confirmation: .none) { model in
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.pip.install-package.title", defaultValue: "Install Package Using pip", comment: "Prompt title asking for permission to install a pip package"),
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.pip.update-requirements-file", defaultValue: "Update requirements.txt", comment: "Progress step title for updating requirements.txt"),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(localized: "lsp.package-manager.pip.writing-requirements-file", defaultValue: "Writing requirements to requirements.txt", comment: "Status message while writing requirements to requirements.txt"))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        let pipCommand = getPipCommand(in: packagePath)
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.pip.verify-installation", defaultValue: "Verify Installation", comment: "Progress step title for verifying package installation"),
            confirmation: .none
        ) { model in
