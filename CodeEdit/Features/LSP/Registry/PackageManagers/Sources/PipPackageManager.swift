...
final class PipPackageManager: PackageManagerProtocol {
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(name: String(
            localized: "lsp.registry.package-managers.pip.initialize-directory-structure",
            defaultValue: "Initialize Directory Structure",
            comment: "Installation step title for initializing directory structure in pip package manager"
    ...
    func runPipInstall(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "lsp.registry.package-managers.pip.install-package-using-pip",
                defaultValue: "Install Package Using pip",
                comment: "Installation step title for installing package with pip"
            ),
            confirmation: .required(
                message: String(format: String(
                    localized: "lsp.registry.package-managers.pip.install-package-confirmation",
                    defaultValue: "This requires the pip package %@.\nAllow CodeEdit to install this package?",
                    comment: "Confirmation message before installing pip package"
                ), "\(source.pkgName)")
            )
        ) { model in
    ...
    private func updateRequirements(in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "lsp.registry.package-managers.pip.update-requirements",
                defaultValue: "Update requirements.txt",
                comment: "Installation step title for updating requirements file"
            ),
            confirmation: .none
        ) { model in
            let requirementsPath = packagePath.appending(path: "requirements.txt")
            ...
            await model.status(String(
                localized: "lsp.registry.package-managers.pip.write-requirements-file",
                defaultValue: "Writing requirements to requirements.txt",
                comment: "Status message while writing requirements to requirements.txt"
            ))
    ...
    private func verifyInstallation(_ source: PackageSource, in packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(
                localized: "lsp.registry.package-managers.pip.verify-installation",
                defaultValue: "Verify Installation",
                comment: "Installation step title for verifying pip package installation"
            ),
            confirmation: .none
        ) { model in
            ...
            guard packageFound else {
                throw PackageManagerError.installationFailed(String(format: String(
                    localized: "lsp.registry.package-managers.pip.package-not-found-in-list",
                    defaultValue: "Package %@ not found in pip list",
                    comment: "Error message when installed package is not found in pip list output"
                ), "\(source.pkgName)"))
            }
        }
    }
