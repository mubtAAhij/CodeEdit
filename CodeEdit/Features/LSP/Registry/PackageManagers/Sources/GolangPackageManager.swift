...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "lsp.package-manager.golang.requires-go", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Confirmation message asking user permission to run go commands for package installation"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.golang.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing Go package directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.golang.install-package", defaultValue: "Install Package Using go", comment: "Step name for installing Go package using go command"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.package-manager.golang.install-confirmation", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation message asking user permission to install a specific go package"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.package-manager.golang.build-from-source", defaultValue: "Build From Source", comment: "Step name for building Go binary from source code"),
            confirmation: .none
        ) { model in
