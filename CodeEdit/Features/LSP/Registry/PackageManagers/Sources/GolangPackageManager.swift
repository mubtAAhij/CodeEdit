...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "package-manager.go.install-permission", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Permission prompt for running go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.go.initialize", defaultValue: "Initialize Directory Structure", comment: "Step name for initializing go directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.go.install", defaultValue: "Install Package Using go", comment: "Step name for installing package using go"),
            confirmation: .required(
                message: String(format: NSLocalizedString("package-manager.go.install-confirmation", comment: "Confirmation message for go package installation"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.go.build", defaultValue: "Build From Source", comment: "Step name for building from source"),
            confirmation: .none
        ) { model in
