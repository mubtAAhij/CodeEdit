...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "go_permission_request", comment: "Permission request message for running go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "initialize_directory_structure", comment: "Step name for initializing directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "install_package_using_go", comment: "Step name for installing package using go"),
            confirmation: .required(
                message: String(localized: "go_package_install_message", comment: "Message explaining go package installation requirement")
                + String(localized: "allow_package_install", comment: "Permission request for package installation")
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "build_from_source", comment: "Step name for building from source"),
            confirmation: .none
        ) { model in
