...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "package-manager.golang.permission.go-commands", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Permission prompt for running go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.golang.step.initialize", defaultValue: "Initialize Directory Structure", comment: "Installation step title for initializing Go directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "package-manager.golang.step.install", defaultValue: "Install Package Using go", comment: "Installation step title for installing Go package"),
            confirmation: .required(
                message: String(format: String(localized: "package-manager.golang.permission.install-package", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt for installing a Go package with package name"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "package-manager.golang.step.build", defaultValue: "Build From Source", comment: "Installation step title for building Go binary from source"),
            confirmation: .none
        ) { model in
