...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "go.permission-prompt", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Prompt asking permission to run go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "go.init-directory", defaultValue: "Initialize Directory Structure", comment: "Go package manager step to initialize directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "go.install-package", defaultValue: "Install Package Using go", comment: "Go package manager step to install package"),
            confirmation: .required(
                message: String(localized: "go.install-prompt", defaultValue: "This requires installing the go package \(installCommand).\nAllow CodeEdit to install this package?", comment: "Prompt asking user permission to install a go package")
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "go.build-from-source", defaultValue: "Build From Source", comment: "Go package manager step to build from source"),
            confirmation: .none
        ) { model in
