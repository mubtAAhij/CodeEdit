...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "lsp.go.install-confirmation", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Confirmation to allow go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.go.initialize", defaultValue: "Initialize Directory Structure", comment: "Go installation step: initialize directory"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.go.install", defaultValue: "Install Package Using go", comment: "Go installation step: install package"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.go.install-package-confirmation", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation for go package installation"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.go.build-from-source", defaultValue: "Build From Source", comment: "Go installation step: build from source"),
            confirmation: .none
        ) { model in
