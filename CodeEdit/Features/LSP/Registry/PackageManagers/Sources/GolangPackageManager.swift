...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "go.install-confirmation", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Confirmation message for allowing go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "go.initialize-directory", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "go.install-package", defaultValue: "Install Package Using go", comment: "Installation step name for go package installation"),
            confirmation: .required(
                message: String(localized: "go.package-install-confirmation", defaultValue: "This requires installing the go package \(installCommand).\nAllow CodeEdit to install this package?", comment: "Confirmation message for go package installation")
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "go.build-from-source", defaultValue: "Build From Source", comment: "Installation step name for building from source"),
            confirmation: .none
        ) { model in
