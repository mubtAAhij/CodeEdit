...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "golang-package-manager.go-permission-prompt", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Permission prompt for running go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "golang-package-manager.initialize-step-name", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "golang-package-manager.install-step-name", defaultValue: "Install Package Using go", comment: "Installation step name for installing package using go"),
            confirmation: .required(
                message: String(format: String(localized: "golang-package-manager.install-permission-prompt", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt for installing go package"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "golang-package-manager.build-step-name", defaultValue: "Build From Source", comment: "Installation step name for building from source"),
            confirmation: .none
        ) { model in
