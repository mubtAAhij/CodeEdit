...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "lsp.golang.install.requires-go-confirmation", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Confirmation message asking permission to run go commands for installation"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.golang.initialize-directory-structure", defaultValue: "Initialize Directory Structure", comment: "Confirmation dialog title for creating Go package directory structure"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "lsp.golang.install-package-title", defaultValue: "Install Package Using go", comment: "Confirmation dialog title for installing package using go"),
            confirmation: .required(
                message: String(format: String(localized: "lsp.golang.install-package-confirmation", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Confirmation dialog message asking permission to install a Go package"), "\(installCommand)")
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "lsp.golang.build-from-source", defaultValue: "Build From Source", comment: "Option title to build language server from source"),
            confirmation: .none
        ) { model in
