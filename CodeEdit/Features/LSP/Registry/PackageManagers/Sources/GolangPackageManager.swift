...
final class GolangPackageManager: PackageManagerProtocol {
    ...
    func isInstalled(method installationMethod: InstallationMethod) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: "",
            confirmation: .required(message: String(localized: "golang.package-manager.go-required-permission", defaultValue: "This package requires go to install. Allow CodeEdit to run go commands?", comment: "Permission prompt asking if CodeEdit can run go commands"))
        ) { model in
    ...
    func initialize(in packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "golang.package-manager.step.initialize", defaultValue: "Initialize Directory Structure", comment: "Installation step name for initializing Go package directory"),
            confirmation: .none
        ) { model in
    ...
    func runGoInstall(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        ...
        return PackageManagerInstallStep(
            name: String(localized: "golang.package-manager.step.install", defaultValue: "Install Package Using go", comment: "Installation step name for installing Go package"),
            confirmation: .required(
                message: String(format: String(localized: "golang.package-manager.install-permission", defaultValue: "This requires installing the go package %@.\nAllow CodeEdit to install this package?", comment: "Permission prompt asking if CodeEdit can install a Go package"), installCommand)
            )
        ) { model in
    ...
    func buildBinary(_ source: PackageSource, packagePath: URL) -> PackageManagerInstallStep {
        PackageManagerInstallStep(
            name: String(localized: "golang.package-manager.step.build", defaultValue: "Build From Source", comment: "Installation step name for building Go package from source"),
            confirmation: .none
        ) { model in
