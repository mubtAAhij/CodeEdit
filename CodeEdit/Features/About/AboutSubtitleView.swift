...
struct AboutSubtitleView: View {
    ...
    var body: some View {
        Text(String(format: String(localized: "about.subtitle.version-line", defaultValue: "Version %@%@ (%@)", comment: "Version line shown in About subtitle with version, postfix, and build"), "\(appVersion)", "\(appVersionPostfix)", "\(appBuild)"))
            ...
            .onTapGesture {
                ...
                NSPasteboard.general.setString(
                    String(format: String(localized: "about.subtitle.copy-summary", defaultValue: "CodeEdit: %@ (%@)\nmacOS: %@", comment: "Copied About subtitle summary containing app and macOS versions"), "\(appVersion)", "\(appBuild)", "\(macOSVersion)"),
                    forType: .string
                )
