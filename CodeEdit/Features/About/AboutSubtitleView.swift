...
struct AboutSubtitleView: View {
    ...
    var body: some View {
        Text(String(format: String(localized: "about.subtitle.version-build", defaultValue: "Version %@%@ (%@)", comment: "Subtitle showing app version, optional postfix, and build number"), "\(appVersion)", "\(appVersionPostfix)", "\(appBuild)"))
            ...
            .onTapGesture {
                ...
                NSPasteboard.general.setString(
                    String(format: String(localized: "about.subtitle.copy-info", defaultValue: "CodeEdit: %@ (%@)\nmacOS: %@", comment: "Multiline about text with CodeEdit and macOS version information"), "\(appVersion)", "\(appBuild)", "\(macOSVersion)"),
                    forType: .string
                )
