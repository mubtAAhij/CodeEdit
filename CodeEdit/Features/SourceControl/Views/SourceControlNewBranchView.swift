        ...
        if let branch = fromBranch ?? sourceControlManager.currentBranch {
            VStack(spacing: 0) {
                Form {
                    Section {
                        LabeledContent(
                            String(localized: "git.from", defaultValue: "From", comment: "From label"),
                            value: branch.isRemote
                                ? branch.longName.replacingOccurrences(of: "refs/remotes/", with: "")
                                : branch.name
                        )
                        TextField(String(localized: "git.to", defaultValue: "To", comment: "To field"), value: $name, formatter: RegexFormatter(pattern: "[^a-zA-Z0-9_-]"))
                    } header: {
                        Text(String(localized: "git.create-new-branch", defaultValue: "Create a new branch", comment: "Create new branch header"))
                        Text(String(localized: "git.create-branch-note", defaultValue: "Create a branch from the current branch and switch to it. All uncommited changes will be preserved on the new branch. ", comment: "Create branch note"))
                    }
                }
                .formStyle(.grouped)
                .scrollDisabled(true)
                ...
                HStack {
                    ...
                    Button {
                        ...
                        Text(String(localized: "git.create", defaultValue: "Create", comment: "Create button"))
                            .frame(minWidth: 56)
                    }
