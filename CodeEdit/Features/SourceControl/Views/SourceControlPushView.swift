                ...
                if loading {
                    HStack(spacing: 7.5) {
                        ProgressView()
                            ...
                            .controlSize(.small)
                        Text(String(localized: "source-control.push.progress", defaultValue: "Pushing changes...", comment: "Push progress message"))
                            .font(.subheadline)
                    }
                }
                Spacer()
    ...
    func submit() {
        Task {
            do {
                ...
                self.loading = false
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.push.error", defaultValue: "Failed to push", comment: "Push error alert title"), error: error)
            }
        }
    }
}
