        ...
        VStack(spacing: 0) {
            Form {
                Section {
                    ...
                    Text(String(localized: "source-control.push.header", defaultValue: "Push local changes to", comment: "Push header text"))
                }
                Section {
                    Toggle(String(localized: "source-control.push.force", defaultValue: "Force", comment: "Force push option"), isOn: $sourceControlManager.operationForce)
                    Toggle(String(localized: "source-control.push.include-tags", defaultValue: "Include Tags", comment: "Include tags option"), isOn: $sourceControlManager.operationIncludeTags)
                }
            }
            .formStyle(.grouped)
            .scrollDisabled(true)
            ...
            HStack {
                if loading {
                    HStack(spacing: 7.5) {
                        ProgressView()
                            ...
                            .controlSize(.small)
                        Text(String(localized: "source-control.push.pushing", defaultValue: "Pushing changes...", comment: "Pushing status message"))
                            .font(.subheadline)
                    }
                }
                Spacer()
                ...
                Button(action: submit) {
                    Text(String(localized: "source-control.push.button", defaultValue: "Push", comment: "Push button"))
                        .frame(minWidth: 56)
                }
