...
struct SourceControlAddExistingRemoteView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section(String(localized: "source-control.add-remote", defaultValue: "Add Remote", comment: "Add remote section header")) {
    ...
    func submit() {
        Task {
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.failed-to-add-remote", defaultValue: "Failed to add remote", comment: "Failed to add remote error"), error: error)
            }
        }
    }
}
