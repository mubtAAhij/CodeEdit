...
struct SourceControlAddExistingRemoteView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section(String(localized: "source-control.add-remote.header", defaultValue: "Add Remote", comment: "Section header for adding a remote")) {
            ...
            HStack {
                ...
                Button {
                    ...
                    Text(String(localized: "source-control.add-remote.cancel", defaultValue: "Cancel", comment: "Cancel button for adding remote"))
                        .frame(minWidth: 56)
                }
                Button {
                    submit()
                } label: {
                    Text(String(localized: "source-control.add-remote.add", defaultValue: "Add", comment: "Add button for adding remote"))
                        .frame(minWidth: 56)
                }
    ...
    func submit() {
        Task {
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.add-remote.error", defaultValue: "Failed to add remote", comment: "Error alert title when adding remote fails"), error: error)
            }
        }
    }
}
