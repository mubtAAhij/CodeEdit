...
struct SourceControlAddExistingRemoteView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section(String(localized: "add_remote", comment: "Add remote section title")) {
                    TextField(String(localized: "remote_name", comment: "Remote name field placeholder"), value: $name, formatter: RegexFormatter(pattern: "[^a-zA-Z0-9_-]"))
    ...
    func submit() {
        Task {
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "failed_to_add_remote", comment: "Error message when failing to add remote"), error: error)
            }
        }
    }
}
