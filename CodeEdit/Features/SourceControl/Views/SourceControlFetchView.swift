            ...
            HStack(alignment: .top, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text(String(format: String(
                        localized: "source-control.fetch.fetching-project",
                        defaultValue: "Fetching changes for “%@”...",
                        comment: "Progress title while fetching changes for a specific project"
                    ), "\(projectName)"))
                        .font(.headline)
                    Text(String(
                        ...
                        comment: "Description text in fetch progress dialog"
                    ))
                        .font(.subheadline)
            ...
            HStack {
                HStack(spacing: 7.5) {
                    ...
                    Text(String(
                        ...
                        comment: "Progress label shown while fetch operation runs"
                    ))
                        .font(.subheadline)
                }
                Spacer()
        ...
        .task {
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(
                    localized: "source-control.fetch.failed",
                    defaultValue: "Failed to fetch changes",
                    comment: "Error title shown when fetch operation fails"
                ), error: error)
            }
        }
    }
