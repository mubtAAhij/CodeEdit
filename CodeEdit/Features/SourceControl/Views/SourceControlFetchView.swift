    ...
    var body: some View {
            ...
            HStack {
                HStack(spacing: 7.5) {
                    ProgressView()
                        ...
                        .controlSize(.small)
                    Text(String(localized: "source-control.fetch.progress", defaultValue: "Fetching changes...", comment: "Fetching changes progress"))
                        .font(.subheadline)
                }
                Spacer()
                Button {
        ...
        .task {
            do {
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.fetch.failed", defaultValue: "Failed to fetch changes", comment: "Failed to fetch error"), error: error)
            }
        }
    }
}
