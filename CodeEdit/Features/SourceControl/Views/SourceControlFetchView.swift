            ...
            HStack(alignment: .top, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text(String(format: String(
                        ...
                        comment: "Title text while fetching changes for a specific project"
                    ), "\(projectName)"))
                        .font(.headline)
                    Text(String(
                        localized: "source-control.fetch.fetching-description",
                        defaultValue: "CodeEdit is fetching changes and updating the status of files in the local repository.",
                        comment: "Description text shown while fetch is in progress"
                    ))
                        .font(.subheadline)
            ...
            HStack {
                HStack(spacing: 7.5) {
                    ...
                    Text(String(
                        ...
                        comment: "Progress label while fetching source control changes"
                    ))
                        .font(.subheadline)
                }
                Spacer()
