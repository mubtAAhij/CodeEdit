...
struct SourceControlFetchView: View {
    ...
    var projectName: String {
        workspace.workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "empty", comment: "Default project name when workspace is empty")
    }
            ...
            HStack(alignment: .top, spacing: 20) {
                ...
                VStack(alignment: .leading, spacing: 5) {
                    Text("Fetching changes for “\(projectName)”...")
                        .font(.headline)
                    Text(String(localized: "fetching_changes_description", comment: "Description of fetch operation"))
                        .font(.subheadline)
            ...
            HStack {
                HStack(spacing: 7.5) {
                    ProgressView()
                        ...
                        .controlSize(.small)
                    Text(String(localized: "fetching_changes", comment: "Progress message while fetching"))
                        .font(.subheadline)
                }
                Spacer()
                Button {
