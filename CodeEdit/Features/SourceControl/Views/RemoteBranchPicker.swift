    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "source-control.add-remote", defaultValue: "Add Existing Remote...", comment: "Menu item to add existing remote"))
                    .tag(GitRemote?(nil))
            } label: {
                Text("Remote")
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(localized: "source-control.branch.create", defaultValue: "\(sourceControlManager.currentBranch?.name ?? "") (Create)", comment: "Label for creating a new branch"))
                    } icon: {
                        Image(symbol: "branch")
                    }
