    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "add_existing_remote", comment: "Menu option to add an existing remote"))
                    .tag(GitRemote?(nil))
            } label: {
                Text("Remote")
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(localized: "branch_name_create", comment: "Branch name with create indicator").replacingOccurrences(of: "{branch_name}", with: sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text(String(localized: "branch", comment: "Branch label"))
            }
        }
        .onAppear {
            if remote == nil {
