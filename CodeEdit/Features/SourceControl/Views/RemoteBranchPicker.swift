    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "source-control.add-existing-remote", defaultValue: "Add Existing Remote...", comment: "Add existing remote menu item"))
                    .tag(GitRemote?(nil))
            } label: {
                Text(String(localized: "source-control.remote-label", defaultValue: "Remote", comment: "Remote picker label"))
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: String(localized: "source-control.branch-create", defaultValue: "%@ (Create)", comment: "Create branch label"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text(String(localized: "source-control.branch-label", defaultValue: "Branch", comment: "Branch picker label"))
            }
        }
        .onAppear {
            if remote == nil {
