    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "source-control.remote-picker.add-remote", defaultValue: "Add Existing Remote...", comment: "Menu item to add existing remote"))
                    .tag(GitRemote?(nil))
            } label: {
            ...
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: String(localized: "source-control.remote-picker.create-branch", defaultValue: "%@ (Create)", comment: "Format for branch creation option"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
