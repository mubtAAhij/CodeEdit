    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "git.add-remote", defaultValue: "Add Existing Remote...", comment: "Add remote menu item"))
                    .tag(GitRemote?(nil))
            } label: {
            ...
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: String(localized: "git.branch-create", defaultValue: "%@ (Create)", comment: "Create branch format"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
