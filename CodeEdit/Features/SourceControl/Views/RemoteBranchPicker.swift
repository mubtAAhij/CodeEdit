        ...
        Group {
            ...
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: NSLocalizedString("source-control.branch.create", comment: "Branch creation label with branch name"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
