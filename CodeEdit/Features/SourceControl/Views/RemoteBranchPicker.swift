    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text("source_control.add_existing_remote", comment: "Add existing remote")
                    .tag(GitRemote?(nil))
            } label: {
                Text("Remote")
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text("source_control.branch_create \(sourceControlManager.currentBranch?.name ?? "")", comment: "Create branch label")
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text("source_control.branch", comment: "Branch label")
            }
        }
        .onAppear {
            if remote == nil {
