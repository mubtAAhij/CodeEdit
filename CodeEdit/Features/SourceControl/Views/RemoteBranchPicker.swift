    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "source-control.remote-picker.add-remote", defaultValue: "Add Existing Remote...", comment: "Menu item to add a new remote"))
                    .tag(GitRemote?(nil))
            } label: {
                Text(String(localized: "source-control.remote-picker.remote-label", defaultValue: "Remote", comment: "Label for remote picker"))
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: String(localized: "source-control.remote-picker.create-branch", defaultValue: "%@ (Create)", comment: "Label for creating a new remote branch"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text(String(localized: "source-control.remote-picker.branch-label", defaultValue: "Branch", comment: "Label for branch picker"))
            }
        }
        .onAppear {
            if remote == nil {
