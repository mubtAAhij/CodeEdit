    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "source-control.add-remote", defaultValue: "Add Existing Remote...", comment: "Add remote menu item"))
                    .tag(GitRemote?(nil))
            } label: {
            ...
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(format: String(localized: "source-control.create-branch", defaultValue: "%@ (Create)", comment: "Create branch label"), sourceControlManager.currentBranch?.name ?? ""))
                    } icon: {
                        Image(symbol: "branch")
                    }
