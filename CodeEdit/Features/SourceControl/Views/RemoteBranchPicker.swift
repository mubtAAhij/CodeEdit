    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text(String(localized: "Add Existing Remote...", comment: "Menu item to add a new remote"))
                    .tag(GitRemote?(nil))
            } label: {
                Text("Remote")
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text(String(localized: "\(sourceControlManager.currentBranch?.name ?? "") (Create)", comment: "Label for creating a new branch on remote"))
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text(String(localized: "Branch", comment: "Label for branch picker"))
            }
        }
        .onAppear {
            if remote == nil {
