    ...
    var body: some View {
        Group {
            Picker(selection: $remote) {
                ...
                Divider()
                Text("String(localized: "add_existing_remote", comment: "Menu item text for adding an existing remote repository")")
                    .tag(GitRemote?(nil))
            } label: {
                Text("Remote")
            }
            Picker(selection: $branch) {
                if shouldCreateBranch {
                    Label {
                        Text("String(localized: "branch_create_format", comment: "Format for branch name with create indicator").replacingOccurrences(of: "{branch}", with: sourceControlManager.currentBranch?.name ?? "")")
                    } icon: {
                        Image(symbol: "branch")
                    }
                ...
                Text("String(localized: "branch_label", comment: "Label for branch picker")")
            }
        }
        .onAppear {
            if remote == nil {
