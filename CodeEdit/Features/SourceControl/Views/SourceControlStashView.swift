...
struct SourceControlStashView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    TextField("", text: $message, prompt: Text(String(localized: "source-control-stash.message-prompt", defaultValue: "Message (optional)", comment: "Message prompt for stash")), axis: .vertical)
                        .labelsHidden()
                        .lineLimit(3...3)
                    ...
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            ...
                            Text(String(localized: "source-control-stash.uncommitted-changes-message", defaultValue: "Your local repository has uncommitted changes that need to be stashed before you can continue. Enter a description for your changes.", comment: "Uncommitted changes stash message"))
                        } else {
                            Text(String(localized: "source-control-stash.description-message", defaultValue: "Enter a description for your stashed changes so you can reference them later. Stashes will appear in the Source Control navigator for your repository.", comment: "Stash description message"))
                        }
                    }
            ...
            HStack {
                ...
                Button {
                    ...
                    Text(String(localized: "source-control-stash.cancel", defaultValue: "Cancel", comment: "Cancel button"))
                        .frame(minWidth: 56)
                }
                Button {
                    submit()
    ...
    func submit() {
        Task {
            do {
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source-control-stash.no-last-stash", defaultValue: "Could not find last stash", comment: "Could not find last stash error")]
                            )
                        }
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control-stash.failed-to-stash", defaultValue: "Failed to stash changes", comment: "Failed to stash changes error"), error: error)
            }
        }
    }
}
