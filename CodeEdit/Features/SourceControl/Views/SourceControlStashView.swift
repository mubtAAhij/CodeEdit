        ...
        VStack(spacing: 0) {
            Form {
                Section {
                    ...
                    Text(String(localized: "source-control.stash.header", defaultValue: "Stash Changes", comment: "Stash changes section header"))
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            || sourceControlManager.switchToBranch != nil {
                            Text(String(localized: "source-control.stash.uncommitted-changes", defaultValue: "Your local repository has uncommitted changes that need to be stashed before you can continue. Enter a description for your changes.", comment: "Uncommitted changes stash message"))
                        } else {
                            Text(String(localized: "source-control.stash.description", defaultValue: "Enter a description for your stashed changes so you can reference them later. Stashes will appear in the Source Control navigator for your repository.", comment: "Stash description instructions"))
                        }
                    }
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source-control.stash.could-not-find-last", defaultValue: "Could not find last stash", comment: "Could not find last stash error")]
                            )
                        }
