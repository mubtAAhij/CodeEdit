                ...
                Button {
                    submit()
                        ...
                        Text(
                            ...
                            : String(localized: "source_control.stash.stash", comment: "Button to stash changes")
                        )
                        .frame(minWidth: 56)
                    }
    ...
    func submit() {
        Task {
            do {
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source_control.error.no_last_stash", comment: "Error when last stash cannot be found")]
                            )
                        }
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source_control.error.failed_to_stash", comment: "Error title when stashing fails"), error: error)
            }
        }
    }
}
