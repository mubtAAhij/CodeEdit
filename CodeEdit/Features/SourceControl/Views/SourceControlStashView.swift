                ...
                Section {
                    ...
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            ...
                            Text(String(localized: "source-control.stash.description-required", defaultValue: "Your local repository has uncommitted changes that need to be stashed before you can continue. Enter a description for your changes.", comment: "Description when stashing is required before continuing"))
                        } else {
                            Text(String(localized: "source-control.stash.description-optional", defaultValue: "Enter a description for your stashed changes so you can reference them later. Stashes will appear in the Source Control navigator for your repository.", comment: "Description for optional stashing"))
                        }
                    }
                ...
                Button {
                    submit()
                        ...
                        Text(
                            ...
                            : String(localized: "source-control.stash.button", defaultValue: "Stash", comment: "Button to stash changes")
                        )
                        .frame(minWidth: 56)
                    }
    ...
    func submit() {
        Task {
            do {
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source-control.stash.not-found", defaultValue: "Could not find last stash", comment: "Error message when last stash cannot be found")]
                            )
                        }
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.stash.failed", defaultValue: "Failed to stash changes", comment: "Error message when stashing fails"), error: error)
            }
        }
    }
}
