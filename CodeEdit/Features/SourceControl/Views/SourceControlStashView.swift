...
struct SourceControlStashView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    TextField("", text: $message, prompt: Text(String(localized: "source-control.stash.message-prompt", defaultValue: "Message (optional)", comment: "Placeholder for stash message field")), axis: .vertical)
                        .labelsHidden()
                        .lineLimit(3...3)
                    ...
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            ...
                            Text(String(localized: "source-control.stash.uncommitted-changes-info", defaultValue: "Your local repository has uncommitted changes that need to be stashed before you can continue. Enter a description for your changes.", comment: "Information about stashing uncommitted changes"))
                        } else {
                            Text(String(localized: "source-control.stash.description-info", defaultValue: "Enter a description for your stashed changes so you can reference them later. Stashes will appear in the Source Control navigator for your repository.", comment: "Information about stash descriptions"))
                        }
                    }
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source-control.stash.error.not-found", defaultValue: "Could not find last stash", comment: "Error message when last stash cannot be found")]
                            )
                        }
