...
struct SourceControlStashView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    TextField("", text: $message, prompt: Text(String(localized: "source-control.stash.message-prompt", defaultValue: "Message (optional)", comment: "Stash message prompt")), axis: .vertical)
                        .labelsHidden()
                        .lineLimit(3...3)
                    ...
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            ...
                            Text(String(localized: "source-control.stash.description.uncommitted", defaultValue: "Your local repository has uncommitted changes that need to be stashed before you can continue. Enter a description for your changes.", comment: "Stash uncommitted changes description"))
                        } else {
                            Text(String(localized: "source-control.stash.description.general", defaultValue: "Enter a description for your stashed changes so you can reference them later. Stashes will appear in the Source Control navigator for your repository.", comment: "Stash general description"))
                        }
                    }
                ...
                if sourceControlManager.pullSheetIsPresented
                    || sourceControlManager.switchToBranch != nil {
                    Section {
                        Toggle(String(localized: "source-control.stash.apply-after", defaultValue: "Apply stash after operation", comment: "Apply stash after operation toggle"), isOn: $applyStashAfterOperation)
                    }
                }
            }
            .formStyle(.grouped)
