...
struct SourceControlStashView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section {
                    TextField("", text: $message, prompt: Text("source_control.stash.message_optional", comment: "Placeholder for stash message"), axis: .vertical)
                        .labelsHidden()
                        .lineLimit(3...3)
                        .contentShape(Rectangle())
                        .frame(height: 48)
                } header: {
                    Text("source_control.stash.title", comment: "Stash changes header")
                    Group {
                        if sourceControlManager.pullSheetIsPresented
                            || sourceControlManager.switchToBranch != nil {
                            Text("source_control.stash.description_required", comment: "Message when stash is required")
                        } else {
                            Text("source_control.stash.description_optional", comment: "Message when stash is optional")
                        }
                    }
    ...
    func submit() {
        Task {
            do {
                    ...
                    if applyStashAfterOperation {
                        guard let lastStashEntry = sourceControlManager.stashEntries.first else {
                            throw NSError(
                                ...
                                code: 1,
                                userInfo: [NSLocalizedDescriptionKey: String(localized: "source_control.stash.error.not_found", comment: "Error when stash not found")]
                            )
                        }
                ...
                await sourceControlManager.showAlertForError(title: String(localized: "source_control.stash.error.failed", comment: "Error when stash fails"), error: error)
            }
        }
    }
}
