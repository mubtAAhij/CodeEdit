    ...
    var body: some View {
        if let branch = fromBranch ?? sourceControlManager.currentBranch {
            VStack(spacing: 0) {
                Form {
                    Section {
                        LabeledContent(String(localized: "source-control.rename-branch.from", defaultValue: "From", comment: "From branch label"), value: branch.name)
                        TextField(String(localized: "source-control.rename-branch.to", defaultValue: "To", comment: "To branch placeholder"), text: $name)
                    } header: {
                ...
                HStack {
                    ...
                    Button {
                        ...
                        Text(String(localized: "source-control.rename-branch.rename-button", defaultValue: "Rename", comment: "Rename button"))
                            .frame(minWidth: 56)
                    }
        ...
        Task {
            do {
                ...
                await sourceControlManager.showAlertForError(
                    title: String(localized: "source-control.rename-branch.error", defaultValue: "Failed to create branch", comment: "Rename branch error alert title"),
                    error: error
                )
            }
        }
