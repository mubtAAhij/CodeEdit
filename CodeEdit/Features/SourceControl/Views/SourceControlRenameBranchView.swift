    ...
    var body: some View {
        if let branch = fromBranch ?? sourceControlManager.currentBranch {
            VStack(spacing: 0) {
                Form {
                    Section {
                        LabeledContent(String(localized: "source-control.rename-branch.from", defaultValue: "From", comment: "Label for current branch name"), value: branch.name)
                        ...
                        Text(String(localized: "source-control.rename-branch.message", defaultValue: "All uncommited changes will be preserved on the renamed branch.", comment: "Message about preserving uncommitted changes"))
                    }
                }
                .formStyle(.grouped)
                .scrollDisabled(true)
                ...
                HStack {
                    ...
                    Button {
                        ...
                        Text(String(localized: "source-control.rename-branch.cancel", defaultValue: "Cancel", comment: "Button to cancel branch rename"))
                            .frame(minWidth: 56)
                    }
                    Button {
                        submit(branch)
                    } label: {
                        Text(String(localized: "source-control.rename-branch.action", defaultValue: "Rename", comment: "Button to rename branch"))
                            .frame(minWidth: 56)
                    }
