        ...
        VStack(spacing: 0) {
            Form {
                Section {
                    ...
                    Text(String(localized: "source_control.pull.title", comment: "Title for pulling remote changes"))
                }
                Section {
                    Toggle(String(localized: "source_control.pull.rebase", comment: "Option to rebase local changes"), isOn: $sourceControlManager.operationRebase)
                }
            }
            .formStyle(.grouped)
            .scrollDisabled(true)
            ...
            HStack {
                if loading {
                    HStack(spacing: 7.5) {
                        ProgressView()
                            ...
                            .controlSize(.small)
                        Text(String(localized: "source_control.pull.progress", comment: "Progress message while pulling changes"))
                            .font(.subheadline)
                    }
                }
                Spacer()
                ...
                Button(action: submit) {
                    Text(String(localized: "source_control.pull.action", comment: "Pull button"))
                        .frame(minWidth: 56)
                }
    ...
    func submit() {
        Task {
            do {
                ...
                self.loading = false
                await sourceControlManager.showAlertForError(title: String(localized: "source_control.pull.error", comment: "Error title when pull fails"), error: error)
            }
        }
    }
}
