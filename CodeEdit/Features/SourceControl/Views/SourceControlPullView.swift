        ...
        VStack(spacing: 0) {
            Form {
                Section {
                    ...
                    Text(String(localized: "source_control.pull.pull_changes_from", comment: "Label for pulling remote changes"))
                }
                Section {
                    Toggle(String(localized: "source_control.pull.rebase_option", comment: "Option to rebase local changes during pull"), isOn: $sourceControlManager.operationRebase)
                }
            }
            .formStyle(.grouped)
            .scrollDisabled(true)
            ...
            HStack {
                if loading {
                    HStack(spacing: 7.5) {
                        ProgressView()
                            ...
                            .controlSize(.small)
                        Text(String(localized: "source_control.pull.pulling_changes", comment: "Status message while pulling changes"))
                            .font(.subheadline)
                    }
                }
                Spacer()
                ...
                Button(action: submit) {
                    Text(String(localized: "source_control.pull.pull_button", comment: "Button to pull changes"))
                        .frame(minWidth: 56)
                }
    ...
    func submit() {
        Task {
            do {
                ...
                self.loading = false
                await sourceControlManager.showAlertForError(title: String(localized: "source_control.pull.failed_error", comment: "Error message when pull operation fails"), error: error)
            }
        }
    }
}
