    ...
    func submit() {
        Task {
            do {
                ...
                self.loading = false
                await sourceControlManager.showAlertForError(title: String(localized: "source-control.pull.error", defaultValue: "Failed to pull", comment: "Pull error alert title"), error: error)
            }
        }
    }
}
