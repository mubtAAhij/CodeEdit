        ...
        VStack(spacing: 0) {
            Form {
                Section {
                    ...
                    Text(String(localized: "source-control.pull-remote-changes", defaultValue: "Pull remote changes from", comment: "Section header for pulling remote changes"))
                }
                Section {
                    Toggle(String(localized: "source-control.rebase-local-changes", defaultValue: "Rebase local changes onto upstream changes", comment: "Toggle for rebasing local changes"), isOn: $sourceControlManager.operationRebase)
                }
            }
            .formStyle(.grouped)
            .scrollDisabled(true)
                ...
                if loading {
                    HStack(spacing: 7.5) {
                        ProgressView()
                            ...
                            .controlSize(.small)
                        Text(String(localized: "source-control.pulling-changes", defaultValue: "Pulling changes...", comment: "Status message while pulling changes"))
                            .font(.subheadline)
                    }
                }
                Spacer()
