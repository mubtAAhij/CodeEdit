...
final class SourceControlManager: ObservableObject {
    ...
    private func showAlert(title: String, message: String) async {
        await MainActor.run {
            ...
            alert.addButton(withTitle: String(localized: "source-control.manager.alert.ok-button-title", defaultValue: "OK", comment: "Confirmation button title in source control alerts"))
