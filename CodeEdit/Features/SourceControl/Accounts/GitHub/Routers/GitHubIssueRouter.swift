...
enum GitHubIssueRouter: GitJSONPostRouter {
    ...
    var params: [String: Any] {
        ...
        case let .readAuthenticatedIssues(_, page, perPage, state):
            return [String(localized: "github.api.param.per-page", defaultValue: "per_page", comment: "GitHub API parameter for per page"): perPage, String(localized: "github.api.param.page", defaultValue: "page", comment: "GitHub API parameter for page"): page, String(localized: "github.api.param.state", defaultValue: "state", comment: "GitHub API parameter for state"): state.rawValue]
        case .readIssue:
            return [:]
        case let .readIssues(_, _, _, page, perPage, state):
            return [String(localized: "github.api.param.per-page", defaultValue: "per_page", comment: "GitHub API parameter for per page"): perPage, String(localized: "github.api.param.page", defaultValue: "page", comment: "GitHub API parameter for page"): page, String(localized: "github.api.param.state", defaultValue: "state", comment: "GitHub API parameter for state"): state.rawValue]
        case let .postIssue(_, _, _, title, body, assignee, labels):
            var params: [String: Any] = [String(localized: "github.api.param.title", defaultValue: "title", comment: "GitHub API parameter for title"): title]
            if let body {
                params[String(localized: "github.api.param.body", defaultValue: "body", comment: "GitHub API parameter for body")] = body
            }
            if let assignee {
        ...
        case let .patchIssue(_, _, _, _, title, body, assignee, state):
            ...
            if let title {
                params[String(localized: "github.api.param.title", defaultValue: "title", comment: "GitHub API parameter for title")] = title
            }
            if let body {
                params[String(localized: "github.api.param.body", defaultValue: "body", comment: "GitHub API parameter for body")] = body
            }
            ...
            if let state {
                params[String(localized: "github.api.param.state", defaultValue: "state", comment: "GitHub API parameter for state")] = state.rawValue
            }
            return params
        case let .commentIssue(_, _, _, _, body):
            return [String(localized: "github.api.param.body", defaultValue: "body", comment: "GitHub API parameter for body"): body]
        case let .readIssueComments(_, _, _, _, page, perPage):
            return [String(localized: "github.api.param.per-page", defaultValue: "per_page", comment: "GitHub API parameter for per page"): perPage, String(localized: "github.api.param.page", defaultValue: "page", comment: "GitHub API parameter for page"): page]
    ...
    var path: String {
        ...
        case let .readIssue(_, owner, repository, number):
            return String(format: String(localized: "github.api.path.repo-issue", defaultValue: "repos/%@/%@/issues/%d", comment: "GitHub API path for repository issue"), owner, repository, number)
        ...
        case let .commentIssue(_, owner, repository, number, _):
            return String(format: String(localized: "github.api.path.repo-issue-comments", defaultValue: "repos/%@/%@/issues/%d/comments", comment: "GitHub API path for repository issue comments"), owner, repository, number)
        case let .readIssueComments(_, owner, repository, number, _, _):
            return String(format: String(localized: "github.api.path.repo-issue-comments", defaultValue: "repos/%@/%@/issues/%d/comments", comment: "GitHub API path for repository issue comments"), owner, repository, number)
        case let .patchIssueComment(_, owner, repository, number, _):
            return String(format: String(localized: "github.api.path.repo-issue-comment", defaultValue: "repos/%@/%@/issues/comments/%d", comment: "GitHub API path for repository issue comment"), owner, repository, number)
        }
    }
}
