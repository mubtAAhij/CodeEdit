    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.error.discard-failed", defaultValue: "Failed to discard changes", comment: "Error alert title when discarding changes fails"), error: error)
            }
        }
    }
