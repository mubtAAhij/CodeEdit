    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source_control.error.failed_discard_changes", comment: "Error title when failing to discard changes"), error: error)
            }
        }
    }
