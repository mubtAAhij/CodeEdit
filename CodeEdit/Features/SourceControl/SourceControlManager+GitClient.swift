    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "git.discard-changes.failed", defaultValue: "Failed to discard changes", comment: "Failed to discard changes alert title"), error: error)
            }
        }
    }
