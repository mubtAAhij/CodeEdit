    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard-error", defaultValue: "Failed to discard changes", comment: "Error title when discarding changes fails"), error: error)
            }
        }
    }
