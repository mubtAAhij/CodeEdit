    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.error.discard-changes", defaultValue: "Failed to discard changes", comment: "Error title when discarding file changes fails"), error: error)
            }
        }
    }
