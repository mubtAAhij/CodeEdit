    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard-changes-failed", defaultValue: "Failed to discard changes", comment: "Alert title when discarding changes fails"), error: error)
            }
        }
    }
