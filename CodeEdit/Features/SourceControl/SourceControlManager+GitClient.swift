    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "git.discard.failed", defaultValue: "Failed to discard changes", comment: "Failed to discard changes error title"), error: error)
            }
        }
    }
