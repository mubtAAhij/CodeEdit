    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.failed-to-discard-changes", defaultValue: "Failed to discard changes", comment: "Error title when failing to discard file changes"), error: error)
            }
        }
    }
