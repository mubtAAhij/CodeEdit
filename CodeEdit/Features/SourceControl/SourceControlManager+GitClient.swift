    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard.error-title", defaultValue: "Failed to discard changes", comment: "Error title when discard changes fails"), error: error)
            }
        }
    }
