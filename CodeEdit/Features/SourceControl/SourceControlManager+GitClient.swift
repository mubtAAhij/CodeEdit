    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.error.failed-discard-changes", defaultValue: "Failed to discard changes", comment: "Alert title when failing to discard changes"), error: error)
            }
        }
    }
