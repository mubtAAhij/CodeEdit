    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard-changes.failed-title", defaultValue: "Failed to discard changes", comment: "Alert title when discarding changes for a file fails"), error: error)
            }
        }
    }
    ...
    func discardAllChanges() {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard-all-changes.failed-title", defaultValue: "Failed to discard changes", comment: "Alert title when discarding all changes fails"), error: error)
            }
        }
    }
