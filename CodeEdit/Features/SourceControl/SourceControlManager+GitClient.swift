    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "source-control.discard-changes-error", defaultValue: "Failed to discard changes", comment: "Error message when discarding changes fails"), error: error)
            }
        }
    }
