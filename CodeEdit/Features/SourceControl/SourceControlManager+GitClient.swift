    ...
    func discardChanges(for file: URL) {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "failed_to_discard_changes", comment: "Error message when failing to discard file changes"), error: error)
            }
        }
    }
    ...
    func discardAllChanges() {
        Task {
            do {
                ...
                await showAlertForError(title: String(localized: "failed_to_discard_changes", comment: "Error message when failing to discard all changes"), error: error)
            }
        }
    }
