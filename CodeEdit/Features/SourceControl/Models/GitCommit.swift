...
struct GitCommit: Equatable, Hashable, Identifiable {
    ...
    var remoteString: String {
        if let remoteURL {
            if remoteURL.absoluteString.contains("github") {
                return String(localized: "GitHub", comment: "Service name")
            }
            if remoteURL.absoluteString.contains("bitbucket") {
                return String(localized: "BitBucket", comment: "Service name")
            }
            if remoteURL.absoluteString.contains("gitlab") {
                return String(localized: "GitLab", comment: "Service name")
            }
        }
        return "Remote"
    }
