...
struct GitCommit: Equatable, Hashable, Identifiable {
    ...
    var remoteString: String {
        if let remoteURL {
            if remoteURL.absoluteString.contains("github") {
                return String(localized: "source-control.remote.github", defaultValue: "GitHub", comment: "GitHub remote provider name")
            }
            if remoteURL.absoluteString.contains("bitbucket") {
                return String(localized: "source-control.remote.bitbucket", defaultValue: "BitBucket", comment: "BitBucket remote provider name")
            }
            if remoteURL.absoluteString.contains("gitlab") {
                return String(localized: "source-control.remote.gitlab", defaultValue: "GitLab", comment: "GitLab remote provider name")
            }
        }
        return "Remote"
    }
