...
struct GitCommit: Equatable, Hashable, Identifiable {
    ...
    var remoteString: String {
        if let remoteURL {
            if remoteURL.absoluteString.contains("github") {
                return String(localized: "source-control.remote.github", defaultValue: "GitHub", comment: "Git remote service name for GitHub")
            }
            if remoteURL.absoluteString.contains("bitbucket") {
                return String(localized: "source-control.remote.bitbucket", defaultValue: "BitBucket", comment: "Git remote service name for BitBucket")
            }
            if remoteURL.absoluteString.contains("gitlab") {
                return String(localized: "source-control.remote.gitlab", defaultValue: "GitLab", comment: "Git remote service name for GitLab")
            }
        }
        return String(localized: "source-control.remote.generic", defaultValue: "Remote", comment: "Generic git remote service name")
    }
}
