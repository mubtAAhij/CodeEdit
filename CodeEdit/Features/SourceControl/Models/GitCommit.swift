...
struct GitCommit: Equatable, Hashable, Identifiable {
    ...
    var remoteString: String {
        if let remoteURL {
            if remoteURL.absoluteString.contains("github") {
                return String(localized: "git.remote.github", defaultValue: "GitHub", comment: "GitHub remote")
            }
            if remoteURL.absoluteString.contains("bitbucket") {
                return String(localized: "git.remote.bitbucket", defaultValue: "BitBucket", comment: "BitBucket remote")
            }
            if remoteURL.absoluteString.contains("gitlab") {
                return String(localized: "git.remote.gitlab", defaultValue: "GitLab", comment: "GitLab remote")
            }
        }
        return String(localized: "git.remote", defaultValue: "Remote", comment: "Generic remote")
    }
}
