...
struct GitCommit: Equatable, Hashable, Identifiable {
    ...
    var remoteString: String {
        if let remoteURL {
            if remoteURL.absoluteString.contains("github") {
                return String(localized: "source-control.provider.github", defaultValue: "GitHub", comment: "GitHub provider")
            }
            if remoteURL.absoluteString.contains("bitbucket") {
                return String(localized: "source-control.provider.bitbucket", defaultValue: "BitBucket", comment: "BitBucket provider")
            }
            if remoteURL.absoluteString.contains("gitlab") {
                return String(localized: "source-control.provider.gitlab", defaultValue: "GitLab", comment: "GitLab provider")
            }
        }
        return String(localized: "source-control.provider.remote", defaultValue: "Remote", comment: "Remote provider")
    }
}
