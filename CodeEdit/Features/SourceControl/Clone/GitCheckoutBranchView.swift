        ...
        VStack(spacing: 8) {
            HStack {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "source-control.clone.checkout-branch.title", defaultValue: "Checkout branch", comment: "Title for checkout branch dialog"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "source-control.clone.checkout-branch.select-branch", defaultValue: "Select a branch to checkout", comment: "Instruction to choose a branch before checkout"))
                        .font(.system(size: 11))
                    ...
                    HStack {
                        Button(String(localized: "source-control.clone.checkout-branch.cancel", defaultValue: "Cancel", comment: "Cancel button title in checkout branch dialog")) {
                            dismiss()
                        }
                        Button(String(localized: "source-control.clone.checkout-branch.checkout", defaultValue: "Checkout", comment: "Confirm button title in checkout branch dialog")) {
                            Task {
