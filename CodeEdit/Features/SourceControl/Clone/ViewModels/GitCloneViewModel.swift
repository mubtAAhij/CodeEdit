...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "source_control.clone.error.git_not_found", comment: "Error message"),
                infoText: String(localized: "source_control.clone.error.git_not_found_info", comment: "Error description")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "source_control.clone.error.url_empty", comment: "Error message"),
                infoText: String(localized: "source_control.clone.error.url_empty_info", comment: "Error description")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "errors.error", comment: "Error title"), infoText: String(localized: "source_control.clone.error.directory_exists", comment: "Error description"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "source_control.clone.error.create_folder_failed", comment: "Error message"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "source_control.clone.error.failed", comment: "Error message"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "source_control.clone.error.failed", comment: "Error message"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "source_control.clone.error.delete_folder_failed", comment: "Error message"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "source_control.clone.action", comment: "Button text")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "source_control.clone.field_label", comment: "Field label")
        dialog.title = String(localized: "source_control.clone.action", comment: "Dialog title")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "actions.ok", comment: "Button text"))
