...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "error_git_not_found", comment: "Error message when Git is not installed"),
                infoText: String(localized: "error_git_not_found_description", comment: "Detailed error message for Git installation")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "error_url_empty", comment: "Error message when URL field is empty"),
                infoText: String(localized: "error_repository_required", comment: "Error message requiring repository specification")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "error_title", comment: "Generic error title"), infoText: String(localized: "error_directory_exists", comment: "Error message when target directory already exists"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "error_folder_creation_failed", comment: "Error message when folder creation fails"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "source_control.clone.failed_to_clone", comment: "Error message when git clone operation fails"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "source_control.clone.failed_to_clone", comment: "Error message when git clone operation fails"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "source_control.clone.failed_to_delete_folder", comment: "Error message when folder deletion fails during clone cleanup"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "source_control.clone.clone", comment: "Button text for clone action")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "source_control.clone.clone_as", comment: "Button text for clone as action")
        dialog.title = String(localized: "source_control.clone.clone", comment: "Button text for clone action")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "common.ok", comment: "OK button text"))
