...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "git-clone.error.git-not-found", defaultValue: "Git installation not found.", comment: "Error when Git is not installed"),
        ...
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "git-clone.error.empty-url", defaultValue: "Url cannot be empty", comment: "Error when repository URL is empty"),
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "error", defaultValue: "Error", comment: "Error alert title"), infoText: String(localized: "git-clone.error.directory-exists", defaultValue: "Directory already exists", comment: "Error when directory already exists"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.create-folder-failed", defaultValue: "Failed to create folder", comment: "Error when creating folder fails"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "git-clone.error.clone-failed", defaultValue: "Failed to clone", comment: "Error when cloning fails"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "git-clone.error.clone-failed", defaultValue: "Failed to clone", comment: "Error when cloning fails"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.delete-folder-failed", defaultValue: "Failed to delete folder", comment: "Error when deleting folder fails"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "git-clone.dialog.prompt", defaultValue: "Clone", comment: "Save panel prompt for clone")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "git-clone.dialog.name-field-label", defaultValue: "Clone as", comment: "Save panel name field label")
        dialog.title = String(localized: "git-clone.dialog.title", defaultValue: "Clone", comment: "Save panel title for clone")
