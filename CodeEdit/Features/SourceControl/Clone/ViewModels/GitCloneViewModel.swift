...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "git.installation-not-found", defaultValue: "Git installation not found.", comment: "Git installation not found error"),
                infoText: String(localized: "git.ensure-installed", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Ensure Git is installed message")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "git.url-cannot-be-empty", defaultValue: "Url cannot be empty", comment: "Url cannot be empty error"),
                infoText: String(localized: "git.must-specify-repository", defaultValue: "You must specify a repository to clone", comment: "You must specify a repository to clone message")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "error.title", defaultValue: "Error", comment: "Error title"), infoText: String(localized: "git.directory-already-exists", defaultValue: "Directory already exists", comment: "Directory already exists error"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "git.failed-to-create-folder", defaultValue: "Failed to create folder", comment: "Failed to create folder error"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "git.failed-to-clone", defaultValue: "Failed to clone", comment: "Failed to clone error"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "git.failed-to-clone", defaultValue: "Failed to clone", comment: "Failed to clone error"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "git.failed-to-delete-folder", defaultValue: "Failed to delete folder", comment: "Failed to delete folder error"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "git.clone", defaultValue: "Clone", comment: "Clone button")
        ...
        dialog.title = String(localized: "git.clone-a-repository", defaultValue: "Clone a Repository", comment: "Clone a Repository dialog title")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "alert.ok", defaultValue: "OK", comment: "OK button"))
