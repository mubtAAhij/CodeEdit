...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                ...
                infoText: String(localized: "git-clone.ensure-git-installed", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Ensure Git is installed message")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                ...
                infoText: String(localized: "git-clone.must-specify-repo", defaultValue: "You must specify a repository to clone", comment: "Must specify repository message")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "common.error", defaultValue: "Error", comment: "Error alert title"), infoText: String(localized: "git-clone.directory-exists", defaultValue: "Directory already exists", comment: "Directory already exists error"))
            return
        }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "git-clone.clone", defaultValue: "Clone", comment: "Clone button")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "git-clone.clone-as", defaultValue: "Clone as", comment: "Clone as field label")
        dialog.title = String(localized: "git-clone.clone-repository", defaultValue: "Clone a Repository", comment: "Clone a repository dialog title")
