...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                ...
                infoText: String(localized: "git-clone.git-not-found-info", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Git installation not found info")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                ...
                infoText: String(localized: "git-clone.url-empty-info", defaultValue: "You must specify a repository to clone", comment: "URL cannot be empty info")
            )
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.failed-to-create-folder", defaultValue: "Failed to create folder", comment: "Failed to create folder error"), infoText: "\(error)")
            return
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.failed-to-delete-folder", defaultValue: "Failed to delete folder", comment: "Failed to delete folder error"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "git-clone.clone-prompt", defaultValue: "Clone", comment: "Clone button in save panel")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "git-clone.clone-as-label", defaultValue: "Clone as", comment: "Clone as label in save panel")
        dialog.title = String(localized: "git-clone.title", defaultValue: "Clone a Repository", comment: "Clone repository dialog title")
