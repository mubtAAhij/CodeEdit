...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "git-clone.error.git-not-found", defaultValue: "Git installation not found.", comment: "Git not found error"),
                infoText: String(localized: "git-clone.error.git-not-found-info", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Git not found info text")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "git-clone.error.url-empty", defaultValue: "Url cannot be empty", comment: "URL empty error"),
                infoText: String(localized: "git-clone.error.url-empty-info", defaultValue: "You must specify a repository to clone", comment: "URL empty info text")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "git-clone.error.title", defaultValue: "Error", comment: "Error alert title"), infoText: String(localized: "git-clone.error.directory-exists", defaultValue: "Directory already exists", comment: "Directory already exists error"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.create-folder-failed", defaultValue: "Failed to create folder", comment: "Failed to create folder error"), infoText: String(describing: error))
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "git-clone.error.clone-failed", defaultValue: "Failed to clone", comment: "Failed to clone error"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "git-clone.error.clone-failed", defaultValue: "Failed to clone", comment: "Failed to clone error"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.delete-folder-failed", defaultValue: "Failed to delete folder", comment: "Failed to delete folder error"), infoText: String(describing: error))
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "git-clone.dialog.prompt", defaultValue: "Clone", comment: "Clone dialog prompt")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "git-clone.dialog.name-field", defaultValue: "Clone as", comment: "Clone as field label")
        dialog.title = String(localized: "git-clone.dialog.title", defaultValue: "Clone a Repository", comment: "Clone repository dialog title")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "git-clone.alert.ok", defaultValue: "OK", comment: "OK button"))
