...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "source-control.clone.git-installation-not-found.title", defaultValue: "Git installation not found.", comment: "Alert title when Git executable is not available"),
                infoText: String(localized: "source-control.clone.git-installation-not-found.message", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Alert message prompting user to install Git")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "source-control.clone.url-empty.title", defaultValue: "Url cannot be empty", comment: "Validation title when clone URL field is empty"),
                infoText: String(localized: "source-control.clone.url-empty.message", defaultValue: "You must specify a repository to clone", comment: "Validation message requiring repository URL before cloning")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "source-control.clone.directory-exists.error-title", defaultValue: "Error", comment: "Generic error title for clone destination validation"), infoText: String(localized: "source-control.clone.directory-exists.message", defaultValue: "Directory already exists", comment: "Error message when clone destination directory already exists"))
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "source-control.clone.failed-to-clone.title", defaultValue: "Failed to clone", comment: "Primary error title when git clone operation fails"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "source-control.clone.failed-to-clone.message", defaultValue: "Failed to clone", comment: "Fallback error message when git clone operation fails"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "source-control.clone.failed-to-delete-folder", defaultValue: "Failed to delete folder", comment: "Error message when cleanup folder deletion fails after clone error"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "source-control.clone.action.clone", defaultValue: "Clone", comment: "Primary action label for cloning repository")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "source-control.clone.action.clone-as", defaultValue: "Clone as", comment: "Label prefix for clone destination naming")
        dialog.title = String(localized: "source-control.clone.title", defaultValue: "Clone a Repository", comment: "Window or sheet title for cloning a repository")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "source-control.clone.ok", defaultValue: "OK", comment: "Confirmation button for clone alerts"))
