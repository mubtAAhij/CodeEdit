...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "git-clone.error.no-git-title", defaultValue: "Git installation not found.", comment: "Git not installed error title"),
                infoText: String(localized: "git-clone.error.no-git-message", defaultValue: "Ensure Git is installed on your system and try again.", comment: "Git not installed error message")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "git-clone.error.empty-url-title", defaultValue: "Url cannot be empty", comment: "Empty URL error title"),
                infoText: String(localized: "git-clone.error.empty-url-message", defaultValue: "You must specify a repository to clone", comment: "Empty URL error message")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "git-clone.error.generic-title", defaultValue: "Error", comment: "Generic error title"), infoText: String(localized: "git-clone.error.directory-exists", defaultValue: "Directory already exists", comment: "Directory already exists error"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.create-folder-title", defaultValue: "Failed to create folder", comment: "Failed to create folder error title"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "git-clone.error.failed-clone-title", defaultValue: "Failed to clone", comment: "Failed to clone error title"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "git-clone.error.failed-clone-title", defaultValue: "Failed to clone", comment: "Failed to clone error title"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "git-clone.error.delete-folder-title", defaultValue: "Failed to delete folder", comment: "Failed to delete folder error title"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.nameFieldLabel = String(localized: "git-clone.dialog.name-field-label", defaultValue: "Clone as", comment: "Clone as field label")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "git-clone.alert.ok-button", defaultValue: "OK", comment: "OK button in git clone alert"))
