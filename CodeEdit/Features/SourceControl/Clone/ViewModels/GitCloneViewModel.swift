...
class GitCloneViewModel: ObservableObject {
    ...
    func cloneRepository(completionHandler: @escaping (URL) -> Void) {
        if !isGitInstalled() {
            showAlert(
                alertMsg: String(localized: "Git installation not found.", comment: "Alert message when Git is not found"),
                infoText: String(localized: "Ensure Git is installed on your system and try again.", comment: "Alert information when Git is not found")
            )
            return
        }
        if repoUrlStr == "" {
            showAlert(
                alertMsg: String(localized: "Url cannot be empty", comment: "Alert message when URL is empty"),
                infoText: String(localized: "You must specify a repository to clone", comment: "Alert information when URL is empty")
            )
            return
        }
        ...
        if FileManager.default.fileExists(atPath: localPath.relativePath, isDirectory: &isDir) {
            showAlert(alertMsg: String(localized: "Error", comment: "Alert message for error"), infoText: String(localized: "Directory already exists", comment: "Alert information when directory already exists"))
            return
        }
        ...
        do {
            ...
            showAlert(alertMsg: String(localized: "Failed to create folder", comment: "Alert message when folder creation fails"), infoText: "\(error)")
            return
        }
        ...
        do {
            ...
            await MainActor.run {
                if let error = error as? GitClient.GitClientError {
                    showAlert(alertMsg: String(localized: "Failed to clone", comment: "Alert message when cloning fails"), infoText: error.description)
                } else {
                    showAlert(alertMsg: String(localized: "Failed to clone", comment: "Alert message when cloning fails"), infoText: error.localizedDescription)
                }
                deleteTemporaryFolder(localPath: localPath)
            }
        }
    ...
    private func deleteTemporaryFolder(localPath: URL) {
        do {
            ...
            showAlert(alertMsg: String(localized: "Failed to delete folder", comment: "Alert message when folder deletion fails"), infoText: "\(error)")
            return
        }
    }
    ...
    private func getPath(saveName: String) -> URL? {
        ...
        dialog.prompt = String(localized: "Clone", comment: "Save panel prompt for cloning")
        dialog.nameFieldStringValue = saveName
        dialog.nameFieldLabel = String(localized: "Clone as", comment: "Save panel field label for clone destination")
        dialog.title = String(localized: "Clone", comment: "Save panel title for cloning")
    ...
    private func showAlert(alertMsg: String, infoText: String) {
        ...
        alert.addButton(withTitle: String(localized: "OK", comment: "Alert button to dismiss"))
