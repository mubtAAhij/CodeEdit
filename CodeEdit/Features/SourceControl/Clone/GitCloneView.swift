...
struct GitCloneView: View {
            ...
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "source-control.clone.title", defaultValue: "Clone a Repository", comment: "Title for clone repository dialog"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "source-control.clone.prompt", defaultValue: "Enter a git repository URL:", comment: "Prompt to enter repository URL"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "source-control.clone.placeholder", defaultValue: "Git Repository URL", comment: "Placeholder for repository URL text field"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "source-control.clone.cancel", defaultValue: "Cancel", comment: "Button to cancel clone operation")) {
                            dismiss()
                        }
                        Button(String(localized: "source-control.clone.action", defaultValue: "Clone", comment: "Button to start clone operation")) {
                            cloneRepository()
                        }
    ...
    @ViewBuilder private var cloningSheet: some View {
        ...
        .toolbar {
            ToolbarItem {
                Button(String(localized: "source-control.clone.cancel-cloning", defaultValue: "Cancel Cloning", comment: "Button to cancel active cloning operation")) {
