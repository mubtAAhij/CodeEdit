            ...
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "git-clone.title", defaultValue: "Clone a Repository", comment: "Clone repository title"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "git-clone.prompt", defaultValue: "Enter a git repository URL:", comment: "Enter repository URL prompt"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "git-clone.placeholder", defaultValue: "Git Repository URL", comment: "Repository URL placeholder"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)
