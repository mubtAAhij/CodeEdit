...
struct GitCloneView: View {
        ...
        VStack(spacing: 8) {
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "git-clone.title", defaultValue: "Clone a Repository", comment: "Clone a repository title"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "git-clone.prompt", defaultValue: "Enter a git repository URL:", comment: "Enter git repository URL prompt"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "git-clone.url-placeholder", defaultValue: "Git Repository URL", comment: "Git repository URL placeholder"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "git-clone.cancel", defaultValue: "Cancel", comment: "Cancel button label")) {
                            dismiss()
                        }
                        Button(String(localized: "git-clone.clone", defaultValue: "Clone", comment: "Clone button label")) {
                            cloneRepository()
                        }
    ...
    @ViewBuilder private var cloningSheet: some View {
        ...
        .toolbar {
            ToolbarItem {
                Button(String(localized: "git-clone.cancel-cloning", defaultValue: "Cancel Cloning", comment: "Cancel cloning button label")) {
