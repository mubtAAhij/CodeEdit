        ...
        VStack(spacing: 8) {
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    ...
                    TextField(String(localized: "git.clone.url-field", defaultValue: "Git Repository URL", comment: "Placeholder for git repository URL field"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "git.clone.cancel", defaultValue: "Cancel", comment: "Cancel button in git clone dialog")) {
                            dismiss()
                        }
                        Button(String(localized: "git.clone.button", defaultValue: "Clone", comment: "Clone button to start cloning repository")) {
                            cloneRepository()
                        }
