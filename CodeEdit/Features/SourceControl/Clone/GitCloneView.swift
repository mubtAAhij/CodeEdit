...
struct GitCloneView: View {
            ...
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "source-control.clone-title", defaultValue: "Clone a Repository", comment: "Title for clone repository dialog"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "source-control.clone-prompt", defaultValue: "Enter a git repository URL:", comment: "Prompt for repository URL"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "source-control.clone-url-placeholder", defaultValue: "Git Repository URL", comment: "Placeholder for repository URL field"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "source-control.cancel", defaultValue: "Cancel", comment: "Button to cancel clone operation")) {
                            dismiss()
                        }
                        Button(String(localized: "source-control.clone", defaultValue: "Clone", comment: "Button to start cloning repository")) {
                            cloneRepository()
                        }
    ...
    @ViewBuilder private var cloningSheet: some View {
        ...
        .toolbar {
            ToolbarItem {
                Button(String(localized: "source-control.cancel-cloning", defaultValue: "Cancel Cloning", comment: "Button to cancel cloning operation")) {
