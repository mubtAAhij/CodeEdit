            ...
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "git.clone.title", defaultValue: "Clone a Repository", comment: "Clone repository title"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "git.clone.prompt", defaultValue: "Enter a git repository URL:", comment: "Prompt to enter repository URL"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "git.clone.url-placeholder", defaultValue: "Git Repository URL", comment: "Repository URL text field placeholder"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "git.clone.cancel", defaultValue: "Cancel", comment: "Cancel clone button")) {
                            dismiss()
                        }
                        Button(String(localized: "git.clone.clone", defaultValue: "Clone", comment: "Clone button")) {
                            cloneRepository()
                        }
