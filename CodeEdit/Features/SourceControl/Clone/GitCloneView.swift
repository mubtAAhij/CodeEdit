...
struct GitCloneView: View {
        ...
        VStack(spacing: 8) {
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "sourcecontrol.clone.title", defaultValue: "Clone a Repository", comment: "Clone repository dialog title"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "sourcecontrol.clone.instruction", defaultValue: "Enter a git repository URL:", comment: "Instruction to enter repository URL"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "sourcecontrol.clone.url.placeholder", defaultValue: "Git Repository URL", comment: "Git repository URL placeholder"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)

                    HStack {
                        Spacer()
                        Button(String(localized: "sourcecontrol.clone.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                            dismiss()
                        }
                        Button(String(localized: "sourcecontrol.clone.button", defaultValue: "Clone", comment: "Clone button")) {
                            cloneRepository()
                        }
    ...
    @ViewBuilder private var cloningSheet: some View {
        ...
        .toolbar {
            ToolbarItem {
                Button(String(localized: "sourcecontrol.clone.cancel.cloning", defaultValue: "Cancel Cloning", comment: "Cancel cloning button")) {
