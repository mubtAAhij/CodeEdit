...
struct GitCloneView: View {
        ...
        VStack(spacing: 8) {
            HStack(alignment: .top) {
                ...
                VStack(alignment: .leading) {
                    Text(String(localized: "git.clone.title", defaultValue: "Clone a Repository", comment: "Clone repository dialog title"))
                        .bold()
                        .padding(.bottom, 2)
                    Text(String(localized: "git.clone.instruction", defaultValue: "Enter a git repository URL:", comment: "Instruction to enter repository URL"))
                        .font(.system(size: 11))
                        .foregroundColor(.secondary)
                        .alignmentGuide(.trailing) { context in
                            context[.trailing]
                        }
                    TextField(String(localized: "git.clone.url_placeholder", defaultValue: "Git Repository URL", comment: "Git repository URL placeholder"), text: $viewModel.repoUrlStr)
                        .lineLimit(1)
                        .padding(.bottom, 15)
    ...
    @ViewBuilder private var cloningSheet: some View {
        ...
        .toolbar {
            ToolbarItem {
                Button(String(localized: "git.clone.cancel_cloning", defaultValue: "Cancel Cloning", comment: "Cancel cloning button")) {
