...
class GitClient {
    enum GitClientError: Error {
        ...
        var description: String {
            ...
            case .notGitRepository: String(localized: "git-client.not-git-repository", defaultValue: "Not a git repository", comment: "Not a git repository error")
            case .failedToDecodeURL: String(localized: "git-client.failed-decode-url", defaultValue: "Failed to decode URL", comment: "Failed to decode URL error")
            case .noRemoteConfigured: String(localized: "git-client.no-remote-configured", defaultValue: "No remote configured", comment: "No remote configured error")
            case .statusParseEarlyEnd: String(localized: "git-client.status-parse-early-end", defaultValue: "Invalid status, found end of string too early", comment: "Status parse early end error")
            case let .invalidStatus(char): String(format: String(localized: "git-client.invalid-status", defaultValue: "Invalid status received: %@", comment: "Invalid status received error"), String(char))
            case let .statusInvalidChangeType(char): String(format: String(localized: "git-client.status-invalid-change-type", defaultValue: "Status invalid change type: %@", comment: "Status invalid change type error"), String(char))
            }
        }
    }
