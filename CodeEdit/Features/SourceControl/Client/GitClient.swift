...
class GitClient {
    enum GitClientError: Error {
        ...
        var description: String {
            ...
            case .notGitRepository: String(localized: "git.error.not-repository", defaultValue: "Not a git repository", comment: "Error when directory is not a git repository")
            case .failedToDecodeURL: String(localized: "git.error.failed-to-decode-url", defaultValue: "Failed to decode URL", comment: "Error when URL decoding fails")
            case .noRemoteConfigured: String(localized: "git.error.no-remote", defaultValue: "No remote configured", comment: "Error when no remote is configured")
            case .statusParseEarlyEnd: String(localized: "git.error.status-parse-early-end", defaultValue: "Invalid status, found end of string too early", comment: "Error when status parsing ends prematurely")
            case let .invalidStatus(char): String(format: String(localized: "git.error.invalid-status", defaultValue: "Invalid status received: %@", comment: "Error for invalid git status character"), String(char))
            case let .statusInvalidChangeType(char): String(format: String(localized: "git.error.invalid-change-type", defaultValue: "Status invalid change type: %@", comment: "Error for invalid change type character"), String(char))
            }
        }
    }
