    ...
    enum GitClientError: Error {
        ...
        var description: String {
            ...
            case .notGitRepository: String(
                ...
                comment: "Error message when target directory is not a git repository"
            )
            case .failedToDecodeURL: String(
                ...
                comment: "Error message when git client fails to decode remote URL"
            )
            case .noRemoteConfigured: String(
                ...
                comment: "Error message when no remote is configured for repository"
            )
            case .statusParseEarlyEnd: String(
                localized: "source-control.git-client.error.invalid-status-early-end",
                defaultValue: "Invalid status, found end of string too early",
                comment: "Error message when git status output ends unexpectedly"
            )
            case let .invalidStatus(char): String(format: String(
                ...
                comment: "Error message when git status output contains unexpected status character"
            ), "\(char)")
            case let .statusInvalidChangeType(char): String(format: String(
                ...
                comment: "Error message when git status output contains invalid change type character"
            ), "\(char)")
            }
        }
    }
