        ...
        do {
            ...
            if let error = error as? GitClientError,
               ...
               error.description.contains("already exists") {
                ...
                logger.error(String(format: String(
                    localized: "source-control.git.failed-to-checkout-branch",
                    defaultValue: "Failed to checkout branch: %@",
                    comment: "Error message when Git branch checkout fails"
                ), "\(error)"))
            }
        }
    }
