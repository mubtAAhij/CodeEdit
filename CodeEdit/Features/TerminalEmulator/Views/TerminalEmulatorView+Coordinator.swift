...
extension TerminalEmulatorView {
    final class Coordinator: NSObject, CELocalShellTerminalViewDelegate {
        ...
        func processTerminated(source: TerminalView, exitCode: Int32?) {
            ...
            if case .shell = mode {
                source.feed(text: String(format: String(localized: "terminal.exit-code", defaultValue: "Exit code: %d\n\r\n", comment: "Terminal exit code message with status code"), exitCode))
                source.feed(text: String(localized: "terminal.new-session-instruction", defaultValue: "To open a new session, create a new terminal tab.", comment: "Instruction message after terminal process exits"))
