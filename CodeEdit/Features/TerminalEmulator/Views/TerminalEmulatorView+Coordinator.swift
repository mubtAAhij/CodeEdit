...
extension TerminalEmulatorView {
    final class Coordinator: NSObject, CELocalShellTerminalViewDelegate {
        ...
        func processTerminated(source: TerminalView, exitCode: Int32?) {
            ...
            if case .shell = mode {
                source.feed(text: String(localized: "terminal.exitCode", comment: "Terminal exit message", arguments: exitCode))
                source.feed(text: String(localized: "terminal.newSession", comment: "Terminal new session message"))
