...
class CELocalShellTerminalView: CETerminalView, TerminalViewDelegate, LocalProcessDelegate {
    ...
    public func startProcess(
        ...
        do {
            ...
            terminal.feed(text: String(format: String(localized: "terminal.local-shell.start-session.failed", defaultValue: "Failed to start a terminal session: %@", comment: "Error shown when local terminal session fails to start"), "\(error.localizedDescription)"))
        }
    }
