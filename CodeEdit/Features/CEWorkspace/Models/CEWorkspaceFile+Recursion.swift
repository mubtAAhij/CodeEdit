    ...
    func childrenDescription(tabCount: Int = 0, using fileManager: CEWorkspaceFileManager) -> String {
        ...
        if !self.isFolder { // if im a file, just return the url
            ...
            if fileManager.hasLoadedChildrenFor(file: self) {
                ...
                childDetails += String(
                    format: String(
                        localized: "workspace.file.recursion.children-not-loaded",
                        defaultValue: "\n%@%@ Children Not Loaded",
                        comment: "Line for unloaded children in recursive workspace file debug description"
                    ),
                    String(repeating: "|  ", count: max(tabCount - 1, 0)),
                    tabCount != 0 ? "╰--" : ""
                )
            }
            return childDetails
