...
extension CEWorkspaceFile {
    ...
    func childrenDescription(tabCount: Int = 0, using fileManager: CEWorkspaceFileManager) -> String {
        var myDetails = "\(String(repeating: "|  ", count: max(tabCount - 1, 0)))\(tabCount != 0 ? String(localized: "workspace.debug.tree_branch", defaultValue: "╰--", comment: "Debug tree branch") : "")"
        ...
        if !self.isFolder { // if im a file, just return the url
            ...
            if fileManager.hasLoadedChildrenFor(file: self) {
                ...
                childDetails += "\n\(String(repeating: "|  ", count: max(tabCount - 1, 0)))\(tabCount != 0 ? String(localized: "workspace.debug.tree_branch_alt", defaultValue: "╰--", comment: "Debug tree branch alternate") : "") Children Not Loaded"
            }
            return childDetails
        }
    }
