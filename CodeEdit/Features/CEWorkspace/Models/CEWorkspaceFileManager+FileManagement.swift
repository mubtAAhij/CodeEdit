...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file-management.delete-single.confirmation-title", defaultValue: "Do you want to delete “%@”?", comment: "Confirmation title for deleting a single file or folder"), "\(fileName)")
        deleteConfirmation.informativeText = String(localized: "workspace.file-management.delete-single.confirmation-message", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Confirmation message for deleting a single file or folder")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-management.delete-single.confirmation-delete", defaultValue: "Delete", comment: "Delete button title in single-item delete confirmation"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-management.delete-single.confirmation-cancel", defaultValue: "Cancel", comment: "Cancel button title in single-item delete confirmation"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file-management.delete-multiple.confirmation-title", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Confirmation title for deleting multiple selected items"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "workspace.file-management.delete-multiple.confirmation-message", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Confirmation message for deleting multiple selected items"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-management.delete-multiple.confirmation-delete", defaultValue: "Delete", comment: "Delete button title in multiple-item delete confirmation"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-management.delete-multiple.confirmation-cancel", defaultValue: "Cancel", comment: "Cancel button title in multiple-item delete confirmation"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "workspace.file-management.duplicate.default-copy-name", defaultValue: "%@ copy%@", comment: "Default duplicated file name pattern with optional extension"), "\(fileName)", "\(fileExtension)"))
        }
