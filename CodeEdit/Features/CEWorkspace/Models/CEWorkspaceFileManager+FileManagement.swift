...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "workspace-file-manager.delete-confirmation", defaultValue: "Do you want to delete \"%@\"?", comment: "Delete file confirmation message"), fileName)
        ...
        deleteConfirmation.addButton(withTitle: String(localized: "workspace-file-manager.delete", defaultValue: "Delete", comment: "Delete button"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace-file-manager.cancel", defaultValue: "Cancel", comment: "Cancel button"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "workspace-file-manager.batch-delete-confirmation", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Batch delete confirmation message"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "workspace-file-manager.batch-delete-warning", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Batch delete warning message"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace-file-manager.batch-delete", defaultValue: "Delete", comment: "Delete button for batch operation"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace-file-manager.batch-cancel", defaultValue: "Cancel", comment: "Cancel button for batch operation"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "workspace-file-manager.file-copy-name", defaultValue: "%@ copy%@", comment: "Duplicate file name format"), fileName, fileExtension))
        }
