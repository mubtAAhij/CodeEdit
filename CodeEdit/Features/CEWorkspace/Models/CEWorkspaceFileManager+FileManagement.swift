...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "filemanager.delete.confirm", defaultValue: "Do you want to delete \"%@\"?", comment: "Confirmation message for deleting a file"), fileName)
        deleteConfirmation.informativeText = String(localized: "filemanager.delete.warning", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Warning that deletion is permanent")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "filemanager.delete.button", defaultValue: "Delete", comment: "Delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "filemanager.cancel.button", defaultValue: "Cancel", comment: "Cancel button title"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "filemanager.batch-delete.confirm", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Confirmation message for batch deleting files"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "filemanager.batch-delete.warning", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Warning that batch deletion is permanent"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "filemanager.delete.button", defaultValue: "Delete", comment: "Delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "filemanager.cancel.button", defaultValue: "Cancel", comment: "Cancel button title"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            let copyString = String(localized: "filemanager.duplicate.copy", defaultValue: "copy", comment: "Suffix added to duplicated file names")
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: "\(fileName) \(copyString)\(fileExtension)")
        }
