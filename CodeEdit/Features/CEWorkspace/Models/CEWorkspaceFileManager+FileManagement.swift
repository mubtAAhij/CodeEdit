...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.delete-confirm", defaultValue: "Do you want to delete \"%@\"?", comment: "Delete file confirmation"), fileName)
        deleteConfirmation.informativeText = String(localized: "file-manager.delete-immediately", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Delete immediately warning")
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.delete-multiple-confirm", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Delete multiple items confirmation"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "file-manager.delete-multiple-immediately", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Delete multiple items warning"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.delete", defaultValue: "Delete", comment: "Delete button"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "common.cancel", defaultValue: "Cancel", comment: "Cancel button"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "file-manager.copy-name", defaultValue: "%@ copy%@", comment: "Duplicate file name format"), fileName, fileExtension))
        }
