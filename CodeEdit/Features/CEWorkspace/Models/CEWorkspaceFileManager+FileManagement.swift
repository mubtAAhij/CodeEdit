...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.delete.confirmation.message", defaultValue: "Do you want to delete \"%@\"?", comment: "Alert message asking user to confirm deletion of a file"), fileName)
        deleteConfirmation.informativeText = String(localized: "file-manager.delete.confirmation.info", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Alert informative text warning that deletion is permanent")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.delete.button", defaultValue: "Delete", comment: "Delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.delete.cancel", defaultValue: "Cancel", comment: "Cancel button title"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "file-manager.batch-delete.confirmation.message", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Alert message asking user to confirm deletion of multiple files"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "file-manager.batch-delete.confirmation.info", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Alert informative text warning that multiple items will be permanently deleted"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.batch-delete.button", defaultValue: "Delete", comment: "Delete button title for batch deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "file-manager.batch-delete.cancel", defaultValue: "Cancel", comment: "Cancel button title for batch deletion"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "file-manager.duplicate.name-format", defaultValue: "%@ copy%@", comment: "Format for duplicated file name (name, extension)"), fileName, fileExtension))
        }
