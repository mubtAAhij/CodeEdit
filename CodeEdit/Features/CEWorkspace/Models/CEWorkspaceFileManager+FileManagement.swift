...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file_manager.delete_confirmation", defaultValue: "Do you want to delete \"%@\"?", comment: "Alert message asking to confirm file deletion"), fileName)
        deleteConfirmation.informativeText = String(localized: "workspace.file_manager.delete_info", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Alert informative text explaining deletion is permanent")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file_manager.delete", defaultValue: "Delete", comment: "Delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file_manager.cancel", defaultValue: "Cancel", comment: "Cancel button title"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file_manager.batch_delete_confirmation", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Alert message asking to confirm batch file deletion"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "workspace.file_manager.batch_delete_info", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Alert informative text explaining batch deletion is permanent"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file_manager.batch_delete", defaultValue: "Delete", comment: "Batch delete button title"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file_manager.batch_cancel", defaultValue: "Cancel", comment: "Batch cancel button title"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: String(format: String(localized: "workspace.file_manager.duplicate_name", defaultValue: "%@ copy%@", comment: "File name pattern for duplicated files"), fileName, fileExtension))
        }
