...
extension CEWorkspaceFileManager {
    ...
    public func delete(file: CEWorkspaceFile, confirmDelete: Bool = true) throws {
        ...
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file-manager.delete-confirmation.message", defaultValue: "Do you want to delete \"%@\"?", comment: "Alert message asking user to confirm file deletion"), fileName)
        deleteConfirmation.informativeText = String(localized: "workspace.file-manager.delete-confirmation.info", defaultValue: "This item will be deleted immediately. You can't undo this action.", comment: "Alert info text explaining deletion is permanent")
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-manager.delete-button", defaultValue: "Delete", comment: "Button title to confirm deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-manager.cancel-button", defaultValue: "Cancel", comment: "Button title to cancel deletion"))
    ...
    public func batchDelete(files: Set<CEWorkspaceFile>, confirmDelete: Bool = true) throws {
        let deleteConfirmation = NSAlert()
        deleteConfirmation.messageText = String(format: String(localized: "workspace.file-manager.batch-delete-confirmation.message", defaultValue: "Are you sure you want to delete the %d selected items?", comment: "Alert message asking user to confirm deletion of multiple files"), files.count)
        // swiftlint:disable:next line_length
        deleteConfirmation.informativeText = String(format: String(localized: "workspace.file-manager.batch-delete-confirmation.info", defaultValue: "%d items will be deleted immediately. You cannot undo this action.", comment: "Alert info text explaining multiple items will be permanently deleted"), files.count)
        deleteConfirmation.alertStyle = .critical
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-manager.batch-delete-button", defaultValue: "Delete", comment: "Button title to confirm batch deletion"))
        deleteConfirmation.buttons.last?.hasDestructiveAction = true
        deleteConfirmation.addButton(withTitle: String(localized: "workspace.file-manager.batch-cancel-button", defaultValue: "Cancel", comment: "Button title to cancel batch deletion"))
    ...
    public func duplicate(file: CEWorkspaceFile) throws {
        ...
        while fileManager.fileExists(atPath: fileUrl.path) {
            ...
            let duplicateName = String(format: String(localized: "workspace.file-manager.duplicate-filename-template", defaultValue: "%@ copy%@", comment: "Template for duplicate file names"), fileName, fileExtension)
            fileUrl = fileUrl.deletingLastPathComponent().appending(path: duplicateName)
        }
