...
extension CEWorkspaceFileManager {
    ...
    enum FileManagerError: LocalizedError {
        ...
        var recoverySuggestion: String? {
            switch self {
            case .fileNotIndexed:
                return String(localized: "workspace.file-manager.recovery.reindex", defaultValue: "Reopen the workspace to reindex the file system.", comment: "Recovery suggestion to reindex workspace")
            case .fileNotFound, .originFileNotFound:
                return String(localized: "workspace.file-manager.recovery.try-again", defaultValue: "The file may have moved during the operation, try again.", comment: "Recovery suggestion when file moved")
            case .destinationFileExists:
                return String(localized: "workspace.file-manager.recovery.rename-or-remove", defaultValue: "Use a different file name or remove the conflicting file.", comment: "Recovery suggestion when destination exists")
            case .invalidFileName:
                return String(localized: "workspace.file-manager.recovery.valid-name", defaultValue: "File names must not contain the : character and be less than 256 characters.", comment: "Recovery suggestion for invalid file name")
            }
        }
    }
}
