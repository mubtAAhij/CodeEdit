...
extension CEWorkspaceFileManager {
    ...
    enum FileManagerError: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .fileNotIndexed:
                return String(localized: "workspace.file-manager.error.file-not-indexed", defaultValue: "File not found in CodeEdit", comment: "Error when file is not indexed in CodeEdit")
            case .originFileNotFound:
        ...
        var recoverySuggestion: String? {
            ...
            case .destinationFileExists:
                return String(localized: "workspace.file-manager.recovery.different-name", defaultValue: "Use a different file name or remove the conflicting file.", comment: "Recovery suggestion to use different file name")
            case .invalidFileName:
                return String(localized: "workspace.file-manager.recovery.invalid-filename", defaultValue: "File names must not contain the : character and be less than 256 characters.", comment: "Recovery suggestion for invalid file name")
            }
        }
    }
}
