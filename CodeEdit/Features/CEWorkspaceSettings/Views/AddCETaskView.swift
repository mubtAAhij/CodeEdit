...
struct AddCETaskView: View {
    ...
    init() {
        self._newTask = StateObject(wrappedValue: CETask(target: String(localized: "task.target.my-mac", defaultValue: "My Mac", comment: "Default task target device")))
    }
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                ...
                Button {
                    ...
                    Text(String(localized: "save", defaultValue: "Save", comment: "Save button"))
                        .frame(minWidth: 56)
                }
