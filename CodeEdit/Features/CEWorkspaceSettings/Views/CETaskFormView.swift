        ...
        Form {
            Section {
                ...
                Picker(String(localized: "task-form.target", defaultValue: "Target", comment: "Label for task target picker"), selection: $task.target) {
                    Text(String(localized: "task-form.target.my-mac", defaultValue: "My Mac", comment: "Local Mac target option"))
                        .tag("My Mac")

                    Text(String(localized: "task-form.target.ssh", defaultValue: "SSH", comment: "SSH target option"))
                        .tag("SSH")

                    Text(String(localized: "task-form.target.docker", defaultValue: "Docker", comment: "Docker target option"))
                        .tag("Docker")

                    Text(String(localized: "task-form.target.docker-compose", defaultValue: "Docker Compose", comment: "Docker Compose target option"))
                        .tag("Docker Compose")
                }
                .disabled(true)
            }
            ...
            Section {
                ...
                .overlay {
                    if task.environmentVariables.isEmpty {
                        Text(String(localized: "task-form.no-environment-variables", defaultValue: "No environment variables", comment: "Placeholder text when environment variables list is empty"))
                ...
                Text(String(localized: "task-form.environment-variables", defaultValue: "Environment Variables", comment: "Section header for environment variables"))
            }
        }
        .formStyle(.grouped)
    }
