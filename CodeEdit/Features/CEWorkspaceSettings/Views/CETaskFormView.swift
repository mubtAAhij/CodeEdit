        ...
        Form {
            Section {
                ...
                Picker(String(localized: "task.target", defaultValue: "Target", comment: "Label for task target picker"), selection: $task.target) {
                    Text(String(localized: "task.target.my-mac", defaultValue: "My Mac", comment: "Target option for local Mac"))
                        .tag("My Mac")

                    Text(String(localized: "task.target.ssh", defaultValue: "SSH", comment: "Target option for SSH"))
                        .tag("SSH")

                    Text(String(localized: "task.target.docker", defaultValue: "Docker", comment: "Target option for Docker"))
                        .tag("Docker")

                    Text(String(localized: "task.target.docker-compose", defaultValue: "Docker Compose", comment: "Target option for Docker Compose"))
                        .tag("Docker Compose")
                }
                .disabled(true)
            }
            ...
            Section {
                ...
                .overlay {
                    if task.environmentVariables.isEmpty {
                        Text(String(localized: "task.env-vars.empty", defaultValue: "No environment variables", comment: "Empty state message for environment variables list"))
                ...
                Text(String(localized: "task.env-vars.header", defaultValue: "Environment Variables", comment: "Header for environment variables section"))
            }
        }
        .formStyle(.grouped)
    }
