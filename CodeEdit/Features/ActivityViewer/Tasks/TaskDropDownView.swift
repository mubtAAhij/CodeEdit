...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "Create Tasks", comment: "Placeholder text when no tasks exist"))
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "Create Tasks", comment: "Placeholder text when no tasks exist"))
        .accessibilityLabel(String(localized: "Active Task", comment: "Accessibility label for active task dropdown"))
        .accessibilityHint(String(localized: "Open the active task menu", comment: "Accessibility hint for active task dropdown"))
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "Add Task...", comment: "Menu item to add a new task")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
            OptionMenuItemView(label: String(localized: "Manage Tasks...", comment: "Menu item to manage tasks")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
        }
        .font(.subheadline)
