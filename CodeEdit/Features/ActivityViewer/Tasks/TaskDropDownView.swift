...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task_dropdown.create_tasks", comment: "Text displayed when no tasks are available"))
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityLabel(String(localized: "task_dropdown.active_task", comment: "Accessibility label for the active task dropdown"))
        .accessibilityHint(String(localized: "task_dropdown.open_menu_hint", comment: "Accessibility hint for opening the active task menu"))
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "task_dropdown.manage_tasks", comment: "Menu item to manage existing tasks")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
        }
        .font(.subheadline)
