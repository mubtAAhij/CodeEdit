...
struct TaskDropDownView: View {
        ...
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(
                    localized: "activity-viewer.task-dropdown.create-tasks",
                    defaultValue: "Create Tasks",
                    comment: "Inline action title to create tasks from task dropdown"
                ))
                    .frame(minWidth: 0)
            }
        }
    ...
    @ViewBuilder private var taskPopoverContent: some View {
        ...
        OptionMenuItemView(label: String(
            ...
            comment: "Menu action to add a new task"
        )) {
        ...
        OptionMenuItemView(label: String(
            ...
            comment: "Menu action to open task management view"
        )) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
    }
