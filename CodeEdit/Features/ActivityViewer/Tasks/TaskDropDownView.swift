...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks label"))
        .accessibilityLabel(String(localized: "task.active-task", defaultValue: "Active Task", comment: "Active task label"))
        .accessibilityHint(String(localized: "task.open-active-task-menu", defaultValue: "Open the active task menu", comment: "Open active task menu hint"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks label"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
