...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Placeholder text when no tasks are created"))
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Placeholder text when no tasks are created"))
        .accessibilityLabel(String(localized: "task.active-task", defaultValue: "Active Task", comment: "Accessibility label for active task"))
        .accessibilityHint(String(localized: "task.open-task-menu", defaultValue: "Open the active task menu", comment: "Accessibility hint for task menu"))
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "task.add-task", defaultValue: "Add Task...", comment: "Menu item to add task")) {
