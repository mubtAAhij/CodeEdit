...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks placeholder"))
        .accessibilityLabel(String(localized: "task-dropdown.active-task", defaultValue: "Active Task", comment: "Active task accessibility label"))
        .accessibilityHint(String(localized: "task-dropdown.active-task.hint", defaultValue: "Open the active task menu", comment: "Active task accessibility hint"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task-dropdown.create-tasks.label", defaultValue: "Create Tasks", comment: "Create tasks label"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
