...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task-dropdown.no-task", defaultValue: "Create Tasks", comment: "Default text when no task is selected"))
        .accessibilityLabel(String(localized: "task-dropdown.accessibility-label", defaultValue: "Active Task", comment: "Active task accessibility label"))
        .accessibilityHint(String(localized: "task-dropdown.accessibility-hint", defaultValue: "Open the active task menu", comment: "Active task accessibility hint"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks label"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
    ...
    @ViewBuilder private var taskPopoverContent: some View {
        ...
        OptionMenuItemView(label: String(localized: "task-dropdown.add-task", defaultValue: "Add Task...", comment: "Add task menu item")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
        OptionMenuItemView(label: String(localized: "task-dropdown.manage-tasks", defaultValue: "Manage Tasks...", comment: "Manage tasks menu item")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
    }
}
