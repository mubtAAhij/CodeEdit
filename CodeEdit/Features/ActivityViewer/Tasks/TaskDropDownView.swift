...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "tasks.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks placeholder"))
        .accessibilityLabel(String(localized: "tasks.active-task", defaultValue: "Active Task", comment: "Active task accessibility label"))
        .accessibilityHint(String(localized: "tasks.open-menu-hint", defaultValue: "Open the active task menu", comment: "Open active task menu hint"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "tasks.create-tasks", defaultValue: "Create Tasks", comment: "Create tasks placeholder"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
    ...
    @ViewBuilder private var taskPopoverContent: some View {
        ...
        OptionMenuItemView(label: String(localized: "tasks.add-task", defaultValue: "Add Task...", comment: "Add task menu item")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
        OptionMenuItemView(label: String(localized: "tasks.manage-tasks", defaultValue: "Manage Tasks...", comment: "Manage tasks menu item")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
    }
}
