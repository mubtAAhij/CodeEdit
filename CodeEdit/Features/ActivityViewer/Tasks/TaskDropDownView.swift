...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text("tasks.create", comment: "Create tasks prompt")
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "tasks.create", comment: "Create tasks prompt"))
        .accessibilityLabel("tasks.active_label", comment: "Active task label")
        .accessibilityHint("tasks.active_hint", comment: "Active task menu hint")
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "tasks.add", comment: "Add task button")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
            OptionMenuItemView(label: String(localized: "tasks.manage", comment: "Manage tasks button")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
        }
        .font(.subheadline)
