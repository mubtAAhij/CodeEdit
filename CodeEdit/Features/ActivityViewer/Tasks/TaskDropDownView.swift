...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Placeholder text when no task is selected"))
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Placeholder text when no task is selected"))
        .accessibilityLabel(String(localized: "accessibility.active-task", defaultValue: "Active Task", comment: "Accessibility label for active task dropdown"))
        .accessibilityHint(String(localized: "accessibility.open-task-menu", defaultValue: "Open the active task menu", comment: "Accessibility hint for active task dropdown"))
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "task.add-task", defaultValue: "Add Task...", comment: "Menu item to add a new task")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
            OptionMenuItemView(label: String(localized: "task.manage-tasks", defaultValue: "Manage Tasks...", comment: "Menu item to manage tasks")) {
                NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
            }
        }
        .font(.subheadline)
