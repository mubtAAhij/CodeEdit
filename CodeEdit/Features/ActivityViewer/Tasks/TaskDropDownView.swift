...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Default text when no task selected"))
        .accessibilityLabel(String(localized: "task-dropdown.accessibility-label", defaultValue: "Active Task", comment: "Accessibility label for task dropdown"))
        .accessibilityHint(String(localized: "task-dropdown.accessibility-hint", defaultValue: "Open the active task menu", comment: "Accessibility hint for task dropdown"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Default text when no task selected"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
    ...
    @ViewBuilder private var taskPopoverContent: some View {
        ...
        OptionMenuItemView(label: String(localized: "task-dropdown.add-task", defaultValue: "Add Task...", comment: "Menu item to add new task")) {
