...
struct TaskDropDownView: View {
    ...
    var body: some View {
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "activity-viewer.task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Default text shown when no task is selected"))
        .accessibilityLabel(String(localized: "activity-viewer.task-dropdown.active-task-label", defaultValue: "Active Task", comment: "Accessibility label for active task dropdown"))
        .accessibilityHint(String(localized: "activity-viewer.task-dropdown.active-task-hint", defaultValue: "Open the active task menu", comment: "Accessibility hint for active task dropdown"))
        .accessibilityAction {
    ...
    @ViewBuilder private var label: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "activity-viewer.task-dropdown.create-tasks", defaultValue: "Create Tasks", comment: "Default text shown when no task is selected"))
                    .frame(minWidth: 0)
            }
        }
        .font(.subheadline)
    ...
    @ViewBuilder private var taskPopoverContent: some View {
        ...
        OptionMenuItemView(label: String(localized: "activity-viewer.task-dropdown.add-task", defaultValue: "Add Task...", comment: "Menu item to add a new task")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
        OptionMenuItemView(label: String(localized: "activity-viewer.task-dropdown.manage-tasks", defaultValue: "Manage Tasks...", comment: "Menu item to manage tasks")) {
            NSApp.sendAction(#selector(CodeEditWindowController.openWorkspaceSettings(_:)), to: nil, from: nil)
        }
    }
}
