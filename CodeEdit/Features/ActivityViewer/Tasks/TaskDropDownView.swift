...
struct TaskDropDownView: View {
    ...
    var body: some View {
        Group {
            if let selectedTask = taskManager.selectedTask {
                ...
                Text(String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Text shown when no tasks exist"))
                    .frame(minWidth: 0)
            }
        }
        ...
        .accessibilityValue(taskManager.selectedTask?.name ?? String(localized: "task.create-tasks", defaultValue: "Create Tasks", comment: "Text shown when no tasks exist"))
        ...
        .accessibilityHint(String(localized: "accessibility.open-task-menu", defaultValue: "Open the active task menu", comment: "Accessibility hint for opening task menu"))
        .accessibilityAction {
    ...
    private var taskPopoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            OptionMenuItemView(label: String(localized: "task.add-task", defaultValue: "Add Task...", comment: "Menu item to add a task")) {
