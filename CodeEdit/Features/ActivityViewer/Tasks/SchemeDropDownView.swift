...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "activity-viewer.scheme.no-project", defaultValue: "No Project found", comment: "Fallback text when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel(String(localized: "activity-viewer.scheme.label", defaultValue: "Active Scheme", comment: "Accessibility label for active scheme dropdown"))
        .accessibilityHint(String(localized: "activity-viewer.scheme.hint", defaultValue: "Open the active scheme menu", comment: "Accessibility hint for scheme dropdown"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        ...
        Divider()
            .padding(.vertical, 5)
        Group {
            OptionMenuItemView(label: String(localized: "activity-viewer.scheme.add-folder", defaultValue: "Add Folder...", comment: "Menu item to add a folder to the workspace")) {
