...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "scheme.no_project_found", comment: "Message when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel(String(localized: "scheme.active_scheme", comment: "Accessibility label for active scheme button"))
        .accessibilityHint(String(localized: "scheme.open_menu_hint", comment: "Accessibility hint for opening scheme menu"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Divider()
                .padding(.vertical, 5)
            Group {
                OptionMenuItemView(label: String(localized: "scheme.add_folder", comment: "Menu item to add folder")) {
                    // TODO: Implment Add Folder
                    print("NOT IMPLEMENTED")
                }
                .disabled(true)
                OptionMenuItemView(label: String(localized: "scheme.workspace_settings", comment: "Menu item to open workspace settings")) {
                    NSApp.sendAction(
