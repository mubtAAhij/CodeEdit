...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "scheme.no-project", defaultValue: "No Project found", comment: "Default text when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel(String(localized: "scheme.dropdown.accessibility-label", defaultValue: "Active Scheme", comment: "Accessibility label for active scheme"))
        .accessibilityHint(String(localized: "scheme.dropdown.accessibility-hint", defaultValue: "Open the active scheme menu", comment: "Accessibility hint for scheme dropdown"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        ...
        Divider()
            .padding(.vertical, 5)
        Group {
            OptionMenuItemView(label: String(localized: "scheme.menu.add-folder", defaultValue: "Add Folder...", comment: "Menu item to add a folder")) {
                // TODO: Implment Add Folder
                print("NOT IMPLEMENTED")
            }
            .disabled(true)
            OptionMenuItemView(label: String(localized: "scheme.menu.workspace-settings", defaultValue: "Workspace Settings...", comment: "Menu item to open workspace settings")) {
                NSApp.sendAction(
