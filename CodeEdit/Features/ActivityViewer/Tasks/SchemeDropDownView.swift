...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "scheme-dropdown.no-project", defaultValue: "No Project found", comment: "Placeholder when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel(String(localized: "scheme-dropdown.active-scheme", defaultValue: "Active Scheme", comment: "Accessibility label for active scheme"))
        .accessibilityHint(String(localized: "scheme-dropdown.open-menu-hint", defaultValue: "Open the active scheme menu", comment: "Accessibility hint for opening scheme menu"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        ...
        Divider()
            .padding(.vertical, 5)
        Group {
            OptionMenuItemView(label: String(localized: "scheme-dropdown.add-folder", defaultValue: "Add Folder...", comment: "Menu item to add folder")) {
                // TODO: Implment Add Folder
                print("NOT IMPLEMENTED")
            }
            .disabled(true)
            OptionMenuItemView(label: String(localized: "scheme-dropdown.workspace-settings", defaultValue: "Workspace Settings...", comment: "Menu item for workspace settings")) {
                NSApp.sendAction(
