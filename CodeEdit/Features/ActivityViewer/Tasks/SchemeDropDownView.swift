...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "no_project_found", comment: "No project found message"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityHint(String(localized: "open_active_scheme_menu", comment: "Active scheme menu accessibility hint"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Divider()
                .padding(.vertical, 5)
            Group {
                OptionMenuItemView(label: String(localized: "add_folder", comment: "Add folder menu item")) {
                    // TODO: Implment Add Folder
                    print("NOT IMPLEMENTED")
                }
                .disabled(true)
                OptionMenuItemView(label: String(localized: "workspace_settings", comment: "Workspace settings menu item")) {
                    NSApp.sendAction(
