...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "workspace.no_project", comment: "Message when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel("accessibility.active_scheme", comment: "Accessibility label for scheme dropdown")
        .accessibilityHint("accessibility.open_scheme_menu", comment: "Accessibility hint for scheme menu")
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Divider()
                .padding(.vertical, 5)
            Group {
                OptionMenuItemView(label: String(localized: "menu.add_folder", comment: "Menu option to add folder")) {
                    // TODO: Implment Add Folder
                    print("NOT IMPLEMENTED")
                }
                .disabled(true)
                OptionMenuItemView(label: String(localized: "menu.workspace_settings", comment: "Menu option for workspace settings")) {
                    NSApp.sendAction(
