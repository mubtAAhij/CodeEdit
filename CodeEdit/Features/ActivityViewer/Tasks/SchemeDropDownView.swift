...
struct SchemeDropDownView: View {
    ...
    var workspaceDisplayName: String {
        ...
        ? (workspaceFileManager?.workspaceItem.fileName() ?? String(localized: "activity-viewer.scheme-dropdown.no-project", defaultValue: "No Project found", comment: "Default text when no project is found"))
        : workspaceName
    }
    ...
    var body: some View {
        ...
        .accessibilityLabel(String(localized: "activity-viewer.scheme-dropdown.active-scheme-label", defaultValue: "Active Scheme", comment: "Accessibility label for active scheme dropdown"))
        .accessibilityHint(String(localized: "activity-viewer.scheme-dropdown.active-scheme-hint", defaultValue: "Open the active scheme menu", comment: "Accessibility hint for active scheme dropdown"))
        .accessibilityAction {
            isSchemePopOverPresented.toggle()
        }
    }
    ...
    @ViewBuilder var popoverContent: some View {
        ...
        Divider()
            .padding(.vertical, 5)
        Group {
            OptionMenuItemView(label: String(localized: "activity-viewer.scheme-dropdown.add-folder", defaultValue: "Add Folder...", comment: "Menu item to add folder")) {
                // TODO: Implment Add Folder
                print("NOT IMPLEMENTED")
            }
            .disabled(true)
            OptionMenuItemView(label: String(localized: "activity-viewer.scheme-dropdown.workspace-settings", defaultValue: "Workspace Settings...", comment: "Menu item to open workspace settings")) {
                NSApp.sendAction(
