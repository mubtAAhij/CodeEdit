...
struct CECircularProgressView: View {
    ...
    var body: some View {
        Circle()
            ...
            .overlay {
                if currentTaskCount > 1 {
                    Text(String(format: String(
                        localized: "activity-viewer.circular-progress.task-count",
                        defaultValue: "%d",
                        comment: "Numeric task count displayed in circular progress view"
                    ), currentTaskCount))
                        .font(.caption)
                }
            }
            ...
            .accessibilityValue(
                progress != nil ? Text(progress!, format: .percent) : Text(String(
                    localized: "activity-viewer.circular-progress.working",
                    defaultValue: "working",
                    comment: "Status label shown under circular progress while tasks are running"
                ))
            )
    }
}
