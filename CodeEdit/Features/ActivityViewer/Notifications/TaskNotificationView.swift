    ...
    private func loaderView(notification: TaskNotificationModel) -> some View {
        if notification.isLoading {
            ...
            if taskNotificationHandler.notifications.count > 1 {
                Text(String(format: String(localized: "task-notifications.count", defaultValue: "%d", comment: "Task notification count badge"), taskNotificationHandler.notifications.count))
                    .font(.caption)
                    .padding(5)
                    .background(
                        Circle()
