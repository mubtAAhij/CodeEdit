    ...
    private func loaderView(notification: TaskNotificationModel) -> some View {
        if notification.isLoading {
            ...
            if taskNotificationHandler.notifications.count > 1 {
                Text(String(format: String(localized: "tasks.notification-count", defaultValue: "%d", comment: "Number of task notifications"), taskNotificationHandler.notifications.count))
                    .font(.caption)
                    .padding(5)
                    .background(
                        Circle()
