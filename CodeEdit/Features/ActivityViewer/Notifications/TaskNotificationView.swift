    ...
    private func loaderView(notification: TaskNotificationModel) -> some View {
        if notification.isLoading {
            ...
            if taskNotificationHandler.notifications.count > 1 {
                Text(String(format: String(localized: "activity-viewer.notifications.count", defaultValue: "%d", comment: "Number of notifications shown in activity viewer"), taskNotificationHandler.notifications.count))
                    .font(.caption)
                    .padding(5)
                    .background(
                        Circle()
