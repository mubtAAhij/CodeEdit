...
final class TaskNotificationHandler: ObservableObject {
    ...
    static func postTask(toWorkspace: URL? = nil, action: Action, model: TaskNotificationModel) {
        NotificationCenter.default.post(name: .taskNotification, object: nil, userInfo: [
            ...
            String(localized: "task.notification.key.isloading", defaultValue: "isLoading", comment: "Notification key for task loading state"): model.isLoading,
    ...
    private func createTask(task: [AnyHashable: Any]) {
        ...
        let task = TaskNotificationModel(
            ...
            isLoading: task[String(localized: "task.notification.key.isloading", defaultValue: "isLoading", comment: "Notification key for task loading state")] as? Bool ?? false
        )

        if action == String(localized: "task.notification.action.create", defaultValue: "create", comment: "Action to create notification") {
    ...
    private func updateTask(task: [AnyHashable: Any]) {
        guard let taskID = task[String(localized: "task.notification.key.id", defaultValue: "id", comment: "Notification key for task id")] as? String else { return }
            if let index = self.notifications.firstIndex(where: { $0.id == taskID }) {
                ...
                if let isLoading = task[String(localized: "task.notification.key.isloading", defaultValue: "isLoading", comment: "Notification key for task loading state")] as? Bool {
