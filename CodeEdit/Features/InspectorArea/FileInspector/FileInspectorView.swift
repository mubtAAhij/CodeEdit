...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.section.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.section.text-settings", defaultValue: "Text Settings", comment: "Section header for text editing settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.label.location", defaultValue: "Location", comment: "Label for file location")) {
                    Button(String(localized: "file-inspector.button.choose-location", defaultValue: "Choose...", comment: "Button to choose new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.label.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.label.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.label.tab", defaultValue: "Tab", comment: "Label for tab width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.help.tab-width", defaultValue: "The visual width of tab characters", comment: "Help text for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.label.indent", defaultValue: "Indent", comment: "Label for indent width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.help.indent-width", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.toggle.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle for line wrapping"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file-inspector.dialog.save-file", defaultValue: "Save File", comment: "Title for save file dialog")
