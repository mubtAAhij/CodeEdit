...
struct FileInspectorView: View {
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file-inspector.name", defaultValue: "Name", comment: "Label for file name text field"), text: $fileName)
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file-inspector.default-detected", defaultValue: "Default - Detected", comment: "Option for auto-detecting file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.spaces", defaultValue: "Spaces", comment: "Option to indent using spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.tabs", defaultValue: "Tabs", comment: "Option to indent using tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
        ...
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.tab", defaultValue: "Tab", comment: "Label for tab width setting"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.tab-help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.indent", defaultValue: "Indent", comment: "Label for indent width setting"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.indent-help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Label for wrap lines toggle"), isOn: $wrapLines)
