...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "inspector.file.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type settings")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "inspector.file.text-settings", defaultValue: "Text Settings", comment: "Section header for text-related file settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "inspector.file.name", defaultValue: "Name", comment: "Field label for file name"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "inspector.file.dismiss", defaultValue: "Dismiss", comment: "Button title to dismiss a file inspector popover or sheet"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "inspector.file.type", defaultValue: "Type", comment: "Field label for file type"),
            selection: $language
        ) {
            Text(String(localized: "inspector.file.type.default-detected", defaultValue: "Default - Detected", comment: "Picker option for automatically detected default file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "inspector.file.location", defaultValue: "Location", comment: "Field label for file location")) {
                    Button(String(localized: "inspector.file.location.choose", defaultValue: "Choose...", comment: "Button title to choose a new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "inspector.file.text-settings.indent-using", defaultValue: "Indent using", comment: "Label for indentation mode selector"), selection: $indentOption.indentType) {
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "inspector.file.text-settings.widths", defaultValue: "Widths", comment: "Section label for indentation width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "inspector.file.text-settings.widths.tab", defaultValue: "Tab", comment: "Field label for tab width value"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "inspector.file.text-settings.widths.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text describing tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "inspector.file.text-settings.widths.indent", defaultValue: "Indent", comment: "Field label for indent width value"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "inspector.file.text-settings.widths.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text describing indent width behavior"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "inspector.file.text-settings.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle label for wrapping long lines in the editor"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "inspector.file.save-file", defaultValue: "Save File", comment: "Button title to save changes made in file inspector")
