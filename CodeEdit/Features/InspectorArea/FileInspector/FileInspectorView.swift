...
struct FileInspectorView: View {
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "common.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file.inspector.type.default-detected", defaultValue: "Default - Detected", comment: "Option for auto-detected file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file.inspector.location", defaultValue: "Location", comment: "Label for file location")) {
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file.inspector.widths", defaultValue: "Widths", comment: "Label for width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file.inspector.width.tab", defaultValue: "Tab", comment: "Label for tab width setting"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file.inspector.width.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "file.inspector.width.indent", defaultValue: "Indent", comment: "Label for indent width setting"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file.inspector.width.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width setting"))
            }
        }
