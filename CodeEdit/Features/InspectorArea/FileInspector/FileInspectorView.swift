...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type settings")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.text-settings", defaultValue: "Text Settings", comment: "Section header for text editing settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file-inspector.name", defaultValue: "Name", comment: "Label for file name field"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss error alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "file-inspector.type", defaultValue: "Type", comment: "Label for file type picker"),
            selection: $language
        ) {
            Text(String(localized: "file-inspector.type.default-detected", defaultValue: "Default - Detected", comment: "Option for default auto-detected file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location", defaultValue: "Location", comment: "Label for file location section")) {
                    Button(String(localized: "file-inspector.location.choose", defaultValue: "Choose...", comment: "Button to choose new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.indent-using.spaces", defaultValue: "Spaces", comment: "Option to indent using spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.indent-using.tabs", defaultValue: "Tabs", comment: "Option to indent using tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "file-inspector.widths.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                ...
                .help(String(localized: "file-inspector.widths.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle for line wrapping"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file-inspector.save-file", defaultValue: "Save File", comment: "Title for save file dialog")
