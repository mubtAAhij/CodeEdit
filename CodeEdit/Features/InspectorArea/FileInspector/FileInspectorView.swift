...
struct FileInspectorView: View {
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField("file_inspector.name", text: $fileName, comment: "Text field label")
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text("file_inspector.default_detected", comment: "Picker option").tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent("file_inspector.location", comment: "Label") {
                    Button("file_inspector.choose", comment: "Button label") {
    ...
    private var indentUsing: some View {
        Picker("file_inspector.indent_using", selection: $indentOption.indentType, comment: "Picker label") {
            Text("file_inspector.spaces", comment: "Picker option").tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text("file_inspector.tabs", comment: "Picker option").tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent("file_inspector.widths", comment: "Label") {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text("file_inspector.tab", comment: "Label")
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help("file_inspector.tab_width_help", comment: "Tooltip text")
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text("file_inspector.indent", comment: "Label")
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help("file_inspector.indent_width_help", comment: "Tooltip text")
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle("file_inspector.wrap_lines", isOn: $wrapLines, comment: "Toggle label")
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file_inspector.save_file", comment: "Dialog title")
