...
struct FileInspectorView: View {
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file_inspector.name", comment: "Placeholder text for file name field"), text: $fileName)
                .background(
        ...
        Group {
            if let file {
                LabeledContent(String(localized: "file_inspector.location", comment: "Label for file location section")) {
                    Button(String(localized: "file_inspector.choose", comment: "Button text to choose file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file_inspector.indent_using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file_inspector.spaces", comment: "Option for using spaces for indentation")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file_inspector.tabs", comment: "Option for using tabs for indentation")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file_inspector.widths", comment: "Label for width settings section")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "file_inspector.tab_width_help", comment: "Help text explaining tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                ...
                .help(String(localized: "file_inspector.indent_spaces_help", comment: "Help text explaining indent spaces setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "wrap_lines", comment: "Toggle to wrap text lines in editor"), isOn: $wrapLines)
