...
struct FileInspectorView: View {
        ...
        Group {
            if file != nil {
                Form {
                    ...
                    Section(String(localized: "inspector.file.text-settings", defaultValue: "Text Settings", comment: "Section header for text editing settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "inspector.file.name", defaultValue: "Name", comment: "Label for file name text field"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "inspector.file.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss error alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "inspector.file.type.default-detected", defaultValue: "Default - Detected", comment: "Option for auto-detected file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "inspector.file.location", defaultValue: "Location", comment: "Label for file location section")) {
                    Button(String(localized: "inspector.file.location.choose", defaultValue: "Choose...", comment: "Button to choose new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "inspector.file.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "inspector.file.indent-using.spaces", defaultValue: "Spaces", comment: "Option for indenting with spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "inspector.file.indent-using.tabs", defaultValue: "Tabs", comment: "Option for indenting with tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "inspector.file.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "inspector.file.widths.tab", defaultValue: "Tab", comment: "Label for tab width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "inspector.file.widths.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "inspector.file.widths.indent", defaultValue: "Indent", comment: "Label for indent width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "inspector.file.widths.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "inspector.file.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle for line wrapping"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "inspector.file.save-file", defaultValue: "Save File", comment: "Title for save file dialog")
