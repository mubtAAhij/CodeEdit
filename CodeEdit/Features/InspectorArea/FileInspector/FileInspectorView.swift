...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "inspector.file.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "inspector.file.text-settings", defaultValue: "Text Settings", comment: "Section header for text settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "inspector.file.name", defaultValue: "Name", comment: "Label for file name field"), text: $fileName)
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "inspector.file.type.default-detected", defaultValue: "Default - Detected", comment: "Default detected file type option")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var indentUsing: some View {
        Picker(String(localized: "inspector.file.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "inspector.file.indent-using.spaces", defaultValue: "Spaces", comment: "Spaces indent option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "inspector.file.indent-using.tabs", defaultValue: "Tabs", comment: "Tabs indent option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "inspector.file.widths", defaultValue: "Widths", comment: "Label for width settings")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "inspector.file.widths.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                ...
                .help(String(localized: "inspector.file.widths.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width"))
            }
        }
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "inspector.file.save-file", defaultValue: "Save File", comment: "Title for save file panel")
