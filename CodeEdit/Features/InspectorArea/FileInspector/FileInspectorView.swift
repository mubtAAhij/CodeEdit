...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section("String(localized: "file_inspector_identity_type", comment: "Section header for file identity and type information")") {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "text_settings", comment: "Section header for text editing settings in file inspector")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "name", comment: "Label for file name text field"), text: $fileName)
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "type", comment: "Label for file type picker"),
            selection: $language
        ) {
            Text(String(localized: "default_detected", comment: "Option for automatically detected file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var indentUsing: some View {
        Picker(String(localized: "indent_using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "spaces", comment: "Option to use spaces for indentation")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "tabs", comment: "Option to use tabs for indentation")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "wrap_lines", comment: "Toggle for line wrapping setting"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "save_file", comment: "Title for save file dialog")
