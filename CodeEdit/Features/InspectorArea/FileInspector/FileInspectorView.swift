...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "Identity and Type", comment: "Section title")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "Text Settings", comment: "Section title")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "Name", comment: "Label text"), text: $fileName)
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "Type", comment: "Label text"),
            selection: $language
        ) {
            Text(String(localized: "Default - Detected", comment: "Menu item")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "Location", comment: "Label text")) {
                    Button(String(localized: "Choose...", comment: "Button text")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "Indent using", comment: "Label text"), selection: $indentOption.indentType) {
            Text(String(localized: "Spaces", comment: "Menu item")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "Tabs", comment: "Menu item")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "Widths", comment: "Label text")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "Tab", comment: "Label text"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "The visual width of tab characters", comment: "Tooltip text"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "Indent", comment: "Label text"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "The number of spaces to insert when the tab key is pressed.", comment: "Tooltip text"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "Wrap lines", comment: "Toggle text"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "Save File", comment: "Panel title")
