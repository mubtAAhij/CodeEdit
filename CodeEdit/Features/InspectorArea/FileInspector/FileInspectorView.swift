...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-type.header", defaultValue: "Identity and Type", comment: "Header for identity and type section")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.text-settings.header", defaultValue: "Text Settings", comment: "Header for text settings section")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.error.dismiss", defaultValue: "Dismiss", comment: "Dismiss button for error alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using.label", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.indent-using.spaces", defaultValue: "Spaces", comment: "Spaces indent option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.indent-using.tabs", defaultValue: "Tabs", comment: "Tabs indent option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths.label", defaultValue: "Widths", comment: "Label for width settings")) {
            HStack(spacing: 5) {
