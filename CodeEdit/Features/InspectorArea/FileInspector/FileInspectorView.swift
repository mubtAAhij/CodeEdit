...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-and-type", defaultValue: "Identity and Type", comment: "File inspector section header")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.text-settings", defaultValue: "Text Settings", comment: "File inspector section header for text settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "alert.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file-inspector.default-detected", defaultValue: "Default - Detected", comment: "Option for default auto-detected file type")).tag(nil as CodeLanguage?)
            Divider()
        ...
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location", defaultValue: "Location", comment: "Label for file location")) {
                    Button(String(localized: "file-inspector.choose", defaultValue: "Choose...", comment: "Button to choose file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.spaces", defaultValue: "Spaces", comment: "Indent using spaces option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.tabs", defaultValue: "Tabs", comment: "Indent using tabs option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
        ...
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.tab", defaultValue: "Tab", comment: "Label for tab width"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.tab-width-help", defaultValue: "The visual width of tab characters", comment: "Tooltip for tab width setting"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.indent", defaultValue: "Indent", comment: "Label for indent width"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file-inspector.indent-width-help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Tooltip for indent width setting"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle for wrapping lines"), isOn: $wrapLines)
