...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file_inspector.identity_type", defaultValue: "Identity and Type", comment: "Section header for identity and type")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file_inspector.text_settings", defaultValue: "Text Settings", comment: "Section header for text settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file_inspector.name", defaultValue: "Name", comment: "Label for file name field"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file_inspector.dismiss", defaultValue: "Dismiss", comment: "Button to dismiss alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "file_inspector.type", defaultValue: "Type", comment: "Label for file type picker"),
            selection: $language
        ) {
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file_inspector.location", defaultValue: "Location", comment: "Label for file location")) {
                    Button(String(localized: "file_inspector.choose", defaultValue: "Choose...", comment: "Button to choose file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file_inspector.indent_using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file_inspector.indent.spaces", defaultValue: "Spaces", comment: "Indent using spaces option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file_inspector.indent.tabs", defaultValue: "Tabs", comment: "Indent using tabs option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file_inspector.widths", defaultValue: "Widths", comment: "Label for width options")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file_inspector.tab", defaultValue: "Tab", comment: "Label for tab width"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file_inspector.tab.help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                    ...
                    .labelsHidden()
                    Text(String(localized: "file_inspector.indent", defaultValue: "Indent", comment: "Label for indent width"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
                .help(String(localized: "file_inspector.indent.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file_inspector.wrap_lines", defaultValue: "Wrap lines", comment: "Toggle for wrapping lines"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file_inspector.save_file", defaultValue: "Save File", comment: "Title for save file dialog")
