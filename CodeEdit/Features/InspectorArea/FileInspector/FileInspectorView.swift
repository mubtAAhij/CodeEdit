...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "inspector.file.identity-and-type-section", defaultValue: "Identity and Type", comment: "Section header for file identity and type")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "inspector.file.text-settings-section", defaultValue: "Text Settings", comment: "Section header for text settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "inspector.file.name-field", defaultValue: "Name", comment: "Text field for file name"), text: $fileName)
                .background(
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "inspector.file.type-label", defaultValue: "Type", comment: "Label for file type picker"),
            selection: $language
        ) {
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "inspector.file.location-label", defaultValue: "Location", comment: "Label for file location")) {
                    Button(String(localized: "inspector.file.choose-button", defaultValue: "Choose...", comment: "Button to choose file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "inspector.file.indent-using-label", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "inspector.file.widths-label", defaultValue: "Widths", comment: "Label for width options")) {
            HStack(spacing: 5) {
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "inspector.file.wrap-lines-toggle", defaultValue: "Wrap lines", comment: "Toggle to wrap lines"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "inspector.file.save-file-dialog-title", defaultValue: "Save File", comment: "Title for save file dialog")
