...
struct FileInspectorView: View {
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file-inspector.name-field", defaultValue: "Name", comment: "Label for file name field"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.dismiss-button", defaultValue: "Dismiss", comment: "Button to dismiss error alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "file-inspector.type-picker", defaultValue: "Type", comment: "Label for file type picker"),
            selection: $language
        ) {
            Text(String(localized: "file-inspector.default-detected", defaultValue: "Default - Detected", comment: "Default option for detected file type")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location-label", defaultValue: "Location", comment: "Label for file location")) {
                    Button(String(localized: "file-inspector.choose-button", defaultValue: "Choose...", comment: "Button to choose new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.spaces-option", defaultValue: "Spaces", comment: "Option to indent using spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.tabs-option", defaultValue: "Tabs", comment: "Option to indent using tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths-label", defaultValue: "Widths", comment: "Label for width options")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.tab-width-label", defaultValue: "Tab", comment: "Label for tab width stepper"))
                ...
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.indent-width-label", defaultValue: "Indent", comment: "Label for indent width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file-inspector.save-file-title", defaultValue: "Save File", comment: "Title for save file dialog")
