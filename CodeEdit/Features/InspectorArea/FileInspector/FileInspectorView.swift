...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-and-type", defaultValue: "Identity and Type", comment: "Identity and type section header")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.text-settings", defaultValue: "Text Settings", comment: "Text settings section header")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileNameField: some View {
        if let file {
            TextField(String(localized: "file-inspector.name", defaultValue: "Name", comment: "Name field label"), text: $fileName)
                .background(
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.dismiss", defaultValue: "Dismiss", comment: "Dismiss button"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            String(localized: "file-inspector.type", defaultValue: "Type", comment: "Type picker label"),
            selection: $language
        ) {
            Text(String(localized: "file-inspector.default-detected", defaultValue: "Default - Detected", comment: "Default detected type option")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location", defaultValue: "Location", comment: "Location label")) {
                    Button(String(localized: "file-inspector.choose", defaultValue: "Choose...", comment: "Choose location button")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Indent using picker label"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.spaces", defaultValue: "Spaces", comment: "Spaces option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.tabs", defaultValue: "Tabs", comment: "Tabs option")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Widths label")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "file-inspector.indent-help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Indent width help text"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Wrap lines toggle label"), isOn: $wrapLines)
