...
struct FileInspectorView: View {
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.dismiss", defaultValue: "Dismiss", comment: "Dismiss button for error alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file-inspector.type.default-detected", defaultValue: "Default - Detected", comment: "Option for default detected file type")).tag(nil as CodeLanguage?)
            Divider()
                    ...
                    Button(String(localized: "file-inspector.location.choose", defaultValue: "Choose...", comment: "Button to choose new file location")) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                let alert = NSAlert(error: error)
                                alert.addButton(withTitle: String(localized: "file-inspector.location.dismiss", defaultValue: "Dismiss", comment: "Dismiss button for location error alert"))
                                alert.runModal()
                            }
                        }
                    }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for width options")) {
            HStack(spacing: 5) {
