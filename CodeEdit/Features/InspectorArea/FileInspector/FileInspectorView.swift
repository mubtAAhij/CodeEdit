...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section("String(localized: "identity_and_type", comment: "Section header for file identity and type information")") {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "text_settings", comment: "File inspector section title for text editing settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "default_detected", comment: "Default option for automatically detected file type")).tag(nil as CodeLanguage?)
            Divider()
        ...
        Group {
            if let file {
                LabeledContent("Location") {
                    Button(String(localized: "choose", comment: "Button text to choose a new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "indent_using", comment: "Label for indent type selection"), selection: $indentOption.indentType) {
            Text(String(localized: "spaces", comment: "Indent type option - spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "tabs", comment: "Indent type option - tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "tab_width_help", comment: "Help text explaining tab character visual width"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                ...
                .help(String(localized: "indent_spaces_help", comment: "Help text explaining how many spaces are inserted when tab key is pressed"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "wrap_lines", comment: "Toggle label for line wrapping setting"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "save_file", comment: "Save panel title for choosing new file location")
