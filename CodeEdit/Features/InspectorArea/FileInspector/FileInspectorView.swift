...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type information")) {
                        fileNameField
                        fileType
                    }
                    Section {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file-inspector.type.default-detected", defaultValue: "Default - Detected", comment: "Option for automatic file type detection")).tag(nil as CodeLanguage?)
            Divider()
    ...
    private var location: some View {
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location", defaultValue: "Location", comment: "Label for file location field")) {
                    Button(String(localized: "file-inspector.choose", defaultValue: "Choose...", comment: "Button to choose new file location")) {
        ...
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.widths.tab", defaultValue: "Tab", comment: "Label for tab width stepper"))
                ...
                VStack(alignment: .center, spacing: 0) {
                    ...
                    .labelsHidden()
                    Text(String(localized: "file-inspector.widths.indent", defaultValue: "Indent", comment: "Label for indent width stepper"))
                        .foregroundColor(.primary)
                        .font(.footnote)
                }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle to enable line wrapping"), isOn: $wrapLines)
