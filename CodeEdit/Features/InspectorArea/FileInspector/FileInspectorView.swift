...
struct FileInspectorView: View {
    ...
    var body: some View {
        Group {
            if file != nil {
                Form {
                    Section(String(localized: "file-inspector.identity-and-type", defaultValue: "Identity and Type", comment: "Section header for file identity and type")) {
                        fileNameField
                        fileType
                    }
                    Section {
                    ...
                    Section(String(localized: "file-inspector.text-settings", defaultValue: "Text Settings", comment: "Section header for text settings")) {
                        indentUsing
                        widthOptions
                        wrapLinesToggle
                    }
                    ...
                    if file.validateFileName(for: fileName) {
                        ...
                        DispatchQueue.main.async { [weak workspace] in
                            do {
                                ...
                                alert.addButton(withTitle: String(localized: "file-inspector.dismiss", defaultValue: "Dismiss", comment: "Button label to dismiss alert"))
                                alert.runModal()
                            }
                        }
                    } else {
    ...
    @ViewBuilder private var fileType: some View {
        Picker(
            ...
            Text(String(localized: "file-inspector.type.default-detected", defaultValue: "Default - Detected", comment: "Picker option for auto-detected file type")).tag(nil as CodeLanguage?)
            Divider()
        ...
        Group {
            if let file {
                LabeledContent(String(localized: "file-inspector.location", defaultValue: "Location", comment: "Label for file location field")) {
                    Button(String(localized: "file-inspector.choose", defaultValue: "Choose...", comment: "Button label to choose new file location")) {
    ...
    private var indentUsing: some View {
        Picker(String(localized: "file-inspector.indent-using", defaultValue: "Indent using", comment: "Label for indent type picker"), selection: $indentOption.indentType) {
            Text(String(localized: "file-inspector.indent-using.spaces", defaultValue: "Spaces", comment: "Indent option for spaces")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            Text(String(localized: "file-inspector.indent-using.tabs", defaultValue: "Tabs", comment: "Indent option for tabs")).tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
        }
    ...
    private var widthOptions: some View {
        LabeledContent(String(localized: "file-inspector.widths", defaultValue: "Widths", comment: "Label for tab and indent width settings")) {
            HStack(spacing: 5) {
                ...
                .help(String(localized: "file-inspector.widths.tab-help", defaultValue: "The visual width of tab characters", comment: "Help text for tab width stepper"))
                VStack(alignment: .center, spacing: 0) {
                    Stepper(
                        "",
                        value: Binding<Double>(
                ...
                .help(String(localized: "file-inspector.widths.indent-help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width stepper"))
            }
        }
    ...
    private var wrapLinesToggle: some View {
        Toggle(String(localized: "file-inspector.wrap-lines", defaultValue: "Wrap lines", comment: "Toggle label for line wrapping"), isOn: $wrapLines)
    ...
    private func chooseNewFileLocation() -> URL? {
        ...
        dialogue.title = String(localized: "file-inspector.save-file", defaultValue: "Save File", comment: "Title for save file panel")
