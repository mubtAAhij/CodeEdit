            ...
            if model.sourceControlManager != nil {
                VStack {
                    if model.commitHistory.isEmpty {
                        CEContentUnavailableView(String(localized: "inspector.history.no-history", defaultValue: "No History", comment: "Empty state when no commit history is available"))
                    } else {
