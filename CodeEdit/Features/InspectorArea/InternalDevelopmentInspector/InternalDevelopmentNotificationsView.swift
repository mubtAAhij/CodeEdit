...
struct InternalDevelopmentNotificationsView: View {
    enum IconType: String, CaseIterable {
        ...
        var displayName: String {
            switch self {
            case .symbol:
                return String(localized: "internal-dev.notifications.icon-type.symbol", defaultValue: "Symbol", comment: "Icon type option for SF Symbol")
            case .image:
    ...
    var body: some View {
        Section(String(localized: "internal-dev.notifications.section-title", defaultValue: "Notifications", comment: "Section title for notifications testing")) {
            Toggle(String(localized: "internal-dev.notifications.delay-toggle", defaultValue: "Delay 5s", comment: "Toggle to enable 5 second delay before notification"), isOn: $delay)
            Toggle(String(localized: "internal-dev.notifications.sticky-toggle", defaultValue: "Sticky", comment: "Toggle to make notification sticky"), isOn: $sticky)
            ...
            Group {
                ...
                case .symbol:
                    Picker(String(localized: "internal-dev.notifications.symbol-picker", defaultValue: "Symbol", comment: "Picker label for symbol selection"), selection: $selectedSymbol) {
                        Label(String(localized: "internal-dev.notifications.random-option", defaultValue: "Random", comment: "Option to select a random item"), systemImage: "dice").tag(nil as String?)
                        Divider()
                ...
                if selectedIconType == .symbol || selectedIconType == .text || selectedIconType == .emoji {
                    Picker(String(localized: "internal-dev.notifications.icon-color-picker", defaultValue: "Icon Color", comment: "Picker label for icon color selection"), selection: $selectedColor) {
                        Label(String(localized: "internal-dev.notifications.random-option", defaultValue: "Random", comment: "Option to select a random item"), systemImage: "dice").tag(nil as Color?)
                        Divider()
            ...
            Button(String(localized: "internal-dev.notifications.add-button", defaultValue: "Add Notification", comment: "Button to add test notification")) {
                let action = {
                    switch selectedIconType {
                    case .symbol:
