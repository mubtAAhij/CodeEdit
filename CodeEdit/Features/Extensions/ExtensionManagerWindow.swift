    ...
    var body: some Scene {
        Window(String(localized: "extensions.title", defaultValue: "Extensions", comment: "Extensions window title"), id: SceneID.extensions.rawValue) {
            NavigationSplitView {
                ...
                case 0:
                    Text(String(localized: "extensions.select_prompt", defaultValue: "Select an extension", comment: "Prompt to select an extension"))
                case 1:
                    ExtensionDetailView(ext: selection.first!)
                default:
                    Text(String(
                        format: String(localized: "extensions.selected_count", defaultValue: "%d selected", comment: "Number of selected extensions"),
                        selection.count
                    ))
                }
            }
