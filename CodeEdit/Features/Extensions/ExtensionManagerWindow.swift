    ...
    var body: some Scene {
        Window(String(localized: "extensions.title", defaultValue: "Extensions", comment: "Extensions window title"), id: SceneID.extensions.rawValue) {
            NavigationSplitView {
                ...
                case 0:
                    Text(String(localized: "extensions.select-extension", defaultValue: "Select an extension", comment: "Select extension placeholder"))
                case 1:
                    ExtensionDetailView(ext: selection.first!)
                default:
                    Text(String(format: String(localized: "extensions.selected-count", defaultValue: "%d selected", comment: "Selected extensions count"), selection.count))
                }
            }
