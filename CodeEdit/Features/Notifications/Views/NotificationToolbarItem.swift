        ...
        if notificationManager.unreadCount > 0 || !visibleNotifications.isEmpty {
            Button {
                ...
                HStack(spacing: 4) {
                    ...
                    Text(String(format: String(localized: "notifications.toolbar.unread-count", defaultValue: "%d", comment: "Unread notifications count shown in toolbar"), notificationManager.unreadCount))
                        .monospacedDigit()
                }
            }
        }
