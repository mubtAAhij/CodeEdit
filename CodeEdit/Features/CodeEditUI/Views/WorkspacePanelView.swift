    ...
    var body: some View {
        VStack(spacing: 0) {
            if let selection = selectedTab {
                ...
                CEContentUnavailableView(String(localized: "workspace-panel.no-selection", defaultValue: "No Selection", comment: "Placeholder title when no workspace panel content is selected"))
            }
        }
