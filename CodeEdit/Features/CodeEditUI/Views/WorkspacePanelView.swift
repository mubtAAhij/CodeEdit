    ...
    var body: some View {
        VStack(spacing: 0) {
            if let selection = selectedTab {
                ...
                CEContentUnavailableView(String(localized: "workspace-panel.no-selection", defaultValue: "No Selection", comment: "Empty state for workspace panel with no selection"))
            }
        }
