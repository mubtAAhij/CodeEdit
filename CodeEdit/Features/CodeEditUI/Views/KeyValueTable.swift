...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "no_selection", comment: "No selection option in picker")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "cancel", comment: "Cancel button in dialog")) {
                    dismiss()
                }
                Button(String(localized: "add", comment: "Add button to confirm adding new item")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "edit_item", comment: "Edit item context menu option")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "remove", comment: "Remove item context menu option")) {
