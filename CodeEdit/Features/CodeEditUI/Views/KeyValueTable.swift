...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "key_value_table.no_selection", defaultValue: "No Selection", comment: "No selection picker option")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "key_value_table.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                    dismiss()
                }
                Button(String(localized: "key_value_table.add", defaultValue: "Add", comment: "Add button")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "key_value_table.edit", defaultValue: "Edit", comment: "Edit context menu item")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "key_value_table.remove", defaultValue: "Remove", comment: "Remove context menu item")) {
