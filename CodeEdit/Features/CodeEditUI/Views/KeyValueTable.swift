...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "ui.key-value-table.no-selection", defaultValue: "No Selection", comment: "Option for no selection in picker")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "ui.key-value-table.cancel", defaultValue: "Cancel", comment: "Cancel button in key-value table dialog")) {
                    dismiss()
                }
                Button(String(localized: "ui.key-value-table.add", defaultValue: "Add", comment: "Add button in key-value table dialog")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "ui.key-value-table.edit", defaultValue: "Edit", comment: "Context menu item to edit key-value entry")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "ui.key-value-table.remove", defaultValue: "Remove", comment: "Context menu item to remove key-value entry")) {
