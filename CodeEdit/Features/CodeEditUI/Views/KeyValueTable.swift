...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "key-value-table.no-selection", defaultValue: "No Selection", comment: "Picker option when no key is selected")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "key-value-table.cancel", defaultValue: "Cancel", comment: "Button to cancel adding item")) {
                    dismiss()
                }
                Button(String(localized: "key-value-table.add", defaultValue: "Add", comment: "Button to add new item")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "key-value-table.edit", defaultValue: "Edit", comment: "Context menu item to edit selected item")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "key-value-table.remove", defaultValue: "Remove", comment: "Context menu item to remove selected item")) {
