...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            HStack {
                Spacer()
                Button(String(localized: "key-value-table.cancel", defaultValue: "Cancel", comment: "Cancel button label")) {
                    dismiss()
                }
                Button(String(localized: "key-value-table.add", defaultValue: "Add", comment: "Add button label")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "key-value-table.edit", defaultValue: "Edit", comment: "Edit button label")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "key-value-table.remove", defaultValue: "Remove", comment: "Remove button label")) {
