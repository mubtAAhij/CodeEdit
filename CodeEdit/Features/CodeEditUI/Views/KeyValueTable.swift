...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "codeedit-ui.key-value-table.no-selection", defaultValue: "No Selection", comment: "Placeholder text shown when no key-value row is selected")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "codeedit-ui.key-value-table.cancel", defaultValue: "Cancel", comment: "Button title to cancel key-value edit action")) {
                    dismiss()
                }
                Button(String(localized: "codeedit-ui.key-value-table.add", defaultValue: "Add", comment: "Button title to add a key-value entry")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "codeedit-ui.key-value-table.edit", defaultValue: "Edit", comment: "Button title to edit selected key-value entry")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "codeedit-ui.key-value-table.remove", defaultValue: "Remove", comment: "Button title to remove selected key-value entry")) {
