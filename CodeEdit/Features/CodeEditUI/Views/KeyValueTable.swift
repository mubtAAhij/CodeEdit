...
private struct NewListTableItemView<HeaderView: View>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
                ...
                Section {
                    if validKeys.isEmpty {
                        ...
                        Picker(keyColumnName, selection: $key) {
                            ...
                            Divider()
                            Text(String(localized: "no_selection", comment: "Default option in picker when nothing is selected")).tag("")
                        }
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "cancel", comment: "Button to cancel adding new item")) {
                    dismiss()
                }
                Button(String(localized: "add", comment: "Button to add new item")) {
                    if !key.isEmpty && !value.isEmpty {
                        completion(key, value)
                    }
                }
    ...
    var body: some View {
        ...
        .contextMenu(
            ...
            menu: { selectedItems in
                Button(String(localized: "edit", comment: "Context menu option to edit item")) {
                    editItem(id: selectedItems.first)
                }
                Button(String(localized: "remove", comment: "Context menu option to remove item")) {
