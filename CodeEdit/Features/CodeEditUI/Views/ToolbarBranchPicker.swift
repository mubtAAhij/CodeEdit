...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "empty", defaultValue: "Empty", comment: "Empty workspace label")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "toolbar.current-branch", defaultValue: "Current Branch", comment: "Current branch section header"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "branches", defaultValue: "Branches", comment: "Branches section header"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "toolbar.failed-to-checkout", defaultValue: "Failed to checkout", comment: "Error title when branch checkout fails"), error: error)
                    }
                }
            }
        }
