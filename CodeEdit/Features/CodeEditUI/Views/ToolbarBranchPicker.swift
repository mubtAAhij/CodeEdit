...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "branch_picker.empty_state", defaultValue: "Empty", comment: "Branch picker empty state title.")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "branch_picker.current_branch_header", defaultValue: "Current Branch", comment: "Header for the current branch section in branch picker."))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "branch_picker.branches_header", defaultValue: "Branches", comment: "Header for the branches list section in branch picker."))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "branch_picker.checkout_failed", defaultValue: "Failed to checkout", comment: "Error message shown when checking out a branch fails."), error: error)
                    }
                }
            }
        }
