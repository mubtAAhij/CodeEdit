...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "toolbar.branch-picker.empty", defaultValue: "Empty", comment: "Empty workspace name placeholder")
    }
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "toolbar.branch-picker.checkout-failed", defaultValue: "Failed to checkout", comment: "Failed to checkout error title"), error: error)
                    }
                }
            }
        }
