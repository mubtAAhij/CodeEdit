...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? "String(localized: "empty", comment: "Placeholder text when no workspace is loaded")"
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel("String(localized: "current_branch", comment: "Section header for current git branch")")
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel("String(localized: "branches", comment: "Section header for git branches list")")
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: "String(localized: "failed_to_checkout", comment: "Error message when git checkout fails")", error: error)
                    }
                }
            }
        }
