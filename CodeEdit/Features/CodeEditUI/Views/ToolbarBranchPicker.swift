...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "toolbar.branch-picker.empty", defaultValue: "Empty", comment: "Empty branch label")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "toolbar.branch-picker.current-branch", defaultValue: "Current Branch", comment: "Current branch label"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "toolbar.branch-picker.branches", defaultValue: "Branches", comment: "Branches section label"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "toolbar.branch-picker.checkout-failed", defaultValue: "Failed to checkout", comment: "Error when checkout fails"), error: error)
                    }
                }
            }
        }
