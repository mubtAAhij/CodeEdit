        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "branch-picker.current-branch", defaultValue: "Current Branch", comment: "Section header for current branch"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "branch-picker.branches", defaultValue: "Branches", comment: "Section header for branches list"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "error.branch.checkout-failed", defaultValue: "Failed to checkout", comment: "Error title when branch checkout fails"), error: error)
                    }
                }
            }
        }
