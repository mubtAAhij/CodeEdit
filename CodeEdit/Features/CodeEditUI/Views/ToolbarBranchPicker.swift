...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? "String(localized: "empty", comment: "Default text when no workspace folder is selected")"
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel("String(localized: "current_branch", comment: "Header label for current branch section")")
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel("String(localized: "branches", comment: "Header label for branches section")")
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: "String(localized: "failed_to_checkout", comment: "Error message when branch checkout fails")", error: error)
                    }
                }
            }
        }
