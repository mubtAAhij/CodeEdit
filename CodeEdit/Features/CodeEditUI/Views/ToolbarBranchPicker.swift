...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "toolbar.branch-picker.empty", defaultValue: "Empty", comment: "Placeholder text when no workspace folder is open")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "toolbar.branch-picker.current-branch", defaultValue: "Current Branch", comment: "Section header for the current branch"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "toolbar.branch-picker.branches", defaultValue: "Branches", comment: "Section header for the list of branches"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "toolbar.branch-picker.checkout-failed", defaultValue: "Failed to checkout", comment: "Error title when branch checkout fails"), error: error)
                    }
                }
            }
        }
