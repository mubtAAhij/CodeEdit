...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "toolbar.branch.empty", defaultValue: "Empty", comment: "Empty workspace title")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "toolbar.branch.current", defaultValue: "Current Branch", comment: "Current branch section header"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "toolbar.branch.branches", defaultValue: "Branches", comment: "Branches section header"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "toolbar.branch.checkout-failed", defaultValue: "Failed to checkout", comment: "Failed to checkout error"), error: error)
                    }
                }
            }
        }
