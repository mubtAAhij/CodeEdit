...
struct ToolbarBranchPicker: View {
    ...
    private var title: String {
        workspaceFileManager?.folderUrl.lastPathComponent ?? String(localized: "empty", comment: "Empty workspace indicator")
    }
        ...
        var body: some View {
            VStack(alignment: .leading) {
                if let currentBranch = sourceControlManager.currentBranch {
                    Section {
                        headerLabel(String(localized: "current_branch", comment: "Current branch section header"))
                ...
                if !branches.isEmpty {
                    Section {
                        headerLabel(String(localized: "branches", comment: "Branches section header"))
            ...
            func switchBranch() {
                Task {
                    do {
                        ...
                        await sourceControlManager.showAlertForError(title: String(localized: "failed_to_checkout", comment: "Failed to checkout error title"), error: error)
                    }
                }
            }
        }
