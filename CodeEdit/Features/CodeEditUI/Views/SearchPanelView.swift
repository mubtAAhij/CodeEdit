    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "search.panel.no.matching.options", defaultValue: "No matching options", comment: "Message when no search results match"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "search.panel.select.option.preview", defaultValue: "Select an option to preview", comment: "Instruction to select option for preview"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
