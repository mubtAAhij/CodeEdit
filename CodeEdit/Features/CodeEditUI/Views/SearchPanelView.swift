    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "search-panel.no-matching-options", defaultValue: "No matching options", comment: "Message when no options match the search"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "search-panel.select-to-preview", defaultValue: "Select an option to preview", comment: "Message prompting user to select an option"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
