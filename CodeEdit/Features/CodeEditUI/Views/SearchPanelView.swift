...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "search-panel.no-matches", defaultValue: "No matching options", comment: "No matching options message"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "search-panel.select-preview", defaultValue: "Select an option to preview", comment: "Select option to preview message"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.close-overlay", defaultValue: "Close Overlay", comment: "Close overlay keyboard shortcut"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-up", defaultValue: "Select Up", comment: "Select up keyboard shortcut"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-down", defaultValue: "Select Down", comment: "Select down keyboard shortcut"))
    }
}
