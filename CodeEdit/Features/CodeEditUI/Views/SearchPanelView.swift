...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "search-panel.no-matching-options", defaultValue: "No matching options", comment: "Message when search has no matching options"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "search-panel.select-option-to-preview", defaultValue: "Select an option to preview", comment: "Instruction message in preview pane"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.close-overlay", defaultValue: "Close Overlay", comment: "Accessibility label for close overlay button"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-up", defaultValue: "Select Up", comment: "Accessibility label for select up button"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-down", defaultValue: "Select Down", comment: "Accessibility label for select down button"))
    }
}
