...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "search-panel.no-matching-options", defaultValue: "No matching options", comment: "Message shown when search returns no results"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "search-panel.select-to-preview", defaultValue: "Select an option to preview", comment: "Message shown in preview pane when no option is selected"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.close-overlay", defaultValue: "Close Overlay", comment: "Accessibility label for escape key action to close search overlay"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-up", defaultValue: "Select Up", comment: "Accessibility label for up arrow key action to select previous option"))
        Button {
            ...
            .accessibilityLabel(String(localized: "search-panel.select-down", defaultValue: "Select Down", comment: "Accessibility label for down arrow key action to select next option"))
    }
}
