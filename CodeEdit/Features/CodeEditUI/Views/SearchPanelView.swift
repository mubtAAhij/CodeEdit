...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text("No matching options", comment: "Message shown when search has no results")
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text("Select an option to preview", comment: "Message shown when no option is selected for preview")
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel("Close Overlay", comment: "Accessibility label for closing the search overlay")
        Button {
            ...
            .accessibilityLabel("Select Up", comment: "Accessibility label for selecting the previous item")
        Button {
            ...
            .accessibilityLabel("Select Down", comment: "Accessibility label for selecting the next item")
    }
}
