...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "no_matching_options", comment: "Message shown when search has no results"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text("String(localized: "select_option_to_preview", comment: "Message shown when no option is selected in preview mode")")
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        ...
        Button {
            ...
            .accessibilityLabel("String(localized: "select_up", comment: "Accessibility label for selecting previous item")")
        Button {
            ...
            .accessibilityLabel("String(localized: "select_down", comment: "Accessibility label for selecting next item")")
    }
}
