...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text(String(localized: "code-edit-ui.search-panel.no-matching-options", defaultValue: "No matching options", comment: "Empty state shown when search panel has no matching options"))
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text(String(localized: "code-edit-ui.search-panel.select-option-to-preview", defaultValue: "Select an option to preview", comment: "Placeholder instruction to select an option for preview"))
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel(String(localized: "code-edit-ui.search-panel.close-overlay", defaultValue: "Close Overlay", comment: "Keyboard shortcut help label to close overlay"))
        Button {
            ...
            .accessibilityLabel(String(localized: "code-edit-ui.search-panel.select-up", defaultValue: "Select Up", comment: "Keyboard shortcut help label to move selection up"))
        Button {
            ...
            .accessibilityLabel(String(localized: "code-edit-ui.search-panel.select-down", defaultValue: "Select Down", comment: "Keyboard shortcut help label to move selection down"))
    }
}
