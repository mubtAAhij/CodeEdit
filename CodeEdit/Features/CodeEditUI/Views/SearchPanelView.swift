...
struct SearchPanelView<RowView: View, PreviewView: View, Option: Identifiable & Hashable>: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if !text.isEmpty || alwaysShowOptions == true {
                ...
                HStack(spacing: 0) {
                    if options.isEmpty {
                        Text("search.no_matching_options", comment: "Message when no search results found")
                            .font(.system(size: 17))
                            .foregroundColor(.secondary)
                    ...
                    if hasPreview && previewVisible {
                        ...
                        if options.isEmpty {
                            ...
                            if let selection, let previewViewBuilder {
                                ...
                                Text("search.select_option_preview", comment: "Prompt to select an option")
                                    .frame(maxWidth: .infinity)
                            }
                        }
                    }
    ...
    @ViewBuilder var keyHandlers: some View {
        Button {
            ...
            .accessibilityLabel("accessibility.close_overlay", comment: "Accessibility label for close button")
        Button {
            ...
            .accessibilityLabel("accessibility.select_up", comment: "Accessibility label for up arrow")
        Button {
            ...
            .accessibilityLabel("accessibility.select_down", comment: "Accessibility label for down arrow")
    }
}
