        ...
        Menu {
            ...
            Picker(String(localized: "status-bar.indent.tab-width", defaultValue: "Tab Width", comment: "Tab width picker label"), selection: $defaultTabWidth) {
                ForEach(2..<9) { index in
                    Text(String(format: String(localized: "status-bar.indent.n-spaces", defaultValue: "%d Spaces", comment: "Number of spaces format"), index))
                        .tag(index)
                }
            }
        } label: {
            Text(String(format: String(localized: "status-bar.indent.n-spaces", defaultValue: "%d Spaces", comment: "Number of spaces format"), defaultTabWidth))
        }
