    ...
    var body: some View {
        Menu {
            ...
            Picker(String(
                ...
                comment: "Menu section title for selecting tab width"
            ), selection: $defaultTabWidth) {
                ForEach(2..<9) { index in
                    Text(String(format: String(
                        localized: "status-bar.indent-selector.index-spaces",
                        defaultValue: "%d Spaces",
                        comment: "Menu option showing indentation width in spaces for each selectable index"
                    ), index))
                        .tag(index)
                }
            }
            ...
            Text(String(format: String(
                ...
                defaultValue: "%d Spaces",
                comment: "Current default indentation width displayed in spaces"
            ), defaultTabWidth))
        }
