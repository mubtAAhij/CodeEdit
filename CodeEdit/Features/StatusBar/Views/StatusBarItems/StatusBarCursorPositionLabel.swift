        ...
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(
                    ...
                    comment: "Status bar text when no cursor selection is available"
                ))
            }
        }
        .fixedSize()
        ...
        func getLabel() -> String {
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(
                        localized: "status-bar.cursor-position.char-len-selection",
                ...
                if lineCount > 1 {
                    return String(format: String(
                        localized: "status-bar.cursor-position.line-count",
                        defaultValue: "%d lines",
                        comment: "Status bar text showing selected line count"
                    ), lineCount)
                }

                return String(format: String(
                    localized: "status-bar.cursor-position.character-count",
                    defaultValue: "%d characters",
                    comment: "Status bar text showing selected character count"
            ...
            if modifierKeys.contains(.option) {
                return String(format: String(
                    ...
                    comment: "Status bar text showing caret location with zero selection length"
            ...
            return String(format: String(
                ...
                comment: "Status bar text showing current line and column"
