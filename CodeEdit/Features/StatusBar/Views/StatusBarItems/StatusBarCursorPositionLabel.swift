    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "status-bar.cursor.no-selection", defaultValue: "No Selection", comment: "No selection message"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor.selected-ranges", defaultValue: "%d selected ranges", comment: "Selected ranges count"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.cursor.lines-count", defaultValue: "%ld lines", comment: "Lines count"), lineCount)
                }

                return String(format: String(localized: "status-bar.cursor.characters-count", defaultValue: "%ld characters", comment: "Characters count"), cursorPositions[0].range.length)
            }
