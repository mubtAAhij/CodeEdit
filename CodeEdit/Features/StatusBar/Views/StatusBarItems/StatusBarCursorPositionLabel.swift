    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(localized: "status-bar.selected-ranges", defaultValue: "\(cursorPositions.count) selected ranges", comment: "Label showing number of selected ranges in editor")
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(localized: "status-bar.char-location-length", defaultValue: "Char: \(cursorPositions[0].range.location) Len: \(cursorPositions[0].range.length)", comment: "Label showing character location and selection length")
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(localized: "status-bar.line-count", defaultValue: "\(lineCount) lines", comment: "Label showing number of selected lines")
                }

                return String(localized: "status-bar.character-count", defaultValue: "\(cursorPositions[0].range.length) characters", comment: "Label showing number of selected characters")
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(localized: "status-bar.char-location-zero", defaultValue: "Char: \(cursorPositions[0].range.location) Len: 0", comment: "Label showing character location with zero length")
            }

            // When there's a single cursor, display the line and column.
            return String(localized: "status-bar.line-column", defaultValue: "Line: \(cursorPositions[0].start.line)  Col: \(cursorPositions[0].start.column)", comment: "Label showing cursor line and column position")
        }
    }
}
