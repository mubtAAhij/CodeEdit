    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "statusbar.cursor.noselection", defaultValue: "No Selection", comment: "No selection accessibility label"))
            }
        }
        .fixedSize()
        .accessibilityIdentifier(String(localized: "statusbar.cursor.identifier", defaultValue: "CursorPositionLabel", comment: "Cursor position label accessibility identifier"))
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "statusbar.cursor.selected.ranges", defaultValue: "%d selected ranges", comment: "Selected ranges count"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "statusbar.cursor.char.len", defaultValue: "Char: %ld Len: %ld", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "statusbar.cursor.lines", defaultValue: "%d lines", comment: "Line count"), lineCount)
                }

                return String(format: String(localized: "statusbar.cursor.characters", defaultValue: "%ld characters", comment: "Character count"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "statusbar.cursor.char.zero", defaultValue: "Char: %ld Len: 0", comment: "Character position with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "statusbar.cursor.line.col", defaultValue: "Line: %ld  Col: %ld", comment: "Line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
