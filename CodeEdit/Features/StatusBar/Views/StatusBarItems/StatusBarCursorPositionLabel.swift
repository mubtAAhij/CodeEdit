    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "statusbar.cursor.no-selection", defaultValue: "No Selection", comment: "Accessibility label when no cursor position"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(localized: "statusbar.cursor.selected-ranges", defaultValue: "\(cursorPositions.count) selected ranges", comment: "Label showing count of selected ranges")
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(localized: "statusbar.cursor.char-len", defaultValue: "Char: \(cursorPositions[0].range.location) Len: \(cursorPositions[0].range.length)", comment: "Label showing character position and length")
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(localized: "statusbar.cursor.lines-count", defaultValue: "\(lineCount) lines", comment: "Label showing count of selected lines")
                }

                return String(localized: "statusbar.cursor.characters-count", defaultValue: "\(cursorPositions[0].range.length) characters", comment: "Label showing count of selected characters")
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(localized: "statusbar.cursor.char-len-zero", defaultValue: "Char: \(cursorPositions[0].range.location) Len: 0", comment: "Label showing character position with zero length")
            }

            // When there's a single cursor, display the line and column.
            return String(localized: "statusbar.cursor.line-col", defaultValue: "Line: \(cursorPositions[0].start.line)  Col: \(cursorPositions[0].start.column)", comment: "Label showing cursor line and column position")
        }
    }
}
