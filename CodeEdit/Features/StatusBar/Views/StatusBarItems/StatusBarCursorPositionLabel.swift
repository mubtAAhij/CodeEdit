    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "cursor.no_selection", comment: "No cursor selection"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(localized: "cursor.selected_ranges \(cursorPositions.count)", comment: "Number of selected ranges")
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(localized: "cursor.char_len \(cursorPositions[0].range.location) \(cursorPositions[0].range.length)", comment: "Character position and length")
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(localized: "cursor.lines \(lineCount)", comment: "Number of lines selected")
                }

                return String(localized: "cursor.characters \(cursorPositions[0].range.length)", comment: "Number of characters selected")
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(localized: "cursor.char_len_zero \(cursorPositions[0].range.location)", comment: "Character position with zero length")
            }

            // When there's a single cursor, display the line and column.
            return String(localized: "cursor.line_col \(cursorPositions[0].start.line) \(cursorPositions[0].start.column)", comment: "Line and column position")
        }
    }
}
