    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor-position.selected-ranges", defaultValue: "%d selected ranges", comment: "Number of selected ranges in editor"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor-position.char-len", defaultValue: "Char: %ld Len: %ld", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }
            ...
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor-position.char-len-zero", defaultValue: "Char: %ld Len: 0", comment: "Character position with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor-position.line-col", defaultValue: "Line: %ld  Col: %ld", comment: "Line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
