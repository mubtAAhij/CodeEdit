    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "status_bar.cursor.no_selection", defaultValue: "No Selection", comment: "No selection accessibility label"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status_bar.cursor.selected_ranges", defaultValue: "%d selected ranges", comment: "Number of selected ranges"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status_bar.cursor.char_len", defaultValue: "Char: %d Len: %d", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status_bar.cursor.lines", defaultValue: "%d lines", comment: "Number of lines selected"), lineCount)
                }

                return String(format: String(localized: "status_bar.cursor.characters", defaultValue: "%d characters", comment: "Number of characters selected"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status_bar.cursor.char_len_zero", defaultValue: "Char: %d Len: 0", comment: "Character position with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status_bar.cursor.line_col", defaultValue: "Line: %d  Col: %d", comment: "Line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
