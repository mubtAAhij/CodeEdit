        ...
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(
                    localized: "status-bar.cursor-position.no-selection",
                    defaultValue: "No Selection",
                    comment: "Status bar text when there is no text selection"
                ))
            }
        }
        .fixedSize()
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(
                    localized: "status-bar.cursor-position.selected-ranges",
                    defaultValue: "%d selected ranges",
                    comment: "Status bar text showing number of selected ranges"
                ), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(
                        localized: "status-bar.cursor-position.char-len",
                        defaultValue: "Char: %d Len: %d",
                        comment: "Status bar text showing character location and selection length"
                    ), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }
                ...
                if lineCount > 1 {
                    return String(format: String(
                        localized: "status-bar.cursor-position.lines-count",
                        defaultValue: "%d lines",
                        comment: "Status bar text showing number of selected lines"
                    ), lineCount)
                }

                return String(format: String(
                    localized: "status-bar.cursor-position.characters-count",
                    defaultValue: "%d characters",
                    comment: "Status bar text showing number of selected characters"
                ), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(
                    localized: "status-bar.cursor-position.char-len-zero",
                    defaultValue: "Char: %d Len: 0",
                    comment: "Status bar text showing cursor character location with zero selection length"
                ), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(
                localized: "status-bar.cursor-position.line-col",
                defaultValue: "Line: %d  Col: %d",
                comment: "Status bar text showing cursor line and column"
            ), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
