    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "status-bar.cursor.no-selection", defaultValue: "No Selection", comment: "No cursor selection accessibility label"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: String(localized: "status-bar.cursor.selected-ranges", defaultValue: "%d selected ranges", comment: "Multiple selected ranges count"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.cursor.char-len", defaultValue: "Char: %ld Len: %ld", comment: "Character position and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.cursor.lines", defaultValue: "%d lines", comment: "Number of lines selected"), lineCount)
                }

                return String(format: String(localized: "status-bar.cursor.characters", defaultValue: "%ld characters", comment: "Number of characters selected"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.cursor.char-len-zero", defaultValue: "Char: %ld Len: 0", comment: "Character position with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor.line-col", defaultValue: "Line: %ld  Col: %ld", comment: "Cursor line and column position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
