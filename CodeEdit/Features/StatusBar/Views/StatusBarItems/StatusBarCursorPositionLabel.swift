        ...
        func getLabel() -> String {
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: String(localized: "status-bar.char-location-length", defaultValue: "Char: %ld Len: %ld", comment: "Character location and length"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: String(localized: "status-bar.lines", defaultValue: "%d lines", comment: "Number of lines"), lineCount)
                }

                return String(format: String(localized: "status-bar.characters", defaultValue: "%ld characters", comment: "Number of characters"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: String(localized: "status-bar.char-location-zero", defaultValue: "Char: %ld Len: 0", comment: "Character location with zero length"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: String(localized: "status-bar.cursor", defaultValue: "Line: %ld  Col: %ld", comment: "Cursor position"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
