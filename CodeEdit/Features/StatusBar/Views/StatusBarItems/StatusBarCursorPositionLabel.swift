    ...
    var body: some View {
        Group {
            if let currentTab = tab {
                ...
                Text("").accessibilityLabel(String(localized: "status-bar.no-selection", defaultValue: "No Selection", comment: "Accessibility label when no cursor position is selected"))
            }
        }
        .fixedSize()
    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(localized: "status-bar.selected-ranges", defaultValue: "\(cursorPositions.count) selected ranges", comment: "Status bar label showing number of selected ranges")
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(localized: "status-bar.char-location-length", defaultValue: "Char: \(cursorPositions[0].range.location) Len: \(cursorPositions[0].range.length)", comment: "Status bar label showing character location and selection length")
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(localized: "status-bar.line-count", defaultValue: "\(lineCount) lines", comment: "Status bar label showing number of lines selected")
                }

                return String(localized: "status-bar.character-count", defaultValue: "\(cursorPositions[0].range.length) characters", comment: "Status bar label showing number of characters selected")
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(localized: "status-bar.char-location-zero", defaultValue: "Char: \(cursorPositions[0].range.location) Len: 0", comment: "Status bar label showing character location with no selection")
            }

            // When there's a single cursor, display the line and column.
            return String(localized: "status-bar.line-column", defaultValue: "Line: \(cursorPositions[0].start.line)  Col: \(cursorPositions[0].start.column)", comment: "Status bar label showing cursor line and column position")
        }
    }
}
