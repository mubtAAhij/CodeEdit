    ...
    struct LineLabel: View {
        ...
        func getLabel() -> String {
            ...
            if cursorPositions.count > 1 {
                return String(format: NSLocalizedString("status-bar.selected-ranges", comment: "Selected ranges count"), cursorPositions.count)
            }
            ...
            if cursorPositions[0].range.length > 0 {
                ...
                if modifierKeys.contains(.option) {
                    return String(format: NSLocalizedString("status-bar.char-range", comment: "Character range"), cursorPositions[0].range.location, cursorPositions[0].range.length)
                }

                let lineCount = getLines(cursorPositions[0].range)

                if lineCount > 1 {
                    return String(format: NSLocalizedString("status-bar.lines-count", comment: "Lines count"), lineCount)
                }

                return String(format: NSLocalizedString("status-bar.characters-count", comment: "Characters count"), cursorPositions[0].range.length)
            }

            // When the option key is pressed display the character offset.
            if modifierKeys.contains(.option) {
                return String(format: NSLocalizedString("status-bar.char-position", comment: "Character position"), cursorPositions[0].range.location)
            }

            // When there's a single cursor, display the line and column.
            return String(format: NSLocalizedString("status-bar.line-col", comment: "Line and column"), cursorPositions[0].start.line, cursorPositions[0].start.column)
        }
    }
}
