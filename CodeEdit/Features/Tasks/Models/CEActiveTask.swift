...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.finished.running", defaultValue: "Finished running \(task.name).", comment: "Terminal output message"))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(localized: "task.finished.title", defaultValue: "Finished Running \(task.name)", comment: "Notification title"),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.cancelled.message", defaultValue: "\(task.name) cancelled.", comment: "Terminal output message"))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(localized: "task.cancelled.title", defaultValue: "\(task.name) cancelled", comment: "Notification title"),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.failed.message", defaultValue: "Failed to run \(task.name)", comment: "Terminal output message"))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(localized: "task.failed.title", defaultValue: "Failed Running \(task.name)", comment: "Notification title"),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(localized: "task.running.title", defaultValue: "Running \(self.task.name)", comment: "Notification title"),
            "message": String(localized: "task.running.message", defaultValue: "Running your task: \(self.task.name).", comment: "Notification message"),
            "isLoading": true,
