...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active.finished_running_message", defaultValue: "Finished running %@.", comment: "Message when task finishes successfully"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active.finished_running_title", defaultValue: "Finished Running %@", comment: "Notification title when task finishes"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active.cancelled_message", defaultValue: "%@ cancelled.", comment: "Message when task is cancelled"), task.name))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active.cancelled_title", defaultValue: "%@ cancelled", comment: "Notification title when task is cancelled"), task.name),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active.failed_message", defaultValue: "Failed to run %@", comment: "Message when task fails"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active.failed_title", defaultValue: "Failed Running %@", comment: "Notification title when task fails"), task.name),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "tasks.active.running_title", defaultValue: "Running %@", comment: "Notification title when task is running"), self.task.name),
            "message": String(format: String(localized: "tasks.active.running_message", defaultValue: "Running your task: %@.", comment: "Notification message when task is running"), self.task.name),
            "isLoading": true,
