...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.finished-running-message", defaultValue: "Finished running %@.", comment: "Task finished running message"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "task.finished-running-title", defaultValue: "Finished Running %@", comment: "Task finished running title"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            ...
            updateTaskNotification(
                title: String(format: String(localized: "task.cancelled-title", defaultValue: "%@ cancelled", comment: "Task cancelled title"), task.name),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.failed-to-run-message", defaultValue: "Failed to run %@", comment: "Task failed to run message"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "task.failed-running-title", defaultValue: "Failed Running %@", comment: "Task failed running title"), task.name),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "task.running-title", defaultValue: "Running %@", comment: "Task running title"), self.task.name),
            "message": String(format: String(localized: "task.running-message", defaultValue: "Running your task: %@.", comment: "Task running message"), self.task.name),
            "isLoading": true,
