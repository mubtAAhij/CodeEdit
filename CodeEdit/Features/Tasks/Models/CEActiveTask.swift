...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.finished-running", defaultValue: "Finished running %@.", comment: "Message when task finishes running with task name"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.notification.finished-running", defaultValue: "Finished Running %@", comment: "Notification title when task finishes with task name"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.cancelled", defaultValue: "%@ cancelled.", comment: "Message when task is cancelled with task name"), task.name))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.notification.cancelled", defaultValue: "%@ cancelled", comment: "Notification title when task is cancelled with task name"), task.name),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.failed-to-run", defaultValue: "Failed to run %@", comment: "Message when task fails to run with task name"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.notification.failed-running", defaultValue: "Failed Running %@", comment: "Notification title when task fails with task name"), task.name),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "tasks.notification.running", defaultValue: "Running %@", comment: "Notification title when task is running with task name"), self.task.name),
            "message": String(format: String(localized: "tasks.notification.running-message", defaultValue: "Running your task: %@.", comment: "Notification message when task is running with task name"), self.task.name),
            "isLoading": true,
