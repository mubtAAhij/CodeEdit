...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.finished-running.message", defaultValue: "Finished running %@.", comment: "Notification body shown when task finishes running"), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.finished-running.title", defaultValue: "Finished Running %@", comment: "Notification title shown when task finishes running"), "\(task.name)"),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.cancelled.message", defaultValue: "%@ cancelled.", comment: "Notification body shown when task is cancelled"), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.cancelled.title", defaultValue: "%@ cancelled", comment: "Notification title shown when task is cancelled"), "\(task.name)"),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.failed-to-run.message", defaultValue: "Failed to run %@", comment: "Notification body shown when task execution fails"), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.failed-running.title", defaultValue: "Failed Running %@", comment: "Notification title shown when task execution fails"), "\(task.name)"),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "tasks.active-task.running.title", defaultValue: "Running %@", comment: "Progress notification title while task is running"), "\(self.task.name)"),
            "message": String(format: String(localized: "tasks.active-task.running.message", defaultValue: "Running your task: %@.", comment: "Progress notification message while task is running"), "\(self.task.name)"),
            "isLoading": true,
