...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.finished-running-message", defaultValue: "Finished running \(task.name).", comment: "Message when task finishes successfully"))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(localized: "task.finished-running-title", defaultValue: "Finished Running \(task.name)", comment: "Notification title when task finishes successfully"),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.cancelled-message", defaultValue: "\(task.name) cancelled.", comment: "Message when task is cancelled"))
            output?.newline()
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(localized: "task.failed-message", defaultValue: "Failed to run \(task.name)", comment: "Message when task fails to run"))
            output?.newline()
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(localized: "task.running-title", defaultValue: "Running \(self.task.name)", comment: "Notification title when task is running"),
            "message": String(localized: "task.running-message", defaultValue: "Running your task: \(self.task.name).", comment: "Notification message when task is running"),
            "isLoading": true,
