...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.finished-running-message", defaultValue: "Finished running %@.", comment: "Message when task finishes running"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "task.finished-running-title", defaultValue: "Finished Running %@", comment: "Title when task finishes running"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.cancelled-message", defaultValue: "%@ cancelled.", comment: "Message when task is cancelled"), task.name))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "task.cancelled-title", defaultValue: "%@ cancelled", comment: "Title when task is cancelled"), task.name),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.failed-message", defaultValue: "Failed to run %@", comment: "Message when task fails to run"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "task.failed-title", defaultValue: "Failed Running %@", comment: "Title when task fails to run"), task.name),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "task.running-title", defaultValue: "Running %@", comment: "Title when task is running"), self.task.name),
            "message": String(format: String(localized: "task.running-message", defaultValue: "Running your task: %@.", comment: "Message when task is running"), self.task.name),
            "isLoading": true,
