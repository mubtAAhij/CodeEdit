...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.notification.finished-running.message", defaultValue: "Finished running %@.", comment: "Notification message when a task finishes running."), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.notification.finished-running.title", defaultValue: "Finished Running %@", comment: "Notification title when a task finishes running."), "\(task.name)"),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.notification.cancelled.message", defaultValue: "%@ cancelled.", comment: "Notification message when a task is cancelled."), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.notification.cancelled.title", defaultValue: "%@ cancelled", comment: "Notification title when a task is cancelled."), "\(task.name)"),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.active-task.notification.failed-to-run.message", defaultValue: "Failed to run %@", comment: "Notification message when task execution fails."), "\(task.name)"))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.active-task.notification.failed-running.title", defaultValue: "Failed Running %@", comment: "Notification title when task execution fails."), "\(task.name)"),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "tasks.active-task.running.status-title", defaultValue: "Running %@", comment: "Status title while a task is running."), "\(self.task.name)"),
            "message": String(format: String(localized: "tasks.active-task.running.status-message", defaultValue: "Running your task: %@.", comment: "Status message while a task is running."), "\(self.task.name)"),
            "isLoading": true,
