...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.finished-running", defaultValue: "Finished running %@.", comment: "Message shown in terminal output when task finishes successfully"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "tasks.notification.finished-running", defaultValue: "Finished Running %@", comment: "Notification title when task finishes successfully"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.cancelled", defaultValue: "%@ cancelled.", comment: "Message shown in terminal output when task is cancelled"), task.name))
            output?.newline()
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "tasks.output.failed-to-run", defaultValue: "Failed to run %@", comment: "Message shown in terminal output when task fails"), task.name))
            output?.newline()
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "tasks.notification.running-title", defaultValue: "Running %@", comment: "Notification title when task starts running"), self.task.name),
            "message": String(format: String(localized: "tasks.notification.running-message", defaultValue: "Running your task: %@.", comment: "Notification message when task starts running"), self.task.name),
            "isLoading": true,
