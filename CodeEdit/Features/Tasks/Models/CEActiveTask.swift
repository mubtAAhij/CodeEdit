...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(localized: "tasks.finished_running \(task.name)", comment: "Task completion message"))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(localized: "tasks.finished_running_title \(task.name)", comment: "Task finished notification title"),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(localized: "tasks.cancelled \(task.name)", comment: "Task cancelled message"))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(localized: "tasks.cancelled_title \(task.name)", comment: "Task cancelled notification title"),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(localized: "tasks.failed_to_run \(task.name)", comment: "Task failed message"))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(localized: "tasks.failed_running_title \(task.name)", comment: "Task failed notification title"),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(localized: "tasks.running_title \(self.task.name)", comment: "Task running notification title"),
            "message": String(localized: "tasks.running_message \(self.task.name)", comment: "Task running notification message"),
            "isLoading": true,
