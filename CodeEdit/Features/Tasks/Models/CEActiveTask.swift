...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            ...
            updateTaskNotification(
                title: "String(localized: "task_finished_running", comment: "Notification title when task finishes successfully").replacingOccurrences(of: "{taskName}", with: task.name)",
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            ...
            updateTaskNotification(
                title: "String(localized: "task_cancelled", comment: "Notification title when task is cancelled").replacingOccurrences(of: "{taskName}", with: task.name)",
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            ...
            updateTaskNotification(
                title: "String(localized: "task_failed_running", comment: "Notification title when task fails to run").replacingOccurrences(of: "{taskName}", with: task.name)",
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": "String(localized: "task_running", comment: "Notification title when task is running").replacingOccurrences(of: "{taskName}", with: self.task.name)",
            "message": "String(localized: "task_running_message", comment: "Notification message when task is running").replacingOccurrences(of: "{taskName}", with: self.task.name)",
            "isLoading": true,
