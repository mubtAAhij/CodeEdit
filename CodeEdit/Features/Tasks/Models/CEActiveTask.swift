...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.finished.message", defaultValue: "Finished running %@.", comment: "Finished running task message"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "task.finished.title", defaultValue: "Finished Running %@", comment: "Finished running task title"), task.name),
                message: "",
                isLoading: false
            )
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.failed.message", defaultValue: "Failed to run %@", comment: "Failed to run task message"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "task.failed.title", defaultValue: "Failed Running %@", comment: "Failed running task title"), task.name),
                message: "",
                isLoading: false
            )
        }
