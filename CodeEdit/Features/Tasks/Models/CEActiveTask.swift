...
class CEActiveTask: ObservableObject, Identifiable, Hashable {
    ...
    func handleProcessFinished(terminationStatus: Int32) {
        ...
        case 0:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.finished.message", defaultValue: "Finished running %@.", comment: "Message shown in terminal when task finishes successfully"), task.name))
            output?.newline()

            updateTaskStatus(to: .finished)
            updateTaskNotification(
                title: String(format: String(localized: "task.finished.title", defaultValue: "Finished Running %@", comment: "Notification title when task finishes successfully"), task.name),
                message: "",
                isLoading: false
            )
        case 2, 15: // SIGINT or SIGTERM
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.cancelled.message", defaultValue: "%@ cancelled.", comment: "Message shown in terminal when task is cancelled"), task.name))
            output?.newline()

            updateTaskStatus(to: .notRunning)
            updateTaskNotification(
                title: String(format: String(localized: "task.cancelled.title", defaultValue: "%@ cancelled", comment: "Notification title when task is cancelled"), task.name),
                message: "",
                isLoading: false
            )
        case 17: // SIGSTOP
        ...
        default:
            output?.newline()
            output?.sendOutputMessage(String(format: String(localized: "task.failed.message", defaultValue: "Failed to run %@", comment: "Message shown in terminal when task fails"), task.name))
            output?.newline()

            updateTaskStatus(to: .failed)
            updateTaskNotification(
                title: String(format: String(localized: "task.failed.title", defaultValue: "Failed Running %@", comment: "Notification title when task fails"), task.name),
                message: "",
                isLoading: false
            )
        }
    ...
    private func createStatusTaskNotification() {
        let userInfo: [String: Any] = [
            ...
            "title": String(format: String(localized: "task.running.title", defaultValue: "Running %@", comment: "Notification title when task starts running"), self.task.name),
            "message": String(format: String(localized: "task.running.message", defaultValue: "Running your task: %@.", comment: "Notification message when task starts running"), self.task.name),
            "isLoading": true,
