...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "task.start.button", defaultValue: "Start", comment: "Start task button label"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "task.start.button.tooltip", defaultValue: "Start selected task", comment: "Start task button tooltip"))
                .frame(width: 28)
