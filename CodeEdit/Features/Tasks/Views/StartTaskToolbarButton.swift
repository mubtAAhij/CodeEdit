...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "task.start", defaultValue: "Start", comment: "Start task button label"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "task.start.help", defaultValue: "Start selected task", comment: "Help text for start task button"))
                .frame(width: 28)
