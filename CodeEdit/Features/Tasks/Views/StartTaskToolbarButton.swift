...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "tasks.toolbar-button.start.title", defaultValue: "Start", comment: "Button title to start a selected task"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "tasks.toolbar-button.start.accessibility-label", defaultValue: "Start selected task", comment: "Accessibility label for start task toolbar button"))
                .frame(width: 28)
