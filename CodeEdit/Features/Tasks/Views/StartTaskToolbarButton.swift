...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "tasks.start.button", comment: "Button text to start a task"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "tasks.start.tooltip", comment: "Tooltip for start task button"))
                .frame(width: 28)
