...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "task.start", defaultValue: "Start", comment: "Start task button"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "task.start-selected-help", defaultValue: "Start selected task", comment: "Start selected task help text"))
                .frame(width: 28)
