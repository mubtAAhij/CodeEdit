...
struct StartTaskToolbarButton: View {
    ...
    var body: some View {
        Button {
            ...
            Label(String(localized: "tasks.start", defaultValue: "Start", comment: "Start task accessibility label"), systemImage: "play.fill")
                .labelStyle(.iconOnly)
                .opacity(activeState == .inactive ? 0.5 : 1.0)
                .font(.system(size: 18, weight: .regular))
                .help(String(localized: "tasks.start-hint", defaultValue: "Start selected task", comment: "Start task accessibility hint"))
                .frame(width: 28)
