        ...
        HStack {
            if let currentSelectedStatus, currentSelectedStatus == .running {
                    Button {
                        ...
                        Label(String(localized: "tasks.stop-button", defaultValue: "Stop", comment: "Stop task button label"), systemImage: "stop.fill")
                            .labelStyle(.iconOnly)
                            .opacity(activeState == .inactive ? 0.5 : 1.0)
                            .font(.system(size: 15, weight: .regular))
                            .help(String(localized: "tasks.stop-button.tooltip", defaultValue: "Stop selected task", comment: "Stop task button tooltip"))
                            .frame(width: 28)
                            .offset(y: 1.5)
                    }
                    .frame(height: 22)
