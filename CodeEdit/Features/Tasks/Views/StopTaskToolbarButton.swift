        ...
        HStack {
            if let currentSelectedStatus, currentSelectedStatus == .running {
                    Button {
                        ...
                        Label(String(localized: "task.stop", defaultValue: "Stop", comment: "Button label to stop a task"), systemImage: "stop.fill")
                            .labelStyle(.iconOnly)
                            .opacity(activeState == .inactive ? 0.5 : 1.0)
                            .font(.system(size: 15, weight: .regular))
                            .help(String(localized: "task.stop.help", defaultValue: "Stop selected task", comment: "Tooltip for stop task button"))
                            .frame(width: 28)
                            .offset(y: 1.5)
                    }
                    .frame(height: 22)
