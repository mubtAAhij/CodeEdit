        ...
        HStack {
            if let currentSelectedStatus, currentSelectedStatus == .running {
                    Button {
                        ...
                        Label("String(localized: "stop_task", comment: "Label for stop task button")", systemImage: "stop.fill")
                            .labelStyle(.iconOnly)
                            .opacity(activeState == .inactive ? 0.5 : 1.0)
                            .font(.system(size: 15, weight: .regular))
                            .help("String(localized: "stop_selected_task", comment: "Help text for stop task button")")
                            .frame(width: 28)
                            .offset(y: 1.5)
                    }
                    .frame(height: 22)
