...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.stop-task", defaultValue: "Stop Task", comment: "Stop task button label")
        toolTip = String(localized: "tasks.stop-task-help", defaultValue: "Stop the selected task", comment: "Help text for stop task button")
        target = self
        isEnabled = false
        isBordered = true
