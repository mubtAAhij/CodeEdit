...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.stop", defaultValue: "Stop Task", comment: "Toolbar item label for stopping a task")
        toolTip = String(localized: "tasks.stop-tooltip", defaultValue: "Stop the selected task", comment: "Tooltip for stop task button")
        target = self
        isEnabled = false
        isBordered = true
