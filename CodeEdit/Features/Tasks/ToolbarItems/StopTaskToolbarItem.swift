...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.stop-task", defaultValue: "Stop Task", comment: "Toolbar item label for stopping a task")
        toolTip = String(localized: "tasks.stop-task.tooltip", defaultValue: "Stop the selected task", comment: "Tooltip for stop task toolbar item")
        target = self
        isEnabled = false
        isBordered = true
