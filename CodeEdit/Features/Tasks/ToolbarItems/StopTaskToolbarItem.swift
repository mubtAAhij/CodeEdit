...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.stop-task", defaultValue: "Stop Task", comment: "Stop task toolbar item label")
        toolTip = String(localized: "tasks.stop-selected-task", defaultValue: "Stop the selected task", comment: "Stop task tooltip")
        target = self
        isEnabled = false
        isBordered = true
