...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.toolbar.stop-task.palette-label", defaultValue: "Stop Task", comment: "Palette label for stop task toolbar item")
        toolTip = String(localized: "tasks.toolbar.stop-task.tool-tip", defaultValue: "Stop the selected task", comment: "Tooltip for stop task toolbar item")
        target = self
        isEnabled = false
        isBordered = true
