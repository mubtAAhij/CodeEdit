...
final class StopTaskToolbarItem: NSToolbarItem {
    ...
    init?(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "task.stop.label", defaultValue: "Stop Task", comment: "Toolbar item label for stopping a task")
        toolTip = String(localized: "task.stop.tooltip", defaultValue: "Stop the selected task", comment: "Tooltip for stop task toolbar button")
        target = self
        isEnabled = false
        isBordered = true
