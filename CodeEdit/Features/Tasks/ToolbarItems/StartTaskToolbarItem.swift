...
final class StartTaskToolbarItem: NSToolbarItem {
    ...
    init(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.start-task", defaultValue: "Start Task", comment: "Start task button label")
        toolTip = String(localized: "tasks.start-task-help", defaultValue: "Run the selected task", comment: "Help text for start task button")
        target = self
