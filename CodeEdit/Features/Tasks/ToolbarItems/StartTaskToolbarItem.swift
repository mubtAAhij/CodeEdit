...
final class StartTaskToolbarItem: NSToolbarItem {
    ...
    init(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(
            localized: "tasks.toolbar-item.start-task.label",
            defaultValue: "Start Task",
            comment: "Label for toolbar item that starts a task"
        )
        toolTip = String(
            localized: "tasks.toolbar-item.start-task.help",
            defaultValue: "Run the selected task",
            comment: "Help text for start task toolbar item"
        )
        target = self
