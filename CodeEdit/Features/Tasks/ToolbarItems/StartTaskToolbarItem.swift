...
final class StartTaskToolbarItem: NSToolbarItem {
    ...
    init(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "tasks.start", defaultValue: "Start Task", comment: "Start task toolbar item label")
        toolTip = String(localized: "tasks.start.tooltip", defaultValue: "Run the selected task", comment: "Start task tooltip")
        target = self
