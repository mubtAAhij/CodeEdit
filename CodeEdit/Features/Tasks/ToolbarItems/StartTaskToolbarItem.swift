...
final class StartTaskToolbarItem: NSToolbarItem {
    ...
    init(workspace: WorkspaceDocument) {
        ...
        paletteLabel = String(localized: "task.start", defaultValue: "Start Task", comment: "Start task toolbar button")
        toolTip = String(localized: "task.start-help", defaultValue: "Run the selected task", comment: "Start task help text")
        target = self
