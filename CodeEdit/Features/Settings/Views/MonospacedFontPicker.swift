...
struct MonospacedFontPicker: View {
    ...
    var body: some View {
        Picker(selection: $selectedFontName, label: Text(title)) {
            Text(String(localized: "font_picker.system_font", comment: "System font option in font picker"))
            ...
            if !otherFontFamilyNames.isEmpty {
                ...
                Menu {
                    ...
                    Text(String(localized: "font_picker.other_fonts", comment: "Menu item for other fonts"))
                }
            }
        }
