...
struct MonospacedFontPicker: View {
    ...
    var body: some View {
        Picker(selection: $selectedFontName, label: Text(title)) {
            Text(String(localized: "system_font_option", comment: "System font option in font picker"))
            ...
            if !otherFontFamilyNames.isEmpty {
                ...
                Menu {
                    ...
                    Text(String(localized: "other_fonts_menu", comment: "Menu item for other fonts"))
                }
            }
        }
