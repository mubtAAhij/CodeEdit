    ...
    var body: some View {
        ...
        .contextMenu(forSelectionType: UUID.self, menu: { selection in
            if let patternID = selection.first, let pattern = patterns.first(where: { $0.id == patternID }) {
                Button(String(localized: "settings.glob-pattern-list.edit", defaultValue: "Edit", comment: "Button title to edit selected glob pattern")) {
                    focusedField = pattern.id.uuidString
                }
                ...
                Divider()
                Button(String(localized: "settings.glob-pattern-list.remove", defaultValue: "Remove", comment: "Button title to remove selected glob pattern")) {
