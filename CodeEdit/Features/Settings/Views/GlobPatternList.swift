    ...
    var body: some View {
        ...
        .contextMenu(forSelectionType: UUID.self, menu: { selection in
            if let patternID = selection.first, let pattern = patterns.first(where: { $0.id == patternID }) {
                Button(String(localized: "globPatternList.edit", comment: "Context menu item")) {
                    focusedField = pattern.id.uuidString
                }
                Button(String(localized: "globPatternList.add", comment: "Context menu item")) {
                    addPattern()
                }
                Divider()
                Button(String(localized: "globPatternList.remove", comment: "Context menu item")) {
