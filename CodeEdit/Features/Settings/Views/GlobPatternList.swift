    ...
    var body: some View {
        ...
        .contextMenu(forSelectionType: UUID.self, menu: { selection in
            if let patternID = selection.first, let pattern = patterns.first(where: { $0.id == patternID }) {
                Button(String(localized: "settings.glob-pattern-list.context-menu.edit", defaultValue: "Edit", comment: "Edit context menu item in glob pattern list")) {
                    focusedField = pattern.id.uuidString
                }
                Button(String(localized: "settings.glob-pattern-list.context-menu.add", defaultValue: "Add", comment: "Add context menu item in glob pattern list")) {
                    addPattern()
                }
                Divider()
                Button(String(localized: "settings.glob-pattern-list.context-menu.remove", defaultValue: "Remove", comment: "Remove context menu item in glob pattern list")) {
