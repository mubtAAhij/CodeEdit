    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "theme.search", defaultValue: "Search", comment: "Search field placeholder"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "theme.create-new-theme", defaultValue: "Create a new Theme", comment: "Create a new Theme tooltip"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "theme.import-theme", defaultValue: "Import Theme...", comment: "Import Theme menu item"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "theme.export-all-custom-themes", defaultValue: "Export All Custom Themes...", comment: "Export All Custom Themes menu item"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "theme.import-or-export-custom-themes", defaultValue: "Import or Export Custom Themes", comment: "Import or Export Custom Themes tooltip"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "theme.import", defaultValue: "Import...", comment: "Import button")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "theme.use-theme-background", defaultValue: "Use theme background ", comment: "Use theme background toggle"), isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "theme.always-use-dark-terminal", defaultValue: "Always use dark terminal appearance", comment: "Always use dark terminal appearance toggle"), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "theme.auto-change-on-system-appearance", defaultValue: "Automatically change theme based on system appearance", comment: "Automatically change theme based on system appearance toggle"),
