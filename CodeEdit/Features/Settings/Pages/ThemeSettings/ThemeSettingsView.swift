    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "settings.theme.search.placeholder", defaultValue: "Search", comment: "Placeholder text for theme search field."), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "settings.theme.create-new-theme.button", defaultValue: "Create a new Theme", comment: "Button title to create a new theme."))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "settings.theme.import-theme.button", defaultValue: "Import Theme...", comment: "Button title to import a theme."))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "settings.theme.export-all-custom-themes.button", defaultValue: "Export All Custom Themes...", comment: "Button title to export all custom themes."))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "settings.theme.import-export-custom-themes.section-title", defaultValue: "Import or Export Custom Themes", comment: "Section title for importing or exporting custom themes."))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "settings.theme.import.button", defaultValue: "Import...", comment: "Button title to import theme data.")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "settings.theme.terminal.use-theme-background.toggle", defaultValue: "Use theme background", comment: "Toggle title to use theme background in terminal.") + " ", isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "settings.theme.terminal.always-dark-appearance.toggle", defaultValue: "Always use dark terminal appearance", comment: "Toggle title to always use dark terminal appearance."), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "settings.theme.automatic-system-appearance.toggle", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle title to automatically change theme based on system appearance."),
