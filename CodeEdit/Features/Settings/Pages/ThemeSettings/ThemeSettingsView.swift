    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "theme-settings.search-field", defaultValue: "Search", comment: "Search field placeholder for themes"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "theme-settings.help.create-theme", defaultValue: "Create a new Theme", comment: "Tooltip for create new theme button"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "theme-settings.menu.import-theme", defaultValue: "Import Theme...", comment: "Menu item to import a theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "theme-settings.menu.export-all", defaultValue: "Export All Custom Themes...", comment: "Menu item to export all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "theme-settings.help.import-export", defaultValue: "Import or Export Custom Themes", comment: "Tooltip for import/export menu button"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "theme-settings.button.import", defaultValue: "Import...", comment: "Button to import a theme")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "theme-settings.toggle.use-theme-background", defaultValue: "Use theme background ", comment: "Toggle to use theme background"), isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "theme-settings.toggle.dark-terminal", defaultValue: "Always use dark terminal appearance", comment: "Toggle to always use dark terminal appearance"), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "theme-settings.toggle.auto-change", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle to automatically change theme based on system appearance"),
