    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "settings.theme.search", defaultValue: "Search", comment: "Search field placeholder"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "settings.theme.create-help", defaultValue: "Create a new Theme", comment: "Tooltip for create theme button"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "settings.theme.import", defaultValue: "Import Theme...", comment: "Import theme menu item"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "settings.theme.export-all", defaultValue: "Export All Custom Themes...", comment: "Export all custom themes menu item"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "settings.theme.import-export-help", defaultValue: "Import or Export Custom Themes", comment: "Tooltip for import/export menu"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "settings.theme.import-button", defaultValue: "Import...", comment: "Import button in theme settings")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "settings.theme.use-background-toggle", defaultValue: "Use theme background ", comment: "Toggle to use theme background"), isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "settings.theme.dark-terminal-toggle", defaultValue: "Always use dark terminal appearance", comment: "Toggle for always using dark terminal appearance"), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "settings.theme.auto-change-toggle", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle for automatically changing theme based on system appearance"),
