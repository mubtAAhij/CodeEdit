    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "theme.search", defaultValue: "Search", comment: "Placeholder for theme search field"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "theme.create.help", defaultValue: "Create a new Theme", comment: "Tooltip for creating a new theme"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "theme.import", defaultValue: "Import Theme...", comment: "Menu item to import a theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "theme.export-all", defaultValue: "Export All Custom Themes...", comment: "Menu item to export all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "theme.import-export.help", defaultValue: "Import or Export Custom Themes", comment: "Tooltip for import/export menu"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "theme.import.button", defaultValue: "Import...", comment: "Button to import a theme")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "theme.use-background", defaultValue: "Use theme background", comment: "Toggle to use theme background color"), isOn: $settings.useThemeBackground)
    }
    ...
    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "theme.auto-change", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle to automatically change theme with system appearance"),
