            ...
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        ...
                        .help(String(localized: "themes.create-new", defaultValue: "Create a new Theme", comment: "Tooltip for creating a new theme"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "themes.import-theme", defaultValue: "Import Theme...", comment: "Menu item to import a theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "themes.export-all", defaultValue: "Export All Custom Themes...", comment: "Menu item to export all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "themes.import-export-help", defaultValue: "Import or Export Custom Themes", comment: "Tooltip for import/export menu"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "themes.import", defaultValue: "Import...", comment: "Button to import a theme")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "themes.use-theme-background", defaultValue: "Use theme background", comment: "Toggle to use theme background color"), isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "themes.always-use-dark-terminal", defaultValue: "Always use dark terminal appearance", comment: "Toggle to always use dark terminal appearance"), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "themes.auto-change-theme", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle to automatically change theme based on system appearance"),
