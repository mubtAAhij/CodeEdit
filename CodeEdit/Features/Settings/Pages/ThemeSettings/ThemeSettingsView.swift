    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "search", comment: "Placeholder text for theme search field"), text: $themeSearchQuery)
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "import_theme_ellipsis", comment: "Menu item for importing a theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "export_all_custom_themes_ellipsis", comment: "Menu item for exporting all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "import_export_custom_themes", comment: "Tooltip for theme import/export menu"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "import_ellipsis", comment: "Button label for importing themes")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "use_theme_background", comment: "Toggle label for using theme background"), isOn: $settings.useThemeBackground)
    }
    ...
    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "auto_change_theme_system_appearance", comment: "Toggle label for automatically changing theme based on system appearance"),
