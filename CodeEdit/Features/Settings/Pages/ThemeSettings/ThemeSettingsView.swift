    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "settings.theme.search", defaultValue: "Search", comment: "Theme search field placeholder"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "settings.theme.create-new-help", defaultValue: "Create a new Theme", comment: "Tooltip for create new theme button"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "settings.theme.import-theme", defaultValue: "Import Theme...", comment: "Menu item to import a theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "settings.theme.export-all-custom", defaultValue: "Export All Custom Themes...", comment: "Menu item to export all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "settings.theme.import-export-help", defaultValue: "Import or Export Custom Themes", comment: "Tooltip for import/export menu button"))
                    }
                }
...
private extension ThemeSettingsView {
    private var useThemeBackground: some View {
        Toggle(String(localized: "settings.theme.use-theme-background", defaultValue: "Use theme background ", comment: "Toggle to use theme background color"), isOn: $settings.useThemeBackground)
    }

    private var alwaysUseDarkTerminalAppearance: some View {
        Toggle(String(localized: "settings.theme.always-dark-terminal", defaultValue: "Always use dark terminal appearance", comment: "Toggle to always use dark terminal appearance"), isOn: $useDarkTerminalAppearance)
    }

    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "settings.theme.auto-change-theme", defaultValue: "Automatically change theme based on system appearance", comment: "Toggle to automatically change theme with system appearance"),
