    ...
    var body: some View {
        VStack {
            SettingsForm {
                Section {
                    HStack(spacing: 10) {
                        SearchField(String(localized: "common.search_placeholder", comment: "Search field placeholder"), text: $themeSearchQuery)
                        ...
                        .help(String(localized: "theme.settings.create_new_help", comment: "Help text for creating new theme"))
                        ...
                        MenuWithButtonStyle(systemImage: "ellipsis", menu: {
                            Group {
                                Button {
                                    ...
                                    Text(String(localized: "theme.settings.import_menu", comment: "Menu item to import theme"))
                                }
                                Button {
                                    themeModel.exportAllCustomThemes()
                                } label: {
                                    Text(String(localized: "theme.settings.export_all_menu", comment: "Menu item to export all custom themes"))
                                }
                                .disabled(themeModel.themes.filter { !$0.isBundled }.isEmpty)
                            }
                        })
                        .padding(.horizontal, 5)
                        .help(String(localized: "theme.settings.import_export_help", comment: "Help text for import/export menu"))
                    }
                }
                ...
                Section {
                    ...
                    HStack {
                        Spacer()
                        Button(String(localized: "theme.settings.import_button", comment: "Button to import themes")) {
                            themeModel.importTheme()
                        }
                    }
                    .padding(.top, 10)
...
private extension ThemeSettingsView {
    ...
    private var changeThemeOnSystemAppearance: some View {
        Toggle(
            String(localized: "theme_settings.auto_change_theme", comment: "Option to automatically change theme based on system appearance"),
