...
struct ThemeSettingsThemeDetails: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Group {
                    ...
                    Section(String(localized: "settings.theme.section.text", defaultValue: "Text", comment: "Section header for text colors")) {
                        SettingsColorPicker(
                            String(localized: "settings.theme.text", defaultValue: "Text", comment: "Label for text color picker"),
                            color: $theme.editor.text.swiftColor
                        )
                        SettingsColorPicker(
                            String(localized: "settings.theme.cursor", defaultValue: "Cursor", comment: "Label for cursor color picker"),
                            color: $theme.editor.insertionPoint.swiftColor
                        )
                        SettingsColorPicker(
                            String(localized: "settings.theme.invisibles", defaultValue: "Invisibles", comment: "Label for invisibles color picker"),
                            color: $theme.editor.invisibles.swiftColor
                        )
                    }
                    Section(String(localized: "settings.theme.section.background", defaultValue: "Background", comment: "Section header for background colors")) {
                        SettingsColorPicker(
                            String(localized: "settings.theme.background", defaultValue: "Background", comment: "Label for background color picker"),
                            color: $theme.editor.background.swiftColor
                        )
                        SettingsColorPicker(
                            String(localized: "settings.theme.current_line", defaultValue: "Current Line", comment: "Label for current line color picker"),
                            color: $theme.editor.lineHighlight.swiftColor
                        )
                        SettingsColorPicker(
                            String(localized: "settings.theme.selection", defaultValue: "Selection", comment: "Label for selection color picker"),
                            color: $theme.editor.selection.swiftColor
                        )
                    }
                    Section(String(localized: "settings.theme.section.tokens", defaultValue: "Tokens", comment: "Section header for token colors")) {
                        VStack(spacing: 0) {
            ...
            HStack {
                if theme.isBundled {
                    HStack {
                        ...
                        Text(String(localized: "settings.theme.duplicate_warning", defaultValue: "Duplicate this theme to make changes.", comment: "Warning text for bundled themes"))
                            .font(.subheadline)
                            .lineLimit(2)
                    }
                    .help(String(localized: "settings.theme.duplicate_help", defaultValue: "Bundled themes must be duplicated to make changes.", comment: "Help text for bundled theme warning"))
                    .accessibilityElement(children: .combine)
                    .accessibilityLabel(String(localized: "settings.theme.duplicate_accessibility", defaultValue: "Warning: Duplicate this theme to make changes.", comment: "Accessibility label for bundled theme warning"))
                } else if !themeModel.isAdding {
                    Button(role: .destructive) {
                        deleteConfirmationIsPresented = true
                    } label: {
                        Text(String(localized: "settings.theme.delete_button", defaultValue: "Delete...", comment: "Button to delete theme"))
                            .foregroundStyle(.red)
                            .frame(minWidth: 56)
                    }
                    Button {
                        ...
                        Text(String(localized: "settings.theme.duplicate_ellipsis_button", defaultValue: "Duplicate...", comment: "Button to duplicate theme with ellipsis"))
                            .frame(minWidth: 56)
                    }
                }
                Spacer()
                if !themeModel.isAdding && theme.isBundled {
                    Button {
                        ...
                        Text(String(localized: "settings.theme.duplicate_button", defaultValue: "Duplicate", comment: "Button to duplicate theme"))
                            .frame(minWidth: 56)
                    }
                } else {
                    Button {
                        ...
                        Text(String(localized: "settings.theme.cancel_button", defaultValue: "Cancel", comment: "Button to cancel theme editing"))
                            .frame(minWidth: 56)
                    }
                ...
                Button {
                    ...
                    Text(String(localized: "settings.theme.done_button", defaultValue: "Done", comment: "Button to finish theme editing"))
                        .frame(minWidth: 56)
                }
        ...
        .alert(
            Text(String(format: String(localized: "settings.theme.delete_confirmation", defaultValue: "Are you sure you want to delete the theme \"%@\"?", comment: "Alert title for deleting theme"), theme.displayName)),
            isPresented: $deleteConfirmationIsPresented
        ) {
            Button(String(localized: "settings.theme.delete_theme_button", defaultValue: "Delete Theme", comment: "Button to confirm theme deletion")) {
                themeModel.delete(theme)
                dismiss()
            }
            Button(String(localized: "settings.theme.delete_cancel_button", defaultValue: "Cancel", comment: "Button to cancel theme deletion")) {
                deleteConfirmationIsPresented = false
            }
        } message: {
            Text(String(localized: "settings.theme.delete_warning", defaultValue: "This action cannot be undone.", comment: "Warning message for theme deletion"))
        }
    }
}
