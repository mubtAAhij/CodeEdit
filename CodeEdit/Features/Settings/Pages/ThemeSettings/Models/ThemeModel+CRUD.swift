...
extension ThemeModel {
    ...
    func importTheme() {
        ...
        openPanel.prompt = String(localized: "theme.import", defaultValue: "Import", comment: "Import theme button")
        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "theme.error.url-not-found", defaultValue: "Theme file URL not found", comment: "Error when theme file URL is missing")]
                )
            }
