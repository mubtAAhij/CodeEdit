...
extension ThemeModel {
    ...
    func importTheme() {
        ...
        openPanel.prompt = String(localized: "settings.theme.import", defaultValue: "Import", comment: "Import button label in theme import dialog")
        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "settings.theme.error.url-not-found", defaultValue: "Theme file URL not found", comment: "Error when theme file URL is not found")]
                )
            }
