...
extension ThemeModel {
    ...
    func loadThemes() throws { // swiftlint:disable:this function_body_length
        if let bundledThemesURL = bundledThemesURL {
            ...
            let userDefinedThemeFilenames = try filemanager.contentsOfDirectory(atPath: themesURL.path).filter {
                $0.contains(String(localized: "theme.extension.cetheme", defaultValue: ".cetheme", comment: "CodeEdit theme file extension with dot"))
            }
            ...
            try themeURLs.forEach { fileURL in
                if var theme = try load(from: fileURL) {
                    ...
                    guard let terminalColors = try theme.terminal.allProperties() as? [String: Theme.Attributes],
                        ...
                        print(String(localized: "theme.load.error", defaultValue: "error", comment: "Generic error loading theme"))
                        // TODO: Throw a proper error
                        throw NSError() // swiftlint:disable:this discouraged_direct_init
                    }

                    // check if there are any overrides in `settings.json`
                    if let overrides = prefs.theme.overrides[theme.name]?[String(localized: "theme.key.terminal", defaultValue: "terminal", comment: "Terminal theme key")] {
                    ...
                    if let overrides = prefs.theme.overrides[theme.name]?[String(localized: "theme.key.editor", defaultValue: "editor", comment: "Editor theme key")] {
    ...
    func importTheme() {
        ...
        let allowedTypes = [UTType(filenameExtension: String(localized: "theme.file-extension", defaultValue: "cetheme", comment: "CodeEdit theme file extension"))!]

        openPanel.prompt = String(localized: "theme.import.prompt", defaultValue: "Import", comment: "Import panel prompt")
    ...
    func duplicate(_ url: URL) {
        do {
            ...
            if isBundled {
                newFileName = String(format: String(localized: "theme.duplicate.name-format", defaultValue: "%@ %d", comment: "Format for duplicated theme name"), fileName, iterator)
            ...
            if let index = self.themes.firstIndex(where: { $0.fileURL == destinationFileURL }) {
                ...
                self.themes[index].name = newFileName.lowercased().replacingOccurrences(of: " ", with: String(localized: "theme.name.separator", defaultValue: "-", comment: "Separator for theme name"))
            ...
            print(String(format: String(localized: "theme.add.error", defaultValue: "Error adding theme: %@", comment: "Error message for adding theme"), error.localizedDescription))
        }
    }
    ...
    func rename(to newName: String, theme: Theme) {
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: String(localized: "theme.error.domain", defaultValue: "ThemeModel", comment: "Error domain for theme operations"),
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "theme.rename.error.no-url", defaultValue: "Theme file URL not found", comment: "Error when theme file URL is not found for rename")]
                )
            }

            var finalName = newName
            var finalURL = themesURL.appending(path: finalName).appendingPathExtension(String(localized: "theme.file-extension", defaultValue: "cetheme", comment: "CodeEdit theme file extension"))
            var iterator = 1

            // Check for existing display names in themes
            while themes.contains(where: { theme != $0 && $0.displayName == finalName }) {
                finalName = String(format: String(localized: "theme.rename.name-format", defaultValue: "%@ %d", comment: "Format for renamed theme with iterator"), newName, iterator)
                finalURL = themesURL.appending(path: finalName).appendingPathExtension(String(localized: "theme.file-extension", defaultValue: "cetheme", comment: "CodeEdit theme file extension"))
                iterator += 1
            }
            ...
            print(String(format: String(localized: "theme.rename.error", defaultValue: "Error renaming theme: %@", comment: "Error message for renaming theme"), error.localizedDescription))
        }
    }
    ...
    func save(_ theme: Theme) {
        do {
            ...
            print(String(format: String(localized: "theme.save.error", defaultValue: "Error saving theme: %@", comment: "Error message for saving theme"), error.localizedDescription))
        }
    }
