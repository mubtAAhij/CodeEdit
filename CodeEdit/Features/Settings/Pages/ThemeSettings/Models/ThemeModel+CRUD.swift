...
extension ThemeModel {
    ...
    func importTheme() {
        ...
        openPanel.prompt = "String(localized: "import_button", comment: "Button to import theme")"
        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "String(localized: "theme_file_url_not_found", comment: "Error message when theme file URL is not found")"]
                )
            }
