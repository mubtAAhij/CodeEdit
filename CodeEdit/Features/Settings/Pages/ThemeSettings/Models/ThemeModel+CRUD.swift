...
extension ThemeModel {
    ...
    func importTheme() {
        ...
        openPanel.prompt = String(localized: "settings.theme.import", defaultValue: "Import", comment: "Import theme open panel button")
        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "settings.theme.error.file-not-found", defaultValue: "Theme file URL not found", comment: "Error message when theme file URL is not found")]
                )
            }
