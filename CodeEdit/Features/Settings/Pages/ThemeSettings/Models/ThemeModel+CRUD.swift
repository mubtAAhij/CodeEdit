        ...
        do {
            guard let oldURL = theme.fileURL else {
                throw NSError(
                    domain: "ThemeModel",
                    code: 1,
                    userInfo: [NSLocalizedDescriptionKey: String(localized: "settings.theme.url-not-found", defaultValue: "Theme file URL not found", comment: "Error when theme file URL is missing")]
                )
            }
