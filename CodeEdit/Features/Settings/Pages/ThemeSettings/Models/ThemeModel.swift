...
final class ThemeModel: ObservableObject {
    ...
    internal var themesURL: URL {
        baseURL.appending(path: String(localized: "settings.theme.folder.themes", defaultValue: "Themes", comment: "Themes folder name"), directoryHint: .isDirectory)
    }

    /// The URL of the `Extensions` folder
    internal var extensionsURL: URL {
        baseURL.appending(path: String(localized: "settings.theme.folder.extensions", defaultValue: "Extensions", comment: "Extensions folder name"), directoryHint: .isDirectory)
    }
    ...
    @Published var selectedLightTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                Settings.shared
                    .preferences.theme.selectedLightTheme = self.selectedLightTheme?.name ?? String(localized: "settings.theme.broken", defaultValue: "Broken", comment: "Broken theme fallback name")
            }
        }
    }
    ...
    @Published var selectedDarkTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                Settings.shared
                    .preferences.theme.selectedDarkTheme = self.selectedDarkTheme?.name ?? String(localized: "settings.theme.broken", defaultValue: "Broken", comment: "Broken theme fallback name")
            }
        }
    }
    ...
    enum ThemeSettingsAppearances: String, CaseIterable {
        ...
        var displayName: String {
            switch self {
            case .light:
                return String(localized: "settings.theme.appearance.light", defaultValue: "Light Appearance", comment: "Light appearance option")
            case .dark:
                return String(localized: "settings.theme.appearance.dark", defaultValue: "Dark Appearance", comment: "Dark appearance option")
            }
        }
    }
    ...
    func exportTheme(_ theme: Theme) {
        ...
        savePanel.prompt = String(localized: "settings.theme.export.prompt", defaultValue: "Export", comment: "Export theme prompt")
    ...
    func exportAllCustomThemes() {
            ...
            openPanel.prompt = String(localized: "settings.theme.export.prompt", defaultValue: "Export", comment: "Export theme prompt")
