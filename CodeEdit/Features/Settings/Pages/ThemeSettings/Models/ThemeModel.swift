...
final class ThemeModel: ObservableObject {
    ...
    @Published var selectedLightTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                Settings.shared
                    .preferences.theme.selectedLightTheme = self.selectedLightTheme?.name ?? String(localized: "settings.theme.model.broken.preview", defaultValue: "Broken", comment: "Fallback text when theme preview data is invalid")
            }
        }
    }
    ...
    @Published var selectedDarkTheme: Theme? {
        didSet {
            DispatchQueue.main.async {
                Settings.shared
                    .preferences.theme.selectedDarkTheme = self.selectedDarkTheme?.name ?? String(localized: "settings.theme.model.broken.name", defaultValue: "Broken", comment: "Fallback text when theme name data is invalid")
            }
        }
    }
    ...
    func exportTheme(_ theme: Theme) {
        ...
        savePanel.prompt = String(localized: "settings.theme.model.export-menu-item", defaultValue: "Export", comment: "Menu item title for exporting selected theme")
    ...
    func exportAllCustomThemes() {
            ...
            openPanel.prompt = String(localized: "settings.theme.model.export-button", defaultValue: "Export", comment: "Button title for exporting theme file")
