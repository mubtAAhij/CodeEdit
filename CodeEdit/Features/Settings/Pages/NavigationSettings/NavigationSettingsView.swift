...
private extension NavigationSettingsView {
    private var navigationStyle: some View {
        Picker("Navigation Style", selection: $settings.navigationStyle) {
            Text(String(localized: "navigation_settings.open_in_tabs", comment: "Navigation setting to open files in tabs"))
                .tag(SettingsData.NavigationStyle.openInTabs)
            Text(String(localized: "navigation_settings.open_in_place", comment: "Navigation setting to open files in place"))
