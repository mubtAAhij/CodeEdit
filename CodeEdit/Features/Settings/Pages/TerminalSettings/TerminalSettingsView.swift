...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(localized: "shell", comment: "Shell selector label in terminal settings"), selection: $settings.shell) {
            Text("System Default")
                .tag(SettingsData.TerminalShell.system)
            Divider()
            Text(String(localized: "zsh", comment: "Zsh shell option"))
                .tag(SettingsData.TerminalShell.zsh)
            Text(String(localized: "bash", comment: "Bash shell option"))
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(localized: "shell_integration", comment: "Label for shell integration toggle"), isOn: $settings.useShellIntegration)
            ...
            if !settings.useShellIntegration {
                HStack {
                    ...
                    Text(String(localized: "shell_integration_warning", comment: "Warning message when shell integration is disabled"))
                    Spacer()
                }
            }
        }
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(localized: "use_login_shell", comment: "Label for login shell toggle"), isOn: $settings.useLoginShell)
            // swiftlint:disable:next line_length
                .help(String(localized: "login_shell_help", comment: "Help text explaining login shell option"))
        } else {
            EmptyView()
        }
    }
