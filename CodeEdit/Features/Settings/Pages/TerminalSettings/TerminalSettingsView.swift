...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(
            localized: "settings.terminal.shell.title",
            defaultValue: "Shell",
            comment: "Title for terminal shell selection setting."
        ), selection: $settings.shell) {
            Text(String(
                localized: "settings.terminal.shell.option.system-default",
                defaultValue: "System Default",
                comment: "Option label for using system default shell."
            ))
                .tag(SettingsData.TerminalShell.system)
            Divider()
            Text(String(
                localized: "settings.terminal.shell.option.zsh",
                defaultValue: "Zsh",
                comment: "Option label for Zsh shell."
            ))
                .tag(SettingsData.TerminalShell.zsh)
            Text(String(
                localized: "settings.terminal.shell.option.bash",
                defaultValue: "Bash",
                comment: "Option label for Bash shell."
            ))
    ...
    private var cursorStyle: some View {
        Picker(String(
            localized: "settings.terminal.cursor-style.title",
            defaultValue: "Terminal Cursor Style",
            comment: "Title for terminal cursor style setting."
        ), selection: $settings.cursorStyle) {
            Text(String(
                localized: "settings.terminal.cursor-style.option.block",
                defaultValue: "Block",
                comment: "Option label for block cursor style."
            ))
                .tag(SettingsData.TerminalCursorStyle.block)
            Text(String(
                localized: "settings.terminal.cursor-style.option.underline",
                defaultValue: "Underline",
                comment: "Option label for underline cursor style."
            ))
                .tag(SettingsData.TerminalCursorStyle.underline)
            Text(String(
                localized: "settings.terminal.cursor-style.option.bar",
                defaultValue: "Bar",
                comment: "Option label for bar cursor style."
            ))
    ...
    private var cursorBlink: some View {
        Toggle(String(
            ...
            comment: "Toggle label for blinking cursor in terminal."
        ), isOn: $settings.cursorBlink)
    }

    private var optionAsMetaToggle: some View {
        Toggle(String(
            localized: "settings.terminal.option-key-as-meta",
            defaultValue: "Use \"Option\" key as \"Meta\"",
            comment: "Toggle label for treating Option key as Meta in terminal."
    ...
    private var useTextEditorFontToggle: some View {
        Toggle(String(
            ...
            comment: "Toggle label for using editor font in terminal."
        ), isOn: $settings.useTextEditorFont)
    }

    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(
            localized: "settings.terminal.font.title",
            defaultValue: "Font",
            comment: "Title for terminal font family setting."
    ...
    private var fontSizeSelector: some View {
        Stepper(
            String(
                localized: "settings.terminal.font-size.title",
                defaultValue: "Font Size",
                comment: "Title for terminal font size setting."
            ),
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(
                ...
                comment: "Section title for terminal shell integration settings."
            ...
            // swiftlint:disable:next line_length
                .help(String(
                    ...
                    comment: "Description text for shell integration feature."
                ))
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(
                localized: "settings.terminal.use-login-shell",
                defaultValue: "Use Login Shell",
                comment: "Toggle label for using a login shell in terminal sessions."
            ...
            // swiftlint:disable:next line_length
                .help(String(
                    ...
                    comment: "Description text explaining login shell behavior in terminal sessions."
                ))
        } else {
            EmptyView()
        }
