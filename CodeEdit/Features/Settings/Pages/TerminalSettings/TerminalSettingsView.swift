...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(
            localized: "terminal-settings.shell",
            defaultValue: "Shell",
            comment: "Label for shell selection picker"
        ), selection: $settings.shell) {
            Text(String(
                localized: "terminal-settings.system-default",
                defaultValue: "System Default",
                comment: "Option for system default shell"
            ))
    ...
    private var cursorStyle: some View {
        Picker(String(
            localized: "terminal-settings.cursor-style",
            defaultValue: "Terminal Cursor Style",
            comment: "Label for terminal cursor style picker"
        ), selection: $settings.cursorStyle) {
            Text(String(
                localized: "terminal-settings.cursor-style.block",
                defaultValue: "Block",
                comment: "Block cursor style option"
            ))
                .tag(SettingsData.TerminalCursorStyle.block)
            Text(String(
                localized: "terminal-settings.cursor-style.underline",
                defaultValue: "Underline",
                comment: "Underline cursor style option"
            ))
                .tag(SettingsData.TerminalCursorStyle.underline)
            Text(String(
                localized: "terminal-settings.cursor-style.bar",
                defaultValue: "Bar",
                comment: "Bar cursor style option"
            ))
    ...
    private var cursorBlink: some View {
        Toggle(String(
            localized: "terminal-settings.blink-cursor",
            defaultValue: "Blink Cursor",
            comment: "Toggle for blinking cursor"
        ), isOn: $settings.cursorBlink)
    }

    private var optionAsMetaToggle: some View {
        Toggle(String(
            localized: "terminal-settings.option-as-meta",
            defaultValue: "Use \"Option\" key as \"Meta\"",
            comment: "Toggle for using Option key as Meta key"
        ), isOn: $settings.optionAsMeta)
    }

    private var useTextEditorFontToggle: some View {
        Toggle(String(
            localized: "terminal-settings.use-text-editor-font",
            defaultValue: "Use text editor font",
            comment: "Toggle for using text editor font in terminal"
        ), isOn: $settings.useTextEditorFont)
    }

    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(
            localized: "terminal-settings.font",
            defaultValue: "Font",
            comment: "Label for font picker"
    ...
    private var fontSizeSelector: some View {
        Stepper(
            String(
                localized: "terminal-settings.font-size",
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(
                localized: "terminal-settings.shell-integration",
                defaultValue: "Shell Integration",
                comment: "Toggle for shell integration"
            ), isOn: $settings.useShellIntegration)
            // swiftlint:disable:next line_length
                .help(String(
                    localized: "terminal-settings.shell-integration-help",
                    ...
                    comment: "Help text explaining shell integration"
                ))
            if !settings.useShellIntegration {
                HStack {
                    ...
                    Text(String(
                        localized: "terminal-settings.shell-integration-warning",
                        defaultValue: "Warning: Disabling integration disables features such as terminal title detection.",
                        comment: "Warning text when shell integration is disabled"
                    ))
                    Spacer()
                }
            }
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(
                localized: "terminal-settings.use-login-shell",
                defaultValue: "Use Login Shell",
                comment: "Toggle for using login shell"
            ), isOn: $settings.useLoginShell)
            // swiftlint:disable:next line_length
                .help(String(
                    localized: "terminal-settings.use-login-shell-help",
                    ...
                    comment: "Help text explaining login shell option"
                ))
        } else {
            EmptyView()
        }
