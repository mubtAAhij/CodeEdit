...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(localized: "terminal-settings.shell", defaultValue: "Shell", comment: "Label for shell selection picker"), selection: $settings.shell) {
            Text(String(localized: "terminal-settings.shell.system-default", defaultValue: "System Default", comment: "Option for system default shell"))
                .tag(SettingsData.TerminalShell.system)
            Divider()
            Text(String(localized: "terminal-settings.shell.zsh", defaultValue: "Zsh", comment: "Option for Zsh shell"))
                .tag(SettingsData.TerminalShell.zsh)
            Text(String(localized: "terminal-settings.shell.bash", defaultValue: "Bash", comment: "Option for Bash shell"))
    ...
    private var useTextEditorFontToggle: some View {
        Toggle(String(localized: "terminal-settings.use-text-editor-font", defaultValue: "Use text editor font", comment: "Toggle for using text editor font in terminal"), isOn: $settings.useTextEditorFont)
    }
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            ...
            // swiftlint:disable:next line_length
                .help(String(localized: "terminal-settings.shell-integration.help", defaultValue: "CodeEdit supports integrating with common shells such as Bash and Zsh. This enables features like terminal title detection.", comment: "Help text explaining shell integration"))
            if !settings.useShellIntegration {
                HStack {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundStyle(Color(NSColor.systemYellow))
                    Text(String(localized: "terminal-settings.shell-integration.warning", defaultValue: "Warning: Disabling integration disables features such as terminal title detection.", comment: "Warning text when shell integration is disabled"))
                    Spacer()
                }
            }
        }
