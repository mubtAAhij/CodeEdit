...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(localized: "terminal.shell", defaultValue: "Shell", comment: "Label for terminal shell selector"), selection: $settings.shell) {
    ...
    private var cursorStyle: some View {
        Picker(String(localized: "terminal.cursor-style", defaultValue: "Terminal Cursor Style", comment: "Label for terminal cursor style picker"), selection: $settings.cursorStyle) {
    ...
    private var cursorBlink: some View {
        Toggle(String(localized: "terminal.blink-cursor", defaultValue: "Blink Cursor", comment: "Toggle for blinking cursor"), isOn: $settings.cursorBlink)
    }

    private var optionAsMetaToggle: some View {
        Toggle(String(localized: "terminal.option-as-meta", defaultValue: "Use \"Option\" key as \"Meta\"", comment: "Toggle for using Option key as Meta"), isOn: $settings.optionAsMeta)
    }

    private var useTextEditorFontToggle: some View {
        Toggle(String(localized: "terminal.use-text-editor-font", defaultValue: "Use text editor font", comment: "Toggle for using text editor font in terminal"), isOn: $settings.useTextEditorFont)
    }

    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "terminal.font", defaultValue: "Font", comment: "Label for terminal font picker"), selectedFontName: $settings.font.name)
    }

    private var fontSizeSelector: some View {
        Stepper(
            String(localized: "terminal.font-size", defaultValue: "Font Size", comment: "Label for terminal font size stepper"),
        ...
        VStack {
            ...
            if !settings.useShellIntegration {
                HStack {
                    ...
                    Text(String(localized: "terminal.shell-integration.warning", defaultValue: "Warning: Disabling integration disables features such as terminal title detection.", comment: "Warning about disabling shell integration"))
                    Spacer()
                }
            }
        }
