...
private extension TerminalSettingsView {
    ...
    private var cursorBlink: some View {
        Toggle(String(localized: "settings.terminal.blink-cursor", defaultValue: "Blink Cursor", comment: "Blink cursor toggle label"), isOn: $settings.cursorBlink)
    }

    private var optionAsMetaToggle: some View {
        Toggle(String(localized: "settings.terminal.option-as-meta", defaultValue: "Use \"Option\" key as \"Meta\"", comment: "Use Option key as Meta toggle label"), isOn: $settings.optionAsMeta)
    }

    private var useTextEditorFontToggle: some View {
        Toggle(String(localized: "settings.terminal.use-text-editor-font", defaultValue: "Use text editor font", comment: "Use text editor font toggle label"), isOn: $settings.useTextEditorFont)
    }
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(localized: "settings.terminal.shell-integration", defaultValue: "Shell Integration", comment: "Shell integration toggle label"), isOn: $settings.useShellIntegration)
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(localized: "settings.terminal.use-login-shell", defaultValue: "Use Login Shell", comment: "Use login shell toggle label"), isOn: $settings.useLoginShell)
            // swiftlint:disable:next line_length
