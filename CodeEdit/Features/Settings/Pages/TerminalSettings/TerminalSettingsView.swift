...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(localized: "settings.terminal.shell", defaultValue: "Shell", comment: "Terminal shell setting label"), selection: $settings.shell) {
    ...
    private var cursorStyle: some View {
        Picker(String(localized: "settings.terminal.cursor-style", defaultValue: "Terminal Cursor Style", comment: "Terminal cursor style setting label"), selection: $settings.cursorStyle) {
    ...
    private var cursorBlink: some View {
        Toggle(String(localized: "settings.terminal.blink-cursor", defaultValue: "Blink Cursor", comment: "Blink cursor setting toggle"), isOn: $settings.cursorBlink)
    }

    private var optionAsMetaToggle: some View {
        Toggle(String(localized: "settings.terminal.option-as-meta", defaultValue: "Use \"Option\" key as \"Meta\"", comment: "Option key as Meta setting toggle"), isOn: $settings.optionAsMeta)
    }

    private var useTextEditorFontToggle: some View {
        Toggle(String(localized: "settings.terminal.use-editor-font", defaultValue: "Use text editor font", comment: "Use text editor font setting toggle"), isOn: $settings.useTextEditorFont)
    }

    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "settings.terminal.font", defaultValue: "Font", comment: "Terminal font setting label"), selectedFontName: $settings.font.name)
    }

    private var fontSizeSelector: some View {
        Stepper(
            String(localized: "settings.terminal.font-size", defaultValue: "Font Size", comment: "Terminal font size setting label"),
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(localized: "settings.terminal.shell-integration", defaultValue: "Shell Integration", comment: "Shell integration setting toggle"), isOn: $settings.useShellIntegration)
            ...
            if !settings.useShellIntegration {
                HStack {
                    ...
                    Text(String(localized: "settings.terminal.shell-integration.warning", defaultValue: "Warning: Disabling integration disables features such as terminal title detection.", comment: "Shell integration disabled warning"))
                    Spacer()
                }
            }
        }
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(localized: "settings.terminal.use-login-shell", defaultValue: "Use Login Shell", comment: "Use login shell setting toggle"), isOn: $settings.useLoginShell)
            // swiftlint:disable:next line_length
