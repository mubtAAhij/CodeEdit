...
private extension TerminalSettingsView {
    ...
    private var optionAsMetaToggle: some View {
        Toggle(String(localized: "settings.terminal.option-as-meta", defaultValue: "Use \"Option\" key as \"Meta\"", comment: "Option as Meta key toggle label"), isOn: $settings.optionAsMeta)
    }
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            ...
            if !settings.useShellIntegration {
                HStack {
                    Image(systemName: String(localized: "settings.terminal.shell-integration.warning-icon", defaultValue: "exclamationmark.triangle.fill", comment: "Warning icon for shell integration"))
