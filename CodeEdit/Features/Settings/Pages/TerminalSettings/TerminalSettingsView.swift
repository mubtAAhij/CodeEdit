...
private extension TerminalSettingsView {
    ...
    private var cursorStyle: some View {
        Picker(String(localized: "terminal.settings.cursor_style", comment: "Terminal cursor style setting label"), selection: $settings.cursorStyle) {
            Text(String(localized: "terminal.settings.cursor_style.block", comment: "Block cursor style option"))
                .tag(SettingsData.TerminalCursorStyle.block)
            Text(String(localized: "terminal.settings.cursor_style.underline", comment: "Underline cursor style option"))
                .tag(SettingsData.TerminalCursorStyle.underline)
            Text(String(localized: "terminal.settings.cursor_style.bar", comment: "Bar cursor style option"))
                .tag(SettingsData.TerminalCursorStyle.bar)
        }
    }

    private var cursorBlink: some View {
        Toggle(String(localized: "terminal.settings.blink_cursor", comment: "Terminal cursor blinking setting label"), isOn: $settings.cursorBlink)
    }
    ...
    private var useTextEditorFontToggle: some View {
        Toggle(String(localized: "terminal.settings.use_text_editor_font", comment: "Toggle for using text editor font in terminal"), isOn: $settings.useTextEditorFont)
    }
    ...
    private var fontSizeSelector: some View {
        Stepper(
            String(localized: "terminal.settings.font_size", comment: "Font size selector label"),
    ...
    @ViewBuilder private var injectionOptions: some View {
        VStack {
            Toggle(String(localized: "terminal.settings.shell_integration", comment: "Shell integration toggle label"), isOn: $settings.useShellIntegration)
            // swiftlint:disable:next line_length
                .help(String(localized: "terminal.settings.shell_integration.help", comment: "Help text explaining shell integration feature"))
    ...
    @ViewBuilder private var useLoginShell: some View {
        if settings.useShellIntegration {
            Toggle(String(localized: "terminal.settings.use_login_shell", comment: "Toggle for using login shell"), isOn: $settings.useLoginShell)
            // swiftlint:disable:next line_length
                .help(String(localized: "terminal.settings.use_login_shell.help", comment: "Help text explaining login shell option"))
        } else {
            EmptyView()
        }
    }
