...
private extension TerminalSettingsView {
    @ViewBuilder private var shellSelector: some View {
        Picker(String(localized: "terminal-settings.shell", defaultValue: "Shell", comment: "Shell picker label"), selection: $settings.shell) {
            ...
            Divider()
            Text(String(localized: "terminal-settings.zsh", defaultValue: "Zsh", comment: "Zsh shell option"))
                .tag(SettingsData.TerminalShell.zsh)
            Text(String(localized: "terminal-settings.bash", defaultValue: "Bash", comment: "Bash shell option"))
    ...
    private var cursorStyle: some View {
        Picker(String(localized: "terminal-settings.cursor-style", defaultValue: "Terminal Cursor Style", comment: "Terminal cursor style picker label"), selection: $settings.cursorStyle) {
            Text(String(localized: "terminal-settings.block", defaultValue: "Block", comment: "Block cursor style option"))
                .tag(SettingsData.TerminalCursorStyle.block)
            Text(String(localized: "terminal-settings.underline", defaultValue: "Underline", comment: "Underline cursor style option"))
                .tag(SettingsData.TerminalCursorStyle.underline)
            Text(String(localized: "terminal-settings.bar", defaultValue: "Bar", comment: "Bar cursor style option"))
    ...
    private var optionAsMetaToggle: some View {
        Toggle(String(localized: "terminal-settings.option-as-meta", defaultValue: "Use \"Option\" key as \"Meta\"", comment: "Use option as meta toggle"), isOn: $settings.optionAsMeta)
    }
    ...
    private var fontSizeSelector: some View {
        Stepper(
            String(localized: "terminal-settings.font-size", defaultValue: "Font Size", comment: "Font size stepper label"),
        ...
        VStack {
            ...
            if !settings.useShellIntegration {
                HStack {
                    ...
                    Text(String(localized: "terminal-settings.integration-warning", defaultValue: "Warning: Disabling integration disables features such as terminal title detection.", comment: "Shell integration warning"))
                    Spacer()
                }
            }
        }
