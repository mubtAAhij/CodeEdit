...
private extension TextEditingSettingsView {
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker("Prefer Indent Using", selection: $textEditing.indentOption.indentType) {
                Text(String(localized: "tabs", comment: "Tab indentation option"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
                Text(String(localized: "spaces_indent", comment: "Space indentation option"))
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "use_custom_color", comment: "Toggle to use custom bracket highlight color"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
    ...
    @ViewBuilder private var showGutter: some View {
        Toggle("Show Gutter", isOn: $textEditing.showGutter)
            .help(String(localized: "gutter_help", comment: "Help text for show gutter setting"))
    }
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "show_minimap", comment: "Toggle to show minimap"), isOn: $textEditing.showMinimap)
            ...
            .help(String(localized: "minimap_help", comment: "Help text for show minimap setting"))
    }

    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "show_code_folding_ribbon", comment: "Toggle to show or hide the code folding ribbon in the editor"), isOn: $textEditing.showFoldingRibbon)
            .disabled(!textEditing.showGutter) // Disabled when the gutter is disabled
            // swiftlint:disable:next line_length
            .help(String(localized: "code_folding_ribbon_description", comment: "Help text explaining what the code folding ribbon does and its dependency on the gutter"))
    }

    @ViewBuilder private var reformatSettings: some View {
        Toggle("Show Reformatting Guide", isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "reformat_guide_description", comment: "Help text explaining what the reformatting guide does"))
        ...
        .help(String(localized: "reformat_column_description", comment: "Help text explaining the reformat column setting"))
    }

    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "show_invisible_characters", comment: "Toggle to show or hide invisible characters in the editor"))
            Spacer()
            Toggle(isOn: $textEditing.invisibleCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingInvisibleCharacterSettings = true
            } label: {
                Text(String(localized: "configure_button", comment: "Button to open configuration settings"))
            }
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "show_warning_characters", comment: "Toggle to show or hide warning characters in the editor"))
            Spacer()
