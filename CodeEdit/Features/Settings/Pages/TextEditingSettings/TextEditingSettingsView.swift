...
private extension TextEditingSettingsView {
    ...
    @ViewBuilder private var fontSizeSelector: some View {
        Stepper(
            String(localized: "settings.text-editing.font-size", defaultValue: "Font Size", comment: "Label for font size stepper"),
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            Text(String(localized: "settings.text-editing.autocomplete-braces", defaultValue: "Autocomplete braces", comment: "Label for autocomplete braces toggle"))
    ...
    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "settings.text-editing.type-over-completion", defaultValue: "Enable type-over completion", comment: "Label for type-over completion toggle"), isOn: $textEditing.enableTypeOverCompletion)
    }

    @ViewBuilder private var wrapLinesToEditorWidth: some View {
        Toggle(String(localized: "settings.text-editing.wrap-lines", defaultValue: "Wrap lines to editor width", comment: "Label for wrap lines toggle"), isOn: $textEditing.wrapLinesToEditorWidth)
    }

    @ViewBuilder private var useSystemCursor: some View {
        if #available(macOS 14, *) {
            Toggle(String(localized: "settings.text-editing.use-system-cursor", defaultValue: "Use System Cursor", comment: "Label for system cursor toggle"), isOn: $textEditing.useSystemCursor)
        } else {
            EmptyView()
        }
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                String(localized: "settings.text-editing.editor-overscroll", defaultValue: "Editor Overscroll", comment: "Label for editor overscroll picker"),
    ...
    @ViewBuilder private var lineHeight: some View {
        Stepper(
            String(localized: "settings.text-editing.line-height", defaultValue: "Line Height", comment: "Label for line height stepper"),
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "settings.text-editing.prefer-indent-using", defaultValue: "Prefer Indent Using", comment: "Label for indent type picker"), selection: $textEditing.indentOption.indentType) {
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            Stepper(
                String(localized: "settings.text-editing.tab-width", defaultValue: "Tab Width", comment: "Label for tab width stepper"),
                value: Binding<Double>(
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "settings.text-editing.use-custom-color", defaultValue: "Use Custom Color", comment: "Toggle for using custom bracket highlight color"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "settings.text-editing.show-code-folding-ribbon", defaultValue: "Show Code Folding Ribbon", comment: "Toggle for showing code folding ribbon"), isOn: $textEditing.showFoldingRibbon)
    ...
    @ViewBuilder private var reformatSettings: some View {
        ...
        Stepper(
            String(localized: "settings.text-editing.reformat-at-column", defaultValue: "Reformat at Column", comment: "Label for reformat at column stepper"),
            value: Binding<Double>(
