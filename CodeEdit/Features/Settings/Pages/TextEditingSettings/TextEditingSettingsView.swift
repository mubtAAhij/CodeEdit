...
private extension TextEditingSettingsView {
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            ...
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    Stepper(
                        String(localized: "settings.text-editing.indent.indent-width", defaultValue: "Indent Width", comment: "Indent width stepper label"),
                    ...
                    Text(String(localized: "settings.text-editing.indent.spaces-unit", defaultValue: "spaces", comment: "Spaces unit label"))
                        .foregroundColor(.secondary)
                }
                .help(String(localized: "settings.text-editing.indent.spaces-help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Indent spaces help text"))
            }
        }
    }
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            ...
            Text(String(localized: "settings.text-editing.tab-width.spaces-unit", defaultValue: "spaces", comment: "Spaces unit label for tab width"))
                .foregroundColor(.secondary)
        }
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "settings.text-editing.folding-ribbon", defaultValue: "Show Code Folding Ribbon", comment: "Show code folding ribbon toggle"), isOn: $textEditing.showFoldingRibbon)
    ...
    @ViewBuilder private var reformatSettings: some View {
        Toggle("Show Reformatting Guide", isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "settings.text-editing.reformat.guide-help", defaultValue: "Shows a vertical guide at the reformat column.", comment: "Reformatting guide help text"))
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "settings.text-editing.invisible-characters", defaultValue: "Show Invisible Characters", comment: "Show invisible characters label"))
            Spacer()
            Toggle(isOn: $textEditing.invisibleCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingInvisibleCharacterSettings = true
            } label: {
                Text(String(localized: "settings.text-editing.invisible-characters.configure", defaultValue: "Configure...", comment: "Configure invisible characters button"))
            }
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "settings.text-editing.warning-characters", defaultValue: "Show Warning Characters", comment: "Show warning characters label"))
            Spacer()
            Toggle(isOn: $textEditing.warningCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingWarningCharactersSettings = true
            } label: {
                Text(String(localized: "settings.text-editing.warning-characters.configure", defaultValue: "Configure...", comment: "Configure warning characters button"))
            }
