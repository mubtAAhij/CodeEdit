...
private extension TextEditingSettingsView {
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            ...
            Text(String(localized: "settings.text-editing.autocomplete-braces.help", defaultValue: "Automatically insert closing braces (\"}\")", comment: "Autocomplete braces help text"))
        }
    }

    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "settings.text-editing.enable-type-over-completion", defaultValue: "Enable type-over completion", comment: "Enable type-over completion toggle label"), isOn: $textEditing.enableTypeOverCompletion)
    }

    @ViewBuilder private var wrapLinesToEditorWidth: some View {
        Toggle(String(localized: "settings.text-editing.wrap-lines", defaultValue: "Wrap lines to editor width", comment: "Wrap lines toggle label"), isOn: $textEditing.wrapLinesToEditorWidth)
    }

    @ViewBuilder private var useSystemCursor: some View {
        if #available(macOS 14, *) {
            Toggle(String(localized: "settings.text-editing.use-system-cursor", defaultValue: "Use System Cursor", comment: "Use system cursor toggle label"), isOn: $textEditing.useSystemCursor)
        } else {
            EmptyView()
        }
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                ...
                Text(String(localized: "settings.text-editing.editor-overscroll.none", defaultValue: "None", comment: "None overscroll option"))
                    .tag(SettingsData.TextEditingSettings.OverscrollOption.none)
                Divider()
                Text(String(localized: "settings.text-editing.editor-overscroll.small", defaultValue: "Small", comment: "Small overscroll option"))
                    .tag(
                        SettingsData.TextEditingSettings.OverscrollOption.small
                    )
                Text(String(localized: "settings.text-editing.editor-overscroll.medium", defaultValue: "Medium", comment: "Medium overscroll option"))
                    .tag(
                        SettingsData.TextEditingSettings.OverscrollOption.medium
                    )
                Text(String(localized: "settings.text-editing.editor-overscroll.large", defaultValue: "Large", comment: "Large overscroll option"))
                    .tag(
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "settings.text-editing.prefer-indent-using", defaultValue: "Prefer Indent Using", comment: "Prefer indent using picker label"), selection: $textEditing.indentOption.indentType) {
                Text(String(localized: "settings.text-editing.prefer-indent-using.tabs", defaultValue: "Tabs", comment: "Tabs indent option"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
                Text(String(localized: "settings.text-editing.prefer-indent-using.spaces", defaultValue: "Spaces", comment: "Spaces indent option"))
            ...
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    ...
                    Text(String(localized: "settings.text-editing.indent-width.unit", defaultValue: "spaces", comment: "Spaces unit label"))
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            ...
            Text(String(localized: "settings.text-editing.tab-width.unit", defaultValue: "spaces", comment: "Spaces unit label"))
                .foregroundColor(.secondary)
        }
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            Picker(
                ...
                Text(String(localized: "settings.text-editing.bracket-pair-highlight.disabled", defaultValue: "Disabled", comment: "Disabled highlight option")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.disabled)
                Divider()
                Text(String(localized: "settings.text-editing.bracket-pair-highlight.bordered", defaultValue: "Bordered", comment: "Bordered highlight option")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.bordered)
                Text(String(localized: "settings.text-editing.bracket-pair-highlight.flash", defaultValue: "Flash", comment: "Flash highlight option")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.flash)
                Text(String(localized: "settings.text-editing.bracket-pair-highlight.underline", defaultValue: "Underline", comment: "Underline highlight option")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.underline)
            }
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "settings.text-editing.bracket-pair-highlight.use-custom-color", defaultValue: "Use Custom Color", comment: "Use custom color toggle label"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
                    String(localized: "settings.text-editing.bracket-pair-highlight.color", defaultValue: "Bracket Pair Highlight Color", comment: "Bracket pair highlight color picker label"),
    ...
    @ViewBuilder private var showGutter: some View {
        Toggle(String(localized: "settings.text-editing.show-gutter", defaultValue: "Show Gutter", comment: "Show gutter toggle label"), isOn: $textEditing.showGutter)
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "settings.text-editing.show-minimap", defaultValue: "Show Minimap", comment: "Show minimap toggle label"), isOn: $textEditing.showMinimap)
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "settings.text-editing.show-folding-ribbon", defaultValue: "Show Code Folding Ribbon", comment: "Show code folding ribbon toggle label"), isOn: $textEditing.showFoldingRibbon)
    ...
    @ViewBuilder private var reformatSettings: some View {
        Toggle(String(localized: "settings.text-editing.show-reformatting-guide", defaultValue: "Show Reformatting Guide", comment: "Show reformatting guide toggle label"), isOn: $textEditing.showReformattingGuide)
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            ...
            Button {
                ...
                Text(String(localized: "settings.text-editing.show-invisible-characters.configure", defaultValue: "Configure...", comment: "Configure invisible characters button"))
            }
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            ...
            Button {
                ...
                Text(String(localized: "settings.text-editing.show-warning-characters.configure", defaultValue: "Configure...", comment: "Configure warning characters button"))
            }
