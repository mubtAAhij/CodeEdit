...
private extension TextEditingSettingsView {
    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "settings.text-editing.font-label", defaultValue: "Font", comment: "Label for font picker"), selectedFontName: $textEditing.font.name)
    }
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            ...
            Text(String(localized: "settings.text-editing.autocomplete-braces-description", defaultValue: "Automatically insert closing braces (\"}\")", comment: "Description for autocomplete braces setting"))
        }
    }
    ...
    @ViewBuilder private var useSystemCursor: some View {
        if #available(macOS 14, *) {
            Toggle(String(localized: "settings.text-editing.use-system-cursor-toggle", defaultValue: "Use System Cursor", comment: "Toggle for using system cursor"), isOn: $textEditing.useSystemCursor)
        } else {
            EmptyView()
        }
    }
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            ...
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    Stepper(
                        String(localized: "settings.text-editing.indent-width-label", defaultValue: "Indent Width", comment: "Label for indent width stepper"),
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "settings.text-editing.use-custom-color-toggle", defaultValue: "Use Custom Color", comment: "Toggle to use custom color for bracket highlighting"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
                    String(localized: "settings.text-editing.bracket-highlight-color-label", defaultValue: "Bracket Pair Highlight Color", comment: "Label for bracket pair highlight color picker"),
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "settings.text-editing.show-minimap-toggle", defaultValue: "Show Minimap", comment: "Toggle to show minimap"), isOn: $textEditing.showMinimap)
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "settings.text-editing.show-folding-ribbon-toggle", defaultValue: "Show Code Folding Ribbon", comment: "Toggle to show code folding ribbon"), isOn: $textEditing.showFoldingRibbon)
            .disabled(!textEditing.showGutter) // Disabled when the gutter is disabled
            // swiftlint:disable:next line_length
            .help(String(localized: "settings.text-editing.show-folding-ribbon-help", defaultValue: "The code folding ribbon lets you fold regions of code. When the gutter is disabled, the folding ribbon is disabled.", comment: "Help text for show code folding ribbon toggle"))
    }
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-invisible-characters-label", defaultValue: "Show Invisible Characters", comment: "Label for show invisible characters setting"))
            Spacer()
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-warning-characters-label", defaultValue: "Show Warning Characters", comment: "Label for show warning characters setting"))
            Spacer()
