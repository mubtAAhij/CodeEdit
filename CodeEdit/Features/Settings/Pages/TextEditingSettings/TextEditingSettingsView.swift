...
private extension TextEditingSettingsView {
    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "text_editing.font", comment: "Label for font selection setting"), selectedFontName: $textEditing.font.name)
    }

    @ViewBuilder private var fontSizeSelector: some View {
        Stepper(
            String(localized: "text_editing.font_size", comment: "Label for font size setting"),
    ...
    @ViewBuilder private var useSystemCursor: some View {
        if #available(macOS 14, *) {
            Toggle(String(localized: "text_editing.use_system_cursor", comment: "Setting to use system cursor"), isOn: $textEditing.useSystemCursor)
        } else {
            EmptyView()
        }
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                ...
                Text(String(localized: "text_editing.none", comment: "None option for cursor blink setting"))
                    .tag(SettingsData.TextEditingSettings.OverscrollOption.none)
                Divider()
                Text(String(localized: "text_editing.small", comment: "Small option for cursor blink setting"))
                    .tag(
                        SettingsData.TextEditingSettings.OverscrollOption.small
                    )
                Text(String(localized: "text_editing.medium", comment: "Medium option for cursor blink setting"))
                    .tag(
                        SettingsData.TextEditingSettings.OverscrollOption.medium
                    )
                Text(String(localized: "text_editing.large", comment: "Large option for cursor blink setting"))
                    .tag(
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker("Prefer Indent Using", selection: $textEditing.indentOption.indentType) {
                Text(String(localized: "text_editing.tabs", comment: "Tabs option for indentation"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
                Text(String(localized: "text_editing.spaces", comment: "Spaces option for indentation"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            }
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    Stepper(
                        String(localized: "text_editing.indent_width", comment: "Label for indent width setting"),
                    ...
                    Text(String(localized: "text_editing.spaces_unit", comment: "Unit label for spaces measurement"))
                        .foregroundColor(.secondary)
                }
                .help(String(localized: "text_editing.indent_width_description", comment: "Description for indent width setting"))
            }
        }
    }
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        ...
        .help(String(localized: "text_editing.tab_width_description", comment: "Description for tab width setting"))
    }
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            Picker(
                ...
                Text(String(localized: "text_editing.disabled", comment: "Disabled option for bracket highlighting")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.disabled)
                Divider()
                Text(String(localized: "text_editing.bordered", comment: "Bordered option for bracket highlighting")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.bordered)
                Text(String(localized: "text_editing.flash", comment: "Flash option for bracket highlighting")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.flash)
                Text(String(localized: "text_editing.underline", comment: "Underline option for bracket highlighting")).tag(SettingsData.TextEditingSettings.BracketPairEmphasis.HighlightType.underline)
            }
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "text_editing.use_custom_color", comment: "Setting to use custom color for bracket highlighting"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "text_editing.show_minimap", comment: "Label for show minimap setting"), isOn: $textEditing.showMinimap)
            ...
            .help(String(localized: "text_editing.minimap_description", comment: "Description of minimap functionality"))
    }
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "text_editing.show_code_folding_ribbon", comment: "Label for show code folding ribbon setting"), isOn: $textEditing.showFoldingRibbon)
            ...
            // swiftlint:disable:next line_length
            .help(String(localized: "text_editing.code_folding_ribbon_description", comment: "Description of code folding ribbon functionality"))
    }

    @ViewBuilder private var reformatSettings: some View {
        Toggle("Show Reformatting Guide", isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "text_editing.vertical_guide_description", comment: "Description of vertical guide functionality"))
        ...
        .help(String(localized: "text_editing.reformat_column_description", comment: "Description of reformat column setting"))
    }
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            ...
            Button {
                ...
                Text(String(localized: "text_editing.configure", comment: "Button label for configure action"))
            }
