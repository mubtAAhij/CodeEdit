...
private extension TextEditingSettingsView {
    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "settings.text-editing.font", defaultValue: "Font", comment: "Text editing font setting label"), selectedFontName: $textEditing.font.name)
    }

    @ViewBuilder private var fontSizeSelector: some View {
        Stepper(
            String(localized: "settings.text-editing.font-size", defaultValue: "Font Size", comment: "Text editing font size setting label"),
    ...
    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "settings.text-editing.type-over-completion", defaultValue: "Enable type-over completion", comment: "Type-over completion toggle label"), isOn: $textEditing.enableTypeOverCompletion)
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                String(localized: "settings.text-editing.overscroll", defaultValue: "Editor Overscroll", comment: "Editor overscroll setting label"),
    ...
    @ViewBuilder private var lineHeight: some View {
        Stepper(
            String(localized: "settings.text-editing.line-height", defaultValue: "Line Height", comment: "Line height setting label"),
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "settings.text-editing.indent-using", defaultValue: "Prefer Indent Using", comment: "Indent type setting label"), selection: $textEditing.indentOption.indentType) {
                Text(String(localized: "settings.text-editing.indent-using.tabs", defaultValue: "Tabs", comment: "Tabs indent option"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
                Text(String(localized: "settings.text-editing.indent-using.spaces", defaultValue: "Spaces", comment: "Spaces indent option"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.spaces)
            }
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    Stepper(
                        String(localized: "settings.text-editing.indent-width", defaultValue: "Indent Width", comment: "Indent width setting label"),
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            Stepper(
                String(localized: "settings.text-editing.tab-width", defaultValue: "Tab Width", comment: "Tab width setting label"),
                value: Binding<Double>(
    ...
    @ViewBuilder private var letterSpacing: some View {
        Stepper(
            String(localized: "settings.text-editing.letter-spacing", defaultValue: "Letter Spacing", comment: "Letter spacing setting label"),
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            Picker(
                String(localized: "settings.text-editing.bracket-highlight", defaultValue: "Bracket Pair Highlight", comment: "Bracket pair highlight setting label"),
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "settings.text-editing.bracket-highlight.use-custom-color", defaultValue: "Use Custom Color", comment: "Use custom color toggle"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
                    String(localized: "settings.text-editing.bracket-highlight.color", defaultValue: "Bracket Pair Highlight Color", comment: "Bracket pair highlight color setting label"),
    ...
    @ViewBuilder private var reformatSettings: some View {
        Toggle(String(localized: "settings.text-editing.show-reformat-guide", defaultValue: "Show Reformatting Guide", comment: "Show reformatting guide toggle label"), isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "settings.text-editing.show-reformat-guide.help", defaultValue: "Shows a vertical guide at the reformat column.", comment: "Show reformatting guide help text"))

        Stepper(
            String(localized: "settings.text-editing.reformat-column", defaultValue: "Reformat at Column", comment: "Reformat column setting label"),
            value: Binding<Double>(
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-invisibles", defaultValue: "Show Invisible Characters", comment: "Show invisible characters setting label"))
            Spacer()
            Toggle(isOn: $textEditing.invisibleCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingInvisibleCharacterSettings = true
            } label: {
                Text(String(localized: "settings.text-editing.configure", defaultValue: "Configure...", comment: "Configure button label"))
            }
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-warning-chars", defaultValue: "Show Warning Characters", comment: "Show warning characters setting label"))
            Spacer()
