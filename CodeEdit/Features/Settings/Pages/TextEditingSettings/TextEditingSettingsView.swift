...
private extension TextEditingSettingsView {
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            Text(String(localized: "settings.text-editing.autocomplete-braces", defaultValue: "Autocomplete braces", comment: "Toggle to enable brace autocompletion"))
            Text(String(localized: "settings.text-editing.autocomplete-braces-description", defaultValue: "Automatically insert closing braces (\"}\")", comment: "Description for brace autocompletion"))
        }
    }

    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "settings.text-editing.enable-typeover-completion", defaultValue: "Enable type-over completion", comment: "Toggle to enable type-over completion"), isOn: $textEditing.enableTypeOverCompletion)
    }
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "settings.text-editing.prefer-indent-using", defaultValue: "Prefer Indent Using", comment: "Picker for indent type preference"), selection: $textEditing.indentOption.indentType) {
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            ...
            Text(String(localized: "settings.text-editing.spaces-unit-tab-width", defaultValue: "spaces", comment: "Unit label for tab width"))
                .foregroundColor(.secondary)
        }
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            Picker(
                String(localized: "settings.text-editing.bracket-pair-highlight", defaultValue: "Bracket Pair Highlight", comment: "Picker for bracket pair highlight mode"),
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "settings.text-editing.show-code-folding-ribbon", defaultValue: "Show Code Folding Ribbon", comment: "Toggle to show code folding ribbon"), isOn: $textEditing.showFoldingRibbon)
            ...
            // swiftlint:disable:next line_length
            .help(String(localized: "settings.text-editing.show-code-folding-ribbon-help", defaultValue: "The code folding ribbon lets you fold regions of code. When the gutter is disabled, the folding ribbon is disabled.", comment: "Help text for code folding ribbon setting"))
    }
    ...
    @ViewBuilder private var reformatSettings: some View {
        Toggle(String(localized: "settings.text-editing.show-reformatting-guide", defaultValue: "Show Reformatting Guide", comment: "Toggle to show reformatting guide"), isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "settings.text-editing.show-reformatting-guide-help", defaultValue: "Shows a vertical guide at the reformat column.", comment: "Help text for reformatting guide setting"))

        Stepper(
            String(localized: "settings.text-editing.reformat-at-column", defaultValue: "Reformat at Column", comment: "Stepper for reformat column position"),
            value: Binding<Double>(
        ...
        .help(String(localized: "settings.text-editing.reformat-at-column-help", defaultValue: "The column at which text should be reformatted.", comment: "Help text for reformat column setting"))
    }

    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-invisible-characters", defaultValue: "Show Invisible Characters", comment: "Toggle to show invisible characters"))
            Spacer()
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "settings.text-editing.show-warning-characters", defaultValue: "Show Warning Characters", comment: "Toggle to show warning characters"))
            Spacer()
            Toggle(isOn: $textEditing.warningCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingWarningCharactersSettings = true
            } label: {
                Text(String(localized: "settings.text-editing.configure-warning-characters", defaultValue: "Configure...", comment: "Button to configure warning characters"))
            }
