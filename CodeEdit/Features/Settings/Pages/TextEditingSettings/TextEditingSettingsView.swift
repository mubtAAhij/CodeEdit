...
private extension TextEditingSettingsView {
    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "text-editing.font", defaultValue: "Font", comment: "Label for font picker"), selectedFontName: $textEditing.font.name)
    }

    @ViewBuilder private var fontSizeSelector: some View {
        Stepper(
            String(localized: "text-editing.font-size", defaultValue: "Font Size", comment: "Label for font size stepper"),
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            Text(String(localized: "text-editing.autocomplete-braces", defaultValue: "Autocomplete braces", comment: "Toggle label for autocomplete braces"))
    ...
    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "text-editing.type-over-completion", defaultValue: "Enable type-over completion", comment: "Toggle label for type-over completion"), isOn: $textEditing.enableTypeOverCompletion)
    }

    @ViewBuilder private var wrapLinesToEditorWidth: some View {
        Toggle(String(localized: "text-editing.wrap-lines", defaultValue: "Wrap lines to editor width", comment: "Toggle label for wrapping lines to editor width"), isOn: $textEditing.wrapLinesToEditorWidth)
    }

    @ViewBuilder private var useSystemCursor: some View {
        if #available(macOS 14, *) {
            Toggle(String(localized: "text-editing.use-system-cursor", defaultValue: "Use System Cursor", comment: "Toggle label for using system cursor"), isOn: $textEditing.useSystemCursor)
        } else {
            EmptyView()
        }
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                String(localized: "text-editing.overscroll", defaultValue: "Editor Overscroll", comment: "Label for editor overscroll picker"),
    ...
    @ViewBuilder private var lineHeight: some View {
        Stepper(
            String(localized: "text-editing.line-height", defaultValue: "Line Height", comment: "Label for line height stepper"),
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "text-editing.indent-using", defaultValue: "Prefer Indent Using", comment: "Label for indent type picker"), selection: $textEditing.indentOption.indentType) {
                Text(String(localized: "text-editing.indent-using.tabs", defaultValue: "Tabs", comment: "Tabs indent option"))
                    .tag(SettingsData.TextEditingSettings.IndentOption.IndentType.tab)
                Text(String(localized: "text-editing.indent-using.spaces", defaultValue: "Spaces", comment: "Spaces indent option"))
            ...
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    ...
                    Text(String(localized: "text-editing.indent-width.spaces-label", defaultValue: "spaces", comment: "Unit label for indent width in spaces"))
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            Stepper(
                String(localized: "text-editing.tab-width", defaultValue: "Tab Width", comment: "Label for tab width stepper"),
                value: Binding<Double>(
            ...
            Text(String(localized: "text-editing.tab-width.spaces-label", defaultValue: "spaces", comment: "Unit label for tab width in spaces"))
                .foregroundColor(.secondary)
        }
    ...
    @ViewBuilder private var letterSpacing: some View {
        Stepper(
            String(localized: "text-editing.letter-spacing", defaultValue: "Letter Spacing", comment: "Label for letter spacing stepper"),
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            Picker(
                String(localized: "text-editing.bracket-highlight", defaultValue: "Bracket Pair Highlight", comment: "Label for bracket pair highlight picker"),
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "text-editing.bracket-highlight.use-custom-color", defaultValue: "Use Custom Color", comment: "Toggle label for using custom bracket highlight color"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
                    String(localized: "text-editing.bracket-highlight.color", defaultValue: "Bracket Pair Highlight Color", comment: "Label for bracket pair highlight color picker"),
    ...
    @ViewBuilder private var showGutter: some View {
        Toggle(String(localized: "text-editing.show-gutter", defaultValue: "Show Gutter", comment: "Toggle label for showing gutter"), isOn: $textEditing.showGutter)
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "text-editing.show-minimap", defaultValue: "Show Minimap", comment: "Toggle label for showing minimap"), isOn: $textEditing.showMinimap)
    ...
    @ViewBuilder private var showFoldingRibbon: some View {
        Toggle(String(localized: "text-editing.show-folding-ribbon", defaultValue: "Show Code Folding Ribbon", comment: "Toggle label for showing code folding ribbon"), isOn: $textEditing.showFoldingRibbon)
            .disabled(!textEditing.showGutter) // Disabled when the gutter is disabled
            // swiftlint:disable:next line_length
            .help(String(localized: "text-editing.show-folding-ribbon.help", defaultValue: "The code folding ribbon lets you fold regions of code. When the gutter is disabled, the folding ribbon is disabled.", comment: "Help text for show code folding ribbon toggle"))
    }

    @ViewBuilder private var reformatSettings: some View {
        Toggle(String(localized: "text-editing.show-reformatting-guide", defaultValue: "Show Reformatting Guide", comment: "Toggle label for showing reformatting guide"), isOn: $textEditing.showReformattingGuide)
        ...
        Stepper(
            String(localized: "text-editing.reformat-at-column", defaultValue: "Reformat at Column", comment: "Label for reformat at column stepper"),
            value: Binding<Double>(
        ...
        .help(String(localized: "text-editing.reformat-at-column.help", defaultValue: "The column at which text should be reformatted.", comment: "Help text for reformat at column setting"))
    }
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            ...
            Button {
                ...
                Text(String(localized: "text-editing.configure-button", defaultValue: "Configure...", comment: "Button label to configure settings"))
            }
