...
private extension TextEditingSettingsView {
    @ViewBuilder private var fontSelector: some View {
        MonospacedFontPicker(title: String(localized: "text-editing.font", defaultValue: "Font", comment: "Text editing font picker label"), selectedFontName: $textEditing.font.name)
    }

    @ViewBuilder private var fontSizeSelector: some View {
        Stepper(
            String(localized: "text-editing.font-size", defaultValue: "Font Size", comment: "Text editing font size stepper label"),
    ...
    @ViewBuilder private var autocompleteBraces: some View {
        Toggle(isOn: $textEditing.autocompleteBraces) {
            Text(String(localized: "text-editing.autocomplete-braces", defaultValue: "Autocomplete braces", comment: "Toggle to autocomplete braces"))
    ...
    @ViewBuilder private var enableTypeOverCompletion: some View {
        Toggle(String(localized: "text-editing.type-over-completion", defaultValue: "Enable type-over completion", comment: "Toggle to enable type-over completion"), isOn: $textEditing.enableTypeOverCompletion)
    }

    @ViewBuilder private var wrapLinesToEditorWidth: some View {
        Toggle(String(localized: "text-editing.wrap-lines", defaultValue: "Wrap lines to editor width", comment: "Toggle to wrap lines to editor width"), isOn: $textEditing.wrapLinesToEditorWidth)
    }
    ...
    @ViewBuilder private var overscroll: some View {
        Group {
            Picker(
                String(localized: "text-editing.overscroll", defaultValue: "Editor Overscroll", comment: "Editor overscroll picker label"),
    ...
    @ViewBuilder private var indentOption: some View {
        Group {
            Picker(String(localized: "text-editing.indent-using", defaultValue: "Prefer Indent Using", comment: "Indent using picker label"), selection: $textEditing.indentOption.indentType) {
            ...
            if textEditing.indentOption.indentType == .spaces {
                HStack {
                    ...
                    Text(String(localized: "text-editing.spaces", defaultValue: "spaces", comment: "Spaces unit label"))
                        .foregroundColor(.secondary)
                }
                .help(String(localized: "text-editing.indent-width.help", defaultValue: "The number of spaces to insert when the tab key is pressed.", comment: "Help text for indent width"))
            }
        }
    }
    ...
    @ViewBuilder private var defaultTabWidth: some View {
        HStack(alignment: .top) {
            ...
            Text(String(localized: "text-editing.spaces-unit", defaultValue: "spaces", comment: "Spaces unit label for tab width"))
                .foregroundColor(.secondary)
        }
        .help(String(localized: "text-editing.tab-width.help", defaultValue: "The visual width of tabs.", comment: "Help text for tab width"))
    }
    ...
    @ViewBuilder private var bracketPairHighlight: some View {
        Group {
            ...
            if [.bordered, .underline].contains(textEditing.bracketEmphasis.highlightType) {
                Toggle(String(localized: "text-editing.use-custom-color", defaultValue: "Use Custom Color", comment: "Toggle to use custom bracket highlight color"), isOn: $textEditing.bracketEmphasis.useCustomColor)
                SettingsColorPicker(
                    String(localized: "text-editing.bracket-highlight-color", defaultValue: "Bracket Pair Highlight Color", comment: "Color picker for bracket pair highlight"),
    ...
    @ViewBuilder private var showGutter: some View {
        Toggle(String(localized: "text-editing.show-gutter", defaultValue: "Show Gutter", comment: "Toggle to show gutter"), isOn: $textEditing.showGutter)
    ...
    @ViewBuilder private var showMinimap: some View {
        Toggle(String(localized: "text-editing.show-minimap", defaultValue: "Show Minimap", comment: "Toggle to show minimap"), isOn: $textEditing.showMinimap)
    ...
    @ViewBuilder private var reformatSettings: some View {
        Toggle(String(localized: "text-editing.show-reformat-guide", defaultValue: "Show Reformatting Guide", comment: "Toggle to show reformatting guide"), isOn: $textEditing.showReformattingGuide)
            .help(String(localized: "text-editing.show-reformat-guide.help", defaultValue: "Shows a vertical guide at the reformat column.", comment: "Help text for show reformatting guide toggle"))

        Stepper(
            String(localized: "text-editing.reformat-at-column", defaultValue: "Reformat at Column", comment: "Stepper for reformat at column"),
            value: Binding<Double>(
    ...
    @ViewBuilder private var invisibles: some View {
        HStack {
            Text(String(localized: "text-editing.invisible-characters.show", defaultValue: "Show Invisible Characters", comment: "Show invisible characters toggle label"))
            Spacer()
            Toggle(isOn: $textEditing.invisibleCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingInvisibleCharacterSettings = true
            } label: {
                Text(String(localized: "text-editing.invisible-characters.configure", defaultValue: "Configure...", comment: "Configure invisible characters button"))
            }
    ...
    @ViewBuilder private var warningCharacters: some View {
        HStack {
            Text(String(localized: "text-editing.warning-characters.show", defaultValue: "Show Warning Characters", comment: "Show warning characters toggle label"))
            Spacer()
            Toggle(isOn: $textEditing.warningCharacters.enabled, label: { EmptyView() })
            Button {
                isShowingWarningCharactersSettings = true
            } label: {
                Text(String(localized: "text-editing.warning-characters.configure", defaultValue: "Configure...", comment: "Configure warning characters button"))
            }
