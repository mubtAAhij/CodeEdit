...
struct LanguageServerInstallView: View {
    ...
    var body: some View {
        ...
        .alert(
            String(localized: "language_server_install.confirm_step", comment: "Alert title for confirming installation step"),
            ...
            Button(String(localized: "common.continue", comment: "Continue button text")) {
        ...
        HStack {
            ...
            case .none:
                ...
                Button {
                    ...
                    Text(String(localized: "language_server_install.install", comment: "Install button text"))
                }
            ...
            case .complete:
                Button {
                    ...
                    Text(String(localized: "common.continue", comment: "Continue button text"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            LabeledContent(String(localized: "language_server_install.installing_package", comment: "Label for package being installed"), value: operation.package.sanitizedName)
            LabeledContent(String(localized: "language_server.install.homepage", comment: "Label for language server homepage link")) {
                sourceButton.cursor(.pointingHand)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(String(localized: "language_server.install.description", comment: "Label for language server description"))
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "language_server.install.error_occurred", comment: "Error message when installation fails"))
                }
                .font(.title3)
    ...
    @ViewBuilder private var progressSection: some View {
        Section {
            LabeledContent(String(localized: "language_server.install.step", comment: "Label for current installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(.green)
                        Text(String(localized: "language_server.install.successfully_installed", comment: "Success message after installation"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                LabeledContent(String(localized: "language_server.install.install_method", comment: "Label for installation method"), value: method.installerDescription)
                    .textSelection(.enabled)
                ...
                LabeledContent(String(localized: "language_server.install.installer", comment: "Label for installer type"), value: "Unknown")
            }
        }
    }
}
