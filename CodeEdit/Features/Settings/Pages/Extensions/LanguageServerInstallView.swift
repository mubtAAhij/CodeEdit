...
struct LanguageServerInstallView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            Button(String(localized: "settings.extensions.install.continue", defaultValue: "Continue", comment: "Button to continue with installation")) {
        ...
        HStack {
            ...
            case .none:
                ...
                Button {
                    ...
                    Text(String(localized: "settings.extensions.install.button", defaultValue: "Install", comment: "Button to start installation"))
                }
            ...
            case .complete:
                Button {
                    ...
                    Text(String(localized: "settings.extensions.install.continue", defaultValue: "Continue", comment: "Button to continue with installation"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            LabeledContent(String(localized: "settings.extensions.install.package-label", defaultValue: "Installing Package", comment: "Label for package being installed"), value: operation.package.sanitizedName)
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "settings.extensions.install.error-occurred", defaultValue: "Error Occurred", comment: "Error message when installation fails"))
                }
                .font(.title3)
            ...
            LabeledContent(String(localized: "settings.extensions.install.step-label", defaultValue: "Step", comment: "Label for current installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        ...
                        Text(String(localized: "settings.extensions.install.success", defaultValue: "Successfully Installed", comment: "Success message when package is installed"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                LabeledContent(String(localized: "settings.extensions.install.method-label", defaultValue: "Install Method", comment: "Label for installation method"), value: method.installerDescription)
                    .textSelection(.enabled)
