        ...
        HStack {
            ...
            case .none:
                Button {
                    ...
                    Text(String(localized: "language-server-install.cancel-button", defaultValue: "Cancel", comment: "Cancel button in footer"))
                }
            ...
            case .complete:
                Button {
                    ...
                    Text(String(localized: "language-server-install.continue-complete", defaultValue: "Continue", comment: "Continue button after completion"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
        ...
        Section {
            LabeledContent(String(localized: "language-server-install.step", defaultValue: "Step", comment: "Step label")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    ...
                    Text(String(localized: "language-server-install.error-status", defaultValue: "Error Occurred", comment: "Error occurred status"))
                } else {
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                ...
                LabeledContent(String(localized: "language-server-install.installer", defaultValue: "Installer", comment: "Installer label"), value: String(localized: "language-server-install.unknown", defaultValue: "Unknown", comment: "Unknown installer"))
            }
        }
    }
}
