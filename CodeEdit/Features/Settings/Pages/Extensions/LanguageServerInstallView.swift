...
struct LanguageServerInstallView: View {
    ...
    var body: some View {
        ...
        .alert(
            ...
            Button(String(localized: "language-server-install.cancel", defaultValue: "Cancel", comment: "Button to cancel installation")) {
                registryManager.cancelInstallation()
            }
            Button(String(localized: "language-server-install.continue", defaultValue: "Continue", comment: "Button to continue installation")) {
        ...
        HStack {
            ...
            case .none:
                Button {
                    ...
                    Text(String(localized: "language-server-install.cancel", defaultValue: "Cancel", comment: "Button to cancel installation"))
                }
                ...
                Button {
                    ...
                    Text(String(localized: "language-server-install.install", defaultValue: "Install", comment: "Button to start installation"))
                }
            ...
            case .complete:
                Button {
                    ...
                    Text(String(localized: "language-server-install.continue", defaultValue: "Continue", comment: "Button to continue installation"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "language-server-install.error-occurred", defaultValue: "Error Occurred", comment: "Label indicating an error occurred"))
                }
                .font(.title3)
            ...
            LabeledContent(String(localized: "language-server-install.step", defaultValue: "Step", comment: "Label for installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        ...
                        Text(String(localized: "language-server-install.successfully-installed", defaultValue: "Successfully Installed", comment: "Status message for successful installation"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                ...
                LabeledContent(String(localized: "language-server-install.installer", defaultValue: "Installer", comment: "Label for installer type"), value: "Unknown")
            }
        }
    }
}
