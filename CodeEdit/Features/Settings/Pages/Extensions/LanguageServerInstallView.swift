...
struct LanguageServerInstallView: View {
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            ...
            LabeledContent(String(localized: "language_server_install.homepage", comment: "Label for language server homepage")) {
                sourceButton.cursor(.pointingHand)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(String(localized: "language_server_install.description", comment: "Label for language server description"))
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "language_server_install.error_occurred", comment: "Error occurred message"))
                }
                .font(.title3)
    ...
    @ViewBuilder private var progressSection: some View {
        Section {
            LabeledContent(String(localized: "language_server_install.step", comment: "Label for installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(.green)
                        Text(String(localized: "language_server_install.successfully_installed", comment: "Success message for completed installation"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                LabeledContent(String(localized: "language_server_install.install_method", comment: "Label for installation method"), value: method.installerDescription)
                    .textSelection(.enabled)
                ...
                LabeledContent(String(localized: "language_server_install.installer", comment: "Label for installer type"), value: "Unknown")
            }
        }
    }
}
