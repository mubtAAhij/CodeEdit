...
struct LanguageServerInstallView: View {
    ...
    var body: some View {
        ...
        .alert(
            String(localized: "confirm_step", comment: "Alert title for confirming installation step"),
            isPresented: Binding(get: { operation.waitingForConfirmation != nil }, set: { _ in }),
            presenting: operation.waitingForConfirmation
        ) { _ in
            Button(String(localized: "cancel", comment: "Cancel button for installation confirmation")) {
                registryManager.cancelInstallation()
            }
            Button(String(localized: "continue", comment: "Continue button for installation confirmation")) {
        ...
        HStack {
            ...
            case .none:
                Button {
                    ...
                    Text(String(localized: "cancel", comment: "Cancel button for installation"))
                }
                ...
                Button {
                    ...
                    Text(String(localized: "install", comment: "Install button to start package installation"))
                }
            ...
            case .running:
                Button {
                    ...
                    Text(String(localized: "cancel", comment: "Cancel button during installation"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.bordered)
            case .complete:
                Button {
                    ...
                    Text(String(localized: "continue", comment: "Continue button after installation completion"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            LabeledContent(String(localized: "installing_package", comment: "Label for package being installed"), value: operation.package.sanitizedName)
            LabeledContent(String(localized: "homepage", comment: "Label for package homepage link")) {
                sourceButton.cursor(.pointingHand)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(String(localized: "description", comment: "Label for package description section"))
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "error_occurred", comment: "Error status message"))
                }
                .font(.title3)
    ...
    @ViewBuilder private var progressSection: some View {
        Section {
            LabeledContent(String(localized: "step", comment: "Label for installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(.green)
                        Text(String(localized: "successfully_installed", comment: "Installation success message"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                LabeledContent(String(localized: "install_method", comment: "Label for installation method"), value: method.installerDescription)
                    .textSelection(.enabled)
                ...
                LabeledContent(String(localized: "installer", comment: "Label for installer information"), value: "Unknown")
            }
        }
    }
}
