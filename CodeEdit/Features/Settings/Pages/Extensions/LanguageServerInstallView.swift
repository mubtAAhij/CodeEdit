...
struct LanguageServerInstallView: View {
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            LabeledContent(String(localized: "lsp.install.installing-package", defaultValue: "Installing Package", comment: "Label for package name being installed"), value: operation.package.sanitizedName)
            LabeledContent(String(localized: "lsp.install.homepage", defaultValue: "Homepage", comment: "Label for package homepage")) {
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "lsp.install.error-occurred", defaultValue: "Error Occurred", comment: "Error section title"))
                }
                .font(.title3)
    ...
    @ViewBuilder private var progressSection: some View {
        Section {
            LabeledContent(String(localized: "lsp.install.step", defaultValue: "Step", comment: "Label for installation step")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(.green)
                        Text(String(localized: "lsp.install.successfully-installed", defaultValue: "Successfully Installed", comment: "Success message for installation"))
        ...
        Section {
            if let method = operation.package.installMethod {
                ...
                if let packageDescription = method.packageDescription {
                    LabeledContent(String(localized: "lsp.install.package", defaultValue: "Package", comment: "Label for package"), value: packageDescription)
                        .textSelection(.enabled)
                }
            } else {
                LabeledContent(String(localized: "lsp.install.installer", defaultValue: "Installer", comment: "Label for installer"), value: String(localized: "lsp.install.unknown", defaultValue: "Unknown", comment: "Unknown installer"))
            }
        }
    }
}
