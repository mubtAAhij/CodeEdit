...
struct LanguageServerInstallView: View {
    ...
    var body: some View {
        ...
        .alert(
            String(localized: "lsp.confirm-step", defaultValue: "Confirm Step", comment: "Confirm Step alert title"),
            ...
            Button(String(localized: "alert.continue", defaultValue: "Continue", comment: "Continue button")) {
        ...
        HStack {
            ...
            case .none:
                ...
                Button {
                    ...
                    Text(String(localized: "lsp.install", defaultValue: "Install", comment: "Install button"))
                }
            ...
            case .complete:
                Button {
                    ...
                    Text(String(localized: "alert.continue", defaultValue: "Continue", comment: "Continue button"))
                        .frame(minWidth: 56)
                }
                .buttonStyle(.borderedProminent)
            }
    ...
    @ViewBuilder private var packageInfoSection: some View {
        Section {
            LabeledContent(String(localized: "lsp.installing-package", defaultValue: "Installing Package", comment: "Installing Package label"), value: operation.package.sanitizedName)
            LabeledContent(String(localized: "lsp.homepage", defaultValue: "Homepage", comment: "Homepage label")) {
                sourceButton.cursor(.pointingHand)
            }
            VStack(alignment: .leading, spacing: 6) {
                Text(String(localized: "lsp.description", defaultValue: "Description", comment: "Description label"))
        ...
        if let error = operation.error {
            Section {
                HStack(spacing: 4) {
                    ...
                    Text(String(localized: "lsp.error-occurred", defaultValue: "Error Occurred", comment: "Error Occurred text"))
                }
                .font(.title3)
    ...
    @ViewBuilder private var progressSection: some View {
        Section {
            LabeledContent(String(localized: "lsp.step", defaultValue: "Step", comment: "Step label")) {
                if registryManager.installedLanguageServers[operation.package.name] != nil {
                    HStack(spacing: 4) {
                        Image(systemName: "checkmark.circle.fill")
                            .foregroundColor(.green)
                        Text(String(localized: "lsp.successfully-installed", defaultValue: "Successfully Installed", comment: "Successfully Installed text"))
    ...
    @ViewBuilder private var notInstalledSection: some View {
        Section {
            if let method = operation.package.installMethod {
                LabeledContent(String(localized: "lsp.install-method", defaultValue: "Install Method", comment: "Install Method label"), value: method.installerDescription)
                    .textSelection(.enabled)
                if let packageDescription = method.packageDescription {
                    LabeledContent(String(localized: "lsp.package", defaultValue: "Package", comment: "Package label"), value: packageDescription)
                        .textSelection(.enabled)
                }
            } else {
                LabeledContent(String(localized: "lsp.installer", defaultValue: "Installer", comment: "Installer label"), value: String(localized: "lsp.unknown", defaultValue: "Unknown", comment: "Unknown value"))
            }
        }
    }
}
