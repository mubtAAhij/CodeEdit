...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text("extensions.install_using \(installerName)", comment: "Installation method")
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? "actions.show_less" : "actions.show_more", comment: "Toggle details button")
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert("extensions.remove_confirmation \(package.sanitizedName)", isPresented: $showingRemovalConfirmation) {
            Button("actions.cancel", comment: "Cancel button")
            Button("actions.remove", comment: "Remove button", role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text("extensions.remove_warning", comment: "Removal warning message")
        }
        .alert("extensions.removal_failed", isPresented: $showingRemovalError) {
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text("actions.remove", comment: "Remove button")
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text("actions.retry", comment: "Retry button")
                .foregroundColor(.red)
        }
    }
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text("actions.install", comment: "Install button")
        }
