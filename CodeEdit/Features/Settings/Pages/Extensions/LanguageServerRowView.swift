...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "extensions.install-using", defaultValue: "Install using %@", comment: "Label showing package manager used for installation"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "extensions.show-less", defaultValue: "Show Less", comment: "Button to collapse description") : String(localized: "extensions.show-more", defaultValue: "Show More", comment: "Button to expand description"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(format: String(localized: "extensions.remove-package", defaultValue: "Remove %@?", comment: "Alert title for removing a package"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "extensions.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) { }
            Button(String(localized: "extensions.remove", defaultValue: "Remove", comment: "Remove button"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "extensions.remove-confirmation", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Confirmation message for removing language server"))
        }
        .alert(String(localized: "extensions.removal-failed", defaultValue: "Removal Failed", comment: "Alert title for failed removal"), isPresented: $showingRemovalError) {
            Button(String(localized: "extensions.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "extensions.unknown-error", defaultValue: "An unknown error occurred", comment: "Generic error message"))
        }
    }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "extensions.remove-button", defaultValue: "Remove", comment: "Button to remove installed language server"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "extensions.retry", defaultValue: "Retry", comment: "Button to retry failed installation"))
                .foregroundColor(.red)
        }
    }
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text(String(localized: "extensions.install", defaultValue: "Install", comment: "Button to install language server"))
        }
