...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "settings.extensions.install-using", defaultValue: "Install using %@", comment: "Install using package manager message"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "settings.extensions.show-less", defaultValue: "Show Less", comment: "Show less details button") : String(localized: "settings.extensions.show-more", defaultValue: "Show More", comment: "Show more details button"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(format: String(localized: "settings.extensions.remove-confirm-title", defaultValue: "Remove %@?", comment: "Confirmation title for removing language server"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "settings.accounts.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) { }
            ...
            Text(String(localized: "settings.extensions.remove-confirm-message", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Confirmation message for removing language server"))
        }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "settings.extensions.remove", defaultValue: "Remove", comment: "Remove button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "settings.extensions.retry", defaultValue: "Retry", comment: "Retry button"))
                .foregroundColor(.red)
        }
    }
