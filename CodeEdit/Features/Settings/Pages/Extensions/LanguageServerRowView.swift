...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(
                                        ...
                                        comment: "Button title to install language server using selected installer"
                                    ), "\(installerName)"))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            ...
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(
                                        ...
                                        comment: "Button title to collapse language server details"
                                    ) : String(
                                        localized: "settings.extensions.language-server-row.show-more",
                                        defaultValue: "Show More",
                                        comment: "Button title to expand language server details"
                                    ))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
        ...
        .alert(String(format: String(
            ...
            defaultValue: "Remove %@?",
            comment: "Alert title asking to remove a specific language server package"
            ...
            Button(String(
                ...
                defaultValue: "Cancel",
                comment: "Cancel button in remove language server confirmation"
            ), role: .cancel) { }
            Button(String(
                localized: "settings.extensions.language-server-row.remove-confirmation.remove",
                defaultValue: "Remove",
                comment: "Destructive button in remove language server confirmation"
            ), role: .destructive) {
            ...
            Text(String(
                ...
                comment: "Warning message shown before removing language server"
            ))
        }
        .alert(String(
            localized: "settings.extensions.language-server-row.removal-failed.title",
            defaultValue: "Removal Failed",
            comment: "Alert title shown when removing language server fails"
        ), isPresented: $showingRemovalError) {
            Button(String(
                localized: "settings.extensions.language-server-row.removal-failed.ok",
                defaultValue: "OK",
                comment: "Acknowledgement button in removal failed alert"
            ), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(
                localized: "settings.extensions.language-server-row.removal-failed.unknown-error",
                defaultValue: "An unknown error occurred",
                comment: "Fallback error message when removal fails without a specific reason"
            ))
        }
    }
            ...
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(
                        ...
                        defaultValue: "Remove",
                        comment: "Button title to remove a language server package"
                    ))
                }
            }
            Toggle(
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(
                localized: "settings.extensions.language-server-row.retry",
                defaultValue: "Retry",
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text(String(
                localized: "settings.extensions.language-server-row.install",
                defaultValue: "Install",
                comment: "Button title to install a language server package"
            ))
        }
