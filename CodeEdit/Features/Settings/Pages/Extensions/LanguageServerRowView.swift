...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "language-server.install-using", defaultValue: "Install using %@", comment: "Label showing which package manager will be used"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
        ...
        .alert(String(format: String(localized: "language-server.remove-confirmation-title", defaultValue: "Remove %@?", comment: "Title for language server removal confirmation dialog"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            ...
            Text(String(localized: "language-server.remove-confirmation-message", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Message for language server removal confirmation"))
        }
        .alert(String(localized: "language-server.removal-failed", defaultValue: "Removal Failed", comment: "Title for language server removal failure alert"), isPresented: $showingRemovalError) {
            Button(String(localized: "common.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "common.error-unknown", defaultValue: "An unknown error occurred", comment: "Message for unknown error"))
        }
    }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "language-server.remove", defaultValue: "Remove", comment: "Remove button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "language-server.retry", defaultValue: "Retry", comment: "Button to retry failed installation"))
                .foregroundColor(.red)
        }
    }
