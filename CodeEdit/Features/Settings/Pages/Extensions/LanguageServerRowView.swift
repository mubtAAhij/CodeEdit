...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(
                                        ...
                                        comment: "Install action text indicating which installer will be used"
                                    ), "\(installerName)"))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
        ...
        .alert(String(format: String(
            localized: "settings.extensions.language-server-row.remove-confirm-title",
            defaultValue: "Remove %@?",
            comment: "Confirmation title asking to remove selected language server package"
        ), "\(package.sanitizedName)"), isPresented: $showingRemovalConfirmation) {
            Button(String(
                localized: "settings.extensions.language-server-row.cancel",
                defaultValue: "Cancel",
                comment: "Cancel action in remove language server confirmation dialog"
            ), role: .cancel) { }
            Button(String(
                localized: "settings.extensions.language-server-row.remove",
                defaultValue: "Remove",
                comment: "Destructive action to remove language server package"
            ), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(
                localized: "settings.extensions.language-server-row.remove-confirm-message",
                defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.",
                comment: "Warning message in remove language server confirmation dialog"
            ))
        }
        .alert(String(
            ...
            Button(String(
                localized: "settings.extensions.language-server-row.ok",
                defaultValue: "OK",
                comment: "Confirmation button in language server error alert"
            ), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(
                ...
                comment: "Fallback error message when language server operation fails with unknown error"
            ))
        }
    }
            ...
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(
                        localized: "settings.extensions.language-server-row.remove",
                        defaultValue: "Remove",
                        comment: "Context menu action to remove language server package"
                    ))
                }
            }
            Toggle(
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(
                ...
                defaultValue: "Retry",
                comment: "Action button to retry failed language server operation"
            ))
                .foregroundColor(.red)
        }
    }
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text(String(
                ...
                comment: "Action button to install language server"
            ))
        }
