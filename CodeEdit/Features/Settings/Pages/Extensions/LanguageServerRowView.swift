...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "lsp.install-using", defaultValue: "Install using %@", comment: "Install using package manager"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "lsp.show-less", defaultValue: "Show Less", comment: "Show less button") : String(localized: "lsp.show-more", defaultValue: "Show More", comment: "Show more button"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(format: String(localized: "lsp.remove-confirmation", defaultValue: "Remove %@?", comment: "Remove language server confirmation"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "lsp.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) { }
            Button(String(localized: "lsp.remove", defaultValue: "Remove", comment: "Remove button"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "lsp.remove-warning", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Remove language server warning"))
        }
        .alert(String(localized: "lsp.removal-failed", defaultValue: "Removal Failed", comment: "Removal failed alert title"), isPresented: $showingRemovalError) {
            Button(String(localized: "lsp.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "lsp.unknown-error", defaultValue: "An unknown error occurred", comment: "Unknown error message"))
        }
    }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "lsp.remove", defaultValue: "Remove", comment: "Remove button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "lsp.retry", defaultValue: "Retry", comment: "Retry button"))
                .foregroundColor(.red)
        }
    }
