...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "settings.extensions.install-using", defaultValue: "Install using %@", comment: "Installation method label"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
        ...
        .alert(String(format: String(localized: "settings.extensions.remove-confirmation-title", defaultValue: "Remove %@?", comment: "Language server removal confirmation title"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "settings.extensions.cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) { }
            Button(String(localized: "settings.extensions.remove", defaultValue: "Remove", comment: "Remove button"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "settings.extensions.remove-confirmation-message", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Language server removal confirmation message"))
        }
        .alert(String(localized: "settings.extensions.removal-failed", defaultValue: "Removal Failed", comment: "Language server removal failed alert title"), isPresented: $showingRemovalError) {
            Button(String(localized: "settings.extensions.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "settings.extensions.unknown-error", defaultValue: "An unknown error occurred", comment: "Unknown error message"))
        }
    }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "settings.extensions.remove-button", defaultValue: "Remove", comment: "Remove language server button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "settings.extensions.retry", defaultValue: "Retry", comment: "Retry installation button"))
                .foregroundColor(.red)
        }
    }
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text(String(localized: "settings.extensions.install", defaultValue: "Install", comment: "Install language server button"))
        }
