...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(format: String(localized: "language-server.install-using", defaultValue: "Install using %@", comment: "Install using package manager"), installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "language-server.show-less", defaultValue: "Show Less", comment: "Show less info") : String(localized: "language-server.show-more", defaultValue: "Show More", comment: "Show more info"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(format: String(localized: "language-server.remove-confirm-title", defaultValue: "Remove %@?", comment: "Remove language server confirmation title"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "language-server.cancel", defaultValue: "Cancel", comment: "Cancel removal"), role: .cancel) { }
            Button(String(localized: "language-server.remove-action", defaultValue: "Remove", comment: "Remove language server action"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "language-server.remove-warning", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Remove language server warning"))
        }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "language-server.remove-button", defaultValue: "Remove", comment: "Remove button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "language-server.retry", defaultValue: "Retry", comment: "Retry installation"))
                .foregroundColor(.red)
        }
    }
    ...
    private func isHoveringRow() -> some View {
        Button {
            ...
            Text(String(localized: "language-server.install", defaultValue: "Install", comment: "Install language server"))
        }
