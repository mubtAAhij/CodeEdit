...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(localized: "install_using", comment: "Installation method description"))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "show_less", comment: "Button to collapse expanded content") : String(localized: "show_more", comment: "Button to expand additional content"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(localized: "remove_package_confirmation", comment: "Confirmation dialog title for package removal"), isPresented: $showingRemovalConfirmation) {
        ...
        .alert(String(localized: "removal_failed", comment: "Error dialog title for failed removal"), isPresented: $showingRemovalError) {
            ...
            Text(removalError?.localizedDescription ?? String(localized: "unknown_error", comment: "Generic error message"))
        }
    }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "remove", comment: "Remove button text"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "retry", comment: "Retry button for failed operations"))
                .foregroundColor(.red)
        }
    }
