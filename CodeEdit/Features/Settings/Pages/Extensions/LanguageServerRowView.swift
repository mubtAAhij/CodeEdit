...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "language-server.show-less", defaultValue: "Show Less", comment: "Show Less button") : String(localized: "language-server.show-more", defaultValue: "Show More", comment: "Show More button"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(format: String(localized: "language-server.remove-confirmation-title", defaultValue: "Remove %@?", comment: "Remove language server confirmation title"), package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "cancel", defaultValue: "Cancel", comment: "Cancel button"), role: .cancel) { }
            Button(String(localized: "remove", defaultValue: "Remove", comment: "Remove button"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "language-server.remove-confirmation-message", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Remove language server confirmation message"))
        }
        ...
        HStack {
            if isRemoving {
                ...
                Button {
                    ...
                    Text(String(localized: "remove", defaultValue: "Remove", comment: "Remove button"))
                }
            }
            Toggle(
                "",
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "retry", defaultValue: "Retry", comment: "Retry button"))
                .foregroundColor(.red)
        }
    }
