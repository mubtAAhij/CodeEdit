...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(localized: "settings.extensions.install-using", defaultValue: "Install using \(installerName)", comment: "Shows the package manager used to install the language server"))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "settings.extensions.show-less", defaultValue: "Show Less", comment: "Button to collapse extended language server details") : String(localized: "settings.extensions.show-more", defaultValue: "Show More", comment: "Button to expand language server details"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(localized: "settings.extensions.remove-confirmation.title", defaultValue: "Remove \(package.sanitizedName)?", comment: "Alert title confirming language server removal"), isPresented: $showingRemovalConfirmation) {
            ...
            Text(String(localized: "settings.extensions.remove-confirmation.message", defaultValue: "Are you sure you want to remove this language server? This action cannot be undone.", comment: "Alert message warning about language server removal"))
        }
        .alert(String(localized: "settings.extensions.removal-failed.title", defaultValue: "Removal Failed", comment: "Alert title when language server removal fails"), isPresented: $showingRemovalError) {
            Button(String(localized: "common.ok", defaultValue: "OK", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "settings.extensions.removal-failed.unknown-error", defaultValue: "An unknown error occurred", comment: "Error message when removal fails with unknown error"))
        }
    }
    ...
    private func failedRow() -> some View {
        Button {
            ...
            Text(String(localized: "settings.extensions.retry", defaultValue: "Retry", comment: "Button to retry installing a language server"))
                .foregroundColor(.red)
        }
    }
