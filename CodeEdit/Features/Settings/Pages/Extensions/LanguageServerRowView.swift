...
struct LanguageServerRowView: View, Equatable {
    ...
    var body: some View {
                        ...
                        VStack(alignment: .leading) {
                            ...
                            if showMore {
                                ...
                                if let installerName = package.installMethod?.packageManagerType?.rawValue {
                                    Text(String(localized: "install_using", comment: "Install method description").replacingOccurrences(of: "%@", with: installerName))
                                        .font(.footnote)
                                        .foregroundColor(.secondary)
                                }
                            }
                        ...
                        if isHovering {
                            HStack {
                                ...
                                Button {
                                    ...
                                    Text(showMore ? String(localized: "show_less", comment: "Button to show less details") : String(localized: "show_more", comment: "Button to show more details"))
                                        .font(.footnote)
                                }
                                .buttonStyle(.plain)
                                .background(
        ...
        .alert(String(localized: "remove_package_confirmation", comment: "Confirmation dialog title for removing package").replacingOccurrences(of: "%@", with: package.sanitizedName), isPresented: $showingRemovalConfirmation) {
            Button(String(localized: "cancel", comment: "Cancel button"), role: .cancel) { }
            Button(String(localized: "remove", comment: "Remove button"), role: .destructive) {
                removeLanguageServer()
            }
        } message: {
            Text(String(localized: "remove_language_server_warning", comment: "Warning message for removing language server"))
        }
        .alert(String(localized: "removal_failed", comment: "Error dialog title for failed removal"), isPresented: $showingRemovalError) {
            Button(String(localized: "ok", comment: "OK button"), role: .cancel) { }
        } message: {
            Text(removalError?.localizedDescription ?? String(localized: "unknown_error", comment: "Unknown error message"))
        }
    }
