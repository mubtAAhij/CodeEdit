...
struct LanguageServersView: View {
        ...
        Group {
            SettingsForm {
                ...
                Section {
                    ...
                    Label(
                        String(localized: "Warning: Language server installation is experimental. Use at your own risk.", comment: "Warning message about experimental language server installation"),
    ...
    private func getInfoString() -> AttributedString {
        let string = String(localized: "CodeEdit makes use of the Mason Registry for language server installation. To install a package, CodeEdit uses the package manager directed by the Mason Registry, and installs a copy of the language server in Application Support.\n\nLanguage server installation is still experimental, there may be bugs and expect this flow to change over time.", comment: "Information about language server installation using Mason Registry")
