...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                ...
                                TextField("", text: $server, prompt: Text(String(localized: "settings.accounts.signin.server-placeholder", defaultValue: "https://git.example.com", comment: "Server field placeholder")))
                                    .labelsHidden()
                            }
                        }
                        VStack(alignment: .leading, spacing: 5) {
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(format: String(localized: "settings.accounts.signin.sign-in-to-provider", defaultValue: "Sign in to %@", comment: "Sign in to provider title"), provider.name))
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(format: String(localized: "settings.accounts.signin.token-scopes-required", defaultValue: "%@ personal access tokens must have these scopes set:", comment: "Token scopes required message"), provider.name))
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        HStack(spacing: 2.5) {
                                            ...
                                            Text(String(localized: "settings.accounts.signin.scope.admin-public-key", defaultValue: "admin:public _key", comment: "GitHub admin:public_key scope label"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "settings.accounts.signin.scope.write-discussion", defaultValue: "write:discussion", comment: "GitHub write:discussion scope label"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "settings.accounts.signin.scope.repo", defaultValue: "repo", comment: "GitHub repo scope label"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "settings.accounts.signin.scope.user", defaultValue: "user", comment: "GitHub user scope label"))
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(format: String(localized: "settings.accounts.signin.create-password-on-provider", defaultValue: "Create a Password on %@", comment: "Create password on provider link"), provider.name))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(format: String(localized: "settings.accounts.signin.create-token-on-provider", defaultValue: "Create a Token on %@", comment: "Create token on provider link"), provider.name))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                Button {
                    ...
                    Text(String(localized: "settings.accounts.signin.cancel", defaultValue: "Cancel", comment: "Cancel button label"))
                        .frame(maxWidth: .infinity)
                }
                .controlSize(.large)
                .frame(maxWidth: .infinity)
                ...
                Button {
                    ...
                    Text(String(localized: "settings.accounts.signin.sign-in", defaultValue: "Sign In", comment: "Sign in button label"))
                        .frame(maxWidth: .infinity)
                }
                ...
                .controlSize(.large)
                .alert(
                    Text(String(format: String(localized: "settings.accounts.signin.unable-to-add-account", defaultValue: "Unable to add account \"%@\"", comment: "Unable to add account alert title"), username)),
                    isPresented: $signinErrorAlertIsPresented
                ) {
                    Button(String(localized: "settings.accounts.signin.ok", defaultValue: "OK", comment: "OK button label")) {
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "settings.accounts.signin.duplicate-account-error", defaultValue: "Account with the same username and provider already exists!", comment: "Duplicate account error message")
            signinErrorAlertIsPresented.toggle()
        } else {
