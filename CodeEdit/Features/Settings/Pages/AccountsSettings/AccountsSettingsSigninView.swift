...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
                    ...
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "accounts.signin.server", defaultValue: "Server", comment: "Label for server URL field"))
                                    .font(.caption3)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        HStack(spacing: 2.5) {
                                            ...
                                            Text(String(localized: "accounts.signin.scope.admin-public-key", defaultValue: "admin:public _key", comment: "GitHub scope for admin public key"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "accounts.signin.scope.write-discussion", defaultValue: "write:discussion", comment: "GitHub scope for write discussion"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "accounts.signin.scope.repo", defaultValue: "repo", comment: "GitHub scope for repository access"))
                                                .font(.system(size: 10.5))
                                        }
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "accounts.signin.scope.user", defaultValue: "user", comment: "GitHub scope for user access"))
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(format: String(localized: "accounts.signin.create-password", defaultValue: "Create a Password on %@", comment: "Button to create password on provider"), provider.name))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(format: String(localized: "accounts.signin.create-token", defaultValue: "Create a Token on %@", comment: "Button to create token on provider"), provider.name))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                Button {
                    ...
                    Text(String(localized: "accounts.signin.cancel", defaultValue: "Cancel", comment: "Cancel button in sign in view"))
                        .frame(maxWidth: .infinity)
                }
                .controlSize(.large)
                .frame(maxWidth: .infinity)
                ...
                .alert(
                    ...
                    Button(String(localized: "accounts.signin.ok", defaultValue: "OK", comment: "OK button in error alert")) {
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "accounts.signin.duplicate-account", defaultValue: "Account with the same username and provider already exists!", comment: "Error message when duplicate account exists")
            signinErrorAlertIsPresented.toggle()
        } else {
