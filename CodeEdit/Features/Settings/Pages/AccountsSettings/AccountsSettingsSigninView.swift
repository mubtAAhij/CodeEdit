...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "settings.accounts.signin.server", defaultValue: "Server", comment: "Server label"))
                                    .font(.caption3)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.accounts.signin.username", defaultValue: "Username", comment: "Username label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.accounts.signin.token", defaultValue: "Personal Access Token", comment: "Personal access token label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(format: String(localized: "settings.accounts.signin.title", defaultValue: "Sign in to %@", comment: "Sign in title"), provider.name))
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(format: String(localized: "settings.accounts.signin.scopes-required", defaultValue: "%@ personal access tokens must have these scopes set:", comment: "Required scopes message"), provider.name))
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(format: String(localized: "settings.accounts.signin.create-password", defaultValue: "Create a Password on %@", comment: "Create password link"), provider.name))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(format: String(localized: "settings.accounts.signin.create-token", defaultValue: "Create a Token on %@", comment: "Create token link"), provider.name))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                Button {
                    ...
                    Text(String(localized: "settings.accounts.signin.cancel", defaultValue: "Cancel", comment: "Cancel button"))
                        .frame(maxWidth: .infinity)
                }
                .controlSize(.large)
                .frame(maxWidth: .infinity)
                ...
                Button {
                    ...
                    Text(String(localized: "settings.accounts.signin.signin-button", defaultValue: "Sign In", comment: "Sign in button"))
                        .frame(maxWidth: .infinity)
                }
                ...
                .controlSize(.large)
                .alert(
                    Text(String(format: String(localized: "settings.accounts.signin.unable-to-add", defaultValue: "Unable to add account \"%@\"", comment: "Unable to add account error title"), username)),
                    isPresented: $signinErrorAlertIsPresented
                ) {
                    Button(String(localized: "settings.accounts.signin.ok", defaultValue: "OK", comment: "OK button")) {
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "settings.accounts.signin.duplicate-account", defaultValue: "Account with the same username and provider already exists!", comment: "Duplicate account error message")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "settings.accounts.signin.auth-failed", defaultValue: "Authentication Failed", comment: "Authentication failed error message")
        case 403:
            signinErrorDetail = String(localized: "settings.accounts.signin.api-forbidden", defaultValue: "API Access Forbidden", comment: "API access forbidden error message")
        default:
