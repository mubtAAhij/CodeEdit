...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text("Server", comment: "Label text")
                                    .font(.caption3)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text("Username", comment: "Label text")
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text("Personal Access Token", comment: "Label text")
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text("Sign in to \(provider.name)", comment: "Header text")
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text("\(provider.name) personal access tokens must have these scopes set:", comment: "Info text")
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text("Create a Password on \(provider.name)", comment: "Link text")
                                        .font(.system(size: 10.5))
                                } else {
                                    Text("Create a Token on \(provider.name)", comment: "Link text")
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                Button {
                    ...
                    Text("Cancel", comment: "Button text")
                        .frame(maxWidth: .infinity)
                }
                .controlSize(.large)
                .frame(maxWidth: .infinity)
                ...
                Button {
                    ...
                    Text("Sign In", comment: "Button text")
                        .frame(maxWidth: .infinity)
                }
                ...
                .controlSize(.large)
                .alert(
                    Text("Unable to add account \"\(username)\"", comment: "Alert title"),
                    isPresented: $signinErrorAlertIsPresented
                ) {
                    Button("OK", comment: "Button text") {
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "Account with the same username and provider already exists!", comment: "Error message")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "Authentication Failed", comment: "Error message")
        case 403:
            signinErrorDetail = String(localized: "API Access Forbidden", comment: "Error message")
        default:
            signinErrorDetail = String(localized: "Unknown Error", comment: "Error message")
        }
        signinErrorAlertIsPresented.toggle()
    }
}
