...
struct AccountsSettingsSigninView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.accounts.signin.token", defaultValue: "Personal Access Token", comment: "Personal access token field label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(format: String(localized: "settings.accounts.signin.scopes-required", defaultValue: "%@ personal access tokens must have these scopes set:", comment: "Scopes requirement message"), provider.name))
            ...
            HStack {
                ...
                .controlSize(.large)
                .alert(
                    Text(String(format: String(localized: "settings.accounts.signin.error-title", defaultValue: "Unable to add account \"%@\"", comment: "Error alert title"), username)),
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "settings.accounts.signin.error-duplicate", defaultValue: "Account with the same username and provider already exists!", comment: "Duplicate account error message")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "settings.accounts.signin.error-auth-failed", defaultValue: "Authentication Failed", comment: "Authentication failed error message")
        case 403:
            signinErrorDetail = String(localized: "settings.accounts.signin.error-api-forbidden", defaultValue: "API Access Forbidden", comment: "API access forbidden error message")
        default:
