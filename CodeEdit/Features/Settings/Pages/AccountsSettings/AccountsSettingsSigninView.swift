...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "settings.accounts.signin.label.server", defaultValue: "Server", comment: "Label for server field"))
                                    .font(.caption3)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.accounts.signin.label.username", defaultValue: "Username", comment: "Label for username field"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.accounts.signin.label.token", defaultValue: "Personal Access Token", comment: "Label for personal access token field"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(localized: "settings.accounts.signin.token-scopes", defaultValue: "\(provider.name) personal access tokens must have these scopes set:", comment: "Message about required token scopes"))
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "settings.accounts.signin.scope.user", defaultValue: "user", comment: "GitHub token scope"))
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(localized: "settings.accounts.signin.create-password", defaultValue: "Create a Password on \(provider.name)", comment: "Link to create password"))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(localized: "settings.accounts.signin.create-token", defaultValue: "Create a Token on \(provider.name)", comment: "Link to create token"))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                ...
                Button {
                    ...
                    Text(String(localized: "settings.accounts.signin.button", defaultValue: "Sign In", comment: "Sign in button"))
                        .frame(maxWidth: .infinity)
                }
                ...
                .controlSize(.large)
                .alert(
                    Text(String(localized: "settings.accounts.signin.error.unable-to-add", defaultValue: "Unable to add account \"\(username)\"", comment: "Error message when unable to add account")),
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "settings.accounts.signin.error.duplicate-account", defaultValue: "Account with the same username and provider already exists!", comment: "Error message when duplicate account exists")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 403:
            signinErrorDetail = String(localized: "settings.accounts.signin.error.api-forbidden", defaultValue: "API Access Forbidden", comment: "Error message when API access is forbidden")
        default:
            signinErrorDetail = String(localized: "settings.accounts.signin.error.unknown", defaultValue: "Unknown Error", comment: "Generic unknown error message")
        }
        signinErrorAlertIsPresented.toggle()
    }
}
