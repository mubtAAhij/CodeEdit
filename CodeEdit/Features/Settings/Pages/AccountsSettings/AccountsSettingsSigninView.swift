...
struct AccountsSettingsSigninView: View {
            ...
            Form {
                Section(
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(localized: "sign_in_to_provider", comment: "Sign in header with provider name"))
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(localized: "provider_token_scopes_required", comment: "Token scopes requirement message"))
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "user_scope", comment: "User scope permission"))
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(localized: "create_password_on_provider", comment: "Create password link text"))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(localized: "create_token_on_provider", comment: "Create token link text"))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "duplicate_account_error", comment: "Duplicate account error message")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "authentication_failed", comment: "Authentication failure error message")
        case 403:
            signinErrorDetail = String(localized: "api_access_forbidden", comment: "API access forbidden error message")
        default:
            signinErrorDetail = String(localized: "unknown_error", comment: "Unknown error message")
        }
        signinErrorAlertIsPresented.toggle()
    }
}
