...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text("String(localized: "sign_in_to_provider", comment: "Sign in to provider title")")
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text("String(localized: "provider_token_scopes_required", comment: "Message about required token scopes for provider")")
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text("String(localized: "user_scope", comment: "User scope for token permissions")")
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text("String(localized: "create_password_on_provider", comment: "Create password on provider link")")
                                        .font(.system(size: 10.5))
                                } else {
                                    Text("String(localized: "create_token_on_provider", comment: "Create token on provider link")")
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                ...
                Button {
                    ...
                    Text("String(localized: "sign_in", comment: "Sign in button")")
                        .frame(maxWidth: .infinity)
                }
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = "String(localized: "authentication_failed", comment: "Authentication failed error message")"
        case 403:
