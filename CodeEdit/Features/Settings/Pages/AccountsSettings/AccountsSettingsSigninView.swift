    ...
    var body: some View {
        VStack(spacing: 0) {
                    ...
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "accounts.signin.server-label", defaultValue: "Server", comment: "Label for server input field"))
                                    .font(.caption3)
                                    .foregroundColor(.secondary)
                                TextField("", text: $server, prompt: Text(String(localized: "accounts.signin.server-placeholder", defaultValue: "https://git.example.com", comment: "Placeholder for server URL input")))
                                    .labelsHidden()
                            }
                        }
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "accounts.signin.username-label", defaultValue: "Username", comment: "Label for username input field"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "accounts.signin.token-label", defaultValue: "Personal Access Token", comment: "Label for personal access token input field"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(format: String(localized: "accounts.signin.title", defaultValue: "Sign in to %@", comment: "Sign in title with provider name"), provider.name))
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(format: String(localized: "accounts.signin.create-password-button", defaultValue: "Create a Password on %@", comment: "Button to create password with provider name"), provider.name))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(format: String(localized: "accounts.signin.create-token-button", defaultValue: "Create a Token on %@", comment: "Button to create token with provider name"), provider.name))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                ...
                .controlSize(.large)
                .alert(
                    Text(String(format: String(localized: "accounts.signin.error-title", defaultValue: "Unable to add account \"%@\"", comment: "Alert title for account add error with username"), username)),
