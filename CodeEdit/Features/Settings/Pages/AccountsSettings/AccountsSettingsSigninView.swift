...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "settings.account.signin.server", defaultValue: "Server", comment: "Server field label"))
                                    .font(.caption3)
                                    .foregroundColor(.secondary)
                                TextField("", text: $server, prompt: Text(String(localized: "settings.account.signin.server-placeholder", defaultValue: "https://git.example.com", comment: "Server field placeholder")))
                                    .labelsHidden()
                            }
                        }
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.account.signin.username", defaultValue: "Username", comment: "Username field label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "settings.account.signin.personal-access-token", defaultValue: "Personal Access Token", comment: "Personal access token field label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(format: NSLocalizedString("settings.account.signin.sign-in-to", comment: "Sign in to provider"), provider.name))
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(format: NSLocalizedString("settings.account.signin.token-scopes-required", comment: "Token scopes required"), provider.name))
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(format: NSLocalizedString("settings.account.signin.create-password-on", comment: "Create password on provider"), provider.name))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(format: NSLocalizedString("settings.account.signin.create-token-on", comment: "Create token on provider"), provider.name))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                ...
                Button {
                    ...
                    Text(String(localized: "settings.account.signin.sign-in", defaultValue: "Sign In", comment: "Sign in button"))
                        .frame(maxWidth: .infinity)
                }
                ...
                .controlSize(.large)
                .alert(
                    Text(String(format: NSLocalizedString("settings.account.signin.unable-to-add-account", comment: "Unable to add account error"), username)),
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "settings.account.signin.account-already-exists", defaultValue: "Account with the same username and provider already exists!", comment: "Account already exists error")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "settings.account.signin.authentication-failed", defaultValue: "Authentication Failed", comment: "Authentication failed error")
        case 403:
            signinErrorDetail = String(localized: "settings.account.signin.api-access-forbidden", defaultValue: "API Access Forbidden", comment: "API access forbidden error")
        default:
            signinErrorDetail = String(localized: "settings.account.signin.unknown-error", defaultValue: "Unknown Error", comment: "Unknown error message")
        }
        signinErrorAlertIsPresented.toggle()
    }
}
