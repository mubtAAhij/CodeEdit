...
struct AccountsSettingsSigninView: View {
        ...
        VStack(spacing: 0) {
            Form {
                Section(
                    content: {
                        if provider.baseURL == nil {
                            VStack(alignment: .leading, spacing: 5) {
                                Text(String(localized: "account.field.server", comment: "Server field label"))
                                    .font(.caption3)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "account.field.username", comment: "Username field label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                        ...
                        VStack(alignment: .leading, spacing: 5) {
                            Text(String(localized: "account.field.personal_access_token", comment: "Personal access token field label"))
                                .font(.caption3)
                                .foregroundColor(.secondary)
                    ...
                    header: {
                        VStack(alignment: .center, spacing: 10) {
                            FeatureIcon(image: Image(provider.iconResource), size: 52)
                                .padding(.top, 5)
                            Text(String(localized: "account.signin.title", arguments: [provider.name], comment: "Sign in title with provider name"))
                    ...
                    footer: {
                        VStack(alignment: .leading, spacing: 5) {
                            if provider == .github {
                                Text(String(localized: "account.signin.scopes_description", arguments: [provider.name], comment: "Description of required scopes for provider"))
                                ...
                                HStack(alignment: .center) {
                                    ...
                                    VStack(alignment: .leading) {
                                        ...
                                        HStack(spacing: 2.5) {
                                            Image(systemName: "checkmark")
                                                .font(.system(size: 10.5, weight: .semibold))
                                            Text(String(localized: "account.scope.user", comment: "User scope label"))
                                                .font(.system(size: 10.5))
                                        }
                                    }
                                    Spacer()
                            ...
                            Button {
                                ...
                                if provider.authType == .password {
                                    Text(String(localized: "account.signin.create_password", arguments: [provider.name], comment: "Create password link text"))
                                        .font(.system(size: 10.5))
                                } else {
                                    Text(String(localized: "account.signin.create_token", arguments: [provider.name], comment: "Create token link text"))
                                        .font(.system(size: 10.5))
                                }
                            }
                            .buttonStyle(.link)
            ...
            HStack {
                Button {
                    ...
                    Text(String(localized: "button.cancel", comment: "Cancel button"))
                        .frame(maxWidth: .infinity)
                }
                .controlSize(.large)
                .frame(maxWidth: .infinity)
                ...
                Button {
                    ...
                    Text(String(localized: "button.sign_in", comment: "Sign in button"))
                        .frame(maxWidth: .infinity)
                }
    ...
    private func signin() {
        if gitAccounts.contains(
            ...
            signinErrorDetail = String(localized: "account.error.duplicate_account", comment: "Error when account already exists")
            signinErrorAlertIsPresented.toggle()
        } else {
    ...
    private func handleGitRequestFailed(_ error: Error) {
        ...
        case 401:
            signinErrorDetail = String(localized: "account.error.authentication_failed", comment: "Authentication failed error")
        case 403:
            signinErrorDetail = String(localized: "account.error.api_access_forbidden", comment: "API access forbidden error")
        default:
            signinErrorDetail = String(localized: "account.error.unknown", comment: "Unknown error message")
        }
        signinErrorAlertIsPresented.toggle()
    }
}
