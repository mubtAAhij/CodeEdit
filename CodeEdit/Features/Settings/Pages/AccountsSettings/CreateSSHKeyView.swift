...
struct CreateSSHKeyView: View {
    ...
    var body: some View {
        VStack {
            Form {
                Section(String(localized: "settings.accounts.ssh.create-title", defaultValue: "Create SSH key", comment: "Title for SSH key creation section")) {
                    Picker(String(localized: "settings.accounts.ssh.key-type", defaultValue: "Key Type", comment: "Key type picker label"), selection: $selectedKeyType) {
                        ...
                        Divider()
                        Group {
                            Text(KeyType.rsa.rawValue) + Text(String(localized: "settings.accounts.ssh.less-secure", defaultValue: " (less secure)", comment: "Indicates less secure key type")).foregroundColor(.secondary)
                        }
                        .tag(KeyType.rsa)
                        Group {
                            Text(KeyType.dsa.rawValue) + Text(String(localized: "settings.accounts.ssh.less-secure", defaultValue: " (less secure)", comment: "Indicates less secure key type")).foregroundColor(.secondary)
                        }
                        .tag(KeyType.dsa)
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "settings.accounts.ssh.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                    dismiss()
                }
                Button(String(localized: "settings.accounts.ssh.create", defaultValue: "Create", comment: "Create SSH key button")) {
                    // create the ssh key
                    dismiss()
                }
