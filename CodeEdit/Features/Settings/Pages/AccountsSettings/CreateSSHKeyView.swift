...
struct CreateSSHKeyView: View {
    ...
    var body: some View {
        VStack {
            Form {
                Section(String(localized: "create-ssh-key.title", defaultValue: "Create SSH key", comment: "Create SSH key section title")) {
                    Picker(String(localized: "create-ssh-key.key-type", defaultValue: "Key Type", comment: "Key type picker label"), selection: $selectedKeyType) {
                        ...
                        Divider()
                        Group {
                            Text(KeyType.rsa.rawValue) + Text(String(localized: "create-ssh-key.less-secure", defaultValue: " (less secure)", comment: "Less secure key type suffix")).foregroundColor(.secondary)
                        }
                        .tag(KeyType.rsa)
                        Group {
                            Text(KeyType.dsa.rawValue) + Text(String(localized: "create-ssh-key.less-secure", defaultValue: " (less secure)", comment: "Less secure key type suffix")).foregroundColor(.secondary)
                        }
                        .tag(KeyType.dsa)
                    }
            ...
            HStack {
                Spacer()
                Button(String(localized: "create-ssh-key.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                    dismiss()
                }
                Button(String(localized: "create-ssh-key.create", defaultValue: "Create", comment: "Create button")) {
                    // create the ssh key
                    dismiss()
                }
