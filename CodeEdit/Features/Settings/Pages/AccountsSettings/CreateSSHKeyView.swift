                    ...
                    Picker(String(
                        ...
                        Group {
                            Text(KeyType.rsa.rawValue) + Text(" " + String(
                                localized: "settings.accounts.create-ssh-key.less-secure-suffix.ed25519-sk",
                                defaultValue: "(less secure)",
                                comment: "Suffix appended to key type indicating lower security"
                            )).foregroundColor(.secondary)
                        }
                        .tag(KeyType.rsa)
                        Group {
                            Text(KeyType.dsa.rawValue) + Text(" " + String(
                                localized: "settings.accounts.create-ssh-key.less-secure-suffix.rsa",
                                defaultValue: "(less secure)",
                                comment: "Suffix appended to key type indicating lower security"
                            )).foregroundColor(.secondary)
                        }
