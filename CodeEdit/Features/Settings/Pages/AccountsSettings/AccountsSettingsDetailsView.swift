...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "settings.accounts.details.account-section", defaultValue: "Account", comment: "Section header title for account details")) {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.details.protocol.https", defaultValue: "HTTPS", comment: "Label for HTTPS clone protocol option"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(localized: "settings.accounts.details.protocol.ssh", defaultValue: "SSH", comment: "Label for SSH clone protocol option"))
                    ...
                    Text(String(format: String(localized: "settings.accounts.details.clone-using-help", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Help text describing provider and selected clone protocol"), "\(currentAccount.provider.name)", "\(currentAccount.urlProtocol.rawValue)"))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key selection control"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.ssh-key.none", defaultValue: "None", comment: "Placeholder text when no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.ssh-key.create-new", defaultValue: "Create New...", comment: "Menu item title for creating a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.ssh-key.choose", defaultValue: "Choose...", comment: "Menu item title for choosing an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "settings.accounts.details.delete-account", defaultValue: "Delete Account...", comment: "Button title for deleting the selected account")) {
                    ...
                    .alert(
                        ...
                        Button(String(localized: "settings.accounts.details.delete-account.ok", defaultValue: "OK", comment: "Confirmation button title for deleting account")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "settings.accounts.details.delete-account.cancel", defaultValue: "Cancel", comment: "Cancellation button title for delete account confirmation")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account.help", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Explanatory text about account deletion impact"))
                    }
