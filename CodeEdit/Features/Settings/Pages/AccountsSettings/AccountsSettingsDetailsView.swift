...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "settings.accounts.account", defaultValue: "Account", comment: "Label for account field")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "settings.accounts.description", defaultValue: "Description", comment: "Placeholder for description field"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "settings.accounts.server", defaultValue: "Server", comment: "Placeholder for server field"), text: $currentAccount.serverURL)
                }
            }

            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.https", defaultValue: "HTTPS", comment: "HTTPS option"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text("SSH")
                        .tag(SourceControlAccount.URLProtocol.ssh)
                } label: {
                    Text(String(localized: "settings.accounts.clone-using", defaultValue: "Clone Using", comment: "Label for clone using picker"))
                    Text(String(format: NSLocalizedString("settings.accounts.clone-description", comment: "Description for clone using setting"), currentAccount.provider.name, currentAccount.urlProtocol.rawValue))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key picker"), selection: $currentAccount.sshKey) {
                        ...
                        Text(String(localized: "settings.accounts.ssh-key-create-new", defaultValue: "Create New...", comment: "Option to create new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.ssh-key-choose", defaultValue: "Choose...", comment: "Option to choose SSH key file"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        Text(String(format: NSLocalizedString("settings.accounts.delete-confirmation", comment: "Confirmation message for deleting account"), account.description)),
                        ...
                        Text(String(localized: "settings.accounts.delete-message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Message explaining account deletion"))
                    }
