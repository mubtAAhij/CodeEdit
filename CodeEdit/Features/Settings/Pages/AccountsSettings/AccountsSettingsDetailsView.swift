        ...
        SettingsForm {
            Section {
                LabeledContent(String(
                    localized: "settings.accounts.details.account.section",
                    defaultValue: "Account",
                    comment: "Section header for account details settings"
                )) {
                    Text(currentAccount.name)
                }
                TextField(String(
                    localized: "settings.accounts.details.description.label",
                    defaultValue: "Description",
                    comment: "Label for account description field"
                ), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(
                        localized: "settings.accounts.details.server.label",
                        defaultValue: "Server",
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(
                        localized: "settings.accounts.details.clone-protocol.https",
                        defaultValue: "HTTPS",
                        comment: "Clone protocol option label for HTTPS"
                    ))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(
                        localized: "settings.accounts.details.clone-protocol.ssh",
                        defaultValue: "SSH",
                        comment: "Clone protocol option label for SSH"
                    ))
                        .tag(SourceControlAccount.URLProtocol.ssh)
                } label: {
                    Text(String(
                        localized: "settings.accounts.details.clone-using.label",
                        defaultValue: "Clone Using",
                        comment: "Label for repository clone protocol selection"
                    ))
                    Text(String(format: String(
                        localized: "settings.accounts.details.clone-using.description",
                        defaultValue: "New repositories will be cloned from %@ using %@.",
                        comment: "Description explaining clone protocol and provider for new repositories"
                    ), "\(currentAccount.provider.name)", "\(currentAccount.urlProtocol.rawValue)"))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(
                        localized: "settings.accounts.details.ssh-key.label",
                        defaultValue: "SSH Key",
                        comment: "Label for SSH key selection control"
                    ), selection: $currentAccount.sshKey) {
                        Text(String(
                            localized: "settings.accounts.details.ssh-key.none-option",
                            defaultValue: "None",
                        ...
                        Text(String(
                            localized: "settings.accounts.details.ssh-key.create-new.button",
                            defaultValue: "Create New...",
                            comment: "Menu action to create a new SSH key"
                        ))
                            .tag("CREATE_NEW")
                        Text(String(
                            localized: "settings.accounts.details.ssh-key.choose.button",
                            defaultValue: "Choose...",
                            comment: "Menu action to choose an existing SSH key"
                        ))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(
                        localized: "settings.accounts.details.delete-account.button",
                        defaultValue: "Delete Account...",
                        comment: "Button title to open account deletion confirmation"
                    )) {
                    ...
                    .alert(
                        Text(String(format: String(
                            ...
                            comment: "Confirmation alert title asking to delete the selected account"
                        ), "\(account.description)")),
                        isPresented: $deleteConfirmationIsPresented
                    ) {
                        Button(String(
                            localized: "settings.accounts.details.delete-account.ok.button",
                            defaultValue: "OK",
                        ...
                        Button(String(
                            localized: "settings.accounts.details.delete-account.cancel.button",
                            defaultValue: "Cancel",
                            comment: "Cancel button title in delete account confirmation alert"
                        )) {
                        ...
                        Text(String(
                            localized: "settings.accounts.details.delete-account.warning.message",
                            defaultValue: "Deleting this account will remove it from CodeEdit.",
                            comment: "Warning message explaining deletion consequence"
                        ))
                    }
