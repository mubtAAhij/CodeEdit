        ...
        SettingsForm {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.details.clone-using.https", defaultValue: "HTTPS", comment: "Clone protocol option for HTTPS"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(localized: "settings.accounts.details.clone-using.ssh", defaultValue: "SSH", comment: "Clone protocol option for SSH"))
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key selection"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.none", defaultValue: "None", comment: "Option indicating no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.create-new", defaultValue: "Create New...", comment: "Menu item title to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.choose", defaultValue: "Choose...", comment: "Menu item title to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        ...
                        Button(String(localized: "settings.accounts.details.ok", defaultValue: "OK", comment: "Button title to confirm account deletion")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "settings.accounts.details.cancel", defaultValue: "Cancel", comment: "Button title to cancel account deletion")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account-warning", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Warning text shown in account deletion confirmation dialog"))
                    }
