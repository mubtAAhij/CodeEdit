...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "settings.accounts.account", defaultValue: "Account", comment: "Account label")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "settings.accounts.description", defaultValue: "Description", comment: "Description field label"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "settings.accounts.server", defaultValue: "Server", comment: "Server field label"), text: $currentAccount.serverURL)
                }
            }

            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.protocol.https", defaultValue: "HTTPS", comment: "HTTPS protocol option"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(localized: "settings.accounts.protocol.ssh", defaultValue: "SSH", comment: "SSH protocol option"))
                        .tag(SourceControlAccount.URLProtocol.ssh)
                } label: {
                    Text(String(localized: "settings.accounts.clone-using", defaultValue: "Clone Using", comment: "Clone using picker label"))
                    Text(String(format: String(localized: "settings.accounts.clone-using-description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Clone using description"), currentAccount.provider.name, currentAccount.urlProtocol.displayName))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.ssh-key", defaultValue: "SSH Key", comment: "SSH Key picker label"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.ssh-key.none", defaultValue: "None", comment: "No SSH key option"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.ssh-key.create-new", defaultValue: "Create New...", comment: "Create new SSH key option"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.ssh-key.choose", defaultValue: "Choose...", comment: "Choose SSH key option"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "settings.accounts.delete-account", defaultValue: "Delete Account...", comment: "Delete account button")) {
                        deleteConfirmationIsPresented.toggle()
                    }
                    .alert(
                        Text(String(format: String(localized: "settings.accounts.delete-confirmation", defaultValue: "Are you sure you want to delete the account \"%@\"?", comment: "Delete account confirmation message"), account.description)),
                        ...
                        Text(String(localized: "settings.accounts.delete-account-message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Delete account alert message"))
                    }
