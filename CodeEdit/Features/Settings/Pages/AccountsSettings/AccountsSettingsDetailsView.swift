...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "settings.accounts.details.account", defaultValue: "Account", comment: "Section header title for account details")) {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.details.protocol.https", defaultValue: "HTTPS", comment: "Clone protocol option label for HTTPS"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(localized: "settings.accounts.details.protocol.ssh", defaultValue: "SSH", comment: "Clone protocol option label for SSH"))
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key selection"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.none", defaultValue: "None", comment: "Placeholder label when no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.create-new", defaultValue: "Create New...", comment: "Button title to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.choose", defaultValue: "Choose...", comment: "Button title to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        ...
                        Text(String(localized: "settings.accounts.details.delete-account.informative-text", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Informative text in delete account alert"))
                    }
