...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "accounts_settings.account", comment: "Account label")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "accounts_settings.description", comment: "Description label"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "accounts_settings.server", comment: "Server label"), text: $currentAccount.serverURL)
                }
            }

            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "accounts_settings.https", comment: "HTTPS protocol option"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text("SSH")
                        .tag(SourceControlAccount.URLProtocol.ssh)
                } label: {
                    Text(String(localized: "accounts_settings.clone_using", comment: "Clone using label"))
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "accounts_settings.ssh_key", comment: "SSH Key label"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "accounts_settings.none", comment: "None option for SSH key"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "accounts_settings.create_new", comment: "Create new SSH key button"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "accounts_settings.choose", comment: "Choose SSH key button"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "accounts_settings.delete_account", comment: "Delete account button")) {
                    ...
                    .alert(
                        ...
                        Button(String(localized: "accounts_settings.ok", comment: "OK button text")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "accounts_settings.cancel", comment: "Cancel button text")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "accounts_settings.delete_warning", comment: "Warning about deleting account"))
                    }
