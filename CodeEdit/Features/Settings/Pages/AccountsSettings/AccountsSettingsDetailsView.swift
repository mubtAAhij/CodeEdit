...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "accounts-settings.account", defaultValue: "Account", comment: "Account label")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "accounts-settings.description", defaultValue: "Description", comment: "Description field label"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "accounts-settings.server", defaultValue: "Server", comment: "Server field label"), text: $currentAccount.serverURL)
                }
            }
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(localized: "accounts-settings.clone-using", defaultValue: "Clone Using", comment: "Clone using picker label"))
                    Text(String(format: String(localized: "accounts-settings.clone-description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Clone method description"), currentAccount.provider.name, currentAccount.urlProtocol.rawValue))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "accounts-settings.ssh-key", defaultValue: "SSH Key", comment: "SSH key picker label"), selection: $currentAccount.sshKey) {
                        ...
                        Text(String(localized: "accounts-settings.ssh-key-create-new", defaultValue: "Create New...", comment: "Create new SSH key option"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "accounts-settings.ssh-key-choose", defaultValue: "Choose...", comment: "Choose SSH key option"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "accounts-settings.delete-account", defaultValue: "Delete Account...", comment: "Delete account button")) {
                        deleteConfirmationIsPresented.toggle()
                    }
                    .alert(
                        Text(String(format: String(localized: "accounts-settings.delete-confirmation", defaultValue: "Are you sure you want to delete the account \"%@\"?", comment: "Delete account confirmation"), account.description)),
                        isPresented: $deleteConfirmationIsPresented
                    ) {
                        Button(String(localized: "accounts-settings.ok", defaultValue: "OK", comment: "OK button")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "accounts-settings.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "accounts-settings.delete-message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Delete account warning message"))
                    }
