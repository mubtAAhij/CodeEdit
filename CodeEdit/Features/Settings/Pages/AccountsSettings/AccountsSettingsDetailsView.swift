...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "accounts-details.account", defaultValue: "Account", comment: "Account label")) {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(format: String(localized: "accounts-details.clone-description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Clone using description"), currentAccount.provider.name, currentAccount.urlProtocol.rawValue))
                }
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "accounts-details.ssh-key", defaultValue: "SSH Key", comment: "SSH Key picker label"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "accounts-details.none", defaultValue: "None", comment: "None option for SSH key"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "accounts-details.create-new", defaultValue: "Create New...", comment: "Create new SSH key option"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "accounts-details.choose", defaultValue: "Choose...", comment: "Choose SSH key option"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        Text(String(format: String(localized: "accounts-details.delete-confirmation", defaultValue: "Are you sure you want to delete the account \"%@\"?", comment: "Delete account confirmation"), account.description)),
                        ...
                        Text(String(localized: "accounts-details.delete-message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Delete account message"))
                    }
