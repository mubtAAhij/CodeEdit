        ...
        SettingsForm {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(format: String(localized: "settings.accounts.details.clone-using.description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Description explaining selected provider and protocol for cloning repositories"), "\(currentAccount.provider.name)", "\(currentAccount.urlProtocol.rawValue)"))
                }
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key selection"), selection: $currentAccount.sshKey) {
                        ...
                        Text(String(localized: "settings.accounts.details.ssh-key.create-new", defaultValue: "Create New...", comment: "Button title to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.ssh-key.choose", defaultValue: "Choose...", comment: "Button title to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "settings.accounts.details.delete-account", defaultValue: "Delete Account...", comment: "Button title to delete selected account")) {
                        deleteConfirmationIsPresented.toggle()
                    }
                    .alert(
                        Text(String(format: String(localized: "settings.accounts.details.delete-account.confirmation", defaultValue: "Are you sure you want to delete the account “%@”?", comment: "Confirmation message shown before deleting an account"), "\(account.description)")),
                        ...
                        Button(String(localized: "settings.accounts.details.delete-account.cancel", defaultValue: "Cancel", comment: "Cancel button title for deleting account confirmation")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account.informative-text", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Informative warning text for deleting account alert"))
                    }
