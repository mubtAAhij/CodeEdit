...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "settings.accounts.details.account.label", defaultValue: "Account", comment: "Label for account name field in account settings details")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "settings.accounts.details.description.placeholder", defaultValue: "Description", comment: "Placeholder for account description field in account settings details"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "settings.accounts.details.server.placeholder", defaultValue: "Server", comment: "Placeholder for server field in account settings details"), text: $currentAccount.serverURL)
                }
            }
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(localized: "settings.accounts.details.clone-using.label", defaultValue: "Clone Using", comment: "Label for clone protocol picker in account settings details"))
                    Text(
                        String(
                            format: String(
                                localized: "settings.accounts.details.clone-using.description",
                                defaultValue: "New repositories will be cloned from %@ using %@.",
                                comment: "Description under clone protocol picker showing provider and selected protocol"
                            ),
                            currentAccount.provider.name,
                            currentAccount.urlProtocol.rawValue
                        )
                    )
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key.picker", defaultValue: "SSH Key", comment: "Title of SSH key picker in account settings details"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.ssh-key.none", defaultValue: "None", comment: "Option indicating no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.ssh-key.create-new", defaultValue: "Create New...", comment: "Option to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.ssh-key.choose", defaultValue: "Choose...", comment: "Option to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "settings.accounts.details.delete-account.button", defaultValue: "Delete Account...", comment: "Button title to delete current account")) {
                    ...
                    .alert(
                        Text(
                            String(
                                format: String(
                                    localized: "settings.accounts.details.delete-account.confirmation.title",
                                    defaultValue: "Are you sure you want to delete the account “%@”?",
                                    comment: "Alert title confirming deletion of an account"
                                ),
                                account.description
                            )
                        ),
                        ...
                        Button(String(localized: "settings.accounts.details.delete-account.confirmation.ok", defaultValue: "OK", comment: "Confirmation button to delete account")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "settings.accounts.details.delete-account.confirmation.cancel", defaultValue: "Cancel", comment: "Cancel button in delete account alert")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account.confirmation.message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Message explaining the consequence of deleting an account"))
                    }
