            ...
            Section {
                ...
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for SSH key selection control"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.none", defaultValue: "None", comment: "Option title when no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.create-new-ssh-key", defaultValue: "Create New...", comment: "Menu item title to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.choose-ssh-key", defaultValue: "Choose...", comment: "Menu item title to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        Text(String(format: String(localized: "settings.accounts.details.delete-account.confirmation", defaultValue: "Are you sure you want to delete the account “%@”?", comment: "Confirmation message asking whether to delete the selected account"), "\(account.description)")),
                        ...
                        Button(String(localized: "settings.accounts.details.delete-account.cancel", defaultValue: "Cancel", comment: "Cancel button title in delete account confirmation")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account.warning", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Informational warning about deleting account from CodeEdit"))
                    }
