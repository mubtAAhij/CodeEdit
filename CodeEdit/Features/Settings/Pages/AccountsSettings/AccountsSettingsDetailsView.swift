        ...
        SettingsForm {
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    Text(String(localized: "settings.accounts.details.protocol.https", defaultValue: "HTTPS", comment: "Picker option for HTTPS protocol"))
                        .tag(SourceControlAccount.URLProtocol.https)
                    Text(String(localized: "settings.accounts.details.protocol.ssh", defaultValue: "SSH", comment: "Picker option for SSH protocol"))
                    ...
                    Text(String(format: String(localized: "settings.accounts.details.clone-using.description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Description for selected clone protocol and provider"), "\(currentAccount.provider.name)", "\(currentAccount.urlProtocol.rawValue)"))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "settings.accounts.details.ssh-key", defaultValue: "SSH Key", comment: "Label for selected SSH key"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "settings.accounts.details.ssh-key.none", defaultValue: "None", comment: "Value shown when no SSH key is selected"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "settings.accounts.details.ssh-key.create-new", defaultValue: "Create New...", comment: "Action title to create a new SSH key"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "settings.accounts.details.ssh-key.choose", defaultValue: "Choose...", comment: "Action title to choose an existing SSH key"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    ...
                    .alert(
                        Text(String(format: String(localized: "settings.accounts.details.delete-account.confirmation.title", defaultValue: "Are you sure you want to delete the account “%@”?", comment: "Confirmation title asking to delete account"), "\(account.description)")),
                        isPresented: $deleteConfirmationIsPresented
                    ) {
                        Button(String(localized: "settings.accounts.details.delete-account.confirmation.ok", defaultValue: "OK", comment: "Confirmation button to proceed with account deletion")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "settings.accounts.details.delete-account.confirmation.cancel", defaultValue: "Cancel", comment: "Confirmation button to cancel account deletion")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "settings.accounts.details.delete-account.confirmation.message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Confirmation message explaining account removal effect"))
                    }
