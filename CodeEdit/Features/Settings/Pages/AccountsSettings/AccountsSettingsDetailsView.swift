    ...
    var body: some View {
        SettingsForm {
            Section {
                ...
                TextField(String(localized: "accounts.details.description", defaultValue: "Description", comment: "Label for account description field"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "accounts.details.server", defaultValue: "Server", comment: "Label for server URL field"), text: $currentAccount.serverURL)
                }
            }
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(localized: "accounts.details.clone_using", defaultValue: "Clone Using", comment: "Label for protocol selection"))
                    Text(String(format: String(localized: "accounts.details.clone_using_description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Description of clone protocol selection"), currentAccount.provider.name, currentAccount.urlProtocol.rawValue))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "accounts.details.ssh_key", defaultValue: "SSH Key", comment: "Label for SSH key picker"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "accounts.details.ssh_key.none", defaultValue: "None", comment: "No SSH key selected option"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "accounts.details.ssh_key.create_new", defaultValue: "Create New...", comment: "Create new SSH key option"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "accounts.details.ssh_key.choose", defaultValue: "Choose...", comment: "Choose SSH key file option"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "accounts.details.delete_account", defaultValue: "Delete Account...", comment: "Delete account button")) {
                        deleteConfirmationIsPresented.toggle()
                    }
                    .alert(
                        Text(String(format: String(localized: "accounts.details.delete_confirmation", defaultValue: "Are you sure you want to delete the account \"%@\"?", comment: "Confirmation message for deleting account"), account.description)),
                        ...
                        Text(String(localized: "accounts.details.delete_message", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Message explaining account deletion"))
                    }
