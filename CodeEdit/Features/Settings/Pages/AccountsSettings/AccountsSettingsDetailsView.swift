...
struct AccountsSettingsDetailsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                LabeledContent(String(localized: "accounts.account", defaultValue: "Account", comment: "Account label")) {
                    Text(currentAccount.name)
                }
                TextField(String(localized: "accounts.description", defaultValue: "Description", comment: "Description field"), text: $currentAccount.description)
                if currentAccount.provider.baseURL == nil {
                    TextField(String(localized: "accounts.server", defaultValue: "Server", comment: "Server field"), text: $currentAccount.serverURL)
                }
            }
            ...
            Section {
                Picker(selection: $currentAccount.urlProtocol) {
                    ...
                    Text(String(localized: "accounts.clone-using", defaultValue: "Clone Using", comment: "Clone using picker label"))
                    Text(String(format: String(localized: "accounts.clone-using-description", defaultValue: "New repositories will be cloned from %@ using %@.", comment: "Clone using description"), currentAccount.provider.name, currentAccount.urlProtocol.rawValue))
                }
                .pickerStyle(.radioGroup)
                if currentAccount.urlProtocol == .ssh {
                    Picker(String(localized: "accounts.ssh-key", defaultValue: "SSH Key", comment: "SSH key picker label"), selection: $currentAccount.sshKey) {
                        Text(String(localized: "accounts.ssh-key-none", defaultValue: "None", comment: "No SSH key option"))
                            .tag("")
                        Divider()
                        ...
                        Text(String(localized: "accounts.ssh-key-create-new", defaultValue: "Create New...", comment: "Create new SSH key option"))
                            .tag("CREATE_NEW")
                        Text(String(localized: "accounts.ssh-key-choose", defaultValue: "Choose...", comment: "Choose SSH key option"))
                            .tag("CHOOSE")
                    }
                ...
                HStack {
                    Button(String(localized: "accounts.delete-account", defaultValue: "Delete Account...", comment: "Delete account button")) {
                        deleteConfirmationIsPresented.toggle()
                    }
                    .alert(
                        Text(String(format: String(localized: "accounts.delete-confirmation", defaultValue: "Are you sure you want to delete the account \"%@\"?", comment: "Delete account confirmation message"), account.description)),
                        isPresented: $deleteConfirmationIsPresented
                    ) {
                        Button(String(localized: "accounts.ok", defaultValue: "OK", comment: "OK button")) {
                            // Handle the account delete
                            handleAccountDelete()
                            dismiss()
                        }
                        Button(String(localized: "accounts.cancel", defaultValue: "Cancel", comment: "Cancel button")) {
                            // Handle the cancel, dismiss the alert
                            deleteConfirmationIsPresented.toggle()
                        }
                    } message: {
                        Text(String(localized: "accounts.delete-warning", defaultValue: "Deleting this account will remove it from CodeEdit.", comment: "Delete account warning message"))
                    }
