...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "Empty state when no accounts are configured"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-client", defaultValue: "This git client is currently not supported.", comment: "Message when git client is not supported"))
            HStack {
                Button(String(localized: "settings.accounts.close", defaultValue: "Close", comment: "Button to close unsupported client sheet")) {
