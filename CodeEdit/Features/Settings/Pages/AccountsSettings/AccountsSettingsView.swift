...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "accounts-settings.no-accounts", defaultValue: "No accounts", comment: "Empty state when there are no accounts"))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "accounts-settings.add-account", defaultValue: "Add Account...", comment: "Add account button")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "accounts-settings.unsupported-client", defaultValue: "This git client is currently not supported.", comment: "Message for unsupported git client"))
            HStack {
                Button(String(localized: "close", defaultValue: "Close", comment: "Close button")) {
