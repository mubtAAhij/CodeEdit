...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "accounts.no-accounts", defaultValue: "No accounts", comment: "No accounts message"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "accounts.client-not-supported", defaultValue: "This git client is currently not supported.", comment: "Git client not supported message"))
            HStack {
