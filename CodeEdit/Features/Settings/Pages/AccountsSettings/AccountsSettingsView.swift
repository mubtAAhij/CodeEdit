...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.empty-state.no-accounts", defaultValue: "No accounts", comment: "Placeholder text shown when no accounts are configured"))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.actions.add-account", defaultValue: "Add Account...", comment: "Button title for adding a new account")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-git-client.message", defaultValue: "This git client is currently not supported.", comment: "Alert message shown when selected git provider is unsupported"))
            HStack {
