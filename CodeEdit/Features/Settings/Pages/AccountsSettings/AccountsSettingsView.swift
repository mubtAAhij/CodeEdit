...
struct AccountsSettingsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-account", defaultValue: "Add Account...", comment: "Button title for adding a new source control account")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-client.message", defaultValue: "This git client is currently not supported.", comment: "Alert message shown when selected git client is unsupported"))
            HStack {
                Button(String(localized: "settings.accounts.unsupported-client.close", defaultValue: "Close", comment: "Button title to close unsupported git client alert")) {
