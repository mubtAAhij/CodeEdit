...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "accounts.no-accounts", defaultValue: "No accounts", comment: "Message when no accounts are available"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "accounts.git-client-not-supported", defaultValue: "This git client is currently not supported.", comment: "Message when git client is not supported"))
            HStack {
