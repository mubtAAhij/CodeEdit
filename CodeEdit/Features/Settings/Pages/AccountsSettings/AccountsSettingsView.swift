...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "Placeholder text shown when no accounts are configured"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.git-client-not-supported", defaultValue: "This git client is currently not supported.", comment: "Alert message shown when a git client type is unsupported"))
            HStack {
                Button(String(localized: "settings.accounts.close", defaultValue: "Close", comment: "Button title to dismiss the unsupported git client alert")) {
