...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "Placeholder shown when no accounts are configured"))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-account-button", defaultValue: "Add Account...", comment: "Button title to add an account in accounts settings view")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-git-client", defaultValue: "This git client is currently not supported.", comment: "Alert message indicating selected git client is unsupported"))
            HStack {
