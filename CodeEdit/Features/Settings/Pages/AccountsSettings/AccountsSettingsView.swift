...
struct AccountsSettingsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-button", defaultValue: "Add Account...", comment: "Button to add a new account")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-client", defaultValue: "This git client is currently not supported.", comment: "Message shown when git client is not supported"))
            HStack {
                Button(String(localized: "settings.accounts.close", defaultValue: "Close", comment: "Close button in unsupported client view")) {
