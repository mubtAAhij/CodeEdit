...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no_accounts", defaultValue: "No accounts", comment: "Empty state when no accounts configured"))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add", defaultValue: "Add Account...", comment: "Add account button")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.not_supported", defaultValue: "This git client is currently not supported.", comment: "Unsupported git client message"))
            HStack {
