...
struct AccountsSettingsView: View {
            ...
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(
                        ...
                        comment: "Empty state text when no accounts are configured"
                    ))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(
                localized: "settings.accounts.unsupported-git-client-message",
                defaultValue: "This git client is currently not supported.",
                comment: "Alert message shown when selected git provider is unsupported"
            ))
            HStack {
                Button(String(
                    localized: "settings.accounts.unsupported-git-client.close",
                    defaultValue: "Close",
                    comment: "Button title to dismiss unsupported git client alert"
                )) {
                    addAccountSheetPresented.toggle()
