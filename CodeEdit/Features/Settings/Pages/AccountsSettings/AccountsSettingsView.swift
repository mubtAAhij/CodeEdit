...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(
                        localized: "settings.accounts.view.no-accounts",
                        defaultValue: "No accounts",
                        comment: "Empty state message when no accounts are configured"
                    ))
                ...
                HStack {
                    Spacer()
                    Button(String(
                        localized: "settings.accounts.view.add-account",
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(
                localized: "settings.accounts.view.unsupported-git-client",
                defaultValue: "This git client is currently not supported.",
                comment: "Warning message for unsupported git client in accounts settings"
            ))
            HStack {
                Button(String(
                    localized: "settings.accounts.view.close",
                    defaultValue: "Close",
                    comment: "Button title to close unsupported client warning"
                )) {
                    addAccountSheetPresented.toggle()
