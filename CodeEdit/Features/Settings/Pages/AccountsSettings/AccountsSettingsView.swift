...
struct AccountsSettingsView: View {
    ...
    var body: some View {
        SettingsForm {
            Section {
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-account.button", defaultValue: "Add Account...", comment: "Button title to add a new source control account")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.unsupported-provider.message", defaultValue: "This git client is currently not supported.", comment: "Message shown when selected git provider is not yet supported"))
            HStack {
                Button(String(localized: "settings.accounts.unsupported-provider.close.button", defaultValue: "Close", comment: "Button title to close unsupported provider sheet")) {
