...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "Message shown when there are no accounts configured"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                Button(String(localized: "settings.accounts.close", defaultValue: "Close", comment: "Button to close unsupported git client message")) {
