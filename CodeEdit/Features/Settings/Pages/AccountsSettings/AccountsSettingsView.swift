...
struct AccountsSettingsView: View {
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.not-supported", defaultValue: "This git client is currently not supported.", comment: "Message when selected git client is not supported"))
            HStack {
                Button(String(localized: "button.close", defaultValue: "Close", comment: "Close button for unsupported client dialog")) {
