...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.empty", defaultValue: "No accounts", comment: "Empty state title when no accounts are configured."))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-account", defaultValue: "Add Account...", comment: "Button title for adding a new account in settings.")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.git-client-unsupported", defaultValue: "This git client is currently not supported.", comment: "Alert message shown when selected git client is unsupported."))
            HStack {
