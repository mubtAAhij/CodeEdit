...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "Message when no accounts exist"))
                ...
                HStack {
                    Spacer()
                    Button(String(localized: "settings.accounts.add-button", defaultValue: "Add Account...", comment: "Add account button")) { addAccountSheetPresented.toggle() }
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "settings.accounts.not-supported", defaultValue: "This git client is currently not supported.", comment: "Unsupported git client message"))
            HStack {
                Button(String(localized: "settings.accounts.close", defaultValue: "Close", comment: "Close button")) {
