...
struct AccountsSettingsView: View {
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            Text(String(localized: "accounts.git_client_not_supported", comment: "Message shown when git client is not supported"))
            HStack {
                Button(String(localized: "common.close", comment: "Close button text")) {
