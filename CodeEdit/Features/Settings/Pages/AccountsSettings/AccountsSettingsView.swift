...
struct AccountsSettingsView: View {
        ...
        SettingsForm {
            Section {
                if $gitAccounts.isEmpty {
                    Text(String(localized: "settings.accounts.no-accounts", defaultValue: "No accounts", comment: "No accounts configured message"))
    ...
    private var implementationNeeded: some View {
        VStack(spacing: 20) {
            ...
            HStack {
                Button(String(localized: "button.close", defaultValue: "Close", comment: "Close button")) {
