...
struct SourceControlAccount: Codable, Identifiable, Hashable {
    ...
    enum URLProtocol: String, Codable, CaseIterable {
        ...
        var displayName: String {
            switch self {
            case .https:
                return String(localized: "settings.accounts.protocol.https", defaultValue: "HTTPS", comment: "HTTPS protocol display name")
            case .ssh:
                return String(localized: "settings.accounts.protocol.ssh", defaultValue: "SSH", comment: "SSH protocol display name")
            }
        }
    }
