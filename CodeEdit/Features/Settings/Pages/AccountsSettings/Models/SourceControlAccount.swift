...
struct SourceControlAccount: Codable, Identifiable, Hashable {
    ...
    enum Provider: Codable, CaseIterable, Identifiable {
        ...
        var name: String {
            switch self {
            case .bitbucketCloud:
                return String(localized: "accounts.bitbucket-cloud", defaultValue: "BitBucket Cloud", comment: "BitBucket Cloud provider name")
            case .bitbucketServer:
                return String(localized: "accounts.bitbucket-server", defaultValue: "BitBucket Server", comment: "BitBucket Server provider name")
            case .github:
                return String(localized: "accounts.github", defaultValue: "GitHub", comment: "GitHub provider name")
            case .githubEnterprise:
                return String(localized: "accounts.github-enterprise", defaultValue: "GitHub Enterprise", comment: "GitHub Enterprise provider name")
            case .gitlab:
                return String(localized: "accounts.gitlab", defaultValue: "GitLab", comment: "GitLab provider name")
            case .gitlabSelfHosted:
                return String(localized: "accounts.gitlab-self-hosted", defaultValue: "GitLab Self-hosted", comment: "GitLab Self-hosted provider name")
            }
        }
