...
struct SourceControlAccount: Codable, Identifiable, Hashable {
    ...
    enum URLProtocol: String, Codable, CaseIterable {
        ...
        var displayName: String {
            switch self {
            case .https:
                return String(localized: "accounts.url-protocol.https", defaultValue: "HTTPS", comment: "HTTPS protocol display name")
            case .ssh:
                return String(localized: "accounts.url-protocol.ssh", defaultValue: "SSH", comment: "SSH protocol display name")
            }
        }
    }
    ...
    enum Provider: Codable, CaseIterable, Identifiable {
        ...
        var name: String {
            switch self {
            case .bitbucketCloud:
                return String(localized: "accounts.provider.bitbucket-cloud", defaultValue: "BitBucket Cloud", comment: "BitBucket Cloud provider name")
            case .bitbucketServer:
                return String(localized: "accounts.provider.bitbucket-server", defaultValue: "BitBucket Server", comment: "BitBucket Server provider name")
            case .github:
                return String(localized: "accounts.provider.github", defaultValue: "GitHub", comment: "GitHub provider name")
            case .githubEnterprise:
                return String(localized: "accounts.provider.github-enterprise", defaultValue: "GitHub Enterprise", comment: "GitHub Enterprise provider name")
            case .gitlab:
                return String(localized: "accounts.provider.gitlab", defaultValue: "GitLab", comment: "GitLab provider name")
            case .gitlabSelfHosted:
                return String(localized: "accounts.provider.gitlab-self-hosted", defaultValue: "GitLab Self-hosted", comment: "GitLab Self-hosted provider name")
            }
        }
