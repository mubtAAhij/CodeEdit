...
struct IgnoredFilesListView: View {
    ...
    var body: some View {
        GlobPatternList(
            ...
            emptyMessage: String(localized: "settings.source-control.ignored-files.empty", defaultValue: "No ignored files", comment: "Empty state text when there are no ignored files")
        )
    }
}
