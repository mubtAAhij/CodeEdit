...
struct IgnoredFilesListView: View {
    ...
    var body: some View {
        GlobPatternList(
            ...
            emptyMessage: String(localized: "settings.source-control.ignored-files.empty-state", defaultValue: "No ignored files", comment: "Empty state message when there are no ignored source control files")
        )
    }
}
