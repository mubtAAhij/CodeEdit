...
struct IgnoredFilesListView: View {
    ...
    var body: some View {
        GlobPatternList(
            ...
            emptyMessage: String(localized: "source-control.no-ignored-files", defaultValue: "No ignored files", comment: "Empty state message when there are no ignored files")
        )
    }
}
