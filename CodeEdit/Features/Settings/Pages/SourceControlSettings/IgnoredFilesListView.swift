...
struct IgnoredFilesListView: View {
    ...
    var body: some View {
        GlobPatternList(
            ...
            emptyMessage: String(localized: "settings.source-control.ignored-files.empty", defaultValue: "No ignored files", comment: "Empty state message when no ignored files are configured")
        )
    }
}
