    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "git.config.title", defaultValue: "Git Configuration", comment: "Git configuration section title"))
                Text(String(localized: "git.config.description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Git configuration description"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                Text(String(localized: "git.ignored-files.description", defaultValue: "Patterns for files and folders that Git should ignore and not track. Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/gitignore)", comment: "Ignored files description"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "git.author-name", defaultValue: "Author Name", comment: "Git author name field"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "git.author-email", defaultValue: "Author Email", comment: "Git author email field"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "git.default-branch", defaultValue: "Default branch name", comment: "Default branch name field"))
            Text(String(localized: "git.branch-rules", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Branch name rules"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "git.prefer-rebase", defaultValue: "Prefer to rebase when pulling", comment: "Prefer rebase toggle"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "git.show-merge-commits", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits toggle"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "git.config-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git config file location"))
                .font(.subheadline)
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "git.ignore-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Git ignore file location"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "git.open-ignore-in-editor", defaultValue: "Open in Editor...", comment: "Open ignore file in editor button"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
