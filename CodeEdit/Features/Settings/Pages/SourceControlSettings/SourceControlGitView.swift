    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "source-control-git.configuration", defaultValue: "Git Configuration", comment: "Git configuration section header"))
                Text(String(localized: "source-control-git.configuration-description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Git configuration description with learn more link"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                Text(String(localized: "source-control-git.ignored-files", defaultValue: "Ignored Files", comment: "Ignored files section header"))
                Text(String(localized: "source-control-git.ignored-files-description", defaultValue: "Patterns for files and folders that Git should ignore and not track. Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/gitignore)", comment: "Ignored files description with learn more link"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "source-control-git.author-name", defaultValue: "Author Name", comment: "Author name field label"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "source-control-git.author-email", defaultValue: "Author Email", comment: "Author email field label"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "source-control-git.default-branch", defaultValue: "Default branch name", comment: "Default branch name field label"))
            Text(String(localized: "source-control-git.default-branch-hint", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name field hint"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "source-control-git.prefer-rebase", defaultValue: "Prefer to rebase when pulling", comment: "Prefer to rebase when pulling toggle"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "source-control-git.show-merge-commits", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits toggle"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "source-control-git.config-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git config file location text"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "source-control-git.open-in-editor", defaultValue: "Open in Editor...", comment: "Open in editor button"), action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "source-control-git.gitignore-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Gitignore file location text"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "source-control-git.open-gitignore-in-editor", defaultValue: "Open in Editor...", comment: "Open gitignore in editor button"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
