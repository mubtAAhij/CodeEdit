    ...
    var body: some View {
        Group {
            ...
            Section {
                ...
                Text("String(localized: "ignored_files", comment: "Section header for ignored files configuration")")
                Text("""
...
private extension SourceControlGitView {
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            ...
            Text("String(localized: "branch_name_restriction", comment: "Help text for default branch name field restrictions")")
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            "String(localized: "prefer_rebase_when_pulling", comment: "Toggle to prefer rebase over merge when pulling changes")",
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            "String(localized: "show_merge_commits_per_file_log", comment: "Toggle to display merge commits in per-file log view")",
    ...
    private var gitConfigEditor: some View {
        HStack {
            ...
            Button("String(localized: "open_in_editor", comment: "Button text to open a file in the editor")", action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text("Ignored file patterns are stored in \"\(resolvedGitIgnorePath)\".")
                ...
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button("String(localized: "open_in_editor", comment: "Button text to open a file in the editor")", action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
