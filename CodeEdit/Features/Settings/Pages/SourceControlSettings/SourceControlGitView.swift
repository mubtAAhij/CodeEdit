    ...
    var body: some View {
        Group {
            Section {
                ...
                """, comment: "Git configuration description with link"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                """, comment: "Ignored files description with link"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "source-control.git.author-name", defaultValue: "Author Name", comment: "Git author name field"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "source-control.git.author-email", defaultValue: "Author Email", comment: "Git author email field"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "source-control.git.default-branch", defaultValue: "Default branch name", comment: "Default branch name field"))
            Text(String(localized: "source-control.git.default-branch-hint", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name validation hint"))
        }
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "source-control.git.config-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git configuration file location message"))
                .font(.subheadline)
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "source-control.git.ignore-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Git ignore file location message"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "source-control.git.open-in-editor", defaultValue: "Open in Editor...", comment: "Open in editor button"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
