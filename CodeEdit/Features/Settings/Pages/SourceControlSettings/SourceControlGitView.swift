    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "settings.source-control.git-config.title", defaultValue: "Git Configuration", comment: "Git configuration section title"))
                Text(String(localized: "settings.source-control.git-config.description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Git configuration description"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                Text(String(localized: "settings.source-control.ignored-files.title", defaultValue: "Ignored Files", comment: "Ignored files section title"))
                Text(String(localized: "settings.source-control.ignored-files.description", defaultValue: "Patterns for files and folders that Git should ignore and not track. Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/gitignore)", comment: "Ignored files description"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "settings.source-control.default-branch", defaultValue: "Default branch name", comment: "Default branch name field label"))
            Text(String(localized: "settings.source-control.default-branch.help", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name help text"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "settings.source-control.prefer-rebase", defaultValue: "Prefer to rebase when pulling", comment: "Prefer rebase toggle"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "settings.source-control.show-merge-commits", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits toggle"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "settings.source-control.git-config.location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git config file location text"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.git-config.open", defaultValue: "Open in Editor...", comment: "Open git config button"), action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "settings.source-control.gitignore.location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Gitignore file location text"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.gitignore.open", defaultValue: "Open in Editor...", comment: "Open gitignore button"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
