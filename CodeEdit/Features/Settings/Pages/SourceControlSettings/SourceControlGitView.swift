    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "settings.source-control.git.section.git-configuration", defaultValue: "Git Configuration", comment: "Git Configuration section header"))
                Text(String(localized: "settings.source-control.git.section.git-configuration.description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Git Configuration section description with link"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                Text(String(localized: "settings.source-control.git.section.ignored-files", defaultValue: "Ignored Files", comment: "Ignored Files section header"))
                Text(String(localized: "settings.source-control.git.section.ignored-files.description", defaultValue: "Patterns for files and folders that Git should ignore and not track. Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/gitignore)", comment: "Ignored Files section description with link"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "settings.source-control.git.author-name", defaultValue: "Author Name", comment: "Git author name field label"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "settings.source-control.git.author-email", defaultValue: "Author Email", comment: "Git author email field label"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "settings.source-control.git.default-branch-name", defaultValue: "Default branch name", comment: "Default branch name field label"))
            Text(String(localized: "settings.source-control.git.default-branch-name.hint", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name field hint"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "settings.source-control.git.prefer-rebase", defaultValue: "Prefer to rebase when pulling", comment: "Prefer to rebase when pulling toggle label"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "settings.source-control.git.show-merge-commits", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits in per-file log toggle label"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "settings.source-control.git.config-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git configuration file location message"))
                .font(.subheadline)
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "settings.source-control.git.ignore-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Git ignore file location message"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.git.open-ignore-in-editor", defaultValue: "Open in Editor...", comment: "Open ignore file in Editor button label"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
