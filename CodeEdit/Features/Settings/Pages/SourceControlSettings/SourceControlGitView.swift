    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "settings.source-control.git.configuration.description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Description for git configuration settings with docs link."))
            }
            Section {
                defaultBranchName
...
private extension SourceControlGitView {
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "settings.source-control.git.default-branch-name.label", defaultValue: "Default branch name", comment: "Label for default git branch name field."))
            Text(String(localized: "settings.source-control.git.default-branch-name.validation", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Validation message for default branch name input."))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "settings.source-control.git.prefer-rebase-when-pulling.toggle", defaultValue: "Prefer to rebase when pulling", comment: "Toggle label for preferring rebase on pull."),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "settings.source-control.git.show-merge-commits.toggle", defaultValue: "Show merge commits in per-file log", comment: "Toggle label for showing merge commits in per-file log."),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "settings.source-control.git.config-location.message", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Informational text showing location of git configuration file."))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.git.config-location.open-in-editor", defaultValue: "Open in Editor...", comment: "Button title to open git configuration file in editor."), action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "settings.source-control.git.ignore-location.message", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Informational text showing location of global gitignore file."), "\(resolvedGitIgnorePath)"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.git.ignore-location.open-in-editor", defaultValue: "Open in Editor...", comment: "Button title to open global gitignore file in editor."), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
