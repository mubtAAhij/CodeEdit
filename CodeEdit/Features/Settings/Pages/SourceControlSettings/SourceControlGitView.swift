    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "git-settings.configuration-title", defaultValue: "Git Configuration", comment: "Git configuration section title"))
                Text(String(localized: "git-settings.configuration-description", defaultValue: "Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/git-config)", comment: "Git configuration description"))
            }
            Section {
                defaultBranchName
            ...
            Section {
                ...
                Text(String(localized: "git-settings.ignored-files-title", defaultValue: "Ignored Files", comment: "Ignored files section title"))
                Text(String(localized: "git-settings.ignored-files-description", defaultValue: "Patterns for files and folders that Git should ignore and not track. Applied globally to all repositories on your Mac. [Learn more...](https://git-scm.com/docs/gitignore)", comment: "Ignored files description"))
            }
            Section {
                gitIgnoreEditor
            }
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "git-settings.author-name", defaultValue: "Author Name", comment: "Author name field label"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "git-settings.author-email", defaultValue: "Author Email", comment: "Author email field label"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            Text(String(localized: "git-settings.default-branch-name", defaultValue: "Default branch name", comment: "Default branch name field label"))
            Text(String(localized: "git-settings.default-branch-name-help", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name validation help"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "git-settings.prefer-rebase-when-pulling", defaultValue: "Prefer to rebase when pulling", comment: "Prefer to rebase when pulling toggle"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "git-settings.show-merge-commits-per-file-log", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits in per-file log toggle"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "git-settings.config-file-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git config file location message"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "git-settings.open-in-editor", defaultValue: "Open in Editor...", comment: "Open in editor button"), action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "git-settings.ignore-file-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Git ignore file location message"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "git-settings.open-in-editor-ignore", defaultValue: "Open in Editor...", comment: "Open ignore file in editor button"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
