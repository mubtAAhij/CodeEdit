    ...
    var body: some View {
        Group {
            Section {
                ...
                Text(String(localized: "settings.source-control.git-configuration.help", defaultValue: """
            ...
            Section {
                ...
                Text(String(localized: "settings.source-control.ignored-files.help", defaultValue: """
...
private extension SourceControlGitView {
    private var gitAuthorName: some View {
        TextField(String(localized: "settings.source-control.author-name", defaultValue: "Author Name", comment: "Git author name field label"), text: $authorName)
    ...
    private var gitEmail: some View {
        TextField(String(localized: "settings.source-control.author-email", defaultValue: "Author Email", comment: "Git author email field label"), text: $authorEmail)
    ...
    private var defaultBranchName: some View {
        TextField(text: $defaultBranch) {
            ...
            Text(String(localized: "settings.source-control.default-branch-name.help", defaultValue: "Cannot contain spaces, backslashes, or other symbols", comment: "Default branch name help text"))
        }
    ...
    private var preferToRebaseWhenPulling: some View {
        Toggle(
            String(localized: "settings.source-control.prefer-rebase-when-pulling", defaultValue: "Prefer to rebase when pulling", comment: "Prefer to rebase when pulling toggle label"),
    ...
    private var showMergeCommitsInPerFileLog: some View {
        Toggle(
            String(localized: "settings.source-control.show-merge-commits-per-file-log", defaultValue: "Show merge commits in per-file log", comment: "Show merge commits in per-file log toggle label"),
    ...
    private var gitConfigEditor: some View {
        HStack {
            Text(String(localized: "settings.source-control.git-config-location", defaultValue: "Git configuration is stored in \"~/.gitconfig\".", comment: "Git config file location info text"))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.open-in-editor", defaultValue: "Open in Editor...", comment: "Open in editor button label"), action: openGitConfigFile)
        }
    ...
    private var gitIgnoreEditor: some View {
        HStack {
            Text(String(format: String(localized: "settings.source-control.gitignore-location", defaultValue: "Ignored file patterns are stored in \"%@\".", comment: "Git ignore file location info text"), resolvedGitIgnorePath))
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .frame(maxWidth: .infinity, alignment: .leading)
            Button(String(localized: "settings.source-control.open-in-editor", defaultValue: "Open in Editor...", comment: "Open in editor button label"), action: openGitIgnoreFile)
        }
        .frame(maxWidth: .infinity)
        .onAppear {
            Task {
