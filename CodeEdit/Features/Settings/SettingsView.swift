    ...
    var body: some View {
        NavigationSplitView {
            ...
            if #unavailable(macOS 26.0) {
                List { }
                    .searchable(text: $searchText, placement: .sidebar, prompt: String(localized: "settings.search", defaultValue: "Search", comment: "Search placeholder in settings"))
                    .scrollDisabled(true)
                    .frame(height: 30)
            ...
            Group {
                ...
                default:
                    Text(String(localized: "settings.implementation-needed", defaultValue: "Implementation Needed", comment: "Message shown for not yet implemented settings pages")).frame(alignment: .center)
                }
            }
