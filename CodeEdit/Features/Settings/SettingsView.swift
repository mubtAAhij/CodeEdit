    ...
    var body: some View {
        NavigationSplitView {
            ...
            if #unavailable(macOS 26.0) {
                List { }
                    .searchable(text: $searchText, placement: .sidebar, prompt: String(localized: "settings.view.search.tab-title", defaultValue: "Search", comment: "Title for search settings section in settings view"))
                    .scrollDisabled(true)
                    .frame(height: 30)
                ...
                .searchable(text: $searchText, placement: .sidebar, prompt: String(localized: "settings.view.search.filter-placeholder", defaultValue: "Search", comment: "Search placeholder in settings sidebar or list"))
                .navigationSplitViewColumnWidth(215)
            }
        } detail: {
            Group {
                ...
                default:
                    Text(String(localized: "settings.view.implementation-needed", defaultValue: "Implementation Needed", comment: "Placeholder text for settings sections pending implementation")).frame(alignment: .center)
                }
            }
