            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "debug.no-task-selected", defaultValue: "No Task Selected", comment: "Message when no task is selected"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "debug.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Message when no tasks are running"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "button.delete", defaultValue: "Delete", comment: "Delete button"))
                                        }
                                    }
                                )
                        }
