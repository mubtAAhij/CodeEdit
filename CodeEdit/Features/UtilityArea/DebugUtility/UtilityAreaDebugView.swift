            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "utility-area.no-task-selected", defaultValue: "No Task Selected", comment: "No task selected placeholder"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "utility-area.no-tasks-running", defaultValue: "No Tasks are Running", comment: "No tasks running message"))
                    .font(.system(size: 16))
