            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "utility-area.task.empty-state", defaultValue: "No Task Selected", comment: "Empty state when no task is selected"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "utility-area.task.no-running-tasks", defaultValue: "No Tasks are Running", comment: "Message when no tasks are running"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "utility-area.task.delete", defaultValue: "Delete", comment: "Delete task context menu item"))
                                        }
                                    }
                                )
                        }
