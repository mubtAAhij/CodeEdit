            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "tasks.no-task-selected", defaultValue: "No Task Selected", comment: "Message shown when no task is selected"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "tasks.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Message shown when no tasks are running"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "tasks.delete", defaultValue: "Delete", comment: "Context menu item to delete a task"))
                                        }
                                    }
                                )
                        }
