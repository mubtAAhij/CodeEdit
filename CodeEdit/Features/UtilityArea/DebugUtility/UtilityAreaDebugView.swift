            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "debug.no-task-selected", defaultValue: "No Task Selected", comment: "Message shown when no task is selected in debug view"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "debug.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Message shown when no tasks are running in debug view"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "debug.delete-task", defaultValue: "Delete", comment: "Context menu item to delete a task"))
                                        }
                                    }
                                )
                        }
