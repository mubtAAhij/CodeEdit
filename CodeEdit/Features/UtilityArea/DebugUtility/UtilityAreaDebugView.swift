        ...
        UtilityAreaTabView(model: utilityAreaViewModel.tabViewModel) { _ in
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(
                        ...
                        comment: "Empty state title when no debug task is selected"
                    ))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(
                    ...
                    comment: "Empty state message when no debug tasks are running"
                ))
                    .font(.system(size: 16))
                                    ...
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(
                                                ...
                                                defaultValue: "Delete",
                                                comment: "Button title to delete a debug task"
                                            ))
                                        }
                                    }
                                )
