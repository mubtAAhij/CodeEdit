            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "utility.debug.no-task-selected", defaultValue: "No Task Selected", comment: "Message shown when no task is selected in debug area"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "utility.debug.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Message shown when no tasks are running in debug area"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "utility.debug.delete-task", defaultValue: "Delete", comment: "Context menu item to delete a task"))
                                        }
                                    }
                                )
                        }
