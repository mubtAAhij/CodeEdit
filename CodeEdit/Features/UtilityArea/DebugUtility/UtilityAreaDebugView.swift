            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "utility-area.debug.no-task-selected", defaultValue: "No Task Selected", comment: "Empty state when no task is selected"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "utility-area.debug.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Empty state when no tasks are running"))
                    .font(.system(size: 16))
