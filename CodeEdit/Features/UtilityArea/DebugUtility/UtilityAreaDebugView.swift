            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView("String(localized: "no_task_selected", comment: "Message shown when no task is selected")")
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text("String(localized: "no_tasks_running", comment: "Message shown when no tasks are currently running")")
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text("String(localized: "delete", comment: "Button label for delete action")")
                                        }
                                    }
                                )
                        }
