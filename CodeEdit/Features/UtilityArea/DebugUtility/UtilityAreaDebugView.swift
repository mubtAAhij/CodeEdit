            ...
            ZStack {
                ...
                if let taskShowingOutput = taskManager.taskShowingOutput,
                   let activeTask = taskManager.activeTasks[taskShowingOutput] {
                    ...
                    CEContentUnavailableView(String(localized: "utility-area.debug.no-task-selected", defaultValue: "No Task Selected", comment: "Message when no debug task is selected"))
                }
            }
        } leadingSidebar: { _ in
            ZStack {
                Text(String(localized: "utility-area.debug.no-tasks-running", defaultValue: "No Tasks are Running", comment: "Message when no debug tasks are running"))
                    .font(.system(size: 16))
                                ...
                                .contextMenu(
                                    ContextMenu {
                                        Button {
                                            ...
                                            Text(String(localized: "utility-area.debug.delete", defaultValue: "Delete", comment: "Delete button in debug task context menu"))
                                        }
                                    }
                                )
                        }
