    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField("String(localized: "name", comment: "Label for terminal name field")", text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button("String(localized: "rename_ellipsis", comment: "Context menu option to rename a terminal tab")") {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button("String(localized: "kill_terminals", comment: "Context menu option to terminate multiple terminal sessions")") {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button("String(localized: "kill_terminal", comment: "Context menu option to terminate a single terminal session")") {
