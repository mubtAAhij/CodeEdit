    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.tab.name", defaultValue: "Name", comment: "Placeholder for terminal name field"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            ...
            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.tab.kill-multiple", defaultValue: "Kill Terminals", comment: "Context menu item to kill multiple terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.tab.kill-single", defaultValue: "Kill Terminal", comment: "Context menu item to kill single terminal")) {
