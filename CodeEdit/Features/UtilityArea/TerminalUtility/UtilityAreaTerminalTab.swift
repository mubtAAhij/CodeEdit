    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.name", defaultValue: "Name", comment: "Terminal name placeholder"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.rename", defaultValue: "Rename...", comment: "Rename terminal context menu item")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.kill-terminals", defaultValue: "Kill Terminals", comment: "Kill multiple terminals context menu item")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.kill-terminal", defaultValue: "Kill Terminal", comment: "Kill terminal context menu item")) {
