    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.tab.name-placeholder", defaultValue: "Name", comment: "Text field placeholder for terminal name"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.tab.rename", defaultValue: "Rename...", comment: "Context menu item to rename terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.tab.kill-terminals", defaultValue: "Kill Terminals", comment: "Context menu item to kill multiple terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.tab.kill-terminal", defaultValue: "Kill Terminal", comment: "Context menu item to kill terminal")) {
