    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.name", defaultValue: "Name", comment: "Terminal name field label"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.rename", defaultValue: "Rename...", comment: "Rename terminal menu item")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.kill-all", defaultValue: "Kill Terminals", comment: "Kill all terminals menu item")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.kill", defaultValue: "Kill Terminal", comment: "Kill terminal menu item")) {
