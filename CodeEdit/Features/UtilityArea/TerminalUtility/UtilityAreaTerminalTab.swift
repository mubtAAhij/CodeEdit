    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.name-field", defaultValue: "Name", comment: "Placeholder for terminal name field"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.rename", defaultValue: "Rename...", comment: "Button to rename a terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.kill-terminals", defaultValue: "Kill Terminals", comment: "Button to kill multiple selected terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.kill-terminal", defaultValue: "Kill Terminal", comment: "Button to kill a single terminal")) {
