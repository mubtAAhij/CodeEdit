    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "terminal.name-placeholder", defaultValue: "Name", comment: "Placeholder text for terminal name field"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "terminal.rename", defaultValue: "Rename...", comment: "Menu item to rename a terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "terminal.kill-terminals", defaultValue: "Kill Terminals", comment: "Menu item to kill multiple terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "terminal.kill-terminal", defaultValue: "Kill Terminal", comment: "Menu item to kill a single terminal")) {
