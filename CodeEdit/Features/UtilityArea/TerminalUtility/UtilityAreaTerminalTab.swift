    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.name-prompt", defaultValue: "Name", comment: "Terminal name prompt"), text: terminalTitle)
                    .focused($isFocused)
            } else {
                // A padding is needed for macOS 13
                TextField(String(localized: "utility-area.terminal.name-title", defaultValue: "Name", comment: "Terminal name title"), text: terminalTitle)
                    .focused($isFocused)
                    .padding(.leading, -8)
            }
        } icon: {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.rename", defaultValue: "Rename...", comment: "Rename terminal button")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.kill-terminals", defaultValue: "Kill Terminals", comment: "Kill terminals button")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.kill-terminal", defaultValue: "Kill Terminal", comment: "Kill terminal button")) {
