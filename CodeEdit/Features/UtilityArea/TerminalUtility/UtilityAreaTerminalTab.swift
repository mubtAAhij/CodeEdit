    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "terminal.name.placeholder", defaultValue: "Name", comment: "Terminal name placeholder"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "terminal.rename", defaultValue: "Rename...", comment: "Rename terminal button")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "terminal.kill.multiple", defaultValue: "Kill Terminals", comment: "Kill multiple terminals button")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "terminal.kill.single", defaultValue: "Kill Terminal", comment: "Kill terminal button")) {
