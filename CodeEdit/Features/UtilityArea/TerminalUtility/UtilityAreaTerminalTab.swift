    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(
                    localized: "utility.terminal.name-placeholder",
                    defaultValue: "Name",
                    comment: "Placeholder for terminal name text field"
                ), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(
                localized: "utility.terminal.rename",
                defaultValue: "Rename...",
                comment: "Context menu item to rename terminal"
            )) {
                isFocused = true
            }
            ...
            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(
                    localized: "utility.terminal.kill-terminals",
                    defaultValue: "Kill Terminals",
                    comment: "Context menu item to kill multiple terminals"
                )) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(
                    localized: "utility.terminal.kill-terminal",
                    defaultValue: "Kill Terminal",
                    comment: "Context menu item to kill single terminal"
                )) {
