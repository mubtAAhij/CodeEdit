    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "terminal.name.placeholder", defaultValue: "Name", comment: "Placeholder for terminal name field"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "terminal.rename", defaultValue: "Rename...", comment: "Context menu item to rename terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "terminal.kill-multiple", defaultValue: "Kill Terminals", comment: "Context menu item to kill multiple selected terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "terminal.kill-single", defaultValue: "Kill Terminal", comment: "Context menu item to kill single terminal")) {
