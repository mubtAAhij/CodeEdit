    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility-area.terminal.tab.name-placeholder", defaultValue: "Name", comment: "Terminal tab name placeholder"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility-area.terminal.tab.rename", defaultValue: "Rename...", comment: "Rename terminal button")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility-area.terminal.tab.kill-terminals", defaultValue: "Kill Terminals", comment: "Kill multiple terminals button")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility-area.terminal.tab.kill-terminal", defaultValue: "Kill Terminal", comment: "Kill single terminal button")) {
