...
struct UtilityAreaTerminalTab: View {
    ...
    var body: some View {
        ...
        .contextMenu {
            Button(String(localized: "rename_ellipsis", comment: "Context menu item to rename terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "kill_terminals", comment: "Context menu item to kill multiple terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "kill_terminal", comment: "Context menu item to kill a terminal")) {
