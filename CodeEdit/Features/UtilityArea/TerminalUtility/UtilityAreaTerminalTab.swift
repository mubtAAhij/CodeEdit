    ...
    var body: some View {
        ...
        Label {
            if #available(macOS 14, *) {
                ...
                TextField(String(localized: "utility.terminal.tab-name-field", defaultValue: "Name", comment: "Terminal tab name text field placeholder"), text: terminalTitle)
                    .focused($isFocused)
            } else {
        ...
        .contextMenu {
            Button(String(localized: "utility.terminal.rename-menu", defaultValue: "Rename...", comment: "Menu item to rename terminal")) {
                isFocused = true
            }

            if selectedIDs.contains(terminal.id) && selectedIDs.count > 1 {
                Button(String(localized: "utility.terminal.kill-terminals", defaultValue: "Kill Terminals", comment: "Menu item to kill multiple terminals")) {
                    removeTerminals(selectedIDs)
                }
            } else {
                Button(String(localized: "utility.terminal.kill-terminal", defaultValue: "Kill Terminal", comment: "Menu item to kill single terminal")) {
