    ...
    var body: some View {
        Picker(String(localized: "utility-area.output.source-picker.label", defaultValue: "Output Source", comment: "Label for output source picker"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.output.no-source", defaultValue: "No Selected Output Source", comment: "Message shown when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility-area.output.no-language-servers", defaultValue: "No Language Servers", comment: "Message shown when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "utility-area.output.no-extensions", defaultValue: "No Extensions", comment: "Message shown when no extensions are available"))
            } else {
