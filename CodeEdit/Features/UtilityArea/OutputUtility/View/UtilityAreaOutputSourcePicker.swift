    ...
    var body: some View {
        Picker(String(localized: "utility-area.output-source", defaultValue: "Output Source", comment: "Output source picker label"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.no-selected-output", defaultValue: "No Selected Output Source", comment: "No selected output source placeholder"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility-area.no-language-servers", defaultValue: "No Language Servers", comment: "No language servers message"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "utility-area.no-extensions", defaultValue: "No Extensions", comment: "No extensions message"))
            } else {
