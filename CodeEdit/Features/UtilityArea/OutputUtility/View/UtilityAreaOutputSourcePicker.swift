...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "utility-area.output.source-label", defaultValue: "Output Source", comment: "Output source label"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.output.no-source", defaultValue: "No Selected Output Source", comment: "No output source selected message"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
