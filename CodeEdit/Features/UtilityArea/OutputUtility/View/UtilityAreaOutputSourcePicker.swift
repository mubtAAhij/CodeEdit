...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "output.source-picker", defaultValue: "Output Source", comment: "Label for output source picker"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "output.no-selected-source", defaultValue: "No Selected Output Source", comment: "Message when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "output.no-language-servers", defaultValue: "No Language Servers", comment: "Message when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "output.no-extensions", defaultValue: "No Extensions", comment: "Message when no extensions are available"))
            } else {
