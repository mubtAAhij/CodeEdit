...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker("String(localized: "output_source", comment: "Label for output source selection")", selection: $selectedSource) {
            ...
            if languageServerClients.isEmpty {
                Text("String(localized: "no_language_servers", comment: "Message when no language servers are available")")
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text("String(localized: "no_extensions", comment: "Message when no extensions are available")")
            } else {
