...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "utility-area.output.source", defaultValue: "Output Source", comment: "Output source menu title"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.output.no-source", defaultValue: "No Selected Output Source", comment: "Message when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility-area.output.no-lsp", defaultValue: "No Language Servers", comment: "Message when no language servers are available"))
            } else {
