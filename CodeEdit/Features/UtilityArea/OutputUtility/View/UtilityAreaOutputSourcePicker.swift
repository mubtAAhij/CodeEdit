...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "output.source-picker", defaultValue: "Output Source", comment: "Output source picker label"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "output.no-selected-source", defaultValue: "No Selected Output Source", comment: "Empty state when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "output.no-language-servers", defaultValue: "No Language Servers", comment: "Empty state when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "output.no-extensions", defaultValue: "No Extensions", comment: "Empty state when no extensions are available"))
            } else {
