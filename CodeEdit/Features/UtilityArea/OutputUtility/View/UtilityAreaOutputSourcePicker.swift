...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "utility_area.output.source_picker", comment: "Picker label for output source selection"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility_area.output.no_source_selected", comment: "Message when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility_area.output.no_language_servers", comment: "Message when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "utility_area.output.no_extensions", comment: "Message when no extensions are available"))
            } else {
