...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "output.source.title", defaultValue: "Output Source", comment: "Output source picker title"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "output.source.none-selected", defaultValue: "No Selected Output Source", comment: "Message when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "output.source.no-language-servers", defaultValue: "No Language Servers", comment: "Message when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "output.source.no-extensions", defaultValue: "No Extensions", comment: "Message when no extensions are available"))
            } else {
