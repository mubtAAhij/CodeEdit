...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(
            localized: "utility-area.output.source-picker.title",
            defaultValue: "Output Source",
            comment: "Title for output source picker"
        ), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(
                    localized: "utility-area.output.source-picker.no-selected-source",
                    defaultValue: "No Selected Output Source",
                    comment: "Placeholder text when no output source is selected"
                ))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            ...
            if languageServerClients.isEmpty {
                Text(String(
                    ...
                    comment: "Placeholder when no language server output sources are available"
                ))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(
                    ...
                    comment: "Placeholder when no extension output sources are available"
                ))
            } else {
