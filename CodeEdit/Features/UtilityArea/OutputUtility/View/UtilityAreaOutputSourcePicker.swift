...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "utility-area.output.source-picker.title", defaultValue: "Output Source", comment: "Title for output source picker in utility area."), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.output.source-picker.no-selected-source", defaultValue: "No Selected Output Source", comment: "Empty state text when no output source is selected."))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility-area.output.source-picker.no-language-servers", defaultValue: "No Language Servers", comment: "Empty state text when no language server output sources are available."))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "utility-area.output.source-picker.no-extensions", defaultValue: "No Extensions", comment: "Empty state text when no extension output sources are available."))
            } else {
