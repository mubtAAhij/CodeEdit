...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "utility-area.output.source", defaultValue: "Output Source", comment: "Output source label"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "utility-area.output.no-source", defaultValue: "No Selected Output Source", comment: "No output source selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "utility-area.output.no-lsp", defaultValue: "No Language Servers", comment: "No language servers message"))
            } else {
