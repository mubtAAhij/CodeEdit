...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "output.label.source", comment: "Output source label"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "output.message.no_source_selected", comment: "Message when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
