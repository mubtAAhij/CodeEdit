...
struct UtilityAreaOutputSourcePicker: View {
    ...
    var body: some View {
        Picker(String(localized: "output_source", comment: "Label for output source picker"), selection: $selectedSource) {
            if selectedSource == nil {
                Text(String(localized: "no_selected_output_source", comment: "Placeholder text when no output source is selected"))
                    .italic()
                    .tag(Sources?.none)
                Divider()
            }
            ...
            if languageServerClients.isEmpty {
                Text(String(localized: "no_language_servers", comment: "Message shown when no language servers are available"))
            } else {
            ...
            if extensionManager.extensions.isEmpty {
                Text(String(localized: "no_extensions", comment: "Message shown when no extensions are available"))
            } else {
