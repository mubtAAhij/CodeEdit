    ...
    enum Sources: Hashable {
        ...
        var title: String {
            ...
            case .extensions(let source):
                String(localized: "output.extension-title", defaultValue: "Extension - \(source.extensionInfo.name)", comment: "Title for extension output source")
            case .languageServer(let source):
                String(localized: "output.language-server-title", defaultValue: "Language Server - \(source.id)", comment: "Title for language server output source")
            case .devOutput:
                String(localized: "output.dev-output-title", defaultValue: "Internal Development Output", comment: "Title for internal development output source")
            }
        }
        ...
        UtilityAreaTabView(model: utilityAreaViewModel.tabViewModel) { _ in
            Group {
                if let selectedSource {
                    ...
                    Text(String(localized: "output.no-output", defaultValue: "No output", comment: "Message shown when no output is available"))
                        .font(.system(size: 16))
                        ...
                        .paneToolbar {
                            UtilityAreaOutputSourcePicker(selectedSource: $selectedSource)
                            Spacer()
                            UtilityAreaFilterTextField(title: String(localized: "output.filter", defaultValue: "Filter", comment: "Placeholder text for output filter field"), text: $filterText)
                                .frame(maxWidth: 175)
                            Button { } label: {
