    ...
    enum Sources: Hashable {
        ...
        var title: String {
            ...
            case .extensions(let source):
                String(format: String(localized: "utility-area.output.source.extension", defaultValue: "Extension - %@", comment: "Extension output source title"), source.extensionInfo.name)
            case .languageServer(let source):
                String(format: String(localized: "utility-area.output.source.language-server", defaultValue: "Language Server - %@", comment: "Language server output source title"), source.id)
            case .devOutput:
                String(localized: "utility-area.output.source.dev-output", defaultValue: "Internal Development Output", comment: "Internal development output source title")
            }
        }
        ...
        UtilityAreaTabView(model: utilityAreaViewModel.tabViewModel) { _ in
            Group {
                if let selectedSource {
                    ...
                    Text(String(localized: "utility-area.output.no-output", defaultValue: "No output", comment: "No output message"))
                        .font(.system(size: 16))
                        ...
                        .paneToolbar {
                            UtilityAreaOutputSourcePicker(selectedSource: $selectedSource)
                            Spacer()
                            UtilityAreaFilterTextField(title: String(localized: "utility-area.output.filter", defaultValue: "Filter", comment: "Filter text field title"), text: $filterText)
                                .frame(maxWidth: 175)
                            Button { } label: {
