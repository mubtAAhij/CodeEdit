    ...
    enum Sources: Hashable {
        ...
        var title: String {
            ...
            case .extensions(let source):
                String(format: String(localized: "utility-area.output.extension-source", defaultValue: "Extension - %@", comment: "Extension output source format"), source.extensionInfo.name)
            case .languageServer(let source):
                String(format: String(localized: "utility-area.output.lsp-source", defaultValue: "Language Server - %@", comment: "Language server output source format"), source.id)
            case .devOutput:
                String(localized: "utility-area.output.internal-dev", defaultValue: "Internal Development Output", comment: "Internal development output source")
            }
        }
                ...
                if let selectedSource {
                    ...
                    Text(String(localized: "utility-area.output.no-output", defaultValue: "No output", comment: "No output message"))
                        ...
                        .paneToolbar {
                            UtilityAreaOutputSourcePicker(selectedSource: $selectedSource)
                            Spacer()
                            UtilityAreaFilterTextField(title: String(localized: "utility-area.output.filter", defaultValue: "Filter", comment: "Filter field placeholder"), text: $filterText)
                                .frame(maxWidth: 175)
                            Button { } label: {
