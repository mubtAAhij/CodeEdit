    ...
    enum Sources: Hashable {
        ...
        var title: String {
            ...
            case .extensions(let source):
                String(format: String(localized: "output.extension-format", defaultValue: "Extension - %@", comment: "Extension output title format"), source.extensionInfo.name)
            case .languageServer(let source):
                String(format: String(localized: "output.language-server-format", defaultValue: "Language Server - %@", comment: "Language server output title format"), source.id)
            case .devOutput:
                String(localized: "output.internal-development", defaultValue: "Internal Development Output", comment: "Internal development output title")
            }
        }
                ...
                if let selectedSource {
                    ...
                    Text(String(localized: "output.no-output", defaultValue: "No output", comment: "No output message"))
                        ...
                        .paneToolbar {
                            UtilityAreaOutputSourcePicker(selectedSource: $selectedSource)
                            Spacer()
                            UtilityAreaFilterTextField(title: String(localized: "output.filter", defaultValue: "Filter", comment: "Filter placeholder"), text: $filterText)
                                .frame(maxWidth: 175)
                            Button { } label: {
