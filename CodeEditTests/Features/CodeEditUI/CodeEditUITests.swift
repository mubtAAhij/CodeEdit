...
final class CodeEditUIUnitTests: XCTestCase {
    ...
    func testSegmentedControlLight() throws {
        let view = SegmentedControl(.constant(0), options: [String(localized: "opt1", comment: "Option 1 test value"), String(localized: "opt2", comment: "Option 2 test value")])
        let hosting = NSHostingView(rootView: view)
        hosting.appearance = .init(named: .aqua)
    ...
    func testSegmentedControlProminentLight() throws {
        let view = SegmentedControl(.constant(0), options: [String(localized: "opt1", comment: "Option 1 test value"), String(localized: "opt2", comment: "Option 2 test value")], prominent: true)
        let hosting = NSHostingView(rootView: view)
        hosting.appearance = .init(named: .aqua)
        hosting.frame = CGRect(origin: .zero, size: .init(width: 100, height: 30))
        assertSnapshot(matching: hosting, as: .image)
