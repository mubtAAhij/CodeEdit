...
final class CodeEditUIUnitTests: XCTestCase {
    ...
    func testSegmentedControlLight() throws {
        let view = SegmentedControl(.constant(0), options: ["String(localized: "test_opt1", comment: "Test option 1 label")", "String(localized: "test_opt2", comment: "Test option 2 label")"])
        let hosting = NSHostingView(rootView: view)
        hosting.appearance = .init(named: .aqua)
    ...
    func testSegmentedControlProminentLight() throws {
        let view = SegmentedControl(.constant(0), options: ["String(localized: "test_opt1", comment: "Test option 1 label")", "String(localized: "test_opt2", comment: "Test option 2 label")"], prominent: true)
        let hosting = NSHostingView(rootView: view)
        hosting.appearance = .init(named: .aqua)
        hosting.frame = CGRect(origin: .zero, size: .init(width: 100, height: 30))
        assertSnapshot(matching: hosting, as: .image)
