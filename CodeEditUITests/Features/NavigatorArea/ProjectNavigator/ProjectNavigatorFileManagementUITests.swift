...
final class ProjectNavigatorFileManagementUITests: XCTestCase {
    ...
    func testNewFilesAppear() throws {
        ...
        guard Query.Navigator.getProjectNavigatorRow(
            fileTitle: String(localized: "projectNavigatorTests.newFolder", comment: "Folder name"),
            navigator
        ).waitForExistence(timeout: 2.0) else {
            XCTFail(String(localized: "projectNavigatorTests.newFolder", comment: "Folder name") + " did not appear")
            return
        }

        let folderRow = Query.Navigator.getProjectNavigatorRow(fileTitle: String(localized: "projectNavigatorTests.newFolder", comment: "Folder name"), navigator)
    ...
    func testCreateNewFiles() throws {
        ...
        for idx in 0..<5 {
            ...
            addMenu.menuItems[String(localized: "projectNavigatorTests.addFile", comment: "Menu item")].click()
