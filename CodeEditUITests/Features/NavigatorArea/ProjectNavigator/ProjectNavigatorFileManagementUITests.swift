...
final class ProjectNavigatorFileManagementUITests: XCTestCase {
    ...
    func testNewFilesAppear() throws {
        ...
        guard Query.Navigator.getProjectNavigatorRow(
            fileTitle: String(localized: "new_folder", comment: "New folder name"),
            navigator
        ).waitForExistence(timeout: 2.0) else {
        ...
        let folderRow = Query.Navigator.getProjectNavigatorRow(fileTitle: String(localized: "new_folder", comment: "New folder name"), navigator)
    ...
    func testCreateNewFiles() throws {
        ...
        for idx in 0..<5 {
            ...
            addMenu.menuItems[String(localized: "add_file", comment: "Add file menu item")].click()
