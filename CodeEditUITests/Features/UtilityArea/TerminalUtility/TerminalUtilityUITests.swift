...
final class TerminalUtilityUITests: XCTestCase {
    ...
    func testTerminalsInputData() throws {
        let terminal = utilityArea.textViews[String(localized: "test.terminal-emulator", defaultValue: "Terminal Emulator", comment: "Accessibility identifier for terminal emulator in UI tests")]
    ...
    func testTerminalsKeepData() throws {
        var terminal = utilityArea.textViews[String(localized: "test.terminal-emulator", defaultValue: "Terminal Emulator", comment: "Accessibility identifier for terminal emulator in UI tests")]
        XCTAssertTrue(terminal.exists)
        terminal.click()
        terminal.typeText("echo hello world")
        terminal.typeKey(.enter, modifierFlags: [])
        ...
        terminal = utilityArea.textViews[String(localized: "test.terminal-emulator", defaultValue: "Terminal Emulator", comment: "Accessibility identifier for terminal emulator in UI tests")]
        XCTAssertTrue(terminal.exists)
