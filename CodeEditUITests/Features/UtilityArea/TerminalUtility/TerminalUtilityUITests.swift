...
final class TerminalUtilityUITests: XCTestCase {
    ...
    func testTerminalsInputData() throws {
        let terminal = utilityArea.textViews[String(localized: "terminal.title", comment: "Title for Terminal Emulator utility")]
    ...
    func testTerminalsKeepData() throws {
        var terminal = utilityArea.textViews[String(localized: "terminal.title", comment: "Title for Terminal Emulator utility")]
        XCTAssertTrue(terminal.exists)
        terminal.click()
        terminal.typeText("echo hello world")
        terminal.typeKey(.enter, modifierFlags: [])
        ...
        terminal = utilityArea.textViews[String(localized: "terminal.title", comment: "Title for Terminal Emulator utility")]
        XCTAssertTrue(terminal.exists)
