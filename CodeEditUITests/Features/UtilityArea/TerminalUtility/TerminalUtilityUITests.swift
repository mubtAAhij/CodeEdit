...
final class TerminalUtilityUITests: XCTestCase {
    ...
    func testTerminalsInputData() throws {
        let terminal = utilityArea.textViews[String(localized: "Terminal Emulator", comment: "Accessibility label")]
    ...
    func testTerminalsKeepData() throws {
        var terminal = utilityArea.textViews[String(localized: "Terminal Emulator", comment: "Accessibility label")]
        XCTAssertTrue(terminal.exists)
        terminal.click()
        terminal.typeText("echo hello world")
        terminal.typeKey(.enter, modifierFlags: [])
        ...
        terminal = utilityArea.textViews[String(localized: "Terminal Emulator", comment: "Accessibility label")]
        XCTAssertTrue(terminal.exists)
