...
final class ActivityViewerTasksMenuTests: XCTestCase {
    ...
    func testTaskMenu() {
        let viewer = window.groups["String(localized: "activity_viewer", comment: "Activity viewer UI component label")"]
        XCTAssertNotNil(viewer, "No Activity Viewer")

        let taskDropdown = viewer.buttons["String(localized: "active_task", comment: "Active task dropdown button label")"]
        XCTAssertTrue(taskDropdown.waitForExistence(timeout: 2.0), "No Task Dropdown")
        XCTAssertEqual(taskDropdown.value as? String, "String(localized: "create_tasks", comment: "Label shown when no tasks exist")", "Incorrect empty tasks label")
    ...
    func testNewTask() {
        ...
        let addTaskListOption = popover.buttons["String(localized: "add_task_ellipsis", comment: "Add task menu item with ellipsis")"]
        ...
        let addTaskButton = workspaceSettingsWindow.buttons["String(localized: "add_task_ellipsis", comment: "Add task menu item with ellipsis")"]
        ...
        let taskName = newSheet.textFields["String(localized: "task_name", comment: "Task name input field label")"]
        ...
        let taskCommand = newSheet.textFields["String(localized: "task_command", comment: "Task command input field label")"]
