...
final class ActivityViewerTasksMenuTests: XCTestCase {
    ...
    func testTaskMenu() {
        let viewer = window.groups[String(localized: "Activity Viewer")]
        XCTAssertNotNil(viewer, "No Activity Viewer")

        let taskDropdown = viewer.buttons[String(localized: "Active Task")]
        XCTAssertTrue(taskDropdown.waitForExistence(timeout: 2.0), "No Task Dropdown")
        XCTAssertEqual(taskDropdown.value as? String, String(localized: "Create Tasks"), "Incorrect empty tasks label")
    ...
    func testNewTask() {
        ...
        let addTaskListOption = popover.buttons[String(localized: "Add Task...")]
        XCTAssertTrue(addTaskListOption.exists, "No add task option in dropdown")
        addTaskListOption.click()

        let workspaceSettingsWindow = window.sheets[String(localized: "Workspace Settings")]
        XCTAssertTrue(workspaceSettingsWindow.waitForExistence(timeout: 1.0), "Workspace settings did not appear")

        let addTaskButton = workspaceSettingsWindow.buttons[String(localized: "Add Task...")]
        ...
        let taskName = newSheet.textFields[String(localized: "Task Name")]
        ...
        let taskCommand = newSheet.textFields[String(localized: "Task Command")]
        ...
        let saveButton = newSheet.buttons[String(localized: "Save")]
        XCTAssertTrue(saveButton.exists)
        saveButton.click()

        workspaceSettingsWindow.buttons[String(localized: "Done")].click()
        XCTAssertFalse(
