...
final class ActivityViewerTasksMenuTests: XCTestCase {
    ...
    func testTaskMenu() {
        let viewer = window.groups[String(localized: "activity_viewer.title", comment: "Activity Viewer panel title")]
        XCTAssertNotNil(viewer, "No Activity Viewer")

        let taskDropdown = viewer.buttons[String(localized: "activity_viewer.active_task", comment: "Active Task dropdown button")]
        XCTAssertTrue(taskDropdown.waitForExistence(timeout: 2.0), "No Task Dropdown")
        XCTAssertEqual(taskDropdown.value as? String, String(localized: "activity_viewer.create_tasks", comment: "Create Tasks placeholder text"), "Incorrect empty tasks label")
    ...
    func testNewTask() {
        ...
        let workspaceSettingsWindow = window.sheets[String(localized: "workspace.settings", comment: "Workspace Settings window title")]
        ...
        let taskName = newSheet.textFields[String(localized: "task.name", comment: "Task Name field label")]
        ...
        let taskCommand = newSheet.textFields[String(localized: "task.command", comment: "Task Command field label")]
