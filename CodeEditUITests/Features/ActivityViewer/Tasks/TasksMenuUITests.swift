...
final class ActivityViewerTasksMenuTests: XCTestCase {
    ...
    func testTaskMenu() {
        let viewer = window.groups[String(localized: "activity_viewer", comment: "Activity viewer panel name")]
        XCTAssertNotNil(viewer, "No Activity Viewer")

        let taskDropdown = viewer.buttons[String(localized: "active_task", comment: "Active task label")]
        XCTAssertTrue(taskDropdown.waitForExistence(timeout: 2.0), "No Task Dropdown")
        XCTAssertEqual(taskDropdown.value as? String, String(localized: "create_tasks", comment: "Create tasks button"), "Incorrect empty tasks label")
    ...
    func testNewTask() {
        ...
        let addTaskListOption = popover.buttons[String(localized: "add_task", comment: "Add task menu item")]
        XCTAssertTrue(addTaskListOption.exists, "No add task option in dropdown")
        addTaskListOption.click()

        let workspaceSettingsWindow = window.sheets[String(localized: "workspace_settings", comment: "Workspace settings menu item")]
        XCTAssertTrue(workspaceSettingsWindow.waitForExistence(timeout: 1.0), "Workspace settings did not appear")

        let addTaskButton = workspaceSettingsWindow.buttons[String(localized: "add_task", comment: "Add task menu item")]
        ...
        let saveButton = newSheet.buttons[String(localized: "save", comment: "Save button")]
        XCTAssertTrue(saveButton.exists)
        saveButton.click()

        workspaceSettingsWindow.buttons[String(localized: "done", comment: "Done button")].click()
        XCTAssertFalse(
